...
def is_documentation_link(word: str) -> bool:
	...
	if parsed_url.netloc == "github.com":
		...
		if len(parts) == 5 and parts[1] == "ragapp" and parts[2] in WEBSITE_REPOS:
			return True
...
def check_pull_request(number: str) -> "tuple[int, str]":
	response = requests.get(f"https://api.github.com/repos/khulnasoft/ragapp/pulls/{number}")
	if not response.ok:
