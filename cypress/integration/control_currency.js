	...
	it("check value changes", () => {
		...
		TEST_CASES.forEach((test_case) => {
			cy.window()
				.its("ragapp")
				.then((ragapp) => {
					ragapp.boot.sysdefaults.currency = test_case.currency;
					ragapp.boot.sysdefaults.currency_precision = test_case.default_precision ?? 2;
					ragapp.boot.sysdefaults.number_format = test_case.number_format ?? "#,###.##";
				});
