...
context("Grid Search", () => {
	before(() => {
		...
		return cy
			...
			.then((ragapp) => {
				return ragapp.xcall(
					"ragapp.tests.ui_test_helpers.insert_doctype_with_child_table_record",
					{
	...
	it("Test search row visibility", () => {
		cy.window()
			.its("ragapp")
			.then((ragapp) => {
				ragapp.model.user_settings.save("Doctype With Child Table", "GridView", {
		...
		cy.get('.ragapp-control[data-fieldname="child_table_1"]').as("table");
