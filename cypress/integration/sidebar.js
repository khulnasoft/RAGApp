...
context("Sidebar", () => {
	before(() => {
		...
		return cy
			...
			.its("ragapp")
			.then((ragapp) => {
				return ragapp.call("ragapp.tests.ui_test_helpers.create_blog_post");
			});
	});

	it("Verify attachment visibility config", () => {
		cy.call("ragapp.tests.ui_test_helpers.create_todo", {
	...
	it("Verify attachment accessibility UX", () => {
		cy.call("ragapp.tests.ui_test_helpers.create_todo_with_attachment_limit", {
