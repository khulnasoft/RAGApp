context("Workspace Blocks", () => {
	before(() => {
		...
		return cy
			...
			.its("ragapp")
			.then((ragapp) => {
				return ragapp.xcall("ragapp.tests.ui_test_helpers.setup_workflow");
			});
	});
	...
	it("Create Test Page", () => {
		cy.intercept({
			method: "POST",
			url: "api/method/ragapp.desk.doctype.workspace.workspace.new_page",
		}).as("new_page");
	...
	it.skip("Quick List Block", () => {
		...
		cy.intercept({
			method: "GET",
			url: "api/method/ragapp.desk.form.load.getdoctype?**",
		}).as("get_doctype");
		...
		cy.intercept({
			method: "POST",
			url: "api/method/ragapp.desk.reportview.get",
		}).as("refresh-list");
