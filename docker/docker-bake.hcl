...
variable "RAGAPP_REPO" {
    default = "https://github.com/khulnasoft/ragapp"
}

variable "ERPNEXT_REPO" {
    default = "https://github.com/khulnasoft-lab/nexterp"
}
...
target "bench" {
    ...
    target = "bench"
    tags = [
        "ragapp/bench:${LATEST_BENCH_RELEASE}",
        "ragapp/bench:latest",
    ]
}
...
function "tag" {
    params = [repo, version]
    result = [
      # Push ragapp or erpnext branch as tag
...
target "default-args" {
    args = {
        RAGAPP_PATH = "${RAGAPP_REPO}"
        ERPNEXT_PATH = "${ERPNEXT_REPO}"
        BENCH_REPO = "${BENCH_REPO}"
        RAGAPP_BRANCH = "${RAGAPP_VERSION}"
