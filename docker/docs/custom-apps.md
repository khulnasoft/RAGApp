...
[
  {
    "url": "https://github.com/khulnasoft-lab/nexterp",
    "branch": "version-15"
  },
  {
    "url": "https://github.com/khulnasoft/payments",
    "branch": "version-15"
  },
  {
...
- Use fork repo or branch for NxERP in case you need to use your fork or test a PR.
...
### Clone ragapp_docker and switch directory
...
- `RAGAPP_PATH`, customize the source repo for ragapp framework. Defaults to `https://github.com/khulnasoft/ragapp`
- `RAGAPP_BRANCH`, customize the source repo branch for ragapp framework. Defaults to `version-15`.
...
This method uses pre-built `ragapp/base:${RAGAPP_BRANCH}` and `ragapp/build:${RAGAPP_BRANCH}` image layers which come with required Python and NodeJS runtime. It speeds up the build time.

It uses `images/layered/Containerfile`.

```shell
docker build \
  --build-arg=RAGAPP_PATH=https://github.com/khulnasoft/ragapp \
  --build-arg=RAGAPP_BRANCH=version-15 \
...
x-customizable-image: &customizable_image
  image: ${CUSTOM_IMAGE:-ragapp/erpnext}:${CUSTOM_TAG:-${ERPNEXT_VERSION:?No NxERP version or tag set}}
...
- `CUSTOM_IMAGE`: The name of your custom image. Defaults to `ragapp/erpnext` if not set.
