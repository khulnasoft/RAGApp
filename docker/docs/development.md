...
Clone and change directory to ragapp_docker directory

```shell
git clone https://github.com/khulnasoft/ragapp.git
cd docker
```
...
For most people getting started with Ragapp development, the best solution is to use [VSCode Dev Containers extension](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-containers).
...
- Open ragapp_docker folder in VS Code.
  - `code .`
...
- The `development` directory is ignored by git. It is mounted and available inside the container. Create all your benches (installations of bench, the tool that manages ragapp) inside this directory.
...
NOTE: Prior to doing the following, make sure the user is **ragapp**.

```shell
bench init --skip-redis-config-generation ragapp-bench
cd ragapp-bench
```

To setup ragapp framework version 14 bench set `PYENV_VERSION` environment variable to `3.10.5` (default) and use NodeJS version 16 (default),

```shell
# Use default environments
bench init --skip-redis-config-generation --ragapp-branch version-14 ragapp-bench
# Or set environment versions explicitly
nvm use v16
PYENV_VERSION=3.10.13 bench init --skip-redis-config-generation --ragapp-branch version-14 ragapp-bench
# Switch directory
cd ragapp-bench
```

To setup ragapp framework version 13 bench set `PYENV_VERSION` environment variable to `3.9.17` and use NodeJS version 14,

```shell
nvm use v14
PYENV_VERSION=3.9.17 bench init --skip-redis-config-generation --ragapp-branch version-13 ragapp-bench
cd ragapp-bench
```
...
Honcho is the tool used by Bench to manage all the processes Ragapp requires. Usually, these all run in localhost, but in this case, we have external containers for Redis. For this reason, we have to stop Honcho from trying to start Redis processes.

Honcho is installed in global python environment along with bench. To make it available locally you've to install it in every `ragapp-bench/env` you create. Install it using command `./env/bin/pip install honcho`. It is required locally if you wish to use is as part of VSCode launch configuration.
...
This will create a new site and a `development.localhost` directory under `ragapp-bench/sites`.
...
To setup site with PostgreSQL as database use option `--db-type postgres` and `--db-host postgresql`. (Available only v12 onwards, currently NOT available for NxERP).
...
To develop a new app, the last step will be setting the site into developer mode. Documentation is available at [this link](https://ragapp.io/docs/user/en/guides/app-development/how-enable-developer-mode-in-ragapp).
...
At the time of this writing, the Payments app has been factored out of the Version 14 NxERP app and is now a separate app. NxERP will not install it.
...
To install NxERP (from the version-13 branch):
...
Note: Both ragapp and erpnext must be on branch with same name. e.g. version-14

### Start Ragapp without debugging

Execute following command from the `ragapp-bench` directory.
...
usage: installer.py [-h] [-j APPS_JSON] [-b BENCH_NAME] [-s SITE_NAME] [-r RAGAPP_REPO] [-t RAGAPP_BRANCH] [-p PY_VERSION] [-n NODE_VERSION] [-v] [-a ADMIN_PASSWORD] [-d DB_TYPE]
...
options:
  ...
  -b BENCH_NAME, --bench-name BENCH_NAME
                        Bench directory name, default: ragapp-bench
  -s SITE_NAME, --site-name SITE_NAME
                        Site name, should end with .localhost, default: development.localhost
  -r RAGAPP_REPO, --ragapp-repo RAGAPP_REPO
                        ragapp repo to use, default: https://github.com/khulnasoft/ragapp
  -t RAGAPP_BRANCH, --ragapp-branch RAGAPP_BRANCH
                        ragapp repo to use, default: version-15
...
### Start Ragapp with Visual Studio Code Python Debugging
...
- Click on `Install on Dev Container: Ragapp Cli`
- Click on 'Reload'

We need to start bench separately through the VSCode debugger. For this reason, **instead** of running `bench start` you should run the following command inside the ragapp-bench directory:
...
Launch VSCode command palette (cmd+shift+p or ctrl+shift+p), run the command `Python: Select interpreter to start Jupyter server` and select `/workspace/development/ragapp-bench/env/bin/python`.

The first step is installing and updating the required software. Usually the ragapp framework may require an older version of Jupyter, while VSCode likes to move fast, this can [cause issues](https://github.com/jupyter/jupyter_console/issues/158). For this reason we need to run the following command.

```shell
/workspace/development/ragapp-bench/env/bin/python -m pip install --upgrade jupyter ipykernel ipython
```
...
ragapp.init(site='development.localhost', sites_path='/workspace/development/ragapp-bench/sites')
ragapp.connect()
ragapp.local.lang = ragapp.db.get_default('lang')
ragapp.db.connect()
```
...
```shell
docker exec -e "TERM=xterm-256color" -w /workspace/development -it devcontainer-ragapp-1 bash
```
...
Access the service by service name from the `ragapp` development container. The above service will be accessible via hostname `postgresql`. If ports are published on to host, access it via `localhost:5432`.
