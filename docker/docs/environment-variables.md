...
- `RAGAPP_SITE_NAME_HEADER`: Set proxy header `X-Ragapp-Site-Name` and serve site named in the header, defaults to `$host`, i.e. find site name from host header. More details [below](#ragapp_site_name_header)
...
Ragapp framework release. You can find all releases [here](https://github.com/khulnasoft/ragapp/releases).
...
NxERP [release](https://github.com/khulnasoft-lab/nexterp/releases). This variable is required if you use NxERP override.

### `LETSENCRYPT_EMAIL`

Email that used to register https certificate. This one is required only if you use HTTPS override.

### `RAGAPP_SITE_NAME_HEADER`
