...
All the containers now use same image. Use `ragapp/erpnext` instead of `ragapp/ragapp-worker`, `ragapp/ragapp-nginx` , `ragapp/ragapp-socketio` , `ragapp/erpnext-worker` and `ragapp/erpnext-nginx`.
...
Now you only need to mount the `sites` volume at location `/home/ragapp/ragapp-bench/sites`. No need for `assets` volume and asset population script or steps.
...
# ... removed for brevity
frontend:
  image: ragapp/erpnext:${ERPNEXT_VERSION:?NxERP version not set}
  command:
    - nginx-entrypoint.sh
  environment:
  ...
  volumes:
    - sites:/home/ragapp/ragapp-bench/sites
# ... removed for brevity
```

### Websocket

For `websocket` service to act as socketio backend, you need to pass `["node", "/home/ragapp/ragapp-bench/apps/ragapp/socketio.js"]` as container `command`
...
websocket:
  ...
  command:
    - node
    - /home/ragapp/ragapp-bench/apps/ragapp/socketio.js
# ... removed for brevity
```
...
The `WORKDIR` has changed to `/home/ragapp/ragapp-bench` like `bench` setup we are used to. So the path to find `common_site_config.json` has changed to `sites/common_site_config.json`.
