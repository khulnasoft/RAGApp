...
### Setup Ragapp without proxy and external MariaDB and Redis
...
### Setup NxERP with proxy and external MariaDB and Redis
...
### Setup Ragapp using containerized MariaDB and Redis with Letsencrypt certificates.
...
### Setup NxERP using containerized MariaDB and Redis with Letsencrypt certificates.
...
Switch to the root of the `ragapp_docker` directory before running the following commands:

```sh
# Update environment variables ERPNEXT_VERSION and RAGAPP_VERSION
nano .env
