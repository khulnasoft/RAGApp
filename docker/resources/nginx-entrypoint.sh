...
if [[ -z "$RAGAPP_SITE_NAME_HEADER" ]]; then
  # shellcheck disable=SC2016
  echo 'RAGAPP_SITE_NAME_HEADER defaulting to $host'
  # shellcheck disable=SC2016
  export RAGAPP_SITE_NAME_HEADER='$host'
fi
...
envsubst '${BACKEND}
  ...
  </templates/nginx/ragapp.conf.template >/etc/nginx/conf.d/ragapp.conf
