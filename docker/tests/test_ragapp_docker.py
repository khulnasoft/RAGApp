...
def test_endpoints(url: str, callback: Any, ragapp_site: str):
    check_url_content(
        url=f"http://127.0.0.1{url}", callback=callback, site_name=ragapp_site
    )


@pytest.mark.skipif(
    os.environ["RAGAPP_VERSION"][0:3] == "v12", reason="v12 doesn't have the asset"
)
def test_assets_endpoint(ragapp_site: str):
    check_url_content(
        url=f"http://127.0.0.1/assets/ragapp/images/ragapp-framework-logo.svg",
        callback=assets_cb,
        site_name=ragapp_site,
    )


def test_files_reachable(ragapp_site: str, tmp_path: Path, compose: Compose):
    content = "lalala\n"
    ...
    compose(
        "cp",
        str(file_path),
        f"backend:/home/ragapp/ragapp-bench/sites/{ragapp_site}/public/files/",
    )
...
@pytest.mark.parametrize("service", BACKEND_SERVICES)
@pytest.mark.usefixtures("ragapp_site")
def test_ragapp_connections_in_backends(
    service: str, python_path: str, compose: Compose
):
    filename = "_ping_ragapp_connections.py"
    compose("cp", f"tests/{filename}", f"{service}:/tmp/")
    compose.exec(
        "-w",
...
def test_push_backup(
    ...
    compose.bench("--site", ragapp_site, "backup", "--with-files")
    restic_args = [
        "--env=RESTIC_REPOSITORY=s3:http://minio:9000/ragapp",
...
def test_https(ragapp_site: str, compose: Compose):
    compose("-f", "overrides/compose.https.yaml", "up", "-d")
    check_url_content(url="https://127.0.0.1", callback=index_cb, site_name=ragapp_site)
