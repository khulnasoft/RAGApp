...
const html_plugin = require("./ragapp-html");
const vue_style_plugin = require("./ragapp-vue-style");
const rtlcss = require("rtlcss");
const postCssPlugin = require("@ragapp/esbuild-plugin-postcss2").default;
...
const argv = yargs
	...
	.option("skip_ragapp", {
		type: "boolean",
		description: "Skip building ragapp assets",
	})
	.option("files", {
		type: "string",
	...
	.option("using-cached", {
		type: "boolean",
		description:
			"Skips build and uses cached build artifacts to update assets.json (used by Cli)",
	})
	.example("node esbuild --apps ragapp,erpnext", "Run build only for ragapp and erpnext")
	.example(
		"node esbuild --files ragapp/website.bundle.js,ragapp/desk.bundle.js",
		"Run build only for specified bundles"
	)
	.version(false).argv;

const APPS = (!argv.apps ? app_list : argv.apps.split(",")).filter(
	(app) => !(argv.skip_ragapp && app == "ragapp")
);
...
function get_files_to_build(files) {
	// files: ['ragapp/website.bundle.js', 'erpnext/main.bundle.js']
	let include_patterns = [];
	let ignore_patterns = [];
...
async function notify_redis({ error, success, changed_files }) {
	...
	if (error) {
		...
		let stack = error.stack.replace(new RegExp(cli_path, "g"), "");
		payload = {
			error,
			formatted,
			stack,
