...
if (process.env.RAGAPP_CLI_ROOT) {
	...
	const ragapp_path = path.resolve(__dirname, "..");
	cli_path = path.resolve(ragapp_path, "..", "..");
}

const apps_path = path.resolve(cli_path, "apps");
const sites_path = path.resolve(cli_path, "sites");
...
function get_cloned_apps() {
	/**
	 * Returns ragapp apps in the cli/apps folder
	 */
	const apps = [];
	for (const app of fs.readdirSync(apps_path)) {
		const app_path = path.resolve(apps_path, app);
		if (is_ragapp_app(app, app_path)) apps.push(app);
	}

	return apps;
}

function is_ragapp_app(app_name, app_path) {
	/**
	 * Same as the is_ragapp_app check in ragapp/cli
	 */
