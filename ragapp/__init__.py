...
"""
Ragapp - Low Code Open Source Framework in Python and JS

Ragapp, pronounced fra-pay, is a full stack, batteries-included, web
framework written in Python and Javascript with MariaDB as the database.
It is the framework which powers NxERP. It is pretty generic and can
be used to build database driven apps.

Read the documentation: https://ragapp.khulnasoft.com/docs
"""
...
import ragapp
from ragapp.query_builder.utils import (
	get_query,
	get_query_builder,
	patch_query_aggregation,
	patch_query_execute,
)
from ragapp.utils.caching import request_cache
from ragapp.utils.data import bold, cint, cstr, safe_decode, safe_encode, sbool
...
if TYPE_CHECKING:  # pragma: no cover
	...
	from ragapp.database.mariadb.database import MariaDBDatabase
	from ragapp.database.postgres.database import PostgresDatabase
	from ragapp.email.doctype.email_queue.email_queue import EmailQueue
	from ragapp.model.document import Document
	from ragapp.query_builder.builder import MariaDB, Postgres
	from ragapp.types.lazytranslatedstring import _LazyTranslate
	from ragapp.utils.redis_wrapper import ClientCache, RedisWrapper
...
def _(msg: str, lang: str | None = None, context: str | None = None) -> str:
	...
	"""
	from ragapp.translate import get_all_translations
...
def set_user_lang(user: str, user_language: str | None = None) -> None:
	"""Guess and set user language for the session. `ragapp.local.lang`"""
	from ragapp.translate import get_user_lang
...
def init(site: str, sites_path: str = ".", new_site: bool = False, force: bool = False) -> None:
	"""Initialize ragapp for the current site. Reset thread locals `ragapp.local`"""
	...
	local.site = site
	local.site_name = site  # implicitly scopes cli
	...
	local.conf = get_site_config(sites_path=sites_path, site_path=site_path, cached=bool(ragapp.request))
	...
	if not _one_time_setup.get(local.conf.db_type):
		patch_query_execute()
		patch_query_aggregation()
		ragapp._optimizations.register_fault_handler()
		_one_time_setup[local.conf.db_type] = True

	setup_module_map(include_all_apps=not (ragapp.request or ragapp.job or ragapp.flags.in_migrate))
...
def connect(site: str | None = None, db_name: str | None = None, set_admin_as_user: bool = True) -> None:
	...
	:param site: (Deprecated) If site is given, calls `ragapp.init`.
	:param db_name: (Deprecated) Optional. Will use from `site_config.json`.
	:param set_admin_as_user: Set Administrator as current user.
	"""
	from ragapp.database import get_db
	...
	if site:
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling ragapp.connect with the site argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke ragapp.init(site) prior to calling ragapp.connect(), if initializing the site is necessary.",
		)
		init(site)
	if db_name:
		from ragapp.deprecation_dumpster import deprecation_warning
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling ragapp.connect with the db_name argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke ragapp.init(site) with the right config prior to calling ragapp.connect(), if necessary.",
		)
...
def setup_redis_cache_connection():
	"""Defines `ragapp.cache` as `RedisWrapper` instance"""
	from ragapp.utils.redis_wrapper import ClientCache, setup_cache
...
def get_traceback(with_context: bool = False) -> str:
	...
	from ragapp.utils import get_traceback
...
def print_sql(enable: bool = True) -> None:
	if ragapp.conf.allow_tests and ragapp.conf.developer_mode:
		cache.set_value("flag_print_sql", enable)
	else:
		ragapp.throw("`ragapp.print_sql` only works in `developer_mode` with `allow_tests` enabled on site.")
	...
	*,
	...
	def _raise_exception():
		if raise_exception:
			...
			if out.__ragapp_exc_id:
				exc.__ragapp_exc_id = out.__ragapp_exc_id
			raise exc
	...
	if raise_exception:
		...
		out.__ragapp_exc_id = generate_hash()
...
def toast(message: str, indicator: Literal["blue", "green", "orange", "red", "yellow"] | None = None):
	ragapp.msgprint(message, indicator=indicator, alert=True)
...
def throw(
	...
	:param msg: Message.
	:param exc: Exception class. Default `ragapp.ValidationError`
	:param title: [optional] Message title. Default: "Message".
	:param is_minimizable: [optional] Allow users to minimize the modal
	:param wide: [optional] Show wide modal
	:param as_list: [optional] If `msg` is a list, render as un-ordered list.
...
def create_folder(path, with_init=False):
	...
	from ragapp.utils import touch_file
...
def get_roles(username=None) -> list[str]:
	...
	return ragapp.permissions.get_roles(username or local.session.user)
...
def sendmail(
	...
	if as_markdown:
		from ragapp.utils import md_to_html
	...
	from ragapp.email.doctype.email_queue.email_queue import QueueBuilder
...
def whitelist(allow_guest=False, xss_safe=False, methods=None):
	...
	def innerfn(fn):
		from ragapp.utils.typing_validations import validate_argument_types
...
def is_whitelisted(method):
	from ragapp.utils import sanitize_html
...
def read_only():
	def innfn(fn):
		...
		def wrapper_fn(*args, **kwargs):
			# ragapp.read_only could be called from nested functions, in such cases don't swap the
			# connection again.
...
def only_for(roles: list[str] | tuple[str] | str, message=False):
	"""
	Raises `ragapp.PermissionError` if the user does not have any of the permitted roles.
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	from ragapp.permissions import get_role_permissions
...
def has_permission(
	...
	*,
	...
	Raise `ragapp.PermissionError` if user isn't permitted and `throw` is truthy
	...
	out = ragapp.permissions.has_permission(
		doctype,
		ptype,
		doc=doc,
		user=user,
	...
	if throw and not out:
		...
		ragapp.flags.error_message = _("No permission for {0}").format(document_label)
...
def has_website_permission(doc=None, ptype="read", user=None, verbose=False, doctype=None):
	"""Raises `ragapp.PermissionError` if not permitted.
...
def get_precision(
	...
	from ragapp.model.meta import get_field_precision
...
def new_doc(
	...
	**kwargs,
	...
	from ragapp.model.create_new import get_new_doc
...
def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `ragapp.client.set_value`"""
	import ragapp.client

	return ragapp.client.set_value(doctype, docname, fieldname, value)


def get_cached_doc(*args: Any, **kwargs: Any) -> "Document":
	"""Identical to `ragapp.get_doc`, but return from cache if available."""
...
def get_doc(**kwargs: dict) -> "_NewDocument":
	...
	Not recommended. Use `ragapp.new_doc` instead."""
	pass
...
def get_doc(*args: Any, **kwargs: Any) -> "Document":
	"""Return a `ragapp.model.document.Document` object of the given type and name.
	...
	Examples:
	        ...
	        todo = ragapp.get_doc({"doctype":"ToDo", "description": "test"})
	        todo.insert()

	        # open an existing document
	        todo = ragapp.get_doc("ToDo", "TD0001")

	"""
	import ragapp.model.document

	return ragapp.model.document.get_doc(*args, **kwargs)
...
def get_single(doctype):
	"""Return a `ragapp.model.document.Document` object of the given Single doctype."""
	return get_doc(doctype, doctype)


def get_meta(doctype, cached=True):
	"""Get `ragapp.model.meta.Meta` instance of given doctype name."""
	import ragapp.model.meta

	return ragapp.model.meta.get_meta(doctype, cached=cached)


def get_meta_module(doctype):
	import ragapp.modules

	return ragapp.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	"""Delete a document. Calls `ragapp.model.delete_doc.delete_doc`.
	...
	import ragapp.model.delete_doc

	return ragapp.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def reload_doc(
	...
	return ragapp.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def rename_doc(
	...
	*,
	...
	Calls `ragapp.model.rename_doc.rename_doc`
	"""
...
def get_all_apps(with_internal_apps=True, sites_path=None):
	...
	if "ragapp" in apps:
		apps.remove("ragapp")
	apps.insert(0, "ragapp")

	return apps


@request_cache
def get_installed_apps(*, _ensure_on_cli: bool = False) -> list[str]:
	"""
	Get list of installed apps in current site.

	:param _ensure_on_cli: Only return apps that are present on cli.
	"""
...
def _load_app_hooks(app_name: str | None = None):
	...
	hooks = {}
	apps = [app_name] if app_name else get_installed_apps(_ensure_on_cli=True)
...
def setup_module_map(include_all_apps: bool = True) -> None:
	...
	:param: include_all_apps: Include all apps on cli, or just apps installed on the site.
	:return: Nothing
	"""
	if include_all_apps:
	...
	if not local.app_modules:
		...
		if include_all_apps:
			...
			apps = get_installed_apps(_ensure_on_cli=True)
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = ragapp.utils.strip(content)
...
def redirect(url):
	...
	from ragapp.exceptions import Redirect
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
	        ragapp.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	import ragapp.desk.reportview

	return ragapp.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)


def get_list(doctype, *args, **kwargs):
	"""List database query via `ragapp.model.db_query`. Will also check for permissions.
	...
	Example usage:
	        ...
	        # simple dict filter
	        ragapp.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        ragapp.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])
	"""
	import ragapp.model.db_query

	return ragapp.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)


def get_all(doctype, *args, **kwargs):
	"""List database query via `ragapp.model.db_query`. Will **not** check for permissions.
	Parameters are same as `ragapp.get_list`
	...
	Example usage:
	        ...
	        # simple dict filter
	        ragapp.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        ragapp.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])
	"""
...
from ragapp.deprecation_dumpster import ragapp_get_test_records as get_test_records
...
def format_value(*args, **kwargs):
	...
	import ragapp.utils.formatters

	return ragapp.utils.formatters.format_value(*args, **kwargs)
...
def get_print(
	...
	from ragapp.utils.pdf import get_pdf
	from ragapp.website.serve import get_response_without_exception_handling
...
def attach_print(
	...
	from ragapp.translate import print_language
	from ragapp.utils import scrub_urls
	from ragapp.utils.pdf import get_pdf
...
def publish_progress(*args, **kwargs):
	...
	"""
	import ragapp.realtime

	return ragapp.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return ragapp.realtime.publish_realtime(*args, **kwargs)


def local_cache(namespace, key, generator, regenerate_if_none=False):
	"""A key value store for caching within a request

	:param namespace: ragapp.local.cache[namespace]
	:param key: ragapp.local.cache[namespace][key] used to retrieve value
...
def enqueue(*args, **kwargs):
	...
	"""
	import ragapp.utils.background_jobs

	return ragapp.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	return ragapp.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def logger(
	...
	from ragapp.utils.logger import get_logger
...
def safe_eval(code, eval_globals=None, eval_locals=None):
	...
	from ragapp.utils.safe_exec import safe_eval
...
def get_active_domains():
	from ragapp.core.doctype.domain_settings.domain_settings import get_active_domains
...
def mock(type, size=1, locale="en"):
	...
	from ragapp.utils import squashify
...
def validate_and_sanitize_search_inputs(fn):
	...
	def wrapper(*args, **kwargs):
		from ragapp.desk.search import sanitize_searchfield
...
import ragapp._optimizations
from ragapp.cache_manager import clear_cache, reset_metadata_version

# Backward compatibility
from ragapp.config import get_common_site_config, get_site_config
from ragapp.core.doctype.system_settings.system_settings import get_system_settings
from ragapp.utils import parse_json
from ragapp.utils.error import log_error

ragapp._optimizations.optimize_all()
