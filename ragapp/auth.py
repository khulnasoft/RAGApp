...
import ragapp
import ragapp.database
import ragapp.utils
import ragapp.utils.user
...
from ragapp.sessions import Session, clear_sessions, delete_session, get_expiry_in_seconds
from ragapp.translate import get_language
from ragapp.twofactor import (
	authenticate_for_2factor,
	confirm_otp_token,
	get_cached_user_pass,
	should_run_2fa,
)
from ragapp.utils import cint, date_diff, datetime, get_datetime, today
from ragapp.utils.password import check_password, get_decrypted_password
...
class HTTPRequest:
	def __init__(self):
		...
		ragapp.local.cookie_manager.init_cookies()

	@property
	def domain(self):
		if not getattr(self, "_domain", None):
			self._domain = ragapp.request.host
	...
	def set_request_ip(self):
		if ragapp.get_request_header("X-Forwarded-For"):
			ragapp.local.request_ip = (ragapp.get_request_header("X-Forwarded-For").split(",", 1)[0]).strip()

		elif ragapp.get_request_header("REMOTE_ADDR"):
			ragapp.local.request_ip = ragapp.get_request_header("REMOTE_ADDR")
	...
	def validate_csrf_token(self):
		if (
			not ragapp.request
			or ragapp.request.method not in UNSAFE_HTTP_METHODS
			or ragapp.conf.ignore_csrf
			or not ragapp.session
			or not (saved_token := ragapp.session.data.csrf_token)
			or (
				(ragapp.get_request_header("X-Ragapp-CSRF-Token") or ragapp.form_dict.pop("csrf_token", None))
				== saved_token
			)
		...
		ragapp.flags.disable_traceback = True
		ragapp.throw(_("Invalid Request"), ragapp.CSRFTokenError)

	def set_lang(self):
		ragapp.local.lang = get_language()

	def is_allowed_referrer(self):
		referrer = ragapp.get_request_header("Referer")
		origin = ragapp.get_request_header("Origin")

		# Get the list of allowed referrers from cache or configuration
		allowed_referrers = ragapp.cache.get_value(
			"allowed_referrers",
			generator=lambda: ragapp.conf.get("allowed_referrers", []),
		)
...
class LoginManager:
	...
	def __init__(self):
		...
		if ragapp.local.form_dict.get("cmd") == "login" or ragapp.local.request.path == "/api/method/login":
	...
	def login(self):
		...
		if ragapp.get_system_settings("disable_user_pass_login"):
			ragapp.throw(_("Login with username and password is not allowed."), ragapp.AuthenticationError)

		# clear cache
		ragapp.clear_cache(user=ragapp.form_dict.get("usr"))
		user, pwd = get_cached_user_pass()
		self.authenticate(user=user, pwd=pwd)
		if self.force_user_to_reset_password():
			doc = ragapp.get_doc("User", self.user)
			ragapp.local.response["redirect_to"] = doc.reset_password(send_email=False, password_expired=True)
			ragapp.local.response["message"] = "Password Reset"
			return False
	...
	def get_user_info(self):
		self.info = ragapp.get_cached_value(
			"User", self.user, ["user_type", "first_name", "last_name", "user_image"], as_dict=1
		)
		self.user_type = self.info.user_type

	def setup_boot_cache(self):
		ragapp.cache_manager.build_table_count_cache()
		ragapp.cache_manager.build_domain_restricted_doctype_cache()
		ragapp.cache_manager.build_domain_restricted_page_cache()
	...
	def set_user_info(self, resume=False):
		...
		if self.info.user_type == "Website User":
			ragapp.local.cookie_manager.set_cookie("system_user", "no", deduplicate=True)
			if not resume:
				ragapp.local.response["message"] = "No App"
				ragapp.local.response["home_page"] = get_default_path() or "/" + get_home_page()
		else:
			ragapp.local.cookie_manager.set_cookie("system_user", "yes", deduplicate=True)
			if not resume:
				ragapp.local.response["message"] = "Logged In"
				ragapp.local.response["home_page"] = get_default_path() or "/app"

		if not resume:
			ragapp.response["full_name"] = self.full_name

		# redirect information
		if not resume and (redirect_to := ragapp.cache.hget("redirect_after_login", self.user)):
			ragapp.local.response["redirect_to"] = redirect_to
			ragapp.cache.hdel("redirect_after_login", self.user)

		ragapp.local.cookie_manager.set_cookie("full_name", self.full_name, deduplicate=True)
		ragapp.local.cookie_manager.set_cookie("user_id", self.user, deduplicate=True)
		ragapp.local.cookie_manager.set_cookie("user_image", self.info.user_image or "", deduplicate=True)
		ragapp.local.cookie_manager.set_cookie("user_lang", ragapp.local.lang, deduplicate=True)

	def clear_preferred_language(self):
		ragapp.local.cookie_manager.delete_cookie("preferred_language")

	def make_session(self, resume=False):
		# start session
		ragapp.local.session_obj = Session(
			user=self.user, resume=resume, full_name=self.full_name, user_type=self.user_type
		)

		# reset user if changed to Guest
		self.user = ragapp.local.session_obj.user
		ragapp.local.session = ragapp.local.session_obj.data
	...
	def clear_active_sessions(self):
		...
		if not (
			cint(ragapp.conf.get("deny_multiple_sessions"))
			or cint(ragapp.db.get_system_setting("deny_multiple_sessions"))
		):
			return

		clear_sessions(ragapp.session.user, keep_current=True)

	def authenticate(self, user: str | None = None, pwd: str | None = None):
		from ragapp.core.doctype.user.user import User

		if not (user and pwd):
			user, pwd = ragapp.form_dict.get("usr"), ragapp.form_dict.get("pwd")
		if not (user and pwd):
		...
		ip_tracker = get_login_attempt_tracker(ragapp.local.request_ip)
		if not user:
		...
		ignore_tracker = should_run_2fa(user.name) and ("otp" in ragapp.form_dict)
	...
	def force_user_to_reset_password(self):
		...
		if self.user in ragapp.STANDARD_USERS:
			return False

		reset_pwd_after_days = cint(ragapp.get_system_settings("force_user_to_reset_password"))

		if reset_pwd_after_days:
			last_password_reset_date = (
				ragapp.db.get_value("User", self.user, "last_password_reset_date") or today()
			)
	...
	def fail(self, message, user=None):
		if not user:
			user = _("Unknown User")
		ragapp.local.response["message"] = message
		add_authentication_log(message, user, status="Failed")
		ragapp.db.commit()
		raise ragapp.AuthenticationError

	def run_trigger(self, event="on_login"):
		for method in ragapp.get_hooks().get(event, []):
			ragapp.call(ragapp.get_attr(method), login_manager=self)

	def validate_hour(self):
		"""check if user is logging in during restricted hours"""
		login_before = cint(ragapp.db.get_value("User", self.user, "login_before", ignore=True))
		login_after = cint(ragapp.db.get_value("User", self.user, "login_after", ignore=True))

		if not (login_before or login_after):
			return

		from ragapp.utils import now_datetime

		current_hour = int(now_datetime().strftime("%H"))

		if login_before and current_hour >= login_before:
			ragapp.throw(_("Login not allowed at this time"), ragapp.AuthenticationError)
	...
	def impersonate(self, user):
		current_user = ragapp.session.user
		self.login_as(user)
		# Flag this session as impersonated session, so other code can log this.
		ragapp.local.session_obj.set_impersonsated(current_user)
	...
	def logout(self, arg="", user=None):
		...
		if user == ragapp.session.user:
			delete_session(ragapp.session.sid, user=user, reason="User Manually Logged Out")
			self.clear_cookies()
			if ragapp.request:
				self.login_as_guest()
		else:
...
class CookieManager:
	...
	def init_cookies(self):
		if not ragapp.local.session.get("sid"):
			return

		if ragapp.session.sid:
			self.set_cookie("sid", ragapp.session.sid, max_age=get_expiry_in_seconds(), httponly=True)
	...
	def set_cookie(
		...
		if not secure and hasattr(ragapp.local, "request"):
			secure = ragapp.local.request.scheme == "https"
		if (
			deduplicate
			and not (expires or max_age)
			and (request := getattr(ragapp.local, "request", None))
...
@ragapp.whitelist()
def get_logged_user():
	return ragapp.session.user


def clear_cookies():
	if hasattr(ragapp.local, "session"):
		ragapp.session.sid = ""
	ragapp.local.cookie_manager.delete_cookie(
...
def validate_ip_address(user):
	...
	checked for those
	"""
	if hasattr(ragapp.local, "request") and ragapp.local.request.path.startswith(
		"/api/method/ragapp.realtime."
	):
		return True

	user_info = ragapp.get_cached_doc("User", user)
	ip_list = user_info.get_restricted_ip_list()
	if not ip_list:
		return

	for ip in ip_list:
		if ragapp.local.request_ip.startswith(ip):
			return

	# check if bypass restrict ip is enabled for all users
	bypass_restrict_ip_check = ragapp.get_system_settings("bypass_restrict_ip_check_if_2fa_enabled")

	# check if two factor auth is enabled
	if ragapp.get_system_settings("enable_two_factor_auth") and not bypass_restrict_ip_check:
	...
	ragapp.throw(_("Access not allowed from this IP Address"), ragapp.AuthenticationError)
...
def get_login_attempt_tracker(key: str, raise_locked_exception: bool = True):
	...
	"""
	sys_settings = ragapp.get_doc("System Settings")
...
class LoginAttemptTracker:
	...
	def login_failed_count(self):
		return ragapp.cache.hget("login_failed_count", self.key)

	@login_failed_count.setter
	def login_failed_count(self, count):
		ragapp.cache.hset("login_failed_count", self.key, count)

	@login_failed_count.deleter
	def login_failed_count(self):
		ragapp.cache.hdel("login_failed_count", self.key)
	...
	def login_failed_time(self):
		...
		"""
		return ragapp.cache.hget("login_failed_time", self.key)

	@login_failed_time.setter
	def login_failed_time(self, timestamp):
		ragapp.cache.hset("login_failed_time", self.key, timestamp)

	@login_failed_time.deleter
	def login_failed_time(self):
		ragapp.cache.hdel("login_failed_time", self.key)
...
def validate_auth():
	...
	"""
	authorization_header = ragapp.get_request_header("Authorization", "").split(" ")
	...
	if len(authorization_header) == 2 and ragapp.session.user in ("", "Guest"):
		raise ragapp.AuthenticationError
...
def validate_oauth(authorization_header):
	...
	from ragapp.integrations.oauth2 import get_oauth_server
	from ragapp.oauth import get_url_delimiter

	if authorization_header[0].lower() != "bearer":
		return

	form_dict = ragapp.local.form_dict
	...
	try:
		required_scopes = ragapp.db.get_value("OAuth Bearer Token", token, "scopes").split(
			get_url_delimiter()
		)
		...
		if valid:
			ragapp.set_user(ragapp.db.get_value("OAuth Bearer Token", token, "user"))
			ragapp.local.form_dict = form_dict
	except AttributeError:
		pass
...
def validate_auth_via_api_keys(authorization_header):
	...
	try:
		...
		authorization_source = ragapp.get_request_header("Ragapp-Authorization-Source")
		if auth_type.lower() == "basic":
			api_key, api_secret = ragapp.safe_decode(base64.b64decode(auth_token)).split(":")
		...
		ragapp.throw(
			...
			ragapp.InvalidAuthorizationToken,
		)
	except (AttributeError, TypeError, ValueError):
		pass


def validate_api_key_secret(api_key, api_secret, ragapp_authorization_source=None):
	"""ragapp_authorization_source to provide api key and secret for a doctype apart from User"""
	doctype = ragapp_authorization_source or "User"
	doc = ragapp.db.get_value(doctype=doctype, filters={"api_key": api_key}, fieldname=["name"])
	if not doc:
		raise ragapp.AuthenticationError
	form_dict = ragapp.local.form_dict
	...
	if api_secret == doc_secret:
		if doctype == "User":
			user = ragapp.db.get_value(doctype="User", filters={"api_key": api_key}, fieldname=["name"])
		else:
			user = ragapp.db.get_value(doctype, doc, "user")
		if ragapp.local.login_manager.user in ("", "Guest"):
...
def validate_auth_via_hooks():
	for auth_hook in ragapp.get_hooks("auth_hooks", []):
		ragapp.get_attr(auth_hook)()
