...
class MilestoneTracker(Document):
	...
	def on_update(self):
		ragapp.cache_manager.clear_doctype_map("Milestone Tracker", self.document_type)
...
def evaluate_milestone(doc, event):
	...
	for d in get_milestone_trackers(doc.doctype):
		ragapp.get_doc("Milestone Tracker", d.get("name")).apply(doc)


def get_milestone_trackers(doctype):
	return ragapp.cache_manager.get_doctype_map(
