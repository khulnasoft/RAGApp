...
def build_missing_files():
	...
	for type in ["css", "js"]:
		folder = os.path.join(sites_path, "assets", "ragapp", "dist", type)
	...
	assets_json = ragapp.read_file("assets/assets.json")
	if assets_json:
		assets_json = ragapp.parse_json(assets_json)

		for bundle_file, output_file in assets_json.items():
			if not output_file.startswith("/assets/ragapp"):
				continue
		...
		if missing_assets:
			...
			files_to_build = ["ragapp/" + name for name in missing_assets]
			bundle(build_mode, files=files_to_build)
	else:
		# no assets.json, run full build
		bundle(build_mode, apps="ragapp")


def get_assets_link(ragapp_head) -> str:
	import requests

	tag = getoutput(
		r"cd ../apps/ragapp && git show-ref --tags -d | grep {} | sed -e 's,.*"
		r" refs/tags/,,' -e 's/\^{{}}//'".format(ragapp_head)
	)

	if tag:
		# if tag exists, download assets from github release
		url = f"https://github.com/khulnasoft/ragapp/releases/download/{tag}/assets.tar.gz"
	else:
		url = f"http://assets.ragapp.khulnasoft.com/{ragapp_head}.tar.gz"

	if not requests.head(url):
		reference = f"Release {tag}" if tag else f"Commit {ragapp_head}"
		raise AssetsDontExistError(f"Assets for {reference} don't exist")

	return url


def fetch_assets(url, ragapp_head):
	click.secho("Retrieving assets...", fg="yellow")

	prefix = mkdtemp(prefix="ragapp-assets-", suffix=ragapp_head)
	assets_archive = download_file(url, prefix)

	if not assets_archive:
		raise AssetsNotDownloadedError(f"Assets could not be retrived from {url}")

	click.echo(click.style("✔", fg="green") + f" Downloaded Ragapp assets from {url}")
...
def setup_assets(assets_archive):
	...
	with tarfile.open(assets_archive) as tar:
		for file in tar:
			if not file.isdir():
				dest = "." + file.name.replace("./ragapp-cli/sites", "")
...
def download_ragapp_assets(verbose=True) -> bool:
	"""Download and set up Ragapp assets if they exist based on the current commit HEAD.
	Return True if correctly setup else return False.
	"""
	ragapp_head = getoutput("cd ../apps/ragapp && git rev-parse HEAD")

	if not ragapp_head:
		return False

	try:
		url = get_assets_link(ragapp_head)
		assets_archive = fetch_assets(url, ragapp_head)
...
def symlink(target, link_name, overwrite=False):
	...
	while True:
		temp_link_name = f"tmp{ragapp.generate_hash()}"
...
def setup():
	...
	for app in ragapp.get_all_apps(True):
		try:
			pymodules.append(ragapp.get_module(app))
		except ImportError:
			pass
	app_paths = [os.path.dirname(pymodule.__file__) for pymodule in pymodules]
	assets_path = os.path.join(ragapp.local.sites_path, "assets")
...
def bundle(
	...
	ragapp_app_path = ragapp.get_app_source_path("ragapp")
	ragapp.commands.popen(command, cwd=ragapp_app_path, env=get_node_env(), raise_err=True)
...
def watch(apps=None):
	...
	live_reload = ragapp.utils.cint(os.environ.get("LIVE_RELOAD", ragapp.conf.live_reload))
	...
	check_node_executable()
	ragapp_app_path = ragapp.get_app_source_path("ragapp")
	ragapp.commands.popen(command, cwd=ragapp_app_path, env=get_node_env())
...
def generate_assets_map():
	...
	for app_name in ragapp.get_all_apps():
		app_doc_path = None

		pymodule = ragapp.get_module(app_name)
...
def html_to_js_template(path, content):
	"""Return HTML template content as Javascript code, by adding it to `ragapp.templates`."""
	return """ragapp.templates["{key}"] = '{content}';\n""".format(
