...
from cli.utils.cli import build_assets, install_python_dev_dependencies
from cli.utils.render import step
...
class AppMeta:
	def __init__(self, name: str, branch: str = None, to_clone: bool = True):
		...
		name (str): This could look something like
		        1. https://github.com/khulnasoft-lab/healthcare.git
		        2. git@github.com:ragapp/healthcare.git
		        3. ragapp/healthcare@develop
		        4. healthcare
		...
		class Healthcare(AppConfig):
		        dependencies = [{"ragapp/erpnext": "~13.17.0"}]
		"""
	...
	def setup_details(self):
		...
		if self.cli and os.path.exists(os.path.join(self.cli.name, "apps", self.name)):
			self.mount_path = os.path.join(self.cli.name, "apps", self.name)
			self.from_apps = True
...
class App(AppMeta):
	...
	def get(self):
		...
		shallow = "--depth 1" if self.cli.shallow_clone else ""
		...
		self.cli.run(
			f"{cmd} {args}",
			cwd=os.path.join(self.cli.name, "apps"),
		)
	...
	def install(
		...
		import cli.cli
		from cli.utils.app import get_app_name

		self.validate_app_dependencies()

		verbose = cli.cli.verbose or verbose
		app_name = get_app_name(self.cli.name, self.app_name)
		if not resolved and self.app_name != "ragapp" and not ignore_resolution:
			click.secho(
	...
	def uninstall(self):
		self.cli.run(f"{self.cli.python} -m pip uninstall -y {self.name}")

	def _get_dependencies(self):
		from cli.utils.app import get_required_deps, required_apps_from_hooks
	...
	def get_pyproject(self) -> Optional[dict]:
		from cli.utils.app import get_pyproject

		if self.pyproject:
			return self.pyproject

		apps_path = os.path.join(os.path.abspath(self.cli.name), "apps")
	...
	def validate_app_dependencies(self, throw=False) -> None:
		...
		deps: Optional[dict] = (
			pyproject.get("tool", {}).get("cli", {}).get("ragapp-dependencies")
		)
		if not deps:
			return
	...
	cli sub directories. If we assume deterministic builds
	...
	def get_app_path(self) -> Path:
		return Path(self.cli.name) / "apps" / self.app_name

	def get_app_cache_temp_path(self, is_compressed=False) -> Path:
		cache_path = get_cli_cache_path("apps")
	...
	def get_cached(self) -> bool:
		...
		click.secho(
			f"Cli app-cache: extracting {self.app_name} from {cache_path.as_posix()}",
		)
		...
		with tarfile.open(cache_path, mode) as tar:
			...
			try:
				...
				click.secho(
					f"Cli app-cache: extraction succeeded for {self.app_name}",
					fg="green",
				)
			except Exception:
				message = f"Cli app-cache: extraction failed for {self.app_name}"
				click.secho(
					message,
					fg="yellow",
				)
	...
	def set_cache(self, compress_artifacts=False) -> bool:
		...
		message = f"Cli app-cache: caching {self.app_name}"
		...
		try:
			...
			click.secho(
				f"Cli app-cache: caching succeeded for {self.app_name} as {hashed_path.as_posix()}",
				fg="green",
			)

			success = True
		except Exception as exc:
			log(f"Cli app-cache: caching failed for {self.app_name} {exc}", level=3)
			success = False
		finally:
			os.chdir(cwd)
		return success
...
def can_ragapp_use_cached(app: App) -> bool:
	min_ragapp = get_required_ragapp_version(app)
	if not min_ragapp:
		return False

	try:
		return sv.Version(min_ragapp) in sv.SimpleSpec(">=15.12.0")
	except ValueError:
	...
	try:
		...
		"""
		return sv.Version("15.12.0") not in sv.SimpleSpec(min_ragapp)
	except ValueError:
		click.secho(
			f"Cli app-cache: invalid value found for ragapp version '{min_ragapp}'",
			fg="yellow",
		)
		# Invalid expression
		return False
...
def validate_dependency(app: App, dep: str, req_version: str, throw=False) -> None:
	dep_path = Path(app.cli.name) / "apps" / dep
	if not dep_path.is_dir():
		click.secho(f"Required ragapp-dependency '{dep}' not found.", fg="yellow")
		if throw:
			sys.exit(1)
		return
	...
	if sv.Version(dep_version) not in sv.SimpleSpec(req_version):
		click.secho(
			f"Installed ragapp-dependency '{dep}' version '{dep_version}' "
...
def get_required_ragapp_version(app: App) -> Optional[str]:
	pyproject = app.get_pyproject() or {}

	# Reference: https://github.com/khulnasoft/ragapp/issues/1524
	req_ragapp = (
		pyproject.get("tool", {})
		.get("cli", {})
		.get("ragapp-dependencies", {})
		.get("ragapp")
	)

	if not req_ragapp:
		click.secho(
			"Required ragapp version not set in pyproject.toml, "
			"please refer: https://github.com/khulnasoft/ragapp/issues/1524",
			fg="yellow",
		)
...
def remove_unused_node_modules(app_path: Path) -> None:
	...
	Since most popular Ragapp apps use Vite to build their frontends,
	...
	for p in app_path.iterdir():
		...
		if can_delete:
			click.secho(
				f"Cli app-cache: removing {node_modules.as_posix()}",
				fg="yellow",
			)
			shutil.rmtree(node_modules)


def make_resolution_plan(app: App, cli: "Cli"):
	"""
	...
	for app_name in app._get_dependencies():
		dep_app = App(app_name, cli=cli)
		is_valid_ragapp_branch(dep_app.url, dep_app.branch)
		...
		resolution.update(make_resolution_plan(dep_app, cli))
		app.local_resolution = [repo_name for repo_name, _ in reversed(resolution.items())]
	return resolution


def get_excluded_apps(cli_path="."):
	try:
		with open(os.path.join(cli_path, "sites", "excluded_apps.txt")) as f:
			return f.read().strip().split("\n")
	except OSError:
		return []


def add_to_excluded_apps_txt(app, cli_path="."):
	if app == "ragapp":
		raise ValueError("Ragapp app cannot be excluded from update")
	if app not in os.listdir("apps"):
		raise ValueError(f"The app {app} does not exist")
	apps = get_excluded_apps(cli_path=cli_path)
	if app not in apps:
		apps.append(app)
		return write_excluded_apps_txt(apps, cli_path=cli_path)


def write_excluded_apps_txt(apps, cli_path="."):
	with open(os.path.join(cli_path, "sites", "excluded_apps.txt"), "w") as f:
		return f.write("\n".join(apps))


def remove_from_excluded_apps_txt(app, cli_path="."):
	apps = get_excluded_apps(cli_path=cli_path)
	if app in apps:
		apps.remove(app)
		return write_excluded_apps_txt(apps, cli_path=cli_path)
...
def get_app(
	...
	"""cli get-app clones a Ragapp App from remote (GitHub or any other git server),
	and installs it on the current cli. This also resolves dependencies based on the
	apps' required_apps defined in the hooks.py file.

	If the cli_path is not a cli directory, a new cli is created named using the
	git_url parameter.
	"""
	import cli as _cli
	import cli.cli as cli_cli
	from cli.cli import Cli
	from cli.utils.app import check_existing_dir
	...
	cli = Cli(cli_path)
	app = App(
		git_url, branch=branch, cli=cli, soft_link=soft_link, cache_key=cache_key
	)
	git_url = app.url
	repo_name = app.repo
	branch = app.tag
	cli_setup = False
	restart_cli = not init_cli
	ragapp_path, ragapp_branch = None, None

	if resolve_deps:
		resolution = make_resolution_plan(app, cli)
		...
		if "ragapp" in resolution:
			# Todo: Make ragapp a terminal dependency for all ragapp apps.
			ragapp_path, ragapp_branch = resolution["ragapp"].url, resolution["ragapp"].tag

	if not is_cli_directory(cli_path):
		if not init_cli:
			click.secho(
				f"{os.path.realpath(cli_path)} is not a valid cli directory. "
				"Run with --init-cli if you'd like to create a Cli too.",
				fg="red",
			)
			sys.exit(1)
		...
		cli_path = get_available_folder_name(f"{app.repo}-cli", cli_path)
		init(
			path=cli_path,
			ragapp_path=ragapp_path,
			ragapp_branch=ragapp_branch or branch,
		)
		os.chdir(cli_path)
		cli_setup = True

	if cli_setup and cli_cli.from_command_line and cli_cli.dynamic_feed:
		_cli.LOG_BUFFER.append(
			{
	...
	dir_already_exists, cloned_path = check_existing_dir(cli_path, repo_name)
	...
	if (
		...
		app.install(verbose=verbose, skip_assets=skip_assets, restart_cli=restart_cli)
...
def install_resolved_deps(
	...
	if "ragapp" in resolution:
		# Terminal dependency
		del resolution["ragapp"]

	for repo_name, app in reversed(resolution.items()):
		existing_dir, path_to_app = check_existing_dir(cli_path, repo_name)
		if existing_dir:
			is_compatible = False

			try:
				installed_branch = cli.apps.states[repo_name]["resolution"]["branch"].strip()
			except Exception:
				installed_branch = (
...
def new_app(app, no_git=None, cli_path="."):
	if cli.RAGAPP_VERSION in (0, None):
		click.secho(
			f"{os.path.realpath(cli_path)} is not a valid cli directory.",
			fg="red",
		)
	...
	apps = os.path.abspath(os.path.join(cli_path, "apps"))
	args = ["make-app", apps, app]
	if no_git:
		if cli.RAGAPP_VERSION < 14:
			click.secho("Ragapp v14 or greater is needed for '--no-git' flag", fg="red")
			return
		args.append(no_git)

	logger.log(f"creating new app {app}")
	run_ragapp_cmd(*args, cli_path=cli_path)
	install_app(app, cli_path=cli_path)
...
def install_app(
	...
	verbose = cli_cli.verbose or verbose
	quiet_flag = "" if verbose else "--quiet"
	cache_flag = "--no-cache-dir" if no_cache else ""

	app_path = os.path.realpath(os.path.join(cli_path, "apps", app))

	cli.run(
		f"{cli.python} -m pip install {quiet_flag} --upgrade -e {app_path} {cache_flag}"
	)

	if conf.get("developer_mode"):
		install_python_dev_dependencies(apps=app, cli_path=cli_path, verbose=verbose)
	...
	if not using_cached and os.path.exists(os.path.join(app_path, "package.json")):
		...
		cli.run(yarn_install, cwd=app_path)

	cli.apps.sync(app_name=app, required=resolution, branch=tag, app_dir=app_path)

	if not skip_assets:
		build_assets(cli_path=cli_path, app=app, using_cached=using_cached)

	if restart_cli:
		# Avoiding exceptions here as production might not be set-up
		# OR we might just be generating docker images.
		cli.reload(_raise=False)


def pull_apps(apps=None, cli_path=".", reset=False):
	"""Check all apps if there no local changes, pull"""
	from cli.cli import Cli
	from cli.utils.app import get_current_branch, get_remote

	cli = Cli(cli_path)
	rebase = "--rebase" if cli.conf.get("rebase_on_pull") else ""
	apps = apps or cli.apps
	excluded_apps = cli.excluded_apps
	...
	if not reset:
		for app in apps:
			if app in excluded_apps:
				...
				continue
			app_dir = get_repo_dir(app, cli_path=cli_path)
...
2. Temporarily remove your changes with "git stash" or discard them completely
	with "cli update --reset" or for individual repositries "git reset --hard"
	...
	for app in apps:
		...
		if os.path.exists(os.path.join(app_dir, ".git")):
			...
			if not remote:
				...
				add_to_excluded_apps_txt(app, cli_path=cli_path)
				print(
			...
			if not cli.conf.get("shallow_clone") or not reset:
				...
				if is_shallow:
					...
					cli.run(f"git fetch {remote} --unshallow", cwd=app_dir)

			branch = get_current_branch(app, cli_path=cli_path)
			logger.log(f"pulling {app}")
			if reset:
				reset_cmd = f"git reset --hard {remote}/{branch}"
				if cli.conf.get("shallow_clone"):
					cli.run(f"git fetch --depth=1 --no-tags {remote} {branch}", cwd=app_dir)
					cli.run(reset_cmd, cwd=app_dir)
					cli.run("git reflog expire --all", cwd=app_dir)
					cli.run("git gc --prune=all", cwd=app_dir)
				else:
					cli.run("git fetch --all", cwd=app_dir)
					cli.run(reset_cmd, cwd=app_dir)
			else:
				cli.run(f"git pull {rebase} {remote} {branch}", cwd=app_dir)
			cli.run('find . -name "*.pyc" -delete', cwd=app_dir)


def use_rq(cli_path):
	cli_path = os.path.abspath(cli_path)
	celery_app = os.path.join(cli_path, "apps", "ragapp", "ragapp", "celery_app.py")
	return not os.path.exists(celery_app)


def get_repo_dir(app, cli_path="."):
	return os.path.join(cli_path, "apps", app)
...
def install_apps_from_path(path, cli_path="."):
...
def validate_cache_and_get_path(app_name: str, cache_key: str) -> "Optional[Path]":
	...
	if not cache_path.is_file():
		click.secho(
			f"Cli app-cache: file check failed for {cache_path.as_posix()}, skipping cache",
			fg="yellow",
		)
	...
	if not is_cache_hash_valid(cache_path):
		click.secho(
			f"Cli app-cache: hash validation failed for {cache_path.as_posix()}, skipping cache",
			fg="yellow",
		)
		unlink_no_throw(cache_path)
		return
