...
from cli.config.common_site_config import update_config, put_config

# imports - third party imports
import click


@click.group(help="Change cli configuration")
def config():
	pass
...
@click.command(
	"dns_multitenant", help="Enable/Disable cli multitenancy on running cli update"
)
@click.argument("state", type=click.Choice(["on", "off"]))
...
def set_common_config(configs):
	...
	update_config(common_site_config, cli_path=".")


@click.command(
	"remove-common-config", help="Remove specific keys from current cli's common config"
)
@click.argument("keys", nargs=-1)
def remove_common_config(keys):
	from cli.cli import Cli

	common_site_config = Cli(".").conf
	for key in keys:
