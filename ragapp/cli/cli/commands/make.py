...
@click.command("init", help="Initialize a new cli instance in the specified path")
@click.argument("path")
@click.option(
	"--version",
	"--ragapp-branch",
	"ragapp_branch",
	default=None,
	help="Clone a particular branch of ragapp",
)
@click.option(
	"--ignore-exist", is_flag=True, default=False, help="Ignore if Cli instance exists."
)
@click.option(
...
@click.option("--ragapp-path", default=None, help="path to ragapp repo")
...
@click.option(
	"--no-backups",
	is_flag=True,
	help="Do not set up automatic periodic backups for all sites on this cli",
)
@click.option(
...
def init(
	...
	if not ignore_exist and os.path.exists(path):
		log(f"Cli instance already exists at {path}", level=2)
		return
	...
	try:
		...
		log(f"Cli {path} initialized", level=1)
	except SystemExit:
		raise
	except Exception:
		import shutil
		...
		from cli.utils import get_traceback
...
def drop(path):
	...
	from cli.exceptions import CliNotFoundError, ValidationError

	cli = Cli(path)

	if not cli.exists:
		raise CliNotFoundError(f"Cli {cli.name} does not exist")

	if cli.sites:
		raise ValidationError("Cannot remove non-empty cli directory")
...
@click.command(
	["get", "get-app"],
	help="Clone an app from the internet or filesystem and set it up in your cli",
)
...
@click.option(
	"--init-cli", is_flag=True, default=False, help="Initialize Cli if not in one"
)
@click.option(
	"--resolve-deps",
	is_flag=True,
...
def get_app(
	...
	"clone an app from the internet and set it up in your cli"
	from cli.app import get_app
...
@click.command("new-app", help="Create a new Ragapp application under apps folder")
@click.option(
	"--no-git",
	is_flag=True,
	flag_value="--no-git",
	help="Do not initialize git repository for the app (available in Ragapp v14+)",
)
@click.argument("app-name")
def new_app(app_name, no_git=None):
	from cli.app import new_app
...
@click.command(
	...
	help=(
		"Completely remove app from cli and re-build assets if not installed on any site"
	),
)
...
def remove_app(app_name, no_backup=False, force=False):
	...
	cli = Cli(".")
	cli.uninstall(app_name, no_backup=no_backup, force=force)


@click.command("exclude-app", help="Exclude app from updating")
@click.argument("app_name")
def exclude_app_for_update(app_name):
	from cli.app import add_to_excluded_apps_txt
...
def include_app_for_update(app_name):
	...
	from cli.app import remove_from_excluded_apps_txt
...
@click.command(
	...
	help="For pip help use `cli pip help [COMMAND]` or `cli pip [COMMAND] -h`",
)
@click.argument("args", nargs=-1)
@click.pass_context
def pip(ctx, args):
	"Run pip commands in cli env"
	import os
...
@click.command(
	...
	help="Validates that all requirements specified in ragapp-dependencies are met curently.",
)
@click.pass_context
def validate_dependencies(ctx):
	"Validate all specified ragapp-dependencies."
	from cli.cli import Cli
	from cli.app import App

	cli = Cli(".")

	for app_name in cli.apps:
		app = App(app_name, cli=cli)
