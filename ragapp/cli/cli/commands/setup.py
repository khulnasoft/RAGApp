...
from cli.utils import exec_cmd, run_playbook, which
from cli.utils.cli import SugaredOption


@click.group(help="Setup command group for enabling setting up a Ragapp environment")
def setup():
	pass
...
def setup_sudoers(user):
	from cli.utils.system import setup_sudoers
...
def setup_nginx(yes=False, logging="combined", log_format=None):
	...
	make_nginx_conf(cli_path=".", yes=yes, logging=logging, log_format=log_format)


@click.command("reload-nginx", help="Checks NGINX config file and reloads service")
def reload_nginx():
	from cli.config.production_setup import reload_nginx
...
def setup_supervisor(user=None, yes=False, skip_redis=False, skip_supervisord=False):
	from cli.utils import get_cmd_output
	from cli.config.supervisor import (
	...
	generate_supervisor_config(cli_path=".", user=user, yes=yes, skip_redis=skip_redis)
...
@click.command("fonts", help="Add Ragapp fonts to system")
def setup_fonts():
	from cli.utils.system import setup_fonts

	setup_fonts()


@click.command(
	"production", help="Setup Ragapp production environment for specific user"
)
@click.argument("user")
@click.option("--yes", help="Yes to regeneration config", is_flag=True, default=False)
def setup_production(user, yes=False):
	from cli.config.production_setup import setup_production

	setup_production(user=user, yes=yes)


@click.command("backups", help="Add cronjob for cli backups")
def setup_backups():
...
@click.command("env", help="Setup Python environment for cli")
@click.option(
...
def setup_env(python="python3"):
	...
	return Cli(".").setup.env(python=python)
...
def setup_firewall(ssh_port=None, force=False):
	...
	run_playbook("roles/cli/tasks/setup_firewall.yml", {"ssh_port": ssh_port})
...
def set_ssh_port(port, force=False):
	...
	run_playbook("roles/cli/tasks/change_ssh_port.yml", {"ssh_port": port})
...
def setup_letsencrypt(site, custom_domain, non_interactive):
	from cli.config.lets_encrypt import setup_letsencrypt

	setup_letsencrypt(site, custom_domain, cli_path=".", interactive=not non_interactive)


@click.command(
	"wildcard-ssl", help="Setup wildcard SSL certificate for multi-tenant cli"
)
...
def setup_wildcard_ssl(domain, email, exclude_base_domain):
	from cli.config.lets_encrypt import setup_wildcard_ssl

	setup_wildcard_ssl(
		domain, email, cli_path=".", exclude_base_domain=exclude_base_domain
	)


@click.command("procfile", help="Generate Procfile for cli start")
def setup_procfile():
...
def setup_requirements(node=False, python=False, dev=False, apps=None):
	...
	if not (node or python or dev):
		cli.setup.requirements(apps=apps)

	elif not node and not dev:
		cli.setup.python(apps=apps)

	elif not python and not dev:
		cli.setup.node(apps=apps)

	else:
		from cli.utils.cli import install_python_dev_dependencies
...
@click.command(
	"manager",
	help="Setup cli-manager.local site with the cli_manager app installed on it",
)
@click.option(
	"--yes", help="Yes to regeneration of nginx config file", default=False, is_flag=True
)
@click.option(
	"--port", help="Port on which you want to run cli manager", default=23624
)
@click.option("--domain", help="Domain on which you want to run cli manager")
def setup_manager(yes=False, port=23624, domain=None):
	from cli.cli import Cli
	from cli.config.nginx import make_cli_manager_nginx_conf

	create_new_site = True

	if "cli-manager.local" in os.listdir("sites"):
		create_new_site = click.confirm("Site already exists. Overwrite existing site?")

	if create_new_site:
		exec_cmd("cli new-site --force cli-manager.local")

	if "cli_manager" in os.listdir("apps"):
		print("App already exists. Skipping app download.")
	else:
		exec_cmd("cli get-app cli_manager")

	exec_cmd("cli --site cli-manager.local install-app cli_manager")

	cli_path = "."
	cli = Cli(cli_path)

	if cli.conf.get("restart_supervisor_on_update") or cli.conf.get(
		"restart_systemd_on_update"
	):
		# implicates a production setup or so I presume
		if not domain:
			print(
				"Please specify the site name on which you want to host cli-manager using the 'domain' flag"
			)
			sys.exit(1)

		if domain not in cli.sites:
			raise Exception("No such site")

		make_cli_manager_nginx_conf(cli_path, yes=yes, port=port, domain=domain)
...
def add_domain(domain, site=None, ssl_certificate=None, ssl_certificate_key=None):
	...
	from cli.config.site_config import add_domain

	add_domain(site, domain, ssl_certificate, ssl_certificate_key, cli_path=".")
...
def remove_domain(domain, site=None):
	...
	from cli.config.site_config import remove_domain

	remove_domain(site, domain, cli_path=".")
...
def sync_domains(domain=None, site=None):
	...
	from cli.config.site_config import sync_domains

	changed = sync_domains(site, domains, cli_path=".")
...
@click.option("--stop", help="Stop cli services", is_flag=True, default=False)
...
def setup_systemd(
	...
	from cli.config.systemd import generate_systemd_config
