...
from cli.app import pull_apps
from cli.utils.cli import post_upgrade, patch_sites, build_assets


@click.command(
	"update",
	help="Performs an update operation on current cli. Without any flags will backup, pull, setup requirements, build, run patches and restart cli. Using specific flags will only do certain tasks instead of all",
)
@click.option("--pull", is_flag=True, help="Pull updates for all the apps in cli")
@click.option("--apps", type=str)
@click.option("--patch", is_flag=True, help="Run migrations for all sites in the cli")
@click.option("--build", is_flag=True, help="Build JS and CSS assets for the cli")
@click.option(
	"--requirements",
	is_flag=True,
	help="Update requirements. If run alone, equivalent to `cli setup requirements`",
)
@click.option(
...
def update(
	...
	from cli.utils.cli import update
...
def switch_to_branch(branch, apps, upgrade=False):
	from cli.utils.app import switch_to_branch
...
def switch_to_develop(upgrade=False):
	"Switch ragapp and erpnext to develop branch"
	from cli.utils.app import switch_to_develop

	switch_to_develop(apps=["ragapp", "erpnext"])
