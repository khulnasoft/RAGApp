...
@click.command("start", help="Start Ragapp development processes")
@click.option("--no-dev", is_flag=True, default=False)
@click.option(
	"--no-prefix",
	is_flag=True,
	default=False,
	help="Hide process name from cli start log",
)
...
def start(no_dev, concurrency, procfile, no_prefix, man):
	from cli.utils.system import start
...
def restart(web, supervisor, systemd):
	...
	Cli(".").reload(web, supervisor, systemd)
...
def set_nginx_port(site, port):
	from cli.config.site_config import set_nginx_port
...
def set_ssl_certificate(site, ssl_certificate_path):
	from cli.config.site_config import set_ssl_certificate
...
def set_ssl_certificate_key(site, ssl_certificate_key_path):
	from cli.config.site_config import set_ssl_certificate_key
...
def set_url_root(site, url_root):
	from cli.config.site_config import set_url_root

	set_url_root(site, url_root)


@click.command("set-mariadb-host", help="Set MariaDB host for cli")
@click.argument("host")
def set_mariadb_host(host):
	from cli.utils.cli import set_mariadb_host

	set_mariadb_host(host)


@click.command("set-redis-cache-host", help="Set Redis cache host for cli")
@click.argument("host")
def set_redis_cache_host(host):
	"""
	Usage: cli set-redis-cache-host localhost:6379/1
	"""
	from cli.utils.cli import set_redis_cache_host

	set_redis_cache_host(host)


@click.command("set-redis-queue-host", help="Set Redis queue host for cli")
@click.argument("host")
def set_redis_queue_host(host):
	"""
	Usage: cli set-redis-queue-host localhost:6379/2
	"""
	from cli.utils.cli import set_redis_queue_host

	set_redis_queue_host(host)


@click.command("set-redis-socketio-host", help="Set Redis socketio host for cli")
@click.argument("host")
def set_redis_socketio_host(host):
	"""
	Usage: cli set-redis-socketio-host localhost:6379/3
	"""
	from cli.utils.cli import set_redis_socketio_host
...
def download_translations():
	from cli.utils.translation import download_translations_p
...
def renew_lets_encrypt():
	from cli.config.lets_encrypt import renew_certs

	renew_certs()


@click.command("backup-all-sites", help="Backup all sites in current cli")
def backup_all_sites():
	from cli.utils.system import backup_all_sites

	backup_all_sites(cli_path=".")


@click.command(
	"disable-production", help="Disables production environment for the cli."
)
def disable_production():
	from cli.config.production_setup import disable_production

	disable_production(cli_path=".")


@click.command(
	"src", help="Prints cli source folder path, which can be used as: cd `cli src`"
)
def cli_src():
	from cli.cli import src

	print(os.path.dirname(src))


@click.command("find", help="Finds clies recursively from location")
@click.argument("location", default="")
def find_clies(location):
	from cli.utils import find_clies

	find_clies(directory=location)
...
def migrate_env(python, backup=True):
	from cli.utils.cli import migrate_env

	migrate_env(python=python, backup=backup)


@click.command("app-cache", help="View or remove items belonging to cli get-app cache")
...
def app_cache_helper(clear=False, remove_app="", remove_key=""):
	from cli.utils.cli import cache_helper
