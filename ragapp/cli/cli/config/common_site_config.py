...
def setup_config(cli_path, additional_config=None):
	make_pid_folder(cli_path)
	cli_config = get_config(cli_path)
	cli_config.update(default_config)
	cli_config.update(get_gunicorn_workers())
	update_config_for_ragapp(cli_config, cli_path)
	if additional_config:
		cli_config.update(additional_config)

	put_config(cli_config, cli_path)


def get_config(cli_path):
	return get_common_site_config(cli_path)


def get_common_site_config(cli_path):
	config_path = get_config_path(cli_path)
...
def put_config(config, cli_path="."):
	config_path = get_config_path(cli_path)
	with open(config_path, "w") as f:
		return json.dump(config, f, indent=1, sort_keys=True)


def update_config(new_config, cli_path="."):
	config = get_config(cli_path=cli_path)
	config.update(new_config)
	put_config(config, cli_path=cli_path)


def get_config_path(cli_path):
	return os.path.join(cli_path, "sites", "common_site_config.json")
...
def update_config_for_ragapp(config, cli_path):
	ports = make_ports(cli_path)
...
def make_ports(cli_path):
	from urllib.parse import urlparse

	clies_path = os.path.dirname(os.path.abspath(cli_path))
	...
	existing_ports = {}
	for folder in os.listdir(clies_path):
		cli_path = os.path.join(clies_path, folder)
...
def make_pid_folder(cli_path):
	pids_path = os.path.join(cli_path, "config", "pids")
