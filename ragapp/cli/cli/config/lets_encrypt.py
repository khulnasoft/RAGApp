...
from cli.config.production_setup import service
from cli.config.site_config import get_domains, remove_domain, update_site_config
from cli.cli import Cli
from cli.utils import exec_cmd, which
from cli.utils.cli import update_common_site_config
from cli.exceptions import CommandFailedError


def setup_letsencrypt(site, custom_domain, cli_path, interactive):

	site_path = os.path.join(cli_path, "sites", site, "site_config.json")
	...
	if not Cli(cli_path).conf.get("dns_multitenant"):
		print("You cannot setup SSL without DNS Multitenancy")
		return

	create_config(site, custom_domain)
	run_certbot_and_setup_ssl(site, custom_domain, cli_path, interactive)
	setup_crontab()
...
def run_certbot_and_setup_ssl(site, custom_domain, cli_path, interactive=True):
	service("nginx", "stop")
	...
	if custom_domain:
		remove_domain(site, custom_domain, cli_path)
		domains = get_domains(site, cli_path)
		ssl_config["domain"] = custom_domain
		domains.append(ssl_config)
		update_site_config(site, {"domains": domains}, cli_path=cli_path)
	else:
		update_site_config(site, ssl_config, cli_path=cli_path)

	make_nginx_conf(cli_path)
	service("nginx", "start")
...
def setup_wildcard_ssl(domain, email, cli_path, exclude_base_domain):
