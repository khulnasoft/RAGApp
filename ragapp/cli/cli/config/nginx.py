...
from cli.utils import get_cli_name


def make_nginx_conf(cli_path, yes=False, logging=None, log_format=None):
	conf_path = os.path.join(cli_path, "config", "nginx.conf")
	...
	if allow_rate_limiting:
		template_vars.update(
			{
				"cli_name_hash": hashlib.sha256(cli_name).hexdigest()[:16],
...
def make_cli_manager_nginx_conf(cli_path, yes=False, port=23624, domain=None):
	from cli.config.site_config import get_site_config

	template = cli.config.env().get_template("cli_manager_nginx.conf")
	cli_path = os.path.abspath(cli_path)
	sites_path = os.path.join(cli_path, "sites")

	config = Cli(cli_path).conf
	site_config = get_site_config(domain, cli_path=cli_path)
	cli_name = get_cli_name(cli_path)

	template_vars = {
		"port": port,
		"domain": domain,
		"cli_manager_site_name": "cli-manager.local",
		"sites_path": sites_path,
		"http_timeout": config.get("http_timeout"),
		"webserver_port": config.get("webserver_port"),
		"socketio_port": config.get("socketio_port"),
	...
	cli_manager_nginx_conf = template.render(**template_vars)

	conf_path = os.path.join(cli_path, "config", "nginx.conf")

	if not yes and os.path.exists(conf_path):
		click.confirm(
			"nginx.conf already exists and cli-manager configuration will be appended to it. Do you want to continue?",
			abort=True,
		)

	with open(conf_path, "a") as myfile:
		myfile.write(cli_manager_nginx_conf)


def prepare_sites(config, cli_path):
	sites = {
		"that_use_port": [],
		"that_use_dns": [],
		"that_use_ssl": [],
	...
	sites_configs = get_sites_with_config(cli_path=cli_path)
...
def get_sites_with_config(cli_path):
	from cli.cli import Cli
	from cli.config.site_config import get_site_config
	...
	for site in sites:
		try:
			site_config = get_site_config(site, cli_path=cli_path)
	...
	use_wildcard_certificate(cli_path, ret)

	return ret


def use_wildcard_certificate(cli_path, ret):
	"""
...
def get_error_pages():
	cli_app_path = os.path.abspath(cli.__path__[0])
	templates = os.path.join(cli_app_path, "config", "templates")
