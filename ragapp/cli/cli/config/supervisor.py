...
from cli.utils import get_cli_name, which

logger = logging.getLogger(cli.PROJECT_NAME)


def generate_supervisor_config(cli_path, user=None, yes=False, skip_redis=False):
	"""Generate supervisor config for respective cli path"""
	if not user:
		user = getpass.getuser()

	config = Cli(cli_path).conf
	template = cli.config.env().get_template("supervisor.conf")
	...
	config = template.render(
		**{
			...
			"multi_queue_consumption": can_enable_multi_queue_consumption(cli_path),
			"supervisor_startretries": 10,
		}
	)

	conf_path = os.path.join(cli_path, "config", "supervisor.conf")
	...
	update_config({"restart_supervisor_on_update": True}, cli_path=cli_path)
	update_config({"restart_systemd_on_update": False}, cli_path=cli_path)
	sync_socketio_port(cli_path)
...
def can_enable_multi_queue_consumption(cli_path: str) -> bool:
	try:
		...
		ragapp_version = Version(get_current_version("ragapp", cli_path=cli_path))

		return ragapp_version > supported_version
	except Exception:
		return False


def check_supervisord_config(user=None):
	"""From cli v5.x, we're moving to supervisor running as user"""
	# i don't think cli should be responsible for this but we're way past this now...
	# removed updating supervisord conf & reload in Aug 2022 - gavin@ragapp.io
	import configparser
