...
from cli import PROJECT_NAME, VERSION
...
paths_in_cli = ("apps", "sites", "config", "logs", "config/pids")
sudoers_file = "/etc/sudoers.d/ragapp"
UNSET_ARG = object()


def is_cli_directory(directory=os.path.curdir):
	is_cli = True

	for folder in paths_in_cli:
		path = os.path.abspath(os.path.join(directory, folder))
		is_cli = is_cli and os.path.exists(path)
		# Once is_cli becomes false, it will always be false, even if other path exists.
		if not is_cli:
			break
...
def is_ragapp_app(directory: str) -> bool:
	is_ragapp_app = True

	for folder in paths_in_app:
		if not is_ragapp_app:
			break

		path = glob(os.path.join(directory, "**", folder))
		is_ragapp_app = is_ragapp_app and path

	return bool(is_ragapp_app)


def get_cli_cache_path(sub_dir: Optional[str]) -> Path:
	relative_path = "~/.cache/cli"
...
def is_valid_ragapp_branch(ragapp_path: str, ragapp_branch: str):
	...
	:param ragapp_path: git url
	:type ragapp_path: str
	:param ragapp_branch: branch to check
	:type ragapp_branch: str
	...
	if ragapp_branch:
		try:
			res = g.ls_remote("--heads", "--tags", ragapp_path, ragapp_branch)
			if not res:
				raise InvalidRemoteException(
					f"Invalid branch or tag: {ragapp_branch} for the remote {ragapp_path}"
				)
		except GitCommandError as e:
			raise InvalidRemoteException(f"Invalid ragapp path: {ragapp_path}") from e
...
def log(message, level=0, no_log=False, stderr=False):
	...
	if cli.cli.from_command_line and cli.cli.dynamic_feed:
		cli.LOG_BUFFER.append({"prefix": prefix, "message": message, "color": color})
...
def check_latest_version():
	...
	try:
		pypi_request = requests.get("https://pypi.org/pypi/ragapp-cli/json")
	except Exception:
	...
	if pypi_request.status_code == 200:
		...
		if pypi_version > local_version:
			log(
				f"A newer version of cli is available: {local_version} → {pypi_version}",
				stderr=True,
			)
...
def setup_logging(cli_path=".") -> logging.Logger:
	LOG_LEVEL = 15
	...
	if os.path.exists(os.path.join(cli_path, "logs")):
		log_file = os.path.join(cli_path, "logs", "cli.log")
...
def run_ragapp_cmd(*args, **kwargs):
	from cli.cli import from_command_line
	from cli.utils.cli import get_env_cmd

	cli_path = kwargs.get("cli_path", ".")
	f = get_env_cmd("python", cli_path=cli_path)
	sites_dir = os.path.join(cli_path, "sites")
	...
	p = subprocess.Popen(
		(f, "-m", "ragapp.utils.cli_helper", "ragapp") + args,
		cwd=sites_dir,
		stdout=stdout,
		stderr=stderr,
	)
...
def get_cli_name(cli_path):
	return os.path.basename(os.path.abspath(cli_path))


def set_git_remote_url(git_url, cli_path="."):
	"Set app remote git url"
	from cli.app import get_repo_dir
	from cli.cli import Cli

	app = git_url.rsplit("/", 1)[1].rsplit(".", 1)[0]

	if app not in Cli(cli_path).apps:
...
def run_playbook(playbook_name, extra_vars=None, tag=None):
	...
	subprocess.check_call(args, cwd=os.path.join(cli.__path__[0], "playbooks"))


def find_clies(directory: str = None) -> List:
	if not directory:
	...
	if is_cli_directory(directory):
		if os.path.curdir == directory:
			print("You are in a cli directory!")
		else:
			print(f"{directory} is a cli directory!")
		return
	...
	for sub in sub_directories:
		...
		if os.path.isdir(sub) and not os.path.islink(sub):
			if is_cli_directory(sub):
				print(f"{sub} found!")
				clies.append(sub)
			else:
				clies.extend(find_clies(sub))
...
def find_parent_cli(path: str) -> str:
	"""Checks if parent directories are clies"""
	if is_cli_directory(directory=path):
		return path
	...
	if path not in {home_path, root_path}:
		...
		return find_parent_cli(parent_dir)


def get_env_ragapp_commands(cli_path=".") -> List:
	"""Caches all available commands (even custom apps) via Ragapp
	Default caching behaviour: generated the first time any command (for a specific cli directory)
	"""
	from cli.utils.cli import get_env_cmd

	python = get_env_cmd("python", cli_path=cli_path)
	sites_path = os.path.join(cli_path, "sites")

	try:
		return json.loads(
			get_cmd_output(
				f"{python} -m ragapp.utils.cli_helper get-ragapp-commands", cwd=sites_path
			)
		)
...
def find_org(org_repo, using_cached: bool = False):
	...
	raise InvalidRemoteException(
		f"{org_repo} not found under ragapp or erpnext GitHub accounts"
	)
...
class _dict(dict):
	...
	# cli port of ragapp._dict
	def __getattr__(self, key):
		ret = self.get(key)
		# "__deepcopy__" exception added to fix ragapp#14833 via DFP
...
def get_cmd_from_sysargv():
	...
	For Command: `cli --profile --site ragappframework.com migrate --no-backup`
	sys.argv: ["/home/ragapp/.local/bin/cli", "--profile", "--site", "ragappframework.com", "migrate", "--no-backup"]
	Actual command run: migrate

	"""
	# context is passed as options to ragapp's cli_helper
	from cli.cli import Cli

	ragapp_context = _dict(params={"--site"}, flags={"--verbose", "--profile", "--force"})
	cmd_from_ctx = None
	sys_argv = sys.argv[1:]
	skip_next = False
	...
	for arg in sys_argv:
		...
		if arg in ragapp_context.flags:
			continue

		elif arg in ragapp_context.params:
			skip_next = True
			continue

		if sys_argv.index(arg) == 0 and arg in Cli(".").apps:
			continue
...
def get_app_cache_extract_filter(
	...
	def filter_function(member: TarInfo, dest_path: str) -> Optional[TarInfo]:
		...
		try:
			...
			# Links created by `ragapp` after extraction
			return None
