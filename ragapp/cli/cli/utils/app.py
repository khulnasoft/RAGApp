...
def is_version_upgrade(app="ragapp", cli_path=".", branch=None):
	upstream_version = get_upstream_version(app=app, branch=branch, cli_path=cli_path)
	...
	local_version = get_major_version(get_current_version(app, cli_path=cli_path))
...
def switch_branch(branch, apps=None, cli_path=".", upgrade=False, check_upgrade=True):
	import git
	from cli.cli import Cli
	from cli.utils import log, exec_cmd
	...
	apps_dir = os.path.join(cli_path, "apps")
	...
	for app in apps:
		...
		if check_upgrade:
			version_upgrade = is_version_upgrade(app=app, cli_path=cli_path, branch=branch)
	...
	if switched_apps:
		...
		print(
			"Please run `cli update --patch` to be safe from any differences in"
			" database schema"
		)

	if version_upgrade[0] and upgrade:
		Cli(cli_path).setup.requirements()
		backup_all_sites()
		patch_sites()
		build_assets()
...
def switch_to_branch(branch=None, apps=None, cli_path=".", upgrade=False):
	switch_branch(branch, apps=apps, cli_path=cli_path, upgrade=upgrade)


def switch_to_develop(apps=None, cli_path=".", upgrade=True):
	switch_branch("develop", apps=apps, cli_path=cli_path, upgrade=upgrade)
...
def get_develop_version(app, cli_path="."):
	repo_dir = get_repo_dir(app, cli_path=cli_path)
...
def get_upstream_version(app, branch=None, cli_path="."):
	repo_dir = get_repo_dir(app, cli_path=cli_path)
	if not branch:
...
def get_current_ragapp_version(cli_path="."):
	try:
		return get_major_version(get_current_version("ragapp", cli_path=cli_path))
	except OSError:
		return 0


def get_current_branch(app, cli_path="."):
...
def get_app_name(cli_path: str, folder_name: str) -> str:
	...
	"""
	app_name = None
	apps_path = os.path.join(os.path.abspath(cli_path), "apps")
...
def check_existing_dir(cli_path, repo_name):
	cloned_path = os.path.join(cli_path, "apps", repo_name)
	dir_already_exists = os.path.isdir(cloned_path)
	return dir_already_exists, cloned_path


def get_current_version(app, cli_path="."):
