...
import cli
from cli.exceptions import PatchError, ValidationError
...
def get_env_cmd(cmd: str, cli_path: str = ".") -> str:
	exact_location = os.path.abspath(
		os.path.join(cli_path, "env", "bin", cmd.strip("*"))
	)
	...
	existing_python_bins = glob(
		os.path.join(cli_path, "env", "**", "bin", cmd), recursive=True
	)
...
def update_node_packages(cli_path=".", apps=None, verbose=None):
	print("Updating node packages...")
	from distutils.version import LooseVersion

	from cli.utils.app import get_develop_version

	v = LooseVersion(get_develop_version("ragapp", cli_path=cli_path))

	# After rollup was merged, ragapp_version = 10.1
	# if develop_verion is 11 and up, only then install yarn
	if v < LooseVersion("11.x.x-develop"):
		update_npm_packages(cli_path, apps=apps, verbose=verbose)
	else:
		update_yarn_packages(cli_path, apps=apps, verbose=verbose)


def install_python_dev_dependencies(cli_path=".", apps=None, verbose=False):
	import cli.cli
	...
	if isinstance(apps, str):
		...
		apps = cli.get_installed_apps()

	for app in apps:
		pyproject_deps = None
		app_path = os.path.join(cli_path, "apps", app)
		...
		if os.path.exists(pyproject_path):
			...
			if pyproject_deps:
				cli.run(f"{cli.python} -m pip install {quiet_flag} --upgrade {pyproject_deps}")

		if not pyproject_deps and os.path.exists(dev_requirements_path):
			cli.run(
				f"{cli.python} -m pip install {quiet_flag} --upgrade -r {dev_requirements_path}"
			)
...
def _generate_dev_deps_pattern(pyproject_path):
	...
	with contextlib.suppress(KeyError):
		for pkg, version in pyproject_config["tool"]["cli"]["dev-dependencies"].items():
			op = "==" if "=" not in version else ""
			requirements_pattern += f"{pkg}{op}{version} "
	return requirements_pattern


def update_yarn_packages(cli_path=".", apps=None, verbose=None):
	...
	apps_dir = os.path.join(cli.name, "apps")
...
def update_npm_packages(cli_path=".", apps=None, verbose=None):
	verbose = cli.cli.verbose or verbose
	...
	with open(os.path.join(cli_path, "package.json"), "w") as f:
		f.write(json.dumps(package_json, indent=1, sort_keys=True))

	exec_cmd(npm_install, cwd=cli_path)
...
def validate_upgrade(from_ver, to_ver, cli_path="."):
	if to_ver >= 6 and not which("npm") and not which("node") and not which("nodejs"):
		raise Exception("Please install nodejs and npm")


def post_upgrade(from_ver, to_ver, cli_path="."):
	from cli.cli import Cli
	from cli.config import redis
	from cli.config.nginx import make_nginx_conf
	from cli.config.supervisor import generate_supervisor_config

	conf = Cli(cli_path).conf
	print("-" * 80 + f"Your cli was upgraded to version {to_ver}")

	if conf.get("restart_supervisor_on_update"):
		redis.generate_config(cli_path=cli_path)
		generate_supervisor_config(cli_path=cli_path)
		make_nginx_conf(cli_path=cli_path)
		print(
			"As you have setup your cli for production, you will have to reload"
...
def patch_sites(cli_path="."):
	from cli.cli import Cli
	from cli.utils.system import migrate_site

	cli = Cli(cli_path)

	for site in cli.sites:
		try:
			migrate_site(site, cli_path=cli_path)
		except subprocess.CalledProcessError:
			raise PatchError


def restart_supervisor_processes(cli_path=".", web_workers=False, _raise=False):
	...
	if cmd:
		cli.run(cmd, _raise=_raise)

	else:
		sudo = ""
		try:
			supervisor_status = get_cmd_output("supervisorctl status", cwd=cli_path)
		except subprocess.CalledProcessError as e:
			if e.returncode == 127:
				log("restart failed: Couldn't find supervisorctl in PATH", level=3)
				return
			sudo = "sudo "
			supervisor_status = get_cmd_output("sudo supervisorctl status", cwd=cli_path)
		...
		if web_workers and f"{cli_name}-web:" in supervisor_status:
			groups = [f"{cli_name}-web:\t"]

		elif f"{cli_name}-workers:" in supervisor_status:
			groups = [f"{cli_name}-web:", f"{cli_name}-workers:"]

		# backward compatibility
		elif f"{cli_name}-processes:" in supervisor_status:
			groups = [f"{cli_name}-processes:"]
		...
		for group in groups:
			failure = cli.run(f"{sudo}supervisorctl restart {group}", _raise=_raise)
			if failure:
				log(
					f"restarting supervisor group `{group}` failed. Use `cli restart` to retry.",
					level=3,
				)


def restart_systemd_processes(cli_path=".", web_workers=False, _raise=True):
	cli_name = get_cli_name(cli_path)
	exec_cmd(
		f"sudo systemctl stop -- $(systemctl show -p Requires {cli_name}.target | cut"
		" -d= -f2)",
		_raise=_raise,
	)
	exec_cmd(
		f"sudo systemctl start -- $(systemctl show -p Requires {cli_name}.target |"
		" cut -d= -f2)",
		_raise=_raise,
	)


def restart_process_manager(cli_path=".", web_workers=False):
	# only overmind has the restart feature, not sure other supported procmans do
	if which("overmind") and os.path.exists(os.path.join(cli_path, ".overmind.sock")):
		worker = "web" if web_workers else ""
		exec_cmd(f"overmind restart {worker}", cwd=cli_path)


def build_assets(cli_path=".", app=None, using_cached=False):
	command = "cli build"
	if app:
		command += f" --app {app}"
	...
	exec_cmd(command, cwd=cli_path, env=env)


def handle_version_upgrade(version_upgrade, cli_path, force, reset, conf):
	from cli.utils import log, pause_exec

	if version_upgrade[0]:
		if force:
			log(
				"""Force flag has been used for a major version change in Ragapp and it's apps.
This will take significant time to migrate and might break custom apps.""",
				level=3,
			)
		else:
			print(
				f"""This update will cause a major version change in Ragapp/NxERP from {version_upgrade[1]} to {version_upgrade[2]}.
This would take significant time to migrate and might break custom apps."""
	...
	if not reset and conf.get("shallow_clone"):
		log(
			"""shallow_clone is set in your cli config.
However without passing the --reset flag, your repositories will be unshallowed.
To avoid this, cancel this operation and run `cli update --reset`.
...
To avoid seeing this warning, set shallow_clone to false in your common_site_config.json
	...
	if version_upgrade[0] or (not version_upgrade[0] and force):
		validate_upgrade(version_upgrade[1], version_upgrade[2], cli_path=cli_path)
...
def update(
	...
	from cli.exceptions import CannotUpdateReleaseCli
	from cli.utils.app import is_version_upgrade
	from cli.utils.system import backup_all_sites

	cli_path = os.path.abspath(".")
	cli = Cli(cli_path)
	patches.run(cli_path=cli_path)
	conf = cli.conf

	if conf.get("release_cli"):
		raise CannotUpdateReleaseCli("Release cli detected, cannot update!")
	...
	handle_version_upgrade(version_upgrade, cli_path, force, reset, conf)

	conf.update({"maintenance_mode": 1, "pause_scheduler": 1})
	update_config(conf, cli_path=cli_path)

	if backup:
		print("Backing up sites...")
		backup_all_sites(cli_path=cli_path)

	if pull:
		print("Updating apps source...")
		pull_apps(apps=apps, cli_path=cli_path, reset=reset)
	...
	if version_upgrade[0] or (not version_upgrade[0] and force):
		post_upgrade(version_upgrade[1], version_upgrade[2], cli_path=cli_path)

	cli.reload(web=False, supervisor=restart_supervisor, systemd=restart_systemd)

	conf.update({"maintenance_mode": 0, "pause_scheduler": 0})
	update_config(conf, cli_path=cli_path)

	print(
		"_" * 80 + "\nCli: Deployment tool for Ragapp and Ragapp Applications"
...
def clone_apps_from(cli_path, clone_from, update_app=True):
	from cli.app import install_app

	print(f"Copying apps from {clone_from}...")
	subprocess.check_output(["cp", "-R", os.path.join(clone_from, "apps"), cli_path])

	node_modules_path = os.path.join(clone_from, "node_modules")
	if os.path.exists(node_modules_path):
		print(f"Copying node_modules from {clone_from}...")
		subprocess.check_output(["cp", "-R", node_modules_path, cli_path])
	...
	def setup_app(app):
		...
		install_app(app, cli_path, restart_cli=False)
...
def remove_backups_crontab(cli_path="."):
	...
	user = Cli(cli_dir).conf.get("ragapp_user")
...
def set_mariadb_host(host, cli_path="."):
	update_common_site_config({"db_host": host}, cli_path=cli_path)


def set_redis_cache_host(host, cli_path="."):
	update_common_site_config({"redis_cache": f"redis://{host}"}, cli_path=cli_path)


def set_redis_queue_host(host, cli_path="."):
	update_common_site_config({"redis_queue": f"redis://{host}"}, cli_path=cli_path)


def set_redis_socketio_host(host, cli_path="."):
	update_common_site_config({"redis_socketio": f"redis://{host}"}, cli_path=cli_path)


def update_common_site_config(ddict, cli_path="."):
	filename = os.path.join(cli_path, "sites", "common_site_config.json")
...
def validate_app_installed_on_sites(app, cli_path="."):
	print("Checking if app installed on active sites...")
	ret = check_app_installed(app, cli_path=cli_path)

	if ret is None:
		check_app_installed_legacy(app, cli_path=cli_path)
	else:
		return ret


def check_app_installed(app, cli_path="."):
	try:
		out = subprocess.check_output(
			["cli", "--site", "all", "list-apps", "--format", "json"],
...
def check_app_installed_legacy(app, cli_path="."):
	site_path = os.path.join(cli_path, "sites")

	for site in os.listdir(site_path):
		req_file = os.path.join(site_path, site, "site_config.json")
		if os.path.exists(req_file):
			out = subprocess.check_output(
				["cli", "--site", site, "list-apps"], cwd=cli_path
			).decode("utf-8")
...
def validate_branch():
	from cli.cli import Cli
	from cli.utils.app import get_current_branch

	apps = Cli(".").apps

	installed_apps = set(apps)
	check_apps = {"ragapp", "erpnext"}
...
To switch to your required branch, run the following commands: cli switch-to-branch [branch-name]"""
			)
...
def cache_list() -> None:
	...
	for item in get_cli_cache_path("apps").iterdir():
