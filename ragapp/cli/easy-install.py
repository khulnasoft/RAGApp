...
def clone_ragapp_docker_repo() -> None:
    try:
        urllib.request.urlretrieve(
            "https://github.com/khulnasoft/ragapp_docker/archive/refs/heads/main.zip",
            "ragapp_docker.zip",
        )
        logging.info("Downloaded ragapp_docker zip file from GitHub")
        unpack_archive("ragapp_docker.zip", ".")
        # Unzipping the ragapp_docker.zip creates a folder "ragapp_docker-main"
        move("ragapp_docker-main", "ragapp_docker")
        logging.info("Unzipped and Renamed ragapp_docker")
        os.remove("ragapp_docker.zip")
        logging.info("Removed the downloaded zip file")
    except Exception as e:
        logging.error("Download and unzip failed", exc_info=True)
        cprint("\nCloning ragapp_docker Failed\n\n", "[ERROR]: ", e, level=1)
...
def write_to_env(
    ...
    example_env = get_from_env(ragapp_docker_dir, "example.env")
    erpnext_version = erpnext_version or example_env["ERPNEXT_VERSION"]
    env_file_lines = [
        # defaults to latest version of NxERP
...
def get_ragapp_docker_path():
    return os.path.join(os.getcwd(), "ragapp_docker")


def check_repo_exists() -> bool:
    return os.path.exists(get_ragapp_docker_path())
...
def start_prod(
    ...
    ragapp_docker_dir = get_ragapp_docker_path()
    ...
    with open(compose_file_name, "w") as f:
        ...
        if not os.path.exists(env_file_path):
            ...
            write_to_env(
                ragapp_docker_dir=ragapp_docker_dir,
...
def setup_dev_instance(project: str):
    ...
    try:
        ...
        subprocess.run(
            command,
            cwd=get_ragapp_docker_path(),
            check=True,
        )
        cprint(
            "Please go through the Development Documentation: https://github.com/khulnasoft/ragapp_docker/tree/main/docs/development.md to fully complete the setup.",
            level=2,
        )
...
def create_site(
    ...
    try:
        ...
        logging.error(f"Cli site creation failed for {sitename}", exc_info=True)
        cprint(f"Cli Site creation failed for {sitename}\n", e)
...
def add_setup_options(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "-s",
        "--sitename",
        help="Site Name(s) for your production cli",
        default=[],
        action="append",
        dest="sites",
    )
...
def add_common_parser(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "-v",
        "--version",
        help="NxERP or image version to install, defaults to latest stable",
    )
    parser.add_argument(
        "-l",
        "--force-pull",
        action="store_true",
        help="Force pull ragapp_docker",
    )
    return parser
...
def add_build_parser(subparsers: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "-r",
        "--ragapp-path",
        help="Ragapp Repository to use, default: https://github.com/khulnasoft/ragapp",
        default="https://github.com/khulnasoft/ragapp",
    )
    parser.add_argument(
        "-b",
        "--ragapp-branch",
        help="Ragapp branch to use, default: version-15",
        default="version-15",
    )
    parser.add_argument(
        "-j",
        "--apps-json",
        help="Path to apps json, default: ragapp_docker/development/apps-example.json",
        default="ragapp_docker/development/apps-example.json",
    )
    parser.add_argument(
        "-t",
        "--tag",
...
def add_develop_parser(subparsers: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "-n", "--project", default="ragapp", help="Compose project name"
    )
...
def build_image(
    ...
    command += [
        ...
        f"--build-arg=RAGAPP_PATH={ragapp_path}",
        f"--build-arg=RAGAPP_BRANCH={ragapp_branch}",
...
def get_args_parser():
    parser = argparse.ArgumentParser(
        description="Easy install script for Ragapp Framework"
    )
    # Setup sub-commands
...
if __name__ == "__main__":
    ...
    if (
        ...
        and args.force_pull
        and os.path.exists(get_ragapp_docker_path())
    ):
        cprint("\nForce pull ragapp_docker again\n", level=2)
        shutil.rmtree(get_ragapp_docker_path(), ignore_errors=True)

    if args.subcommand == "build":
        build_image(
            push=args.push,
            ragapp_path=args.ragapp_path,
            ragapp_branch=args.ragapp_branch,
