...
def pass_context(f):
	...
	def _func(ctx, *args, **kwargs):
		...
		try:
			...
			ragapp.exceptions.SiteNotSpecifiedError,
			ragapp.exceptions.IncorrectSitePath,
			ragapp.exceptions.CommandFailedError,
		) as e:
...
def get_site(context, raise_err=True):
	try:
		...
		if raise_err:
			raise ragapp.SiteNotSpecifiedError
		return None
...
def get_commands():
	...
	clickable_link = "https://ragapp.khulnasoft.com/docs"
	all_commands = (
		scheduler_commands
		+ site_commands
		+ testing_commands
