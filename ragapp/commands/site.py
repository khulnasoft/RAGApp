...
from ragapp.utils import CallbackManager
...
def new_site(
	...
	"Create a new site"
	from ragapp.installer import _new_site

	ragapp.init(site, new_site=True)

	if site in ragapp.get_all_apps():
		click.secho(
			f"Your cli has an app called {site}, please choose another name for the site.", fg="red"
		)
		sys.exit(1)
...
def _restore(
	*,
	...
	from ragapp.installer import extract_files
	from ragapp.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key

	err, out = ragapp.utils.execute_in_shell(f"file {sql_file_path}", check_exit_code=True)
	if err:
...
def restore_backup(
	...
	from ragapp.installer import _new_site, is_downgrade, is_partial, validate_database_sql

	# Check for the backup file in the backup directory, as well as the main cli directory
	...
	if is_partial(sql_file_path):
		click.secho(
			"Partial Backup file detected. You cannot use a partial file to restore a Ragapp site.",
			fg="red",
		)
		click.secho(
			"Use `cli partial-restore` to restore a partial backup to an existing site.",
			fg="yellow",
		)
		sys.exit(1)

	# Check if the backup is of an older version of ragapp and the user hasn't specified force
...
def partial_restore(context: CliCtxObj, sql_file_path, verbose, encryption_key=None):
	from ragapp.installer import is_partial, partial_restore
	from ragapp.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key
	...
	if "cipher" in out.decode().split(":")[-1].strip():
		...
		with decrypt_backup(sql_file_path, key):
			if not is_partial(sql_file_path):
				click.secho(
					"Full backup file detected. Use `cli restore` to restore a Ragapp Site.",
					fg="red",
				)
				sys.exit(1)
...
def _reinstall(
	...
	try:
		...
		ragapp.connect()
		ragapp.clear_cache()
		installed = ragapp.get_installed_apps()
		ragapp.clear_cache()
	except Exception:
		installed = []
	finally:
...
def install_app(context: CliCtxObj, apps, force=False):
	...
	from ragapp.installer import install_app as _install_app
	from ragapp.utils.synchronization import filelock
	...
	for site in context.sites:
		...
		with filelock("install_app", timeout=1):
			for app in apps:
				...
				except ragapp.IncompatibleApp as err:
					err_msg = f":\n{err}" if str(err) else ""
					print(f"App {app} is Incompatible with Site {site}{err_msg}")
					exit_code = 1
				except Exception as err:
					err_msg = f": {err!s}\n{ragapp.get_traceback(with_context=True)}"
...
def list_apps(context: CliCtxObj, format):
	...
	for site in context.sites:
		...
		apps = ragapp.get_single("Installed Applications").installed_applications
		if apps:
			installed_apps_info.extend(format_app(app) for app in apps)
		else:
			installed_apps_info.extend(ragapp.get_installed_apps())
	...
	if format == "json":
		click.echo(ragapp.as_json(summary_dict))
...
def add_db_index(context: CliCtxObj, doctype, column):
	...
	for site in context.sites:
		...
		ragapp.connect()
		try:
			ragapp.db.add_index(doctype, columns)
			if len(columns) == 1:
...
def describe_database_table(context, doctype, column):
	...
	from ragapp.core.doctype.recorder.recorder import _fetch_table_stats

	for site in context.sites:
		ragapp.init(site=site)
		ragapp.connect()
		try:
...
def add_system_manager(context: CliCtxObj, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		...
		ragapp.connect()
		try:
			ragapp.utils.user.add_system_manager(email, first_name, last_name, send_welcome_email, password)
			ragapp.db.commit()
		finally:
			ragapp.destroy()
	if not context.sites:
...
def disable_user(context: CliCtxObj, email):
	...
	with ragapp.init_site(site):
		ragapp.connect()
		user = ragapp.get_doc("User", email)
		user.enabled = 0
...
def migrate(context: CliCtxObj, skip_failing=False, skip_search_index=False):
	...
	from ragapp.migrate import SiteMigration
...
def migrate_to():
	...
	from ragapp.integrations.ragapp_providers import migrate_to
...
def run_patch(context: CliCtxObj, module, force):
	...
	import ragapp.modules.patch_handler

	for site in context.sites:
		ragapp.init(site)
		try:
			ragapp.connect()
			ragapp.modules.patch_handler.run_single(module, force=force or context.force)
		finally:
			ragapp.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def reload_doc(context: CliCtxObj, module, doctype, docname):
	...
	for site in context.sites:
		try:
			ragapp.init(site)
			ragapp.connect()
			ragapp.reload_doc(module, doctype, docname, force=context.force)
			ragapp.db.commit()
		finally:
			ragapp.destroy()
	if not context.sites:
...
def reload_doctype(context: CliCtxObj, doctype):
	...
	for site in context.sites:
		try:
			ragapp.init(site)
			ragapp.connect()
			ragapp.reload_doctype(doctype, force=context.force)
			ragapp.db.commit()
		finally:
			ragapp.destroy()
	if not context.sites:
...
def add_to_hosts(context: CliCtxObj):
	"Add site to hosts"
	for site in context.sites:
		ragapp.commands.popen(f"echo '127.0.0.1\t{site}\n::1\t{site}' | sudo tee -a /etc/hosts")
	if not context.sites:
		raise SiteNotSpecifiedError
	...
	for site in context.sites:
		try:
			...
			if rollback_callback:
				...
				rollback_callback = None
			if verbose:
				print(ragapp.get_traceback(with_context=True))
			exit_code = 1
			continue
		if ragapp.get_system_settings("encrypt_backup") and ragapp.get_site_config().encryption_key:
			click.secho(
...
def remove_from_installed_apps(context: CliCtxObj, app):
	...
	ensure_app_not_ragapp(app)
	from ragapp.installer import remove_from_installed_apps
...
def uninstall(context: CliCtxObj, app, dry_run, yes, no_backup, force):
	...
	ensure_app_not_ragapp(app)
	from ragapp.installer import remove_app
	from ragapp.utils.synchronization import filelock
...
def _drop_site(
	...
	from ragapp.database import drop_user_and_database
	from ragapp.utils.backups import scheduled_backup
	...
	try:
		...
		if force:
			...
			messages = [
				...
				f"Hint: Use 'cli drop-site {site} --force' to force the removal of {site}",
			]
			click.echo("\n".join(messages))
			sys.exit(1)
	...
	ragapp.flags.root_login = db_root_username
	ragapp.flags.root_password = db_root_password

	drop_user_and_database(ragapp.conf.db_name, ragapp.conf.db_user)

	archived_sites_path = archived_sites_path or os.path.join(
		ragapp.utils.get_cli_path(), "archived", "sites"
	)
...
def move(dest_dir, site):
	...
	ragapp.init(site)
	old_path = ragapp.utils.get_site_path()
...
def set_last_active_for_user(context: CliCtxObj, user=None):
	...
	with ragapp.init_site(site):
		...
		ragapp.db.set_value("User", user, "last_active", now_datetime())
		ragapp.db.commit()
...
def publish_realtime(context: CliCtxObj, event, message, room, user, doctype, docname, after_commit):
	"Publish realtime event from cli"
	from ragapp import publish_realtime
...
def browse(context: CliCtxObj, site, user=None):
	...
	if site not in ragapp.utils.get_sites():
...
def start_ngrok(context: CliCtxObj, bind_tls, use_default_authtoken):
	...
	ngrok_authtoken = ragapp.conf.ngrok_authtoken
...
def build_search_index(context):
	...
	from ragapp.search.website_search import build_index_for_all_routes
...
def clear_log_table(context: CliCtxObj, doctype, days, no_backup):
	...
	"""
	from ragapp.core.doctype.log_settings.log_settings import LOG_DOCTYPES
	from ragapp.core.doctype.log_settings.log_settings import clear_log_table as clear_logs
	from ragapp.utils.backups import scheduled_backup
	...
	if doctype not in LOG_DOCTYPES:
		raise ragapp.ValidationError(f"Unsupported logging DocType: {doctype}")
...
def trim_database(context: CliCtxObj, dry_run, format, no_backup, yes=False):
	...
	for site in context.sites:
		...
		database_tables: list[str] = (
			...
			.select(table_name)
			.where(information_schema.tables.table_schema == ragapp.conf.db_name)
			.where(information_schema.tables.table_type == "BASE TABLE")
			.run(pluck=True)
		)
		doctype_tables = ragapp.get_all("DocType", pluck="name")
		...
		if not TABLES_TO_DROP:
			...
			for table in TABLES_TO_DROP:
				...
				ragapp.db.sql_ddl(f"drop table `{table}`")

			ALL_DATA[ragapp.local.site] = TABLES_TO_DROP
		ragapp.destroy()
...
def get_standard_tables():
	...
	sql_file = os.path.join(
		...
		"database",
		ragapp.conf.db_type,
		f"framework_{ragapp.conf.db_type}.sql",
	)
...
def trim_tables(context: CliCtxObj, dry_run, format, no_backup):
	...
	from ragapp.model.meta import trim_tables
	from ragapp.utils.backups import scheduled_backup
	...
	for site in context.sites:
		...
		if not (no_backup or dry_run):
			click.secho(f"Taking backup for {ragapp.local.site}", fg="green")
		...
		try:
			...
			if format == "table" and not dry_run:
				click.secho(f"The following data have been removed from {ragapp.local.site}", fg="green")
...
def handle_data(data: dict, format="json"):
	if format == "json":
		...
		print(json.dumps({ragapp.local.site: data}, indent=1, sort_keys=True))
	else:
...
def ensure_app_not_ragapp(app: str) -> None:
	"""
	Ensure that the app name passed is not 'ragapp'

	:param app: Name of the app
	:return: Nothing
	"""
	if app == "ragapp":
		click.secho("You cannot remove or uninstall the app `ragapp`", fg="red")
		sys.exit(1)
...
def bypass_patch(context: CliCtxObj, patch_name: str, yes: bool):
	...
	from ragapp.modules.patch_handler import update_patch_log
