...
if TYPE_CHECKING:
	...
	from ragapp.testing import TestRunner
...
def main(
	...
	from ragapp.testing.environment import _cleanup_after_tests, _initialize_test_environment

	if debug and not debug_exceptions:
		debug_exceptions = (Exception,)

	testing_module_logger = logging.getLogger("ragapp.testing")
	...
	try:
		...
		if doctype or doctype_list_path:
			...
			apps = [app] if app else ragapp.get_installed_apps()
...
def _load_doctype_list(doctype_list_path):
	...
	with open(ragapp.get_app_path(app, path)) as f:
...
def _run_module_def_tests(app, module_def, runner: "TestRunner", force) -> "TestRunner":
	...
	from ragapp.testing import discover_doctype_tests
...
def _get_doctypes_for_module_def(app, module_def):
	...
	doctypes = []
	doctypes_ = ragapp.get_list(
		"DocType",
		filters={"module": module_def, "istable": 0},
		fields=["name", "module"],
		as_list=True,
	)
	from ragapp.modules import get_module_name
...
def run_tests(
	...
	with CodeCoverage(coverage, app):
		...
		ragapp.init(site)
		allow_tests = ragapp.get_conf().allow_tests

		if not (allow_tests or os.environ.get("CI")):
			click.secho("Testing is disabled for the site!", bold=True)
			click.secho("You can enable tests by entering following command:")
			click.secho(f"cli --site {site} set-config allow_tests true", fg="green")
			return
...
@click.option("--app", help="For App", default="ragapp")
...
def run_parallel_tests(
	...
	with CodeCoverage(with_coverage, app) as cc:
		site = get_site(context)
		if use_orchestrator:
			from ragapp.parallel_test_runner import ParallelTestWithOrchestrator

			runner = ParallelTestWithOrchestrator(app, site=site)
		else:
			from ragapp.parallel_test_runner import ParallelTestRunner
...
def run_ui_tests(
	...
	site = get_site(context)
	ragapp.init(site)
	app_base_path = ragapp.get_app_source_path(app)
	site_url = ragapp.utils.get_site_url(site)
	admin_password = ragapp.get_conf().admin_password
	...
	node_bin = subprocess.getoutput("(cd ../ragapp && yarn bin)")
	...
	if not (
		...
		ragapp.commands.popen(f"(cd ../ragapp && yarn add {packages} --no-lockfile)")
	...
	try:
		ragapp.commands.popen(formatted_command, cwd=app_base_path, raise_err=True)
