...
import ragapp
from ragapp.contacts.doctype.address.address import address_query, get_address_display
...
class TestAddress(IntegrationTestCase):
	def test_template_works(self):
		if not ragapp.db.exists("Address Template", "India"):
			ragapp.get_doc({"doctype": "Address Template", "country": "India", "is_default": 1}).insert()

		if not ragapp.db.exists("Address", "_Test Address-Office"):
			ragapp.get_doc(
				{
		...
		address = ragapp.get_list("Address")[0].name
		display = get_address_display(ragapp.get_doc("Address", address).as_dict())
