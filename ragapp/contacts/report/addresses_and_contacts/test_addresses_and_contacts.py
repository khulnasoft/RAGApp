...
from ragapp.contacts.report.addresses_and_contacts.addresses_and_contacts import get_data
from ragapp.tests import IntegrationTestCase


def get_custom_linked_doctype():
	if bool(ragapp.get_all("DocType", filters={"name": "Test Custom Doctype"})):
		return
...
def create_linked_address(link_list):
	if ragapp.flags.test_address_created:
		return

	address = ragapp.get_doc(
		{
			"doctype": "Address",
	...
	address.insert()
	ragapp.flags.test_address_created = True

	return address.name


def create_linked_contact(link_list, address):
	if ragapp.flags.test_contact_created:
		return
	...
	ragapp.flags.test_contact_created = True
