...
from ragapp.utils import cstr, get_site_url
...
class TestAccessLog(IntegrationTestCase):
	def setUp(self):
		...
		generate_keys(ragapp.session.user)
		ragapp.db.commit()
		generated_secret = ragapp.utils.password.get_decrypted_password(
			"User", ragapp.session.user, fieldname="api_secret"
		)
		...
		self.file_name = ragapp.utils.random_string(10) + ".txt"
		self.test_content = ragapp.utils.random_string(1024)
	...
	def test_make_full_access_log(self):
		...
		last_doc = ragapp.get_last_doc("Access Log")
	...
	def test_private_file_download(self):
		...
		new_private_file = ragapp.get_doc(
			{
				"doctype": self.test_doctype,
		...
		private_file_link = get_site_url(ragapp.local.site) + new_private_file.file_url
