...
import ragapp
from ragapp.core.utils import set_timeline_doc
...
from ragapp.utils import get_fullname, now, strip_html
...
class ActivityLog(Document):
	...
	def set_ip_address(self):
		if self.operation in ("Login", "Logout"):
			self.ip_address = ragapp.local.request_ip
	...
	def clear_old_logs(days=None):
		...
		ragapp.db.delete(doctype, filters=(doctype.creation < (Now() - Interval(days=days))))


def on_doctype_update():
	"""Add indexes in `tabActivity Log`"""
	ragapp.db.add_index("Activity Log", ["reference_doctype", "reference_name"])
	ragapp.db.add_index("Activity Log", ["timeline_doctype", "timeline_name"])
