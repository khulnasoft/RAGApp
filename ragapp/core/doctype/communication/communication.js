ragapp.ui.form.on("Communication", {
	onload: function (frm) {
		if (frm.doc.content) {
			frm.doc.content = ragapp.dom.remove_script_and_style(frm.doc.content);
		}
			...
			fields: [
				{
					...
					get_query: function () {
						return { query: "ragapp.email.get_communication_doctype" };
					},
				},
				{
	...
	show_move_dialog: function (frm) {
		var d = new ragapp.ui.Dialog({
			...
			primary_action(values) {
				d.hide();
				ragapp.call({
					method: "ragapp.email.inbox.move_email",
					args: {
	...
	mark_as_read_unread: function (frm) {
		...
		return ragapp.call({
			method: "ragapp.email.inbox.create_email_flag_queue",
			args: {
				names: [frm.doc.name],
				action: action,
				flag: flag,
	...
	mark_as_closed_open: function (frm) {
		...
		return ragapp.call({
			method: "ragapp.email.inbox.mark_as_closed_open",
			args: {
				communication: frm.doc.name,
				status: status,
			},
	...
	get_mail_args: function (frm) {
		var sender_email_id = "";
		$.each(ragapp.boot.email_accounts, function (idx, account) {
	...
	add_to_contact: function (frm) {
		...
		ragapp.new_doc("Contact");
	},

	mark_as_spam: function (frm) {
		ragapp.call({
			method: "ragapp.email.inbox.mark_as_spam",
			args: {
				communication: frm.doc.name,
			...
			freeze: true,
			callback: function (r) {
				ragapp.msgprint(__("Email has been marked as spam"));
			},
		});
	},
	...
	move_to_trash: function (frm) {
		ragapp.call({
			method: "ragapp.email.inbox.mark_as_trash",
			args: {
				communication: frm.doc.name,
			},
			freeze: true,
			callback: function (r) {
				ragapp.msgprint(__("Email has been moved to trash"));
			},
		});
	},
});
