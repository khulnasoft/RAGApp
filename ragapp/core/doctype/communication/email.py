...
def make(
	...
	**kwargs,
	...
	"""
	if kwargs:
		from ragapp.utils.commands import warn

		warn(
			f"Options {kwargs} used in ragapp.core.doctype.communication.email.make "
			"are deprecated or unsupported",
			category=DeprecationWarning,
		)

	if doctype and name and not ragapp.has_permission(doctype=doctype, ptype="email", doc=name):
		raise ragapp.PermissionError(f"You are not allowed to send emails related to: {doctype} {name}")
...
def _make(
	...
	sender = sender or get_formatted_email(ragapp.session.user)
...
def add_attachments(name: str, attachments: Iterable[str | dict]) -> None:
	...
	for a in attachments:
		if isinstance(a, str):
			attach = ragapp.db.get_value("File", {"name": a}, ["file_url", "is_private"], as_dict=1)
			file_args = {
				"file_url": attach.file_url,
				"is_private": attach.is_private,
			}
		elif isinstance(a, dict) and "fcontent" in a and "fname" in a:
			# dict returned by ragapp.attach_print()
			file_args = {
				"file_name": a["fname"],
				"content": a["fcontent"],
				"is_private": 1,
...
def mark_email_as_seen(name: str | None = None):
	ragapp.request.after_response.add(lambda: _mark_email_as_seen(name))
...
def _mark_email_as_seen(name):
	try:
		...
		ragapp.log_error("Unable to mark as seen", None, "Communication", name)

	ragapp.db.commit()  # nosemgrep: after_response requires explicit commit
...
def update_communication_as_read(name):
	...
	communication = ragapp.db.get_value("Communication", name, "read_by_recipient", as_dict=True)
