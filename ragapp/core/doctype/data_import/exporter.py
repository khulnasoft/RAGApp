...
from ragapp.model import display_fieldtypes, no_value_fields
from ragapp.model import table_fields as table_fieldtypes
from ragapp.utils import flt, format_duration, groupby_metric
from ragapp.utils.csvutils import build_csv_response
from ragapp.utils.xlsxutils import build_xlsx_response
...
class Exporter:
	...
	def get_all_exportable_fields(self):
		...
		exportable_fields = ragapp._dict({})
	...
	def serialize_exportable_fields(self):
		...
		for key, exportable_fields in self.exportable_fields.items():
			for _df in exportable_fields:
				...
				if isinstance(_df, ragapp.core.doctype.docfield.docfield.DocField):
					df = _df.as_dict()
				else:
					df = _df.copy()
	...
	def get_data_to_export(self):
		...
		if not ragapp.permissions.can_export(self.doctype):
			if ragapp.permissions.can_export(self.doctype, is_owner=True):
				for doc in data:
					if doc.get("owner") != ragapp.session.user:
		...
		for i, df in enumerate(self.fields):
			if df.parent == doctype:
				...
				if df.fieldtype == "Text Editor" and value:
					value = ragapp.core.utils.html2text(value)
				row[i] = value
		return rows
	...
	def get_data_as_docs(self):
		...
		parent_data = ragapp.db.get_list(
			self.doctype,
			filters=filters,
