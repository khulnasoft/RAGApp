...
from ragapp.core.doctype.data_import.test_importer import create_doctype_if_not_exists
...
class TestExporter(IntegrationTestCase):
	...
	def test_exports_specified_fields(self):
		if not ragapp.db.exists(doctype_name, "Test"):
			doc = ragapp.get_doc(
			...
			doc = ragapp.get_doc(doctype_name, "Test")
	...
	def test_export_csv_response(self):
		...
		self.assertEqual(ragapp.response["doctype"], doctype_name)
