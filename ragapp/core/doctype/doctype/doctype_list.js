ragapp.listview_settings["DocType"] = {
	...
	new_doctype_dialog(args) {
		...
		let non_developer = ragapp.session.user !== "Administrator" || !ragapp.boot.developer_mode;
		let fields = [
			{
				label: __("Name"),
				fieldname: "name",
		...
		let new_d = new ragapp.ui.Dialog({
			...
			primary_action(values) {
				...
				ragapp.db
					...
					.then((doc) => {
						ragapp.set_route("Form", "DocType", doc.name);
					});
			},
			secondary_action_label: __("Cancel"),
			secondary_action() {
				new_d.hide();
				if (ragapp.get_route()[0] === "Form") {
