...
class TestDocumentNamingRule(IntegrationTestCase):
	def test_naming_rule_by_series(self):
		naming_rule = ragapp.get_doc(
			doctype="Document Naming Rule", document_type="ToDo", prefix="test-todo-", prefix_digits=5
		).insert()

		todo = ragapp.get_doc(
			doctype="ToDo", description="Is this my name " + ragapp.generate_hash()
		).insert()
	...
	def test_naming_rule_by_condition(self):
		...
		# another rule
		naming_rule_1 = ragapp.copy_doc(naming_rule)
		...
		naming_rule_2 = ragapp.copy_doc(naming_rule)
		...
		todo = ragapp.get_doc(
			doctype="ToDo", priority="High", description="Is this my name " + ragapp.generate_hash()
		).insert()

		todo_1 = ragapp.get_doc(
			doctype="ToDo", priority="Medium", description="Is this my name " + ragapp.generate_hash()
		).insert()

		todo_2 = ragapp.get_doc(
			doctype="ToDo", priority="Low", description="Is this my name " + ragapp.generate_hash()
		).insert()
