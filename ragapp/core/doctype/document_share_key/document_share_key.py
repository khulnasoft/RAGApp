...
class DocumentShareKey(Document):
	...
	def before_insert(self):
		self.key = ragapp.generate_hash(length=randrange(25, 35))
		if not self.expires_on and not self.flags.no_expiry:
			self.expires_on = ragapp.utils.add_days(
				None, days=ragapp.get_system_settings("document_share_key_expiry") or 90
			)


def is_expired(expires_on):
	return expires_on and expires_on < ragapp.utils.getdate()
