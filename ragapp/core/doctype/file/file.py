...
from ragapp.database.schema import SPECIAL_CHAR_PATTERN
from ragapp.model.document import Document
from ragapp.permissions import SYSTEM_USER_ROLE, get_doctypes_with_read
from ragapp.utils import call_hook_method, cint, get_files_path, get_hook_method, get_url
from ragapp.utils.file_manager import is_safe_path
...
class File(Document):
	...
	def autoname(self):
		...
		if self.is_folder:
			...
			self.name = ragapp.generate_hash(length=10)
	...
	def before_insert(self):
		...
		if self.is_remote_file:
			...
			ragapp.db.after_rollback.add(self.on_rollback)
	...
	def validate(self):
		...
		self.file_size = ragapp.form_dict.file_size or self.file_size
	...
	def validate_attachment_references(self):
		...
		if not self.attached_to_name or not isinstance(self.attached_to_name, str | int):
			ragapp.throw(_("Attached To Name must be a string or an integer"), ragapp.ValidationError)

		if self.attached_to_field and SPECIAL_CHAR_PATTERN.search(self.attached_to_field):
			ragapp.throw(_("The fieldname you've specified in Attached To Field is invalid"))
	...
	def on_trash(self):
		if self.is_home_folder or self.is_attachments_folder:
			ragapp.throw(_("Cannot delete Home and Attachments folders"))
	...
	def get_successors(self):
		return ragapp.get_all("File", filters={"folder": self.name}, pluck="name")
	...
	def handle_is_private_changed(self):
		...
		private_file_path = Path(ragapp.get_site_path("private", "files", file_name))
		public_file_path = Path(ragapp.get_site_path("public", "files", file_name))
		...
		if ragapp.get_meta(self.attached_to_doctype).issingle:
	...
	def fetch_attached_to_field(self, old_file_url):
		...
		reference_dict = ragapp.get_doc(self.attached_to_doctype, self.attached_to_name).as_dict()
	...
	def validate_attachment_limit(self):
		...
		if self.attached_to_doctype and self.attached_to_name:
			attachment_limit = cint(ragapp.get_meta(self.attached_to_doctype).max_attachments)
			...
			if current_attachment_count >= attachment_limit:
				ragapp.throw(
					_("Maximum Attachment Limit of {0} has been reached for {1} {2}.").format(
						ragapp.bold(attachment_limit), self.attached_to_doctype, self.attached_to_name
					),
	...
	def set_folder_name(self):
		...
		if self.attached_to_doctype:
			self.folder = ragapp.db.get_value("File", {"is_attachments_folder": 1})
	...
	def validate_file_on_disk(self):
		...
		if not os.path.exists(full_path):
			ragapp.throw(_("File {0} does not exist").format(self.file_url), IOError)

	def validate_file_extension(self):
		# Only validate uploaded files, not generated by code/integrations.
		if not self.file_type or not ragapp.request:
			return

		allowed_extensions = ragapp.get_system_settings("allowed_file_extensions")
		if not allowed_extensions:
			return

		if self.file_type not in allowed_extensions.splitlines():
			ragapp.throw(_("File type of {0} is not allowed").format(self.file_type), exc=FileTypeNotAllowed)
	...
	def validate_duplicate_entry(self):
		if not self.flags.ignore_duplicate_entry_error and not self.is_folder:
			...
			duplicate_file = ragapp.db.get_value("File", filters, ["name", "file_url"], as_dict=1)

			if duplicate_file:
				duplicate_file_doc = ragapp.get_cached_doc("File", duplicate_file.name)
	...
	def set_file_name(self):
		if not self.file_name and not self.file_url:
			ragapp.throw(
				_("Fields `file_name` or `file_url` must be set for File"), exc=ragapp.MandatoryError
			)
	...
	def generate_content_hash(self):
		...
		try:
			...
			ragapp.throw(_("File {0} does not exist").format(file_path))
	...
	def make_thumbnail(
		...
		path = os.path.abspath(ragapp.get_site_path("public", thumbnail_url.lstrip("/")))
		...
		try:
			...
			ragapp.msgprint(_("Unable to write file format for {0}").format(path))
			return
	...
	def validate_empty_folder(self):
		...
		if self.is_folder and ragapp.get_all("File", filters={"folder": self.name}, limit=1):
			ragapp.throw(_("Folder {0} is not empty").format(self.name), FolderNotEmpty)

	def _delete_file_on_disk(self):
		"""If file not attached to any other record, delete it"""
		on_disk_file_not_shared = self.content_hash and not ragapp.get_all(
			"File",
			filters={
				"content_hash": self.content_hash,
				"name": ["!=", self.name],
	...
	def unzip(self) -> list["File"]:
		...
		if not self.file_url.endswith(".zip"):
			ragapp.throw(_("{0} is not a zip file").format(self.file_name))
		...
		with zipfile.ZipFile(zip_path) as z:
			for file in z.filelist:
				...
				file_doc = ragapp.new_doc("File")
				try:
					file_doc.content = z.read(file.filename)
				except zipfile.BadZipFile:
					ragapp.throw(_("{0} is a not a valid zip file").format(self.file_name))
		...
		ragapp.delete_doc("File", self.name)
		return files
	...
	def get_content(self, encodings=None) -> bytes | str:
		if self.is_folder:
			ragapp.throw(_("Cannot get file contents of a Folder"))
	...
	def get_full_path(self):
		...
		if file_path.startswith("/private/files/"):
			...
			ragapp.throw(_("There is some problem with the file url: {0}").format(file_path))

		if not is_safe_path(file_path):
			ragapp.throw(_("Cannot access file path {0}").format(file_path))

		if os.path.sep in self.file_name:
			ragapp.throw(_("File name cannot have {0}").format(os.path.sep))
	...
	def save_file(
		...
		if (
			...
			and ragapp.get_system_settings("strip_exif_metadata_from_uploaded_images")
		):
		...
		if not ignore_existing_file_check:
			duplicate_file = ragapp.get_value(
				"File",
		...
		if duplicate_file:
			file_doc: File = ragapp.get_cached_doc("File", duplicate_file.name)
	...
	def check_max_file_size(self):
		...
		if file_size > max_file_size:
			...
			if ragapp.has_permission("System Settings", "write"):
				msg += ".<br>" + _("You can increase the limit from System Settings.")
			ragapp.throw(msg, exc=MaxFileSizeReachedError)
	...
	def create_attachment_record(self):
		...
		file_url = quote(ragapp.safe_encode(self.file_url), safe="/:") if self.file_url else self.file_name
	...
	def add_comment_in_reference_doc(self, comment_type, text):
		if self.attached_to_doctype and self.attached_to_name:
			try:
				doc = ragapp.get_doc(self.attached_to_doctype, self.attached_to_name)
	...
	def zip_files(files):
		...
		for _file in files:
			if isinstance(_file, str):
				_file = ragapp.get_doc("File", _file)
...
def on_doctype_update():
	ragapp.db.add_index("File", ["attached_to_doctype", "attached_to_name"])
	ragapp.db.add_index("File", ["file_url(100)"])
...
def has_permission(doc, ptype=None, user=None, debug=False):
	...
	if doc.attached_to_doctype and doc.attached_to_name:
		...
		try:
			ref_doc = ragapp.get_doc(attached_to_doctype, attached_to_name)
		except ragapp.DoesNotExistError:
...
def get_permission_query_conditions(user: str | None = None) -> str:
	...
	if SYSTEM_USER_ROLE not in ragapp.get_roles(user):
		return f""" `tabFile`.`owner` = {ragapp.db.escape(user)} """

	readable_doctypes = ", ".join(repr(dt) for dt in get_doctypes_with_read())
	return f"""
		(`tabFile`.`is_private` = 0)
		OR (`tabFile`.`attached_to_doctype` IS NULL AND `tabFile`.`owner` = {ragapp.db.escape(user)})
