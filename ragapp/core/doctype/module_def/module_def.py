...
from ragapp.modules.export_file import delete_folder
...
class ModuleDef(Document):
	...
	def on_update(self):
		...
		ragapp.clear_cache()
		if not self.custom and ragapp.conf.get("developer_mode"):
	...
	def create_modules_folder(self):
		...
		module_path = ragapp.get_app_path(self.app_name, self.name)
	...
	def add_to_modules_txt(self):
		...
		modules = None
		if not ragapp.local.module_app.get(ragapp.scrub(self.name)):
			with open(ragapp.get_app_path(self.app_name, "modules.txt")) as f:
				content = f.read()
			...
			if modules:
				with open(ragapp.get_app_path(self.app_name, "modules.txt"), "w") as f:
					f.write("\n".join(modules))
	...
	def on_trash(self):
		...
		if not ragapp.conf.get("developer_mode") or ragapp.flags.in_uninstall or self.custom:
			return

		if ragapp.local.module_app.get(ragapp.scrub(self.name)):
			ragapp.db.after_commit.add(self.delete_module_from_file)

	def delete_module_from_file(self):
		delete_folder(self.module_name, "Module Def", self.name)
		modules = []

		modules_txt = Path(ragapp.get_app_path(self.app_name, "modules.txt"))
...
def get_installed_apps():
	return json.dumps(ragapp.get_installed_apps())
