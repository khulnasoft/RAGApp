...
class NavbarSettings(Document):
	...
	if TYPE_CHECKING:
		from ragapp.core.doctype.navbar_item.navbar_item import NavbarItem
...
def get_app_logo():
	app_logo = ragapp.get_website_settings("app_logo") or ragapp.get_cached_value(
		"Navbar Settings",
		"Navbar Settings",
		"app_logo",
	)
	...
	if not app_logo:
		logos = ragapp.get_hooks("app_logo_url")
		app_logo = logos[0]
		if len(logos) == 2:
			app_logo = logos[1]
...
def get_navbar_settings():
	return ragapp.get_single("Navbar Settings")
...
def sync_table(key, hook):
	...
	for item in ragapp.get_hooks(hook):
