...
class TestPackage(IntegrationTestCase):
	def test_package_release(self):
		...
		# make release
		ragapp.get_doc(doctype="Package Release", package="Test Package", publish=1).insert()

		self.assertTrue(os.path.exists(ragapp.get_site_path("packages", "test-package")))
		self.assertTrue(
			os.path.exists(ragapp.get_site_path("packages", "test-package", "test_module_for_package"))
		)
		self.assertTrue(
			os.path.exists(
				ragapp.get_site_path(
					"packages",
					"test-package",
...
def make_test_package():
	if not ragapp.db.exists("Package", "Test Package"):
		ragapp.get_doc(
...
def make_test_module():
	if not ragapp.db.exists("Module Def", "Test Module for Package"):
		ragapp.get_doc(
			doctype="Module Def",
...
def make_test_doctype():
	if not ragapp.db.exists("DocType", "Test DocType for Package"):
		ragapp.get_doc(
			doctype="DocType",
...
def make_test_server_script():
	if not ragapp.db.exists("Server Script", "Test Script for Package"):
		ragapp.get_doc(
			...
			script='ragapp.msgprint("Test")',
		).insert()


def make_test_web_page():
	if not ragapp.db.exists("Web Page", "test-web-page-for-package"):
		ragapp.get_doc(
			doctype="Web Page",
			module="Test Module for Package",
