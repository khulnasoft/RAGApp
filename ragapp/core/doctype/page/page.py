...
import ragapp
from ragapp import _, conf, get_module_path, safe_decode
from ragapp.build import html_to_js_template
from ragapp.core.doctype.custom_role.custom_role import get_custom_allowed_roles
from ragapp.desk.form.meta import get_code_files_via_hooks, get_js
...
class Page(Document):
	...
	def autoname(self):
		...
		if (self.name and self.name.startswith("New Page")) or not self.name:
			...
			if ragapp.db.exists("Page", self.name):
				cnt = ragapp.db.sql(
	...
	def validate(self):
		...
		if self.is_new() and not getattr(conf, "developer_mode", 0):
			ragapp.throw(_("Not in Developer Mode"))

		# setting ignore_permissions via update_setup_wizard_access (setup_wizard.py)
		if ragapp.session.user != "Administrator" and not self.flags.ignore_permissions:
			ragapp.throw(_("Only Administrator can edit"))
	...
	def on_update(self):
		...
		from ragapp.core.doctype.doctype.doctype import make_module_and_roles
		...
		if path:
			...
			if not os.path.exists(path + ".js"):
				with open(path + ".js", "w") as f:
					f.write(
						f"""ragapp.pages['{self.name}'].on_page_load = function(wrapper) {{
	...
	def on_trash(self):
		...
		ragapp.db.after_commit(self.delete_folder_with_contents)

	def delete_folder_with_contents(self):
		module_path = get_module_path(self.module)
		dir_path = os.path.join(module_path, "page", ragapp.scrub(self.name))
			...
			if fname.endswith(".html"):
				with open(os.path.join(path, fname)) as f:
					...
					if "<!-- jinja -->" in template:
						...
						try:
							out = ragapp.get_attr(
								"{app}.{module}.page.{page}.{page}.get_context".format(
									app=ragapp.local.module_app[scrub(self.module)],
						...
						template = ragapp.render_template(template, context)
...
def delete_custom_role(field, docname):
	name = ragapp.db.get_value("Custom Role", {field: docname}, "name")
	if name:
		ragapp.delete_doc("Custom Role", name)
