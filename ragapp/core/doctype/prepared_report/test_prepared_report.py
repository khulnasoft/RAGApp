...
import ragapp
from ragapp.desk.query_report import generate_report_result, get_report_doc
...
class TestPreparedReport(IntegrationTestCase):
	...
	def tearDownClass(cls):
		for r in ragapp.get_all("Prepared Report", pluck="name"):
			ragapp.delete_doc("Prepared Report", r, force=True, delete_permanently=True)

		ragapp.db.commit()

	@timeout(seconds=20)
	def wait_for_status(self, report, status):
		ragapp.db.commit()  # Flush changes first
		while True:
			ragapp.db.rollback()  # read new data
			report.reload()
	...
	def test_queueing(self):
		...
		doc = ragapp.get_last_doc("Prepared Report")
	...
	def test_start_status_and_kill_jobs(self):
		with test_report(report_type="Query Report", query="select sleep(10)") as report:
			...
			doc.delete()
			time.sleep(1)
			job = ragapp.get_doc("RQ Job", job_id)
...
def test_report(**args):
	try:
		...
		if not report.report_name:
			report.report_name = ragapp.generate_hash()
