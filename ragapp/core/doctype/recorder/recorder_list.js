ragapp.listview_settings["Recorder"] = {
	...
	onload(listview) {
		...
		listview.page.add_button(__("Clear"), () => {
			ragapp.xcall("ragapp.recorder.delete").then(listview.refresh);
		});
		...
		listview.page.add_menu_item(__("Import"), () => {
			new ragapp.ui.FileUploader({
				...
				on_success: (file) => {
					...
					ragapp.call({
						method: "ragapp.recorder.import_data",
						args: {
							file: file.file_url,
						},
						callback: function () {
		...
		listview.page.add_menu_item(__("Export"), () => {
			ragapp.call({
				method: "ragapp.recorder.export_data",
				callback: function (r) {
	...
	fetch_recorder_status(listview) {
		return ragapp.xcall("ragapp.recorder.status").then((status) => {
	...
	stop_recorder(listview) {
		let me = this;
		ragapp.xcall("ragapp.recorder.stop", {}).then(() => {
			listview.refresh();
	...
	start_recorder(listview) {
		...
		ragapp.prompt(
			...
			(values) => {
				ragapp.xcall("ragapp.recorder.start", values).then(() => {
					listview.refresh();
