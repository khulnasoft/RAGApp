...
from ragapp.custom.doctype.customize_form.customize_form import reset_customization
from ragapp.desk.query_report import add_total_row, run, save_report
from ragapp.desk.reportview import delete_report
from ragapp.desk.reportview import save_report as _save_report
...
class TestReport(IntegrationTestCase):
	...
	def test_report_builder(self):
		if ragapp.db.exists("Report", "User Activity Report"):
			ragapp.delete_doc("Report", "User Activity Report")

		with open(os.path.join(os.path.dirname(__file__), "user_activity_report.json")) as f:
			ragapp.get_doc(json.loads(f.read())).insert()

		report = ragapp.get_doc("Report", "User Activity Report")
	...
	def test_query_report(self):
		report = ragapp.get_doc("Report", "Permitted Documents For User")
	...
	def test_save_or_delete_report(self):
		...
		try:
			...
			ragapp.set_user("test_report_owner@example.com")
			self.assertRaises(ragapp.PermissionError, delete_report, report.name)
	...
	def test_custom_report(self):
		...
		custom_report = ragapp.get_doc("Report", custom_report_name)
		columns, result = custom_report.run_query_report(user=ragapp.session.user)

		self.assertListEqual(["email"], [column.get("fieldname") for column in columns])
		admin_dict = ragapp.core.utils.find(result, lambda d: d["name"] == "Administrator")
	...
	def test_report_permissions(self):
		ragapp.set_user("test@example.com")
		ragapp.db.delete("Has Role", {"parent": ragapp.session.user, "role": "Test Has Role"})
		ragapp.db.commit()
		if not ragapp.db.exists("Role", "Test Has Role"):
			ragapp.get_doc({"doctype": "Role", "role_name": "Test Has Role"}).insert(ignore_permissions=True)

		if not ragapp.db.exists("Report", "Test Report"):
			report = ragapp.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = ragapp.get_doc("Report", "Test Report")
	...
	def test_report_custom_permissions(self):
		ragapp.set_user("test@example.com")
		ragapp.db.delete("Custom Role", {"report": "Test Custom Role Report"})
		ragapp.db.commit()  # nosemgrep
		if not ragapp.db.exists("Report", "Test Custom Role Report"):
			report = ragapp.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = ragapp.get_doc("Report", "Test Custom Role Report")
	...
	def test_format_method(self):
		if ragapp.db.exists("Report", "User Activity Report Without Sort"):
			ragapp.delete_doc("Report", "User Activity Report Without Sort")
		with open(os.path.join(os.path.dirname(__file__), "user_activity_report_without_sort.json")) as f:
			ragapp.get_doc(json.loads(f.read())).insert()

		report = ragapp.get_doc("Report", "User Activity Report Without Sort")
		columns, data = report.get_data()
	...
	def test_non_standard_script_report(self):
		...
		if not ragapp.db.exists("Report", report_name):
			report = ragapp.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
...
totals = {}
for user in ragapp.get_all('User', fields = ['name', 'user_type', 'creation']):
...
"""
	...
	def test_script_report_with_columns(self):
		...
		if ragapp.db.exists("Report", report_name):
			ragapp.delete_doc("Report", report_name)
...
"""
	...
	def test_toggle_disabled(self):
		...
		reports = ragapp.get_all(doctype="Report", limit=1)
		...
		doc = ragapp.get_doc("Report", report_name)
		with self.assertRaises(ragapp.exceptions.ValidationError):
	...
	def test_cte_in_query_report(self):
		...
		if ragapp.db.db_type == "mariadb":
			...
			self.assertRaises(ragapp.PermissionError, report.execute_query_report, filters={})
