...
import ragapp
from ragapp.core.doctype.report.report import is_prepared_report_enabled
...
class RolePermissionforPageandReport(Document):
	...
	def set_custom_roles(self):
		...
		name = ragapp.db.get_value("Custom Role", args, "name")
		if name:
			doc = ragapp.get_doc("Custom Role", name)
			roles = doc.roles
		else:
	...
	def update_custom_roles(self):
		...
		if self.report:
			args.update({"ref_doctype": ragapp.db.get_value("Report", self.report, "ref_doctype")})

		if name:
			custom_role = ragapp.get_doc("Custom Role", name)
			custom_role.set("roles", roles)
			custom_role.save()
		else:
			ragapp.get_doc(args).insert()

	def update_disable_prepared_report(self):
		if self.report:
			# intentionally written update query in ragapp.db.sql instead of ragapp.db.set_value
			ragapp.db.sql(
