...
class TestRoleProfile(IntegrationTestCase):
	def test_make_new_role_profiles(self):
		ragapp.delete_doc_if_exists("Role Profile", "Test 1", force=1)
		new_role_profile = ragapp.get_doc(doctype="Role Profile", role_profile="Test 1").insert()
	...
	def test_multiple_role_profiles(self):
		ragapp.delete_doc_if_exists("Role Profile", "_Test Role Profile 1", force=1)
		ragapp.delete_doc_if_exists("Role Profile", "_Test Role Profile 2", force=1)

		role_profile_one = ragapp.get_doc(doctype="Role Profile", role_profile="_Test Role Profile 1").insert(
			ignore_if_duplicate=True
		)
		role_profile_two = ragapp.get_doc(doctype="Role Profile", role_profile="_Test Role Profile 2").insert(
			ignore_if_duplicate=True
		)
		...
		ragapp.get_doc(doctype="Role", role_name="_Test Role 1").insert(ignore_if_duplicate=True)
		ragapp.get_doc(doctype="Role", role_name="_Test Role 2").insert(ignore_if_duplicate=True)
		ragapp.get_doc(doctype="Role", role_name="_Test Role 3").insert(ignore_if_duplicate=True)
		# add role
		...
		user_one = ragapp.get_doc(
			{
				"doctype": "User",
				"email": ragapp.mock("email"),
				"enabled": 1,
				"first_name": ragapp.mock("name"),
				"new_password": "Eastern_43A1W",
				"role_profiles": [
	...
	def test_update_role_profile(self):
		role_profile = ragapp.get_doc("Role Profile", "_Test Role Profile 1")
