...
from ragapp.utils.background_jobs import get_queues, get_redis_conn
...
class RQJob(Document):
	...
	def stop_job(self):
		try:
			...
			ragapp.msgprint(_("Job is not running."), title=_("Invalid Operation"))
...
def serialize_job(job: Job) -> ragapp._dict:
	...
	if job_name == "ragapp.utils.background_jobs.run_doc_method":
	...
	return ragapp._dict(
		...
		exc_info=exc_info,
		arguments=ragapp.as_json(job.kwargs),
		timeout=job.timeout,
...
def for_current_site(job: Job) -> bool:
	return job.kwargs.get("site") == ragapp.local.site
...
@ragapp.whitelist()
def stop_job(job_id):
	ragapp.get_doc("RQ Job", job_id).stop_job()
