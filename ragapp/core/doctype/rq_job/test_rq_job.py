...
import ragapp
from ragapp.core.doctype.rq_job.rq_job import RQJob, remove_failed_jobs, stop_job
from ragapp.installer import update_site_config
from ragapp.tests import IntegrationTestCase, UnitTestCase, timeout
from ragapp.utils import cstr, execute_in_shell
from ragapp.utils.background_jobs import get_job_status, is_job_enqueued
...
class TestRQJob(IntegrationTestCase):
	BG_JOB = "ragapp.core.doctype.rq_job.test_rq_job.test_func"

	def check_status(self, job: Job, status, wait=True):
		if wait:
			wait_for_completion(job)
		self.assertEqual(ragapp.get_doc("RQ Job", job.id).status, status)

	def test_serialization(self):
		job = ragapp.enqueue(method=self.BG_JOB, queue="short")
		rq_job = ragapp.get_doc("RQ Job", job.id)
	...
	def test_configurable_ttl(self):
		ragapp.conf.rq_job_failure_ttl = 600
		job = ragapp.enqueue(method=self.BG_JOB, queue="short")

		self.assertEqual(job.failure_ttl, 600)

	def test_func_obj_serialization(self):
		job = ragapp.enqueue(method=test_func, queue="short")
		rq_job = ragapp.get_doc("RQ Job", job.id)
		self.assertEqual(rq_job.job_name, "ragapp.core.doctype.rq_job.test_rq_job.test_func")

	@timeout
	def test_get_list_filtering(self):
		# Check failed job clearning and filtering
		remove_failed_jobs()
		jobs = ragapp.get_all("RQ Job", {"status": "failed"})
		...
		# Fail a job
		job = ragapp.enqueue(method=self.BG_JOB, queue="short", fail=True)
		...
		non_failed_jobs = ragapp.get_all("RQ Job", {"status": ("!=", "failed")})
		self.assertGreaterEqual(len(non_failed_jobs), 1)

		# Create a slow job and check if it's stuck in "Started"
		job = ragapp.enqueue(method=self.BG_JOB, queue="short", sleep=10)
		time.sleep(3)
	...
	def test_delete_doc(self):
		job = ragapp.enqueue(method=self.BG_JOB, queue="short")
		ragapp.get_doc("RQ Job", job.id).delete()
	...
	def test_multi_queue_burst_consumption(self):
		for _ in range(3):
			for q in ["default", "short"]:
				ragapp.enqueue(self.BG_JOB, sleep=1, queue=q)

		_, stderr = execute_in_shell("cli worker --queue short,default --burst", check_exit_code=True)
	...
	def test_multi_queue_burst_consumption_worker_pool(self):
		...
		_, stderr = execute_in_shell(
			"cli worker-pool --queue short,default --burst --num-workers=4", check_exit_code=True
		)
		self.assertIn("quitting", cstr(stderr))

	def test_job_id_manual_dedup(self):
		job_id = "test_dedup"
		job = ragapp.enqueue(self.BG_JOB, sleep=5, job_id=job_id)
	...
	def test_auto_job_dedup(self):
		job_id = "test_dedup"
		job1 = ragapp.enqueue(self.BG_JOB, sleep=2, job_id=job_id, deduplicate=True)
		job2 = ragapp.enqueue(self.BG_JOB, sleep=5, job_id=job_id, deduplicate=True)
		...
		job3 = ragapp.enqueue(self.BG_JOB, fail=True, job_id=job_id, deduplicate=True)
		self.check_status(job3, "failed")
		job4 = ragapp.enqueue(self.BG_JOB, sleep=1, job_id=job_id, deduplicate=True)
		self.check_status(job4, "finished")

	@timeout
	def test_enqueue_after_commit(self):
		job_id = ragapp.generate_hash()

		ragapp.enqueue(self.BG_JOB, enqueue_after_commit=True, job_id=job_id)
	...
	def test_memory_usage(self):
		if ragapp.db.db_type != "mariadb":
			return
		job = ragapp.enqueue("ragapp.utils.data._get_rss_memory_usage")
		self.check_status(job, "finished")
		...
		# Refer this PR: https://github.com/khulnasoft/ragapp/pull/21467
	...
	def test_clear_failed_jobs(self):
		...
		jobs = [ragapp.enqueue(method=self.BG_JOB, queue="short", fail=True) for _ in range(limit * 2)]
