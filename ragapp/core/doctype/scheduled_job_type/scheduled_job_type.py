...
from ragapp.utils import get_datetime, now_datetime
...
class ScheduledJobType(Document):
	...
	def validate(self):
		...
		if self.frequency == "Cron":
			if not self.cron_format:
				ragapp.throw(_("Cron format is required for job types with Cron frequency."))
			try:
	...
	def enqueue(self, force=False) -> bool:
		...
		if self.is_event_due() or force:
			if not self.is_job_in_queue():
				enqueue(
					"ragapp.core.doctype.scheduled_job_type.scheduled_job_type.run_scheduled_job",
				...
				ragapp.logger("scheduler").error(
					f"Skipped queueing {self.method} because it was found in queue for {ragapp.local.site}"
				)
	...
	def get_next_execution(self):
		CRON_MAP = {
			...
			"Hourly": "0 * * * *",
			"Hourly Long": "0 * * * *",
			"All": f"*/{(ragapp.get_conf().scheduler_interval or 240) // 60} * * * *",
		}
	...
	def execute(self):
		if ragapp.job:
			ragapp.job.frequency = self.frequency
			ragapp.job.cron_format = self.cron_format

		self.scheduler_log = None
		try:
			self.log_status("Start")
			if self.server_script:
				script_name = ragapp.db.get_value("Server Script", self.server_script)
				if script_name:
					ragapp.get_doc("Server Script", script_name).execute_scheduled_method()
			else:
				ragapp.get_attr(self.method)()
			ragapp.db.commit()
	...
	def log_status(self, status):
		# log file
		ragapp.logger("scheduler").info(f"Scheduled Job {status}: {self.method} for {ragapp.local.site}")
	...
	def update_scheduler_log(self, status):
		...
		if not self.scheduler_log:
			self.scheduler_log = ragapp.get_doc(
		...
		if status == "Failed":
			self.scheduler_log.db_set("details", ragapp.get_traceback(with_context=True))
		if status == "Start":
			self.db_set("last_execution", now_datetime(), update_modified=False)
		ragapp.db.commit()
	...
	def on_trash(self):
		ragapp.db.delete("Scheduled Job Log", {"scheduled_job_type": self.name})
...
def execute_event(doc: str):
	...
	doc = json.loads(doc)
	ragapp.get_doc("Scheduled Job Type", doc.get("name")).enqueue(force=True)
	return doc
...
def run_scheduled_job(scheduled_job_type: str, job_type: str | None = None):
	...
	try:
		ragapp.get_doc("Scheduled Job Type", scheduled_job_type).execute()
	except Exception:
		print(ragapp.get_traceback())


def sync_jobs(hooks: dict | None = None):
	ragapp.reload_doc("core", "doctype", "scheduled_job_type")
	scheduler_events = hooks or ragapp.get_hooks("scheduler_events")
...
def insert_single_event(frequency: str, event: str, cron_format: str | None = None):
	...
	if not ragapp.db.exists("Scheduled Job Type", {"method": event, "frequency": frequency, **cron_expr}):
