...
class TestScheduledJobType(IntegrationTestCase):
	def setUp(self):
		ragapp.db.rollback()
		ragapp.db.truncate("Scheduled Job Type")
		sync_jobs()
		ragapp.db.commit()
	...
	def test_throws_on_duplicate_job(self):
		job_config = dict(
			...
			method="ragapp.desk.notifications.clear_notifications",
			frequency="Weekly",
		)
		ragapp.get_doc(job_config).insert()

		duplicate_job = ragapp.get_doc(job_config)
	...
	def test_sync_jobs(self):
		all_job = ragapp.get_doc("Scheduled Job Type", dict(method="ragapp.email.queue.flush"))
		self.assertEqual(all_job.frequency, "All")

		daily_job = ragapp.get_doc(
			"Scheduled Job Type", dict(method="ragapp.desk.notifications.clear_notifications")
		)
		self.assertEqual(daily_job.frequency, "Daily")

		# check if cron jobs are synced
		cron_job = ragapp.get_doc("Scheduled Job Type", dict(method="ragapp.oauth.delete_oauth2_data"))
		...
		updated_scheduler_events = {"hourly": ["ragapp.email.queue.flush"]}
		sync_jobs(updated_scheduler_events)
		updated_scheduled_job = ragapp.get_doc("Scheduled Job Type", {"method": "ragapp.email.queue.flush"})
	...
	def test_weekly_job(self):
		job = ragapp.get_doc(
			"Scheduled Job Type",
			dict(method="ragapp.social.doctype.energy_point_log.energy_point_log.send_weekly_summary"),
		)
		job.db_set("last_execution", "2019-01-01 00:00:00")
	...
	def test_monthly_job(self):
		job = ragapp.get_doc(
			"Scheduled Job Type",
			dict(method="ragapp.email.doctype.auto_email_report.auto_email_report.send_monthly"),
		)
		job.db_set("last_execution", "2019-01-01 00:00:00")
	...
	def test_cron_job(self):
		# runs every 15 mins
		job = ragapp.get_doc("Scheduled Job Type", dict(method="ragapp.oauth.delete_oauth2_data"))
		job.db_set("last_execution", "2019-01-01 00:00:00")
	...
	def test_cold_start(self):
		...
		job = ragapp.new_doc("Scheduled Job Type")
		job.frequency = "Daily"
