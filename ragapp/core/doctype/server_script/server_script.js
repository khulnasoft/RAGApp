...
ragapp.ui.form.on("Server Script", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Compare Versions"), () => {
				new ragapp.ui.DiffView("Server Script", "script", frm.doc.name);
			});
		}

		ragapp
			.call("ragapp.core.doctype.server_script.server_script.get_autocompletion_items")
			.then((r) => r.message)
			.then((items) => {
	...
	check_safe_exec(frm) {
		ragapp.xcall("ragapp.core.doctype.server_script.server_script.enabled").then((enabled) => {
			if (enabled === false) {
				let docs_link =
					"https://ragapp.khulnasoft.com/docs/user/en/desk/scripting/server-script";
...
<p>Payment processing events have a special state. See the <a href="https://github.com/khulnasoft/payments/blob/develop/payments/controllers/payment_controller.py">PaymentController in Ragapp Payments</a> for details.</p>
<pre>
	<code>
...
if ragapp.form_dict.message == "ping":
	ragapp.response['message'] = "pong"
else:
	ragapp.response['message'] = "ok"
</code></pre>
