...
# this is a separate file since it is imported in ragapp.model.document
...
def run_server_script_for_doc_event(doc, event):
	...
	if scripts:
		...
		for script_name in scripts:
			ragapp.get_cached_doc("Server Script", script_name).execute_doc(doc)
...
def get_server_script_map():
	...
	# 	}
	# }
	if ragapp.flags.in_patch and not ragapp.db.table_exists("Server Script"):
		return {}

	script_map = ragapp.client_cache.get_value("server_script_map")
	if script_map is None:
		script_map = {"permission_query": {}}
		enabled_server_scripts = ragapp.get_all(
			"Server Script",
		...
		ragapp.client_cache.set_value("server_script_map", script_map)
