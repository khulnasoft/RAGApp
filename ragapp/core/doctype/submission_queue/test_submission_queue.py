...
from ragapp.utils.background_jobs import get_queue
...
class TestSubmissionQueue(IntegrationTestCase):
	...
	def test_queue_operation(self):
		...
		if not ragapp.db.table_exists("Test Submission Queue", cached=False):
			doc = new_doctype("Test Submission Queue", is_submittable=True, queue_in_background=True)
			doc.insert()

		d = ragapp.new_doc("Test Submission Queue")
		...
		time.sleep(4)
		submission_queue = ragapp.get_last_doc("Submission Queue")
