ragapp.ui.form.on("System Settings", {
	refresh: function (frm) {
		ragapp.call({
			method: "ragapp.core.doctype.system_settings.system_settings.load",
			callback: function (data) {
				ragapp.all_timezones = data.message.timezones;
				frm.set_df_property("time_zone", "options", ragapp.all_timezones);

				$.each(data.message.defaults, function (key, val) {
					frm.set_value(key, val, null, true);
					ragapp.sys_defaults[key] = val;
				});
				if (frm.re_setup_moment) {
					ragapp.app.setup_moment();
	...
	after_save: function (frm) {
		...
		const attr_tuples = [
			[frm.doc.language, ragapp.boot.sysdefaults.language, ragapp.boot.user.language],
			[frm.doc.rounding_method, ragapp.boot.sysdefaults.rounding_method], // no user override.
		];
	...
	rounding_method: function (frm) {
		if (frm.doc.rounding_method == ragapp.boot.sysdefaults.rounding_method) return;
		let msg = __(
		...
		ragapp.confirm(
			...
			() => {},
			() => {
				frm.set_value("rounding_method", ragapp.boot.sysdefaults.rounding_method);
			}
		);
	},
