...
from ragapp.utils import cint, today
...
class SystemSettings(Document):
	...
	def validate(self):
		from ragapp.twofactor import toggle_two_factor_auth
		...
		if enable_password_policy and minimum_password_score <= 0:
			ragapp.throw(_("Please select Minimum Password Score"))
		...
		if self.session_expiry:
			...
			if len(parts) != 2 or not (cint(parts[0]) or cint(parts[1])):
				ragapp.throw(_("Session Expiry must be in format {0}").format("hh:mm"))

		if self.enable_two_factor_auth:
			if self.two_factor_method == "SMS":
				if not ragapp.db.get_single_value("SMS Settings", "sms_gateway_url"):
					ragapp.throw(
		...
		ragapp.flags.update_last_reset_password_date = False
		if self.force_user_to_reset_password and not cint(
			ragapp.db.get_single_value("System Settings", "force_user_to_reset_password")
		):
			ragapp.flags.update_last_reset_password_date = True
	...
	def validate_user_pass_login(self):
		...
		social_login_enabled = ragapp.db.exists("Social Login Key", {"enable_social_login": 1})
		ldap_enabled = ragapp.db.get_single_value("LDAP Settings", "enabled")
		login_with_email_link_enabled = ragapp.db.get_single_value("System Settings", "login_with_email_link")
	...
	def validate_backup_limit(self):
		if not self.backup_limit or self.backup_limit < 1:
			ragapp.msgprint(_("Number of backups must be greater than zero."), alert=True)
			self.backup_limit = 1
	...
	def on_update(self):
		...
		if ragapp.flags.update_last_reset_password_date:
			update_last_reset_password_date()

	def set_defaults(self):
		from ragapp.translate import set_default_language

		for df in self.meta.get("fields"):
			if df.fieldtype not in no_value_fields and self.has_value_changed(df.fieldname):
				ragapp.db.set_default(df.fieldname, self.get(df.fieldname))
...
@ragapp.whitelist()
def load():
	from ragapp.utils.momentjs import get_all_timezones

	if "System Manager" not in ragapp.get_roles():
		ragapp.throw(_("Not permitted"), ragapp.PermissionError)

	all_defaults = ragapp.db.get_defaults()
	defaults = {}

	for df in ragapp.get_meta("System Settings").get("fields"):
...
cache_key = ragapp.get_document_cache_key("System Settings", "System Settings")


def get_system_settings(key: str):
	"""Return the value associated with the given `key` from System Settings DocType."""
	if not (system_settings := getattr(ragapp.local, "system_settings", None)):
		try:
			system_settings = ragapp.client_cache.get_doc("System Settings")
			ragapp.local.system_settings = system_settings
		except ragapp.DoesNotExistError:  # possible during new install
...
def clear_system_settings_cache():
	ragapp.client_cache.delete_value(cache_key)
	ragapp.cache.delete_value("system_settings")
	ragapp.cache.delete_value("time_zone")
