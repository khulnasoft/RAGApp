ragapp.ui.form.on("User", {
	setup: function (frm) {
		frm.set_query("default_workspace", () => {
			return {
				filters: {
					for_user: ["in", [null, ragapp.session.user]],
	...
	before_load: function (frm) {
		let update_tz_options = function () {
			frm.fields_dict.time_zone.set_data(ragapp.all_timezones);
		};

		if (!ragapp.all_timezones) {
			ragapp.call({
				method: "ragapp.core.doctype.user.user.get_timezones",
				callback: function (r) {
					ragapp.all_timezones = r.message.timezones;
					update_tz_options();
				},
			});
		} else {
	...
	module_profile: function (frm) {
		if (frm.doc.module_profile) {
			ragapp.call({
				method: "ragapp.core.doctype.user.user.get_module_profile",
				args: {
	...
	onload: function (frm) {
		...
		if (
			...
			if (!frm.roles_editor) {
				...
				frm.roles_editor = new ragapp.RoleEditor(
					role_area,
					frm,
				...
				if (frm.doc.user_type == "System User") {
					...
					frm.module_editor = new ragapp.ModuleEditor(frm, module_area);
				}
			} else {
				frm.roles_editor.show();
			}
	...
	refresh: function (frm) {
		...
		ragapp.xcall("ragapp.apps.get_apps").then((r) => {
			let apps = r?.map((r) => r.name) || [];
			frm.set_df_property("default_app", "options", [" ", ...apps]);
		});

		if (frm.is_new()) {
			frm.set_value("time_zone", ragapp.sys_defaults.time_zone);
		}
		...
		if (!frm.is_new()) {
			if (has_access_to_edit_user()) {
				...
				frm.add_custom_button(
					...
					() =>
						ragapp.set_route("query-report", "Permitted Documents For User", {
							user: frm.doc.name,
						}),
					__("Permissions")
				);
				...
				frm.add_custom_button(
					...
					() =>
						ragapp.set_route("query-report", "User Doctype Permissions", {
							user: frm.doc.name,
						}),
					__("Permissions")
				);
			...
			frm.add_custom_button(
				...
				function () {
					ragapp.call({
						method: "ragapp.core.doctype.user.user.reset_password",
						args: {
							user: frm.doc.name,
						},
					});
			...
			if (ragapp.user.has_role("System Manager")) {
				ragapp.db.get_single_value("LDAP Settings", "enabled").then((value) => {
								...
								const d = new ragapp.ui.Dialog({
									...
									primary_action: (values) => {
										...
										if (values.new_password !== values.confirm_password) {
											ragapp.throw(__("Passwords do not match!"));
										}
										ragapp.call(
											"ragapp.integrations.doctype.ldap_settings.ldap_settings.reset_password",
											{
												user: frm.doc.email,
			...
			if (
				cint(ragapp.boot.sysdefaults.enable_two_factor_auth) &&
				(ragapp.session.user == doc.name || ragapp.user.has_role("System Manager"))
			) {
				frm.add_custom_button(
					__("Reset OTP Secret"),
					function () {
						ragapp.call({
							method: "ragapp.twofactor.reset_otp_secret",
							args: {
								user: frm.doc.name,
							},
						});
			...
			if (ragapp.session.user == doc.name) {
				// update display settings
				if (doc.user_image) {
					ragapp.boot.user_info[ragapp.session.user].image = ragapp.utils.get_file_link(
						doc.user_image
					);
				}
			}
		}
		if (frm.doc.user_emails && ragapp.model.can_create("Email Account")) {
			var found = 0;
			...
			if (!found) {
				frm.add_custom_button(__("Create User Email"), function () {
					if (!frm.doc.email) {
						ragapp.msgprint(__("Email is mandatory to create User Email"));
						return;
					}
		...
		if (ragapp.route_flags.unsaved === 1) {
			delete ragapp.route_flags.unsaved;
	...
	create_user_email: function (frm) {
		ragapp.call({
			method: "ragapp.core.doctype.user.user.has_email_account",
			args: {
				email: frm.doc.email,
			},
			callback: function (r) {
				if (!Array.isArray(r.message) || !r.message.length) {
					...
					ragapp.model.with_doctype("Email Account", function (doc) {
						doc = ragapp.model.get_new_doc("Email Account");
						ragapp.route_flags.linked_user = frm.doc.name;
						ragapp.route_flags.delete_user_from_locals = true;
						ragapp.set_route("Form", "Email Account", doc.name);
					});
				} else {
					ragapp.route_flags.create_user_account = frm.doc.name;
					ragapp.set_route("Form", "Email Account", r.message[0]["name"]);
				}
			},
		});
	},
	generate_keys: function (frm) {
		ragapp.call({
			method: "ragapp.core.doctype.user.user.generate_keys",
			args: {
				user: frm.doc.name,
			},
			callback: function (r) {
				if (r.message) {
					ragapp.msgprint(__("Save API Secret: {0}", [r.message.api_secret]));
					frm.reload_doc();
				}
			},
		});
	...
	after_save: function (frm) {
		...
		if (doc.name === ragapp.session.user && attr_tuples.some(has_effectively_changed)) {
			ragapp.msgprint(__("Refreshing..."));
		...
		if (
			...
			frm.add_custom_button(__("Impersonate"), () => {
				...
				ragapp.prompt(
					...
					(values) => {
						ragapp
							.xcall("ragapp.core.doctype.user.user.impersonate", {
								user: frm.doc.name,
...
ragapp.ui.form.on("User Email", {
	email_account(frm, cdt, cdn) {
		let child_row = locals[cdt][cdn];
		ragapp.model.get_value(
			"Email Account",
...
function has_access_to_edit_user() {
	return has_common(ragapp.user_roles, get_roles_for_editing_user());
}
