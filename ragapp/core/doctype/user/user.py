...
import ragapp.share
from ragapp import STANDARD_USERS, _, msgprint, throw
from ragapp.apps import get_default_path
from ragapp.auth import MAX_PASSWORD_SIZE
from ragapp.core.doctype.user_type.user_type import user_linked_with_permission_on_doctype
...
from ragapp.utils.data import sha256_hash
from ragapp.utils.password import check_password, get_password_reset_limit
from ragapp.utils.password import update_password as _update_password
from ragapp.utils.user import get_system_managers
from ragapp.website.utils import get_home_page, is_signup_disabled
...
class User(Document):
	...
	if TYPE_CHECKING:
		from ragapp.core.doctype.block_module.block_module import BlockModule
		from ragapp.core.doctype.defaultvalue.defaultvalue import DefaultValue
		from ragapp.core.doctype.has_role.has_role import HasRole
		from ragapp.core.doctype.user_email.user_email import UserEmail
		from ragapp.core.doctype.user_role_profile.user_role_profile import UserRoleProfile
		from ragapp.core.doctype.user_social_login.user_social_login import UserSocialLogin
	...
	def onload(self):
		from ragapp.utils.modules import get_modules_from_all_apps
	...
	def after_insert(self):
		...
		ragapp.cache.delete_key("users_for_mentions")
		ragapp.cache.delete_key("enabled_users")
	...
	def validate(self):
		...
		if (self.name not in ["Administrator", "Guest"]) and (not self.get_social_login_userid("ragapp")):
			self.set_social_login_userid("ragapp", ragapp.generate_hash(length=39))
	...
	def populate_role_profile_roles(self):
		...
		for role_profile in self.role_profiles:
			role_profile = ragapp.get_cached_doc("Role Profile", role_profile.role_profile)
	...
	from ragapp.deprecation_dumpster import validate_roles
	...
	def validate_allowed_modules(self):
		if self.module_profile:
			module_profile = ragapp.get_doc("Module Profile", self.module_profile)
	...
	def validate_user_image(self):
		if self.user_image and len(self.user_image) > 2000:
			ragapp.throw(_("Not a valid User Image."))

	def on_update(self):
		# clear new password
		self.share_with_self()
		clear_notifications(user=self.name)
		ragapp.clear_cache(user=self.name)
		now = ragapp.flags.in_test or ragapp.flags.in_install
		self.send_password_notification(self.__new_password)
		ragapp.enqueue(
			"ragapp.core.doctype.user.user.create_contact",
			user=self,
		...
		if self.name not in STANDARD_USERS and not self.user_image:
			ragapp.enqueue(
				"ragapp.core.doctype.user.user.update_gravatar",
				name=self.name,
				now=now,
				enqueue_after_commit=True,
			)
		...
		if self.time_zone:
			ragapp.defaults.set_default("time_zone", self.time_zone, self.name)

		if self.has_value_changed("language"):
			locale_keys = ("date_format", "time_format", "number_format", "first_day_of_the_week")
			if self.language:
				language = ragapp.get_doc("Language", self.language)
				for key in locale_keys:
					value = language.get(key)
					if value:
						ragapp.defaults.set_default(key, value, self.name)
			else:
				for key in locale_keys:
					ragapp.defaults.clear_default(key, parent=self.name)
	...
	def has_website_permission(self, ptype, user, verbose=False):
		...
		return self.name == ragapp.session.user
	...
	def check_enable_disable(self):
		...
		if not cint(self.enabled) and self.name in STANDARD_USERS:
			ragapp.throw(_("User {0} cannot be disabled").format(self.name))

		# clear sessions if disabled
		if not cint(self.enabled) and getattr(ragapp.local, "login_manager", None):
			ragapp.local.login_manager.logout(user=self.name)
	...
	def set_system_user(self):
		...
		if self.user_type and not ragapp.get_cached_value("User Type", self.user_type, "is_standard"):
	...
	def set_roles_and_modules_based_on_user_type(self):
		user_type_doc = ragapp.get_cached_doc("User Type", self.user_type)
	...
	def validate_share(self, docshare):
		...
		# 		if docshare.share != 1:
		# 			ragapp.throw(_("Sorry! User should have complete access to their own record."))
		# 	else:
		# 		ragapp.throw(_("Sorry! Sharing with Website User is prohibited."))
	...
	def password_reset_mail(self, link):
		reset_password_template = ragapp.db.get_system_setting("reset_password_template")
	...
	def send_welcome_mail_to_user(self):
		...
		subject = None
		method = ragapp.get_hooks("welcome_email")
		if method:
			subject = ragapp.get_attr(method[-1])()
		if not subject:
			site_name = ragapp.db.get_default("site_name") or ragapp.get_conf().get("site_name")
			if site_name:
				subject = _("Welcome to {0}").format(site_name)
			else:
				subject = _("Complete Registration")

		welcome_email_template = ragapp.db.get_system_setting("welcome_email_template")
	...
	def send_login_mail(self, subject, template, add_args, now=None, custom_template=None):
		...
		from ragapp.utils import get_url
		from ragapp.utils.user import get_user_fullname

		created_by = get_user_fullname(ragapp.session["user"])
		if created_by == "Guest":
		...
		sender = (
			ragapp.session.user not in STANDARD_USERS and get_formatted_email(ragapp.session.user)
		) or None

		if custom_template:
			from ragapp.email.doctype.email_template.email_template import get_email_template
	...
	def on_trash(self):
		...
		self.enabled = 0
		if getattr(ragapp.local, "login_manager", None):
			ragapp.local.login_manager.logout(user=self.name)

		# delete todos
		ragapp.db.delete("ToDo", {"allocated_to": self.name})
		todo_table = DocType("ToDo")
		(
			ragapp.qb.update(todo_table)
			.set(todo_table.assigned_by, None)
			.where(todo_table.assigned_by == self.name)
		).run()

		# delete events
		ragapp.db.delete("Event", {"owner": self.name, "event_type": "Private"})

		# delete shares
		ragapp.db.delete("DocShare", {"user": self.name})
		# unlink contact
		table = DocType("Contact")
		ragapp.qb.update(table).where(table.user == self.name).set(table.user, None).run()

		# delete notification settings
		ragapp.delete_doc("Notification Settings", self.name, ignore_permissions=True)
		...
		ragapp.db.delete("User Permission", {"user": self.name})

		# Delete OAuth data
		ragapp.db.delete("OAuth Authorization Code", {"user": self.name})
		ragapp.db.delete("Token Cache", {"user": self.name})

		# Delete EPS data
		ragapp.db.delete("Energy Point Log", {"user": self.name})

		# Remove user link from Workflow Action
		ragapp.db.set_value("Workflow Action", {"user": self.name}, "user", None)

		# Delete user's List Filters
		ragapp.db.delete("List Filter", {"for_user": self.name})

		# Remove user from Note's Seen By table
		seen_notes = ragapp.get_all("Note", filters=[["Note Seen By", "user", "=", self.name]], pluck="name")
		for note_id in seen_notes:
			note = ragapp.get_doc("Note", note_id)
	...
	def before_rename(self, old_name, new_name, merge=False):
		...
		if merge:
			ragapp.delete_doc("Notification Settings", old_name, ignore_permissions=True)

		ragapp.clear_cache(user=old_name)
	...
	def after_rename(self, old_name, new_name, merge=False):
		tables = ragapp.db.get_tables()
		for tab in tables:
			desc = ragapp.db.get_table_columns_description(tab)
		...
		if ragapp.db.exists("Notification Settings", old_name):
			ragapp.rename_doc("Notification Settings", old_name, new_name, force=True, show_alert=False)

		# set email
		ragapp.db.set_value("User", new_name, "email", new_name)
	...
	def remove_disabled_roles(self):
		disabled_roles = [d.name for d in ragapp.get_all("Role", filters={"disabled": 1})]
	...
	def validate_username(self):
		if not self.username and self.is_new() and self.first_name:
			self.username = ragapp.scrub(self.first_name)
		...
		if self.username_exists():
			if self.user_type == "System User":
				ragapp.msgprint(_("Username {0} already exists").format(self.username))
	...
	def suggest_username(self):
		...
		# @firstname
		username = _check_suggestion(ragapp.scrub(self.first_name))

		if not username:
			# @firstname_last_name
			username = _check_suggestion(ragapp.scrub("{} {}".format(self.first_name, self.last_name or "")))

		if username:
			ragapp.msgprint(_("Suggested Username: {0}").format(username))

		return username

	def username_exists(self, username=None):
		return ragapp.db.get_value("User", {"username": username or self.username, "name": ("!=", self.name)})
	...
	def validate_user_email_inbox(self):
		...
		if len(email_accounts) != len(set(email_accounts)):
			ragapp.throw(_("Email Account added multiple times"))
	...
	def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
		...
		login_with_mobile = cint(ragapp.get_system_settings("allow_login_using_mobile_number"))
		login_with_username = cint(ragapp.get_system_settings("allow_login_using_user_name"))
		...
		users = ragapp.get_all("User", fields=["name", "enabled"], or_filters=or_filters, limit=1)
		if not users:
			return
		...
		if validate_password:
			...
			except ragapp.AuthenticationError:
	...
	def check_roles_added(self):
		...
		ragapp.msgprint(
			_("Newly created user {0} has no roles enabled.").format(ragapp.bold(self.name)),
			title=_("No Roles Specified"),
			indicator="orange",
			primary_action={
				"label": _("Add Roles"),
				"client_action": "ragapp.set_route",
...
def get_all_roles():
	...
	roles = ragapp.get_all(
		"Role",
		filters={
			"name": ("not in", ragapp.permissions.AUTOMATIC_ROLES),
			"disabled": 0,
		},
...
def get_roles(arg=None):
	...
	return ragapp.get_roles(ragapp.form_dict.get("uid", ragapp.session.user))


@ragapp.whitelist()
def get_perm_info(role):
	"""get permission info"""
	from ragapp.permissions import get_all_perms

	return get_all_perms(role)


@ragapp.whitelist(allow_guest=True, methods=["POST"])
def update_password(
	...
	if len(new_password) > MAX_PASSWORD_SIZE:
		ragapp.throw(_("Password size exceeded the maximum allowed size."))
	...
	if res.get("message"):
		ragapp.local.response.http_status_code = 410
		return res["message"]
	else:
		user = res["user"]

	logout_all_sessions = cint(logout_all_sessions) or ragapp.get_system_settings("logout_on_password_reset")
	_update_password(user, new_password, logout_all_sessions=cint(logout_all_sessions))

	user_doc, redirect_url = reset_user_data(user)

	# get redirect url from cache
	redirect_to = ragapp.cache.hget("redirect_after_login", user)
	if redirect_to:
		redirect_url = redirect_to
		ragapp.cache.hdel("redirect_after_login", user)

	ragapp.local.login_manager.login_as(user)

	ragapp.db.set_value("User", user, "last_password_reset_date", today())
	ragapp.db.set_value("User", user, "reset_password_key", "")
...
def test_password_strength(new_password: str, key=None, old_password=None, user_data: tuple | None = None):
	from ragapp.utils.password_strength import test_password_strength as _test_password_strength
	...
	enable_password_policy = ragapp.get_system_settings("enable_password_policy")

	if not enable_password_policy:
		return {}

	if not user_data:
		user_data = ragapp.db.get_value(
			"User", ragapp.session.user, ["first_name", "middle_name", "last_name", "email", "birth_date"]
		)

	if new_password:
		result = _test_password_strength(new_password, user_inputs=user_data)
		password_policy_validation_passed = False
		minimum_password_score = cint(ragapp.get_system_settings("minimum_password_score"))
...
def has_email_account(email: str):
	return ragapp.get_list("Email Account", filters={"email_id": email})


@ragapp.whitelist(allow_guest=False)
...
def ask_pass_update():
	...
	from ragapp.utils import set_default

	password_list = ragapp.get_all(
	...
	if key:
		...
		if result.user:
			reset_password_link_expiry = cint(
				ragapp.get_system_settings("reset_password_link_expiry_duration")
			)
			if (
		...
		# verify old password
		ragapp.local.login_manager.check_password(ragapp.session.user, old_password)
...
def reset_user_data(user):
	user_doc = ragapp.get_doc("User", user)
...
@ragapp.whitelist(methods=["POST"])
def verify_password(password):
	ragapp.local.login_manager.check_password(ragapp.session.user, password)


@ragapp.whitelist(allow_guest=True)
def sign_up(email: str, full_name: str, redirect_to: str) -> tuple[int, str]:
	if is_signup_disabled():
		ragapp.throw(_("Sign Up is disabled"), title=_("Not Allowed"))

	user = ragapp.db.get("User", {"email": email})
	if user:
		if user.enabled:
		...
		if ragapp.db.get_creation_count("User", 60) > 300:
		...
		from ragapp.utils import random_string
		...
		default_role = ragapp.db.get_single_value("Portal Settings", "default_role")
		if default_role:
			user.add_roles(default_role)

		if redirect_to:
			ragapp.cache.hset("redirect_after_login", user.name, redirect_to)
...
def reset_password(user: str) -> str:
	try:
		user: User = ragapp.get_doc("User", user)
		...
		ragapp.local.response["http_status_code"] = 404
...
def user_query(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters:
		if not (filters.get("ignore_user_type") and ragapp.session.data.user_type == "System User"):
			list_filters["user_type"] = ["!=", "Website User"]

		filters.pop("ignore_user_type", None)
		list_filters.update(filters)

	return ragapp.get_list(
		doctype,
...
def get_website_users():
	...
	return ragapp.db.count("User", filters={"enabled": True, "user_type": "Website User"})
...
def get_permission_query_conditions(user):
	if user == "Administrator":
		...
		return """(`tabUser`.name not in ({standard_users}))""".format(
			standard_users=", ".join(ragapp.db.escape(user) for user in STANDARD_USERS)
		)
...
def notify_admin_access_to_system_manager(login_manager=None):
	if (
		...
		and ragapp.local.conf.notify_admin_access_to_system_manager
	):
		site = '<a href="{0}" target="_blank">{0}</a>'.format(ragapp.local.request.host_url)
		date_and_time = "<b>{}</b>".format(format_datetime(now_datetime(), format_string="medium"))
		ip_address = ragapp.local.request_ip
...
def handle_password_test_fail(feedback: dict):
	...
	ragapp.throw(msg=" ".join([warning, *suggestions]), title=_("Invalid Password"))


def update_gravatar(name):
	gravatar = has_gravatar(name)
	if gravatar:
		ragapp.db.set_value("User", name, "user_image", gravatar)
...
def throttle_user_creation():
	...
	if ragapp.db.get_creation_count("User", 60) > ragapp.local.conf.get("throttle_user_limit", 60):
		ragapp.throw(_("Throttled"))


@ragapp.whitelist()
def get_module_profile(module_profile: str):
	module_profile = ragapp.get_doc("Module Profile", {"module_profile_name": module_profile})
	return module_profile.get("block_modules")


def create_contact(user, ignore_links=False, ignore_mandatory=False):
	from ragapp.contacts.doctype.contact.contact import get_contact_name
	...
	if not contact_name:
		...
		except ragapp.DuplicateEntryError:
			pass
	else:
		try:
			contact = ragapp.get_doc("Contact", contact_name)
		...
		except ragapp.TimestampMismatchError:
			raise ragapp.RetryBackgroundJobError
...
def generate_keys(user: str):
	...
	user_details: User = ragapp.get_doc("User", user)
	api_secret = ragapp.generate_hash(length=15)
	# if api key is not set generate api key
	if not user_details.api_key:
		api_key = ragapp.generate_hash(length=15)
...
def switch_theme(theme):
	if theme in ["Dark", "Light", "Automatic"]:
		ragapp.db.set_value("User", ragapp.session.user, "desk_theme", theme)


def get_enabled_users():
	def _get_enabled_users():
		enabled_users = ragapp.get_all("User", filters={"enabled": "1"}, pluck="name")
		return enabled_users

	return ragapp.cache.get_value("enabled_users", _get_enabled_users)
...
def impersonate(user: str, reason: str):
	...
	impersonator = ragapp.session.user
	ragapp.get_doc(
		{
