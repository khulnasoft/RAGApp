...
from ragapp.desk.form.linked_with import get_linked_doctypes
...
class UserPermission(Document):
	...
	def on_update(self):
		ragapp.cache.hdel("user_permissions", self.user)
		ragapp.publish_realtime("update_user_permissions", user=self.user, after_commit=True)
	...
	def validate_user_permission(self):
		...
		duplicate_exists = ragapp.get_all(
			self.doctype,
			filters={
				"allow": self.allow,
	...
	def validate_default_permission(self):
		...
		if overlap_exists:
			ref_link = ragapp.get_desk_link(self.doctype, overlap_exists[0].name)
			ragapp.throw(_("{0} has already assigned default value for {1}.").format(ref_link, self.allow))
...
def get_user_permissions(user=None):
	...
	if ragapp.request and ragapp.local.form_dict.cmd == "get_user_permissions":
	...
	cached_user_permissions = ragapp.cache.hget("user_permissions", user)
	...
	try:
		for perm in ragapp.get_all(
			...
			meta = ragapp.get_meta(perm.allow)
			add_doc_to_perm(perm, perm.for_value, perm.is_default)

			if meta.is_nested_set() and not perm.hide_descendants:
				decendants = ragapp.db.get_descendants(perm.allow, perm.for_value)
				for doc in decendants:
					add_doc_to_perm(perm, doc, False)

		out = ragapp._dict(out)
		ragapp.cache.hset("user_permissions", user, out)
	except ragapp.db.SQLError as e:
...
def clear_user_permissions(user, for_doctype):
	ragapp.only_for("System Manager")
	total = ragapp.db.count("User Permission", {"user": user, "allow": for_doctype})
...
def insert_user_perm(
	...
	user_perm = ragapp.new_doc("User Permission")
	user_perm.user = user
