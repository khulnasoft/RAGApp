...
class TestUserType(IntegrationTestCase):
	...
	def test_add_select_perm_doctypes(self):
		...
		doc = ragapp.get_meta("Contact")
		link_fields = doc.get_link_fields()
		select_doctypes = ragapp.get_all(
	...
	def test_print_share_email_default(self):
		...
		perm = ragapp.get_all("Custom DocPerm", filters={"role": "_Test User Type"}, fields=["*"])[0]
...
def create_user_type(user_type):
	if ragapp.db.exists("User Type", user_type):
		ragapp.delete_doc("User Type", user_type)

	user_type_limit = {ragapp.scrub(user_type): 1}
...
def create_role():
	if not ragapp.db.exists("Role", "_Test User Type"):
		ragapp.get_doc(
