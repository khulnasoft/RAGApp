...
from ragapp.permissions import add_permission, add_user_permission
from ragapp.utils import get_link_to_form
from ragapp.utils.modules import get_modules_from_app
...
class UserType(Document):
	...
	if TYPE_CHECKING:
		from ragapp.core.doctype.user_document_type.user_document_type import UserDocumentType
		from ragapp.core.doctype.user_select_document_type.user_select_document_type import (
			UserSelectDocumentType,
		)
		from ragapp.core.doctype.user_type_module.user_type_module import UserTypeModule
	...
	def clear_cache(self):
		...
		if not self.is_standard:
			ragapp.cache.delete_value("non_standard_user_types")
	...
	def on_trash(self):
		if self.is_standard:
			ragapp.throw(_("Standard user type {0} can not be deleted.").format(ragapp.bold(self.name)))

	def set_modules(self):
		if not self.user_doctypes:
			return

		modules = ragapp.get_all(
			"DocType",
	...
	def validate_document_type_limit(self):
		limit = ragapp.conf.get("user_type_doctype_limit", {}).get(ragapp.scrub(self.name))

		if not limit and ragapp.session.user != "Administrator":
			ragapp.throw(
				_("User does not have permission to create the new {0}").format(ragapp.bold(_("User Type"))),
	...
	def validate_role(self):
		if not self.role:
			ragapp.throw(_("The field {0} is mandatory").format(ragapp.bold(_("Role"))))

		if not ragapp.db.get_value("Role", self.role, "is_custom"):
			ragapp.throw(
				_("The role {0} should be a custom role.").format(
					ragapp.bold(get_link_to_form("Role", self.role))
				)
			)

	def update_users(self):
		for row in ragapp.get_all("User", filters={"user_type": self.name}):
			user = ragapp.get_cached_doc("User", row.name)
	...
	def add_select_perm_doctypes(self):
		if ragapp.flags.ignore_select_perm:
			return
	...
	def remove_permission_for_deleted_doctypes(self):
		...
		for perm in ragapp.get_all(
			"Custom DocPerm", filters={"role": self.role, "parent": ["not in", doctypes]}
		):
			ragapp.delete_doc("Custom DocPerm", perm.name)


def add_role_permissions(doctype, role):
	name = ragapp.get_value("Custom DocPerm", dict(parent=doctype, role=role, permlevel=0))
...
def get_user_linked_doctypes(doctype, txt, searchfield, start, page_len, filters):
	modules = [d.get("module_name") for d in get_modules_from_app("ragapp")]
	...
	custom_doctypes = ragapp.get_all(
...
def user_linked_with_permission_on_doctype(doc, user):
	...
	if not doc.user_id_field:
		ragapp.throw(_("User Id Field is mandatory in the user type {0}").format(ragapp.bold(doc.name)))

	if ragapp.db.get_value(doc.apply_user_permission_on, {doc.user_id_field: user}, "name"):
		return True
	else:
		label = ragapp.get_meta(doc.apply_user_permission_on).get_field(doc.user_id_field).label

		ragapp.msgprint(
			_(
				"To set the role {0} in the user {1}, kindly set the {2} field as {3} in one of the {4} record."
			).format(
				ragapp.bold(doc.role),
				ragapp.bold(user),
				ragapp.bold(label),
				ragapp.bold(user),
				ragapp.bold(doc.apply_user_permission_on),
			)
		)
...
def apply_permissions_for_non_standard_user_type(doc, method=None):
	...
	if not ragapp.db.table_exists("User Type") or ragapp.flags.in_migrate:
		return

	user_types = ragapp.cache.get_value(
	...
	for user_type, data in user_types.items():
		...
		if doc.get(data[1]) and (
			...
			if not perm_data:
				...
				user_perm = ragapp.get_doc("User Permission", perm_data[0])
