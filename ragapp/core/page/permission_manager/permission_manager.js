ragapp.pages["permission-manager"].on_page_load = (wrapper) => {
	let page = ragapp.ui.make_app_page({
		parent: wrapper,
	...
	$(ragapp.render_template("permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new ragapp.PermissionEngine(wrapper);
};

ragapp.pages["permission-manager"].refresh = function (wrapper) {
	wrapper.permission_engine.set_from_route();
};

ragapp.PermissionEngine = class PermissionEngine {
	constructor(wrapper) {
	...
	setup_page() {
		this.doctype_select = this.wrapper.page.add_field({
			...
			options: "DocType",
			change: function () {
				ragapp.set_route("permission-manager", this.get_value());
			},
		});
		...
		this.page.add_inner_button(__("Set User Permissions"), () => {
			return ragapp.set_route("List", "User Permission");
		});
		this.set_from_route();
	}
	...
	set_from_route() {
		...
		if (ragapp.get_route()[1]) {
			this.doctype_select.set_value(ragapp.get_route()[1]);
		} else if (ragapp.route_options) {
			if (ragapp.route_options.doctype) {
				this.doctype_select.set_value(ragapp.route_options.doctype);
			}
			if (ragapp.route_options.role) {
				this.role_select.set_value(ragapp.route_options.role);
			}
	...
	reset_std_permissions(data) {
		let doctype = this.get_doctype();
		let d = ragapp.confirm(__("Reset Permissions for {0}?", [__(doctype)]), () => {
			return ragapp
				.call({
					module: "ragapp.core",
					page: "permission_manager",
		...
		data.message.forEach((d) => {
			let rights = this.rights
				.filter((r) => d[r])
				.map((r) => {
					return __(toTitle(ragapp.unscrub(r)));
				});
	...
	add_delete_button(row, d) {
		$(
			`<button class='btn btn-danger btn-remove-perm btn-xs'>${ragapp.utils.icon(
				"delete"
			)}</button>`
		)
			...
			.on("click", () => {
				return ragapp.call({
					...
					callback: (r) => {
						if (r.exc) {
							ragapp.msgprint(__("Did not remove"));
						} else {
							this.refresh();
						}
					},
	...
	add_check_events() {
		...
		this.body.on("click", ".show-user-permissions", () => {
			ragapp.route_options = { allow: this.get_doctype() || "" };
			...
			() => {
				...
				d.set_primary_action(__("Add"), () => {
					...
					ragapp.call({
						...
						callback: (r) => {
							if (r.exc) {
								ragapp.msgprint(__("Did not add"));
							} else {
								this.refresh();
							}
						},
	...
	get_link_fields(doctype) {
		return ragapp.get_children("DocType", doctype, "fields", {
			fieldtype: "Link",
