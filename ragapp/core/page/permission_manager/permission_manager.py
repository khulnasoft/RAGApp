...
from ragapp.modules.import_file import get_file_path, read_doc_from_file
...
from ragapp.utils.user import get_users_with_role as _get_user_with_role
...
def get_roles_and_doctypes():
	...
	if ragapp.session.user != "Administrator":
		custom_user_type_roles = ragapp.get_all("User Type", filters={"is_standard": 0}, fields=["role"])
...
def get_permissions(doctype: str | None = None, role: str | None = None):
	...
	if role:
		...
		if ragapp.session.user != "Administrator":
			custom_roles = ragapp.get_all("Role", filters={"is_custom": 1}, pluck="name")
			filters["role"] = ["not in", custom_roles]

		out = ragapp.get_all("Custom DocPerm", fields="*", filters=filters, order_by="permlevel")
		if not out:
			out = ragapp.get_all("DocPerm", fields="*", filters=filters, order_by="permlevel")
	...
	for d in out:
		...
		if meta := ragapp.get_meta(d.parent):
...
def update(doctype: str, role: str, permlevel: int, ptype: str, value=None, if_owner=0) -> str | None:
	...
	if ptype == "report" and value == "1" and if_owner == "1":
		ragapp.throw(_("Cannot set 'Report' permission if 'Only If Creator' permission is set"))
	...
	ragapp.db.after_commit.add(clear_cache)
...
def remove(doctype, role, permlevel, if_owner=0):
	...
	custom_docperms = ragapp.db.get_values(
		"Custom DocPerm", {"parent": doctype, "role": role, "permlevel": permlevel, "if_owner": if_owner}
	)
	for name in custom_docperms:
		ragapp.delete_doc("Custom DocPerm", name, ignore_permissions=True, force=True)

	if not ragapp.get_all("Custom DocPerm", {"parent": doctype}):
		ragapp.throw(_("There must be atleast one permission rule."), title=_("Cannot Remove"))
