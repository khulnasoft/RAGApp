...
ragapp.ui.form.on("Custom Field", {
	setup: function (frm) {
	...
	dt: function (frm) {
		...
		return ragapp.call({
			method: "ragapp.custom.doctype.custom_field.custom_field.get_fields_label",
	...
	label: function (frm) {
		if (frm.doc.label && ragapp.utils.has_special_chars(frm.doc.label)) {
			...
			frm.add_custom_button(__("Rename Fieldname"), () => {
				ragapp.prompt(
					...
					function (data) {
						ragapp
							.xcall(
								"ragapp.custom.doctype.custom_field.custom_field.rename_fieldname",
								{
...
ragapp.utils.has_special_chars = function (t) {
	var iChars = "!@#$%^&*()+=-[]\\';,./{}|\":<>?";
