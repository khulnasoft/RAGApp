...
ragapp.provide("ragapp.customize_form");

ragapp.ui.form.on("Customize Form", {
	setup: function (frm) {
		...
		frm.set_query("doc_type", function () {
			return {
				filters: [
					...
					["DocType", "restrict_to_domain", "in", ragapp.boot.active_domains],
				],
			};
		});
	...
	refresh: function (frm) {
		...
		if (frm.doc.doc_type) {
			ragapp.model.with_doctype(frm.doc.doc_type).then(() => {
				frm.page.set_title(__("Customize Form - {0}", [__(frm.doc.doc_type)]));
				ragapp.customize_form.set_primary_action(frm);

				frm.add_custom_button(
					__("Go to {0} List", [__(frm.doc.doc_type)]),
					function () {
						ragapp.set_route("List", frm.doc.doc_type);
					},
					__("Actions")
				);
				...
				frm.add_custom_button(
					__("Set Permissions"),
					function () {
						ragapp.set_route("permission-manager", frm.doc.doc_type);
					},
					__("Actions")
				);
				...
				frm.add_custom_button(
					...
					function () {
						ragapp.customize_form.confirm(__("Remove all customizations?"), frm);
					},
					__("Actions")
				);
				...
				frm.toggle_display(
					["queue_in_background"],
					ragapp.get_meta(frm.doc.doc_type).is_submittable || 0
				);
	...
	set_default_doc_type(frm) {
		let doc_type;
		if (ragapp.route_options && ragapp.route_options.doc_type) {
			doc_type = ragapp.route_options.doc_type;
				...
				return frm.call({
					...
					callback: function (r) {
						if (!r.exc) {
							...
							ragapp.customize_form.clear_locals_and_refresh(frm);
						}
					},
				});
			}
	...
	async trim_table(frm) {
		let dropped_columns = await ragapp.xcall(
			"ragapp.custom.doctype.customize_form.customize_form.get_orphaned_columns",
			{ doctype: frm.doc.doc_type }
		);

		if (!dropped_columns?.length) {
			ragapp.toast(__("This doctype has no orphan fields to trim"));
			return;
		}
		let msg = __(
			...
			frm.add_custom_button(
				...
				function () {
					ragapp.prompt(
						...
						function (data) {
							ragapp.call({
								method: "ragapp.modules.utils.export_customizations",
								args: {
									doctype: frm.doc.doc_type,
									module: data.module,
	...
	setup_sort_order(frm) {
		...
		if (frm.doc.doc_type) {
			var fields = $.map(frm.doc.fields, function (df) {
				return ragapp.model.is_value_type(df.fieldtype) ? df.fieldname : null;
			});
	...
	setup_default_views(frm) {
		ragapp.model.set_default_views_for_doctype(frm.doc.doc_type, frm);
	},
});

// can't delete standard fields
ragapp.ui.form.on("Customize Form Field", {
	before_fields_remove: function (frm, doctype, name) {
		const row = ragapp.get_doc(doctype, name);
	...
	fields_add: function (frm, cdt, cdn) {
		var f = ragapp.model.get_doc(cdt, cdn);
...
ragapp.ui.form.on("DocType Link", {
	before_links_remove: function (frm, doctype, name) {
		let row = ragapp.get_doc(doctype, name);
		...
		if (!(row.custom || row.__islocal)) {
			ragapp.msgprint(__("Cannot delete standard link. You can hide it if you want"));
			throw "cannot delete standard link";
		}
	},
	links_add: function (frm, cdt, cdn) {
		let f = ragapp.model.get_doc(cdt, cdn);
		f.custom = 1;
	},
...
ragapp.ui.form.on("DocType Action", {
	before_actions_remove: function (frm, doctype, name) {
		...
		if (!(row.custom || row.__islocal)) {
			ragapp.msgprint(__("Cannot delete standard action. You can hide it if you want"));
...
ragapp.ui.form.on("DocType State", {
	before_states_remove: function (frm, doctype, name) {
		...
		if (!(row.custom || row.__islocal)) {
			ragapp.msgprint(__("Cannot delete standard document state."));
...
ragapp.customize_form.save_customization = function (frm) {
	if (frm.doc.doc_type) {
		return frm.call({
			doc: frm.doc,
			freeze: true,
...
ragapp.customize_form.update_fields_from_form_builder = function (frm) {
...
ragapp.customize_form.set_primary_action = function (frm) {
...
ragapp.customize_form.confirm = function (msg, frm) {
	...
	ragapp.customize_form.confirm.dialog = d;
	d.show();
};

ragapp.customize_form.clear_locals_and_refresh = function (frm) {
	delete frm.doc.__unsaved;
	// clear doctype from locals
	ragapp.model.clear_doc("DocType", frm.doc.doc_type);
	delete ragapp.meta.docfield_copy[frm.doc.doc_type];
	frm.refresh();
};

function render_form_builder(frm) {
	if (ragapp.form_builder && ragapp.form_builder.doctype === frm.doc.doc_type) {
	...
	if (ragapp.form_builder) {
		...
		ragapp.form_builder.doctype = frm.doc.doc_type;
		ragapp.form_builder.customize = true;
