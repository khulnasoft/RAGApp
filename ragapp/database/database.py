...
from ragapp.database.utils import (
	DefaultOrderBy,
	EmptyQueryValues,
	FallBackDateTimeStr,
	FilterValue,
...
from ragapp.exceptions import DoesNotExistError, ImplicitCommitError
from ragapp.monitor import get_trace_id
from ragapp.query_builder import Case
from ragapp.query_builder.functions import Count
from ragapp.utils import CallbackManager, cint, get_datetime, get_table_name, getdate, now, sbool
from ragapp.utils import cast as cast_fieldtype
...
class Database:
	...
	class InvalidColumnName(ragapp.ValidationError):
		pass
	...
	def __init__(
		...
		self.value_cache = {}
		self.logger = ragapp.logger("database")
	...
	def sql(
		...
		*,
		...
		Examples:
		        ...
		        ragapp.db.sql("select name from tabCustomer", as_dict=True)

		        # return names beginning with a
		        ragapp.db.sql("select name from tabCustomer where name like %s", "a%")

		        # values as dict
		        ragapp.db.sql("select name from tabCustomer where name like %(name)s and owner=%(owner)s",
		                {"name": "a%", "owner":"test@example.com"})

		"""
		if isinstance(query, MySQLQueryBuilder | PostgreSQLQueryBuilder):
			ragapp.log("Use run method to execute SQL queries generated by Query Builder")
		...
		if trace_id := get_trace_id():
			query += f" /* RAGAPP_TRACE_ID: {trace_id} */"

		try:
			self._cursor.execute(query, values)
		except Exception as e:
			if self.is_syntax_error(e):
				ragapp.log(f"Syntax error in query:\n{query} {values or ''}")

			elif self.is_deadlocked(e):
				raise ragapp.QueryDeadlockError(e) from e

			elif self.is_timedout(e):
				raise ragapp.QueryTimeoutError(e) from e
				...
				ragapp.throw(
					...
					exc=ragapp.InReadOnlyMode,
				)

			# TODO: added temporarily
			elif self.db_type == "postgres":
				traceback.print_stack()
				ragapp.log(f"Error in query:\n{e}")
				raise
		...
		if debug:
			time_end = time()
			ragapp.log(f"Execution time: {time_end - time_start:.2f} sec")
	...
	def _return_as_iterator(self, *, pluck, as_dict, as_list, update):
		while result := self._transform_result(self._cursor.fetchmany(SQL_ITERATOR_BATCH_SIZE)):
			if pluck:
				...
				for row in result:
					row = ragapp._dict(zip(keys, row, strict=False))
					if update:
						row.update(update)
					yield row
				...
				ragapp.throw(_("`as_iterator` only works with `as_list=True` or `as_dict=True`"))
	...
	def _log_query(
		...
		if (
			ragapp.conf.allow_tests
			and ragapp.conf.developer_mode
			and ragapp.cache.get_value("flag_print_sql")
		):
	...
	def mogrify(self, query: Query, values: QueryValues):
		...
		try:
			...
			if isinstance(values, dict):
				return query % {
					k: ragapp.db.escape(v) if isinstance(v, str) else v for k, v in values.items()
				}
			elif isinstance(values, list | tuple):
				return query % tuple(ragapp.db.escape(v) if isinstance(v, str) else v for v in values)
			return query, values
	...
	def explain_query(self, query, values=EmptyQueryValues):
		...
		ragapp.log("--- query explain ---")
		try:
			results = self.sql(f"EXPLAIN {query}", values, as_dict=1)
		except Exception as e:
			ragapp.log(f"error in query explain: {e}")
		else:
			ragapp.log(json.dumps(results, indent=1))
			ragapp.log("--- query explain end ---")
	...
	def sql_list(self, query, values=(), debug=False, **kwargs):
		...
		Example:
		        ...
		        doctypes = ragapp.db.sql_list("select name from DocType")
		"""
		...
		if query.lstrip()[:6].lower() in ("update", "insert", "delete"):
			...
			if self.transaction_writes > self.MAX_WRITES_PER_TRANSACTION:
				if self.auto_commit_on_many_writes:
					...
					raise ragapp.TooManyWritesError(msg)
	...
	def fetch_as_dict(self, result) -> list[ragapp._dict]:
		"""Internal. Convert results to dict."""
		if result:
			keys = [column[0] for column in self._cursor.description]

		return [ragapp._dict(zip(keys, row, strict=False)) for row in result]

	@staticmethod
	def clear_db_table_cache(query):
		if query and is_query_type(query, ("drop", "create")):
			ragapp.cache.delete_key("db_tables")
	...
	def get_value(
		...
		*,
		...
		Example:
		        ...
		        ragapp.db.get_value("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        ragapp.db.get_value("User", "test@example.com", "last_login")

		        last_login, last_ip = ragapp.db.get_value("User", "test@example.com",
		                ["last_login", "last_ip"])

		        # returns default date_format
		        ragapp.db.get_value("System Settings", None, "date_format")
		"""
	...
	def get_values(
		...
		*,
		...
		Example:
		        ...
		        # return first customer starting with a
		        customers = ragapp.db.get_values("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        user = ragapp.db.get_values("User", "test@example.com", "*")[0]
		"""
		out = None
		cache_key = None
		...
		if isinstance(filters, list):
			...
			if (filters is not None) and (filters != doctype or doctype == "DocType"):
				try:
					...
					if ignore and (
						ragapp.db.is_missing_column(e)
						or ragapp.db.is_table_missing(e)
						or str(e).startswith("Invalid DocType")
					):
						out = None
					elif (not ignore) and ragapp.db.is_table_missing(e):
	...
	def get_singles_dict(self, doctype, debug=False, *, for_update=False, cast=False):
		...
		Example:
		        ...
		        account_settings = ragapp.db.get_singles_dict("Accounts Settings")
		"""
		queried_result = ragapp.qb.get_query(
			"Singles",
		...
		if not cast:
			return ragapp._dict(queried_result)
	...
	def get_all(*args, **kwargs):
		return ragapp.get_all(*args, **kwargs)

	@staticmethod
	def get_list(*args, **kwargs):
		return ragapp.get_list(*args, **kwargs)
	...
	def _get_update_dict(
		...
		if update_modified:
			...
			modified_by = modified_by or ragapp.session.user
	...
	def set_single_value(
		...
		*,
		...
		Example:
		        ...
		        ragapp.db.set_single_value("System Settings", "deny_multiple_sessions", True)
		"""
		...
		ragapp.qb.into("Singles").columns("doctype", "field", "value").insert(*singles_data).run(debug=debug)
		ragapp.clear_document_cache(doctype, doctype)
	...
	def get_single_value(self, doctype: str, fieldname: str, cache: bool = True):
		...
		Example:
		        ...
		        company = ragapp.db.get_single_value('Global Defaults', 'default_company')
		"""
		...
		df = ragapp.get_meta(doctype).get_field(fieldname)

		if not df:
			ragapp.throw(
				_("Field {0} does not exist on {1}").format(
					ragapp.bold(fieldname), ragapp.bold(doctype), self.InvalidColumnName
				)
			)
	...
	def set_value(
		...
		**Warning:** this function will not call Document events and should be avoided in normal cases.
		...
		"""
		from ragapp.model.utils import is_single_doctype
		...
		if isinstance(dn, FilterValue):
			ragapp.clear_document_cache(dt, convert_to_value(dn))
		else:
	...
	def _build_and_run_bulk_update_query(
		...
		update_query = ragapp.qb.update(dt)
	...
	def set_default(key, val, parent="__default", parenttype=None):
		...
		ragapp.defaults.set_default(key, val, parent, parenttype)
	...
	def add_default(key, val, parent="__default", parenttype=None):
		...
		ragapp.defaults.add_default(key, val, parent, parenttype)

	@staticmethod
	def get_defaults(key=None, parent="__default"):
		"""Get all defaults"""
		defaults = ragapp.defaults.get_defaults_for(parent)
		if not key:
			return defaults
		...
		return defaults.get(ragapp.scrub(key))

	def begin(self, *, read_only=False):
		read_only = read_only or ragapp.flags.read_only
	...
	def savepoint(self, save_point):
		...
		Changes can be undone to a save point by doing ragapp.db.rollback(save_point)
	...
	def a_row_exists(self, doctype):
		...
		return ragapp.get_all(doctype, limit=1, order_by=None, as_list=True)
	...
	def count(self, dt, filters=None, debug=False, cache=False, distinct: bool = True):
		...
		if cache and not filters:
			cache_count = ragapp.cache.get_value(f"doctype:count:{dt}")
			if cache_count is not None:
				return cache_count
		count = ragapp.qb.get_query(
			table=dt,
			filters=filters,
			fields=Count("*"),
			distinct=distinct,
		...
		if not filters and cache:
			ragapp.cache.set_value(f"doctype:count:{dt}", count, expires_in_sec=86400)
		return count
	...
	def get_db_table_columns(self, table) -> list[str]:
		...
		columns = ragapp.client_cache.get_value(key)
		if columns is None:
			information_schema = ragapp.qb.Schema("information_schema")

			columns = (
				ragapp.qb.from_(information_schema.columns)
			...
			if columns:
				ragapp.cache.set_value(key, columns)
	...
	def get_system_setting(self, key):
		return ragapp.get_system_settings(key)
	...
	from ragapp.deprecation_dumpster import is_column_missing as _is_column_missing

	is_column_missing = staticmethod(_is_column_missing)

	def get_descendants(self, doctype, name):
		"""Return descendants of the group node in tree"""
		from ragapp.utils.nestedset import get_descendants_of
	...
	def log_touched_tables(self, query):
		if is_query_type(query, ("insert", "delete", "update", "alter", "drop", "rename")):
			...
			if ragapp.flags.touched_tables is None:
				ragapp.flags.touched_tables = set()
			ragapp.flags.touched_tables.update(tables)
	...
	def bulk_insert(
		...
		*,
		...
		query = ragapp.qb.into(table).columns(fields)
	...
	def create_sequence(self, *args, **kwargs):
		from ragapp.database.sequence import create_sequence

		return create_sequence(*args, **kwargs)

	def set_next_sequence_val(self, *args, **kwargs):
		from ragapp.database.sequence import set_next_val

		set_next_val(*args, **kwargs)

	def get_next_sequence_val(self, *args, **kwargs):
		from ragapp.database.sequence import get_next_val
	...
	def unbuffered_cursor(self):
		...
		Usage:
		        with ragapp.db.unbuffered_cursor():
		                for row in ragapp.db.sql("query with huge result", as_iterator=True):
...
def savepoint(catch: type | tuple[type, ...] = Exception):
	...
	try:
		...
		ragapp.db.release_savepoint(savepoint)
...
def get_query_execution_timeout() -> int:
	...
	if not ragapp.conf.get("enable_db_statement_timeout"):
		return 0

	# Zero means no timeout, which is the default value in db.
	timeout = 0
	with suppress(Exception):
		if getattr(ragapp.local, "request", None):
			timeout = ragapp.conf.http_timeout or 300
