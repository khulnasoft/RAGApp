...
from ragapp.database.mariadb.schema import MariaDBTable
from ragapp.utils import UnicodeWithAttrs, cstr, get_datetime, get_table_name
...
class MariaDBConnectionUtil:
	...
	def get_connection_settings(self) -> dict:
		...
		if ragapp.conf.local_infile:
			conn_settings["local_infile"] = ragapp.conf.local_infile

		if ragapp.conf.db_ssl_ca and ragapp.conf.db_ssl_cert and ragapp.conf.db_ssl_key:
			conn_settings["ssl"] = {
				"ca": ragapp.conf.db_ssl_ca,
				"cert": ragapp.conf.db_ssl_cert,
				"key": ragapp.conf.db_ssl_key,
			}
...
class MariaDBDatabase(MariaDBConnectionUtil, MariaDBExceptionUtil, Database):
	...
	def escape(s, percent=True):
		...
		# Also, given we're promoting use of the PyPika builder via ragapp.qb, the use
		# of this method should be limited.

		# pymysql expects unicode argument to escape_string with Python 3
		s = ragapp.as_unicode(escape_string(ragapp.as_unicode(s)), "utf-8").replace("`", "\\`")
	...
	def get_column_index(self, table_name: str, fieldname: str, unique: bool = False) -> ragapp._dict | None:
	...
	def get_tables(self, cached=True):
		...
		if cached:
			tables = ragapp.cache.get_value("db_tables")
			to_query = not tables
		...
		if to_query:
			...
			tables = (
				...
				.where(information_schema.tables.table_schema == ragapp.db.cur_db_name)
				.run(pluck=True)
			)
			ragapp.cache.set_value("db_tables", tables)
