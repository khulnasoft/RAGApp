...
def get_mariadb_variables():
	return ragapp._dict(ragapp.db.sql("show variables"))
...
def setup_database(force, verbose, mariadb_user_host_login_scope=None):
	ragapp.local.session = ragapp._dict({"user": "Administrator"})

	db_user = ragapp.conf.db_user
	db_name = ragapp.local.conf.db_name
	...
	dbman.create_user(db_user, ragapp.conf.db_password, **dbman_kwargs)
	if verbose:
...
def drop_user_and_database(
	...
	ragapp.local.db = get_root_connection()
	dbman = DbManager(ragapp.local.db)
	dbman.drop_database(db_name)
...
def bootstrap_database(verbose, source_sql=None):
	...
	ragapp.connect()
	if "tabDefaultValue" not in ragapp.db.get_tables(cached=False):
...
def check_compatible_versions():
	try:
		...
		if version_tuple < (10, 6):
			click.secho(
				f"Warning: MariaDB version {version} is older than 10.6 which is not supported by Ragapp",
				fg="yellow",
			)
		elif version_tuple > (11, 3):
			click.secho(
				f"Warning: MariaDB version {version} is newer than 11.3 which is not yet tested with Ragapp Framework.",
				fg="yellow",
			)
	except Exception:
		click.secho(
	...
	if not ragapp.local.flags.root_connection:
		...
		if not ragapp.flags.root_login:
			ragapp.flags.root_login = (
				ragapp.conf.get("mariadb_root_login")
		...
		if not ragapp.flags.root_password:
			ragapp.flags.root_password = (
				ragapp.conf.get("mariadb_root_password")
