...
from ragapp.database.postgres.schema import PostgresTable
from ragapp.database.utils import EmptyQueryValues, LazyDecode
from ragapp.utils import cstr, get_table_name
...
class PostgresExceptionUtil:
	...
	def is_statement_timeout(e):
		return PostgresDatabase.is_timedout(e) or isinstance(e, ragapp.QueryTimeoutError)
...
class PostgresDatabase(PostgresExceptionUtil, Database):
	...
	@property
	def db_schema(self):
		return ragapp.conf.get("db_schema", "public").replace("'", "").replace('"', "")
	...
	def get_db_table_columns(self, table) -> list[str]:
		...
		if (columns := ragapp.client_cache.get_value(key)) is not None:
			return columns
		...
		ragapp.client_cache.set_value(key, columns)
	...
	def describe(self, doctype: str) -> list | tuple:
		...
		return self.sql(
			f"SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_NAME = '{table_name}' and table_schema='{ragapp.conf.get('db_schema', 'public')}'"
		)
