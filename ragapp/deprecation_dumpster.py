...
This file is the final resting place (or should we say, "retirement home"?) for all the deprecated functions and methods of the Ragapp framework. It's like a code nursing home, but with more monkey-patching and less bingo.
...
2. The Ragapp version at the beginning of which it becomes an error and at the end of which it will be removed (its "graduation" to the great codebase in the sky)
...
# see also ragapp.__init__ enabling them when a dev_server
class RagappDeprecationError(Warning):
...
class RagappDeprecationWarning(Warning):
	"""Deprecated feature in next version"""


class PendingRagappDeprecationWarning(RagappDeprecationWarning):
...
warnings.simplefilter("error", RagappDeprecationError)
warnings.simplefilter("ignore", PendingRagappDeprecationWarning)


class V15RagappDeprecationWarning(RagappDeprecationError):
	pass


class V16RagappDeprecationWarning(RagappDeprecationWarning):
	pass


class V17RagappDeprecationWarning(PendingRagappDeprecationWarning):
	pass
...
def __get_deprecation_class(graduation: str | None = None, class_name: str | None = None) -> type:
	if graduation:
		...
		class_name = f"{cleaned_graduation}RagappDeprecationWarning"
		current_module = sys.modules[__name__]
	try:
		return getattr(current_module, class_name)
	except AttributeError:
		return PendingRagappDeprecationWarning
...
for warning_filter in pythonwarnings.split(","):
	...
	if len(parts) >= 3 and (
		parts[2] in ("RagappDeprecationError", "RagappDeprecationWarning", "PendingRagappDeprecationWarning")
		or parts[2].endswith("RagappDeprecationWarning")
	):
		try:
...
try:
	...
	def _deprecated(message: str, category=RagappDeprecationWarning, stacklevel=1) -> Callable[[T], T]:
...
def deprecated(original: str, marked: str, graduation: str, msg: str, stacklevel: int = 1):
	...
	Arguments:
	        - original: ragapp.utils.make_esc  (fully qualified)
	...
	def decorator(func):
		...
		if os.path.basename(caller_filepath) != "deprecation_dumpster.py":
			raise RuntimeError(
				...
				+ colorize("ragapp/deprecation_dumpster.py\n", Color.CYAN)
				+ colorize("Move the entire function there and import it back via adding\n ", Color.YELLOW)
				+ colorize(f"from ragapp.deprecation_dumpster import {func.__name__}\n", Color.CYAN)
				+ colorize("to file\n ", Color.YELLOW)
...
def get_local_with_deprecations() -> "Local":
	...
	class LocalWithDeprecations(Local):
		...
		# 		"2024-12-06",
		# 		"v17",
		# 		"'local.sites_path' will be deprecated: use 'ragapp.cli.sites.path instead'",
		# 	),
		# )
...
def _old_deprecated(func):
	return deprecated(
		"ragapp.deprecations.deprecated",
		"2024-09-13",
		"v17",
		"Make use of the ragapp/deprecation_dumpster.py file, instead. 🎉🗑️",
	)(_deprecated("")(func))


def _old_deprecation_warning(msg):
	@deprecated(
		"ragapp.deprecations.deprecation_warning",
		"2024-09-13",
		"v17",
		"Use ragapp.deprecation_dumpster.deprecation_warning, instead. 🎉🗑️",
	)
...
@deprecated("ragapp.utils.make_esc", "unknown", "v17", "Not used anymore.")
...
@deprecated(
	"ragapp.db.is_column_missing",
	"unknown",
	"v17",
	"Renamed to ragapp.db.is_missing_column.",
)
def is_column_missing(e):
	import ragapp

	return ragapp.db.is_missing_column(e)


@deprecated(
	"ragapp.desk.doctype.bulk_update.bulk_update",
	"unknown",
	"v17",
	"Unknown.",
)
def show_progress(docnames, message, i, description):
	...
	n = len(docnames)
	ragapp.publish_progress(float(i) * 100 / n, title=message, description=description)


@deprecated(
	"ragapp.client.get_js",
	"unknown",
	"v17",
	"Unknown.",
)
def get_js(items):
	...
	for src in items:
		...
		if ".." in src or src[0] != "assets":
			ragapp.throw(_("Invalid file path: {0}").format("/".join(src)))

		contentpath = os.path.join(ragapp.local.sites_path, *src)
		with open(contentpath) as srcfile:
			code = ragapp.utils.cstr(srcfile.read())
...
@deprecated(
	"ragapp.utils.print_format.read_multi_pdf",
	"unknown",
	"v17",
	"Unknown.",
)
...
@deprecated("ragapp.gzip_compress", "unknown", "v17", "Use py3 methods directly (this was compat for py2).")
...
@deprecated("ragapp.gzip_decompress", "unknown", "v17", "Use py3 methods directly (this was compat for py2).")
...
@deprecated(
	"ragapp.email.doctype.email_queue.email_queue.send_mail",
	"unknown",
	"v17",
	"Unknown.",
)
def send_mail(email_queue_name, smtp_server_instance=None):
	...
	"""
	from ragapp.email.doctype.email_queue.email_queue import EmailQueue
...
@deprecated(
	"ragapp.geo.country_info.get_translated_dict",
	"unknown",
	"v17",
	"Use ragapp.geo.country_info.get_translated_countries, instead.",
)
def get_translated_dict():
	from ragapp.geo.country_info import get_translated_countries
...
@deprecated("ragapp.tests_runner.get_modules", "2024-20-08", "v17", "use ragapp.tests.utils.get_modules")
def test_runner_get_modules(doctype):
	from ragapp.tests.utils import get_modules

	return get_modules(doctype)


@deprecated(
	"ragapp.tests_runner.make_test_records", "2024-20-08", "v17", "use ragapp.tests.utils.make_test_records"
)
def test_runner_make_test_records(*args, **kwargs):
	from ragapp.tests.utils import make_test_records

	return make_test_records(*args, **kwargs)


@deprecated(
	"ragapp.tests_runner.make_test_objects", "2024-20-08", "v17", "use ragapp.tests.utils.make_test_objects"
)
def test_runner_make_test_objects(*args, **kwargs):
	from ragapp.tests.utils import make_test_objects

	return make_test_objects(*args, **kwargs)


@deprecated(
	"ragapp.tests_runner.make_test_records_for_doctype",
	"2024-20-08",
	"v17",
	"use ragapp.tests.utils.make_test_records_for_doctype",
)
def test_runner_make_test_records_for_doctype(*args, **kwargs):
	from ragapp.tests.utils import make_test_records_for_doctype

	return make_test_records_for_doctype(*args, **kwargs)


@deprecated(
	"ragapp.tests_runner.print_mandatory_fields",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_runner_print_mandatory_fields(*args, **kwargs):
	from ragapp.tests.utils.generators import print_mandatory_fields

	return print_mandatory_fields(*args, **kwargs)


@deprecated(
	"ragapp.tests_runner.get_test_record_log",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_runner_get_test_record_log(doctype):
	from ragapp.tests.utils.generators import TestRecordManager

	return TestRecordManager().get(doctype)


@deprecated(
	"ragapp.tests_runner.add_to_test_record_log",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
...
@deprecated(
	"ragapp.tests_runner.main",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_runner_main(*args, **kwargs):
	from ragapp.commands.testing import main

	return main(*args, **kwargs)


@deprecated(
	"ragapp.tests_runner.xmlrunner_wrapper",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_xmlrunner_wrapper(output):
	...
	try:
		...
		print("$ cli setup requirements --dev")
		raise
...
@deprecated(
	"ragapp.tests.upate_system_settings",
	"2024-20-08",
	"v17",
...
@deprecated(
	"ragapp.tests.get_system_setting",
	"2024-20-08",
	"v17",
	"use `ragapp.db.get_single_value('System Settings', key)`",
)
def tests_get_system_setting(key):
	import ragapp

	return ragapp.db.get_single_value("System Settings", key)


@deprecated(
	"ragapp.tests.utils.change_settings",
	"2024-20-08",
	"v17",
	"use `ragapp.tests.change_settings` or the cls.change_settings",
)
def tests_change_settings(*args, **kwargs):
	from ragapp.tests.classes.context_managers import change_settings

	return change_settings(*args, **kwargs)


@deprecated(
	"ragapp.tests.utils.patch_hooks",
	"2024-20-08",
	"v17",
	"use `ragapp.tests.patch_hooks` or the cls.patch_hooks",
)
def tests_patch_hooks(*args, **kwargs):
	from ragapp.tests.classes.context_managers import patch_hooks

	return patch_hooks(*args, **kwargs)


@deprecated(
	"ragapp.tests.utils.debug_on",
	"2024-20-08",
	"v17",
	"use `ragapp.tests.debug_on` or the cls.debug_on",
)
def tests_debug_on(*args, **kwargs):
	from ragapp.tests.classes.context_managers import debug_on

	return debug_on(*args, **kwargs)


@deprecated(
	"ragapp.tests.utils.timeout",
	"2024-20-08",
	"v17",
	"use `ragapp.tests.timeout` or the cls.timeout",
)
def tests_timeout(*args, **kwargs):
	from ragapp.tests.classes.context_managers import timeout

	return timeout(*args, **kwargs)


def get_tests_CompatRagappTestCase():
	...
	import ragapp
	from ragapp.model.base_document import BaseDocument
	...
	class RagappTestCase(unittest.TestCase):
		"""Base test class for Ragapp tests.
		...
		@deprecated(
			"ragapp.tests.utils.RagappTestCase",
			"2024-20-08",
			"v17",
			"Import `ragapp.tests.UnitTestCase` or `ragapp.tests.IntegrationTestCase` respectively instead of `ragapp.tests.utils.RagappTestCase` - also see wiki for more info: https://github.com/khulnasoft/ragapp/wiki#testing-guide",
		)
		...
		def setUpClass(cls) -> None:
			...
			cls.addClassCleanup(_restore_thread_locals, copy.deepcopy(ragapp.local.flags))
			cls.addClassCleanup(_rollback_db)

			return super().setUpClass()

		def _apply_debug_decorator(self, exceptions=()):
			from ragapp.tests.utils import debug_on
		...
		# --- Ragapp Framework specific assertions
		...
		def assertRedisCallCounts(self, count):
			...
			try:
				orig_execute = ragapp.cache.execute_command
				ragapp.cache.execute_command = execute_command_and_count
				yield
				self.assertLessEqual(
					len(commands), count, msg="commands executed: \n" + "\n".join(str(c) for c in commands)
				)
			finally:
				ragapp.cache.execute_command = orig_execute
		...
		def enable_safe_exec(cls) -> None:
			...
			from ragapp.utils.safe_exec import SAFE_EXEC_CONFIG_KEY

			cls._common_conf = os.path.join(ragapp.local.sites_path, "common_site_config.json")
		...
		def switch_site(self, site: str):
			...
			try:
				old_site = ragapp.local.site
				ragapp.init(site, force=True)
				ragapp.connect()
				yield
			finally:
				ragapp.init(old_site, force=True)
				ragapp.connect()
		...
		def freeze_time(self, time_to_freeze, is_utc=False, *args, **kwargs):
			...
			from ragapp.utils.data import convert_utc_to_timezone, get_datetime, get_system_timezone
	...
	return RagappTestCase


# remove alongside get_tests_CompatRagappTestCase
def get_compat_ragapp_test_case_preparation(cfg):
	import unittest

	import ragapp
	from ragapp.testing.environment import IntegrationTestPreparation

	class RagappTestCasePreparation(IntegrationTestPreparation):
		def __call__(self, suite: unittest.TestSuite, app: str, category: str) -> None:
			super().__call__(suite, app, category)
			candidates = []
			app_path = ragapp.get_app_path(app)
			for path, folders, files in os.walk(ragapp.get_app_path(app)):
				...
				for filename in files:
					if filename.startswith("test_") and filename.endswith(".py"):
						...
						module = ragapp.get_module(module_name)
						candidates.append((module, path, filename))
			compat_preload_test_records_upfront(candidates)

	return RagappTestCasePreparation(cfg)


@deprecated(
	"ragapp.model.trace.traced_field_context",
	"2024-20-08",
	"v17",
	"use `cls.trace_fields`",
)
def model_trace_traced_field_context(*args, **kwargs):
	from ragapp.tests.classes.context_managers import trace_fields

	return trace_fields(*args, **kwargs)


@deprecated(
	"ragapp.tests.utils.get_dependencies",
	"2024-20-09",
	"v17",
	"refactor to use ragapp.tests.utils.get_missing_records_doctypes",
)
def tests_utils_get_dependencies(doctype):
	"""Get the dependencies for the specified doctype"""
	import ragapp
	from ragapp.tests.utils.generators import get_modules
	...
	for df in meta.get_table_fields():
		link_fields.extend(ragapp.get_meta(df.options).get_link_fields())
...
@deprecated(
	"ragapp.tests_runner.get_dependencies",
	"2024-20-08",
	"v17",
	"refactor to use ragapp.tests.utils.get_missing_record_doctypes",
)
...
@deprecated(
	"ragapp.get_test_records",
	"2024-20-09",
	"v17",
	"""Please access the global test records pool via cls.globalTestRecords['Some Doc'] -> list.
If not an IntegrationTestCase, use ragapp.tests.utils.load_test_records_for (check return type).
""",
)
def ragapp_get_test_records(doctype):
	import ragapp
	from ragapp.tests.utils.generators import load_test_records_for
...
@deprecated(
	"ragapp.utils.data.get_number_format_info",
	"unknown",
	"v16",
	"Use `NumberFormat.from_string()` from `ragapp.utils.number_format` instead",
)
def get_number_format_info(format: str) -> tuple[str, str, int]:
	"""DEPRECATED: use `NumberFormat.from_string()` from `ragapp.utils.number_format` instead.
...
@deprecated(
	...
	"2024-11-12",
	"yet unknown",
	"""It has been added for compatibility in addition to the new .ragapp sentinel file inside the module. This is for your info: you don't have to do anything.
""",
)
def boilerplate_modules_txt(dest, app_name, app_title):
	...
	with open(os.path.join(dest, app_name, app_name, "modules.txt"), "w") as f:
		f.write(ragapp.as_unicode(app_title))
