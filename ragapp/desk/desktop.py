...
# Author - Shivam Mishra <shivam@ragapp.khulnasoft.com>

from functools import wraps
from json import JSONDecodeError, dumps, loads

import ragapp
from ragapp import DoesNotExistError, ValidationError, _, _dict
from ragapp.boot import get_allowed_pages, get_allowed_reports
from ragapp.cache_manager import (
	build_domain_restricted_doctype_cache,
	build_domain_restricted_page_cache,
	build_table_count_cache,
)
from ragapp.core.doctype.custom_role.custom_role import get_custom_allowed_roles
...
class Workspace:
	def __init__(self, page, minimal=False):
		...
		self.workspace_manager = "Workspace Manager" in ragapp.get_roles()

		self.user = ragapp.get_user()
		self.allowed_modules = self.get_cached("user_allowed_modules", self.get_allowed_modules)

		self.doc = ragapp.get_cached_doc("Workspace", self.page_name)
		if (
			self.doc
			and self.doc.module
		...
		self.restricted_doctypes = (
			ragapp.cache.get_value("domain_restricted_doctypes") or build_domain_restricted_doctype_cache()
		)
		self.restricted_pages = (
			ragapp.cache.get_value("domain_restricted_pages") or build_domain_restricted_page_cache()
		)

	def is_permitted(self):
		"""Return true if `Has Role` is not set or the user is allowed."""
		from ragapp.utils import has_common
	...
	def get_cached(self, cache_key, fallback_fn):
		value = ragapp.cache.get_value(cache_key, user=ragapp.session.user)
		if value:
			return value
		...
		ragapp.cache.set_value(cache_key, value, ragapp.session.user, 21600)
		return value
	...
	def get_onboarding_doc(self, onboarding):
		...
		if ragapp.db.get_value("Module Onboarding", onboarding, "is_complete"):
			return None

		doc = ragapp.get_doc("Module Onboarding", onboarding)

		# Check if user is allowed
		allowed_roles = set(doc.get_allowed_roles())
		user_roles = set(ragapp.get_roles())
	...
	def _doctype_contains_a_record(self, name):
		...
		if not exists and ragapp.db.exists(name):
			if not ragapp.db.get_value("DocType", name, "issingle"):
				exists = bool(ragapp.get_all(name, limit=1))
			else:
				exists = True
	...
	def _prepare_item(self, item):
		...
		if item.get("link_type") == "DocType":
			item["description"] = ragapp.get_meta(item.link_to).description
	...
	def is_custom_block_permitted(self, custom_block_name):
		...
		allowed = [
			d.role for d in ragapp.get_all("Has Role", fields=["role"], filters={"parent": custom_block_name})
		]
	...
	def get_links(self):
		...
		default_country = ragapp.db.get_default("country")
	...
	def get_charts(self):
		all_charts = []
		if ragapp.has_permission("Dashboard Chart", throw=False):
			charts = self.doc.charts

			for chart in charts:
				if ragapp.has_permission("Dashboard Chart", doc=chart.chart_name):
					# Translate label
	...
	def get_shortcuts(self):
		def _in_active_domains(item):
			if not item.restrict_to_domain:
				...
				return item.restrict_to_domain in ragapp.get_active_domains()
	...
	def get_onboarding_steps(self, onboarding_doc):
		...
		for doc in onboarding_doc.get_steps():
			...
			if step.action == "Create Entry":
				step.is_submittable = ragapp.db.get_value(
	...
	def get_number_cards(self):
		all_number_cards = []
		if ragapp.has_permission("Number Card", throw=False):
			number_cards = self.doc.number_cards
			for number_card in number_cards:
				if ragapp.has_permission("Number Card", doc=number_card.number_card_name):
					# Translate label
					number_card.label = (
	...
	def get_custom_blocks(self):
		all_custom_blocks = []
		if ragapp.has_permission("Custom HTML Block", throw=False):
			custom_blocks = self.doc.custom_blocks

			for custom_block in custom_blocks:
				if ragapp.has_permission("Custom HTML Block", doc=custom_block.custom_block_name):
...
def get_desktop_page(page):
	...
	try:
		...
		ragapp.log_error("Workspace Missing")
		return {}
...
def get_workspace_sidebar_items():
	...
	has_access = "Workspace Manager" in ragapp.get_roles()

	# don't get domain restricted pages
	blocked_modules = ragapp.get_cached_doc("User", ragapp.session.user).get_blocked_modules()
	blocked_modules.append("Dummy Module")

	# adding None to allowed_domains to include pages without domain restriction
	allowed_domains = [None, *ragapp.get_active_domains()]
	...
	try:
		workspace_visibilty = loads(
			ragapp.db.get_single_value("Workspace Settings", "workspace_visibility_json") or "{}"
		)
		...
		try:
			...
			if has_access or workspace.is_permitted():
				if page.public and (has_access or not page.is_hidden) and page.title != "Welcome Workspace":
					pages.append(page)
				elif page.for_user == ragapp.session.user:
			...
			if not page["app"] and page["module"]:
				page["app"] = ragapp.db.get_value("Module Def", page["module"], "app_name") or get_module_app(
					page["module"]
				)
			if page["link_type"] == "Report":
				report_type, ref_doctype = ragapp.db.get_value(
	...
	return {
		"workspace_setup_completed": ragapp.db.get_single_value(
			"Workspace Settings", "workspace_setup_completed"
		),
		"pages": pages,
		"has_access": has_access,
		"has_create_access": ragapp.has_permission(doctype="Workspace", ptype="create"),
	}


def get_table_with_counts():
	counts = ragapp.cache.get_value("information_schema:counts")
	if not counts:
...
def update_onboarding_step(name, field, value):
	...
	ragapp.db.set_value("Onboarding Step", name, field, value)

	capture(ragapp.scrub(name), app="ragapp_onboarding", properties={field: value})
