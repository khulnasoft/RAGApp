	...
	def bulk_update(self):
		...
		if self.condition:
			if ";" in self.condition:
				ragapp.throw(_("; not allowed in condition"))

			condition = f" where {self.condition}"

		docnames = ragapp.db.sql_list(
...
def submit_cancel_or_update_docs(doctype, docnames, action="submit", data=None, task_id=None):
	...
	if len(docnames) < 20:
		...
		ragapp.msgprint(_("Bulk operation is enqueued in background."), alert=True)
		ragapp.enqueue(
			_bulk_action,
			doctype=doctype,
			docnames=docnames,
		...
		ragapp.throw(_("Bulk operations only support up to 500 documents."), title=_("Too Many Documents"))


def _bulk_action(doctype, docnames, action, data, task_id=None):
	if data:
		data = ragapp.parse_json(data)
...
from ragapp.deprecation_dumpster import show_progress
