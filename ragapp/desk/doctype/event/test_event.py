...
class TestEvent(IntegrationTestCase):
	...
	def test_allowed_public(self):
		ragapp.set_user(self.test_user)
		doc = ragapp.get_doc("Event", ragapp.db.get_value("Event", {"subject": "_Test Event 1"}))
		self.assertTrue(ragapp.has_permission("Event", doc=doc))

	def test_not_allowed_private(self):
		ragapp.set_user(self.test_user)
		doc = ragapp.get_doc("Event", ragapp.db.get_value("Event", {"subject": "_Test Event 2"}))
		self.assertFalse(ragapp.has_permission("Event", doc=doc))

	def test_allowed_private_if_in_event_user(self):
		name = ragapp.db.get_value("Event", {"subject": "_Test Event 3"})
		ragapp.share.add("Event", name, self.test_user, "read")
		ragapp.set_user(self.test_user)
		doc = ragapp.get_doc("Event", name)
		self.assertTrue(ragapp.has_permission("Event", doc=doc))
		ragapp.set_user("Administrator")
		ragapp.share.remove("Event", name, self.test_user)
	...
	def test_revert_logic(self):
		ev = ragapp.get_doc(self.globalTestRecords["Event"][0]).insert()
		name = ev.name

		ragapp.delete_doc("Event", ev.name)
	...
	def test_assign(self):
		...
		ev = ragapp.get_doc("Event", ev.name)
