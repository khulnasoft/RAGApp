...
class Note(Document):
	...
	if TYPE_CHECKING:
		from ragapp.desk.doctype.note_seen_by.note_seen_by import NoteSeenBy
	...
	def validate(self):
		if self.notify_on_login and not self.expire_notification_on:
			...
			self.expire_notification_on = ragapp.utils.add_days(self.creation, 7)
	...
	def clear_cache(self):
		ragapp.cache.delete_keys(UNSEEN_NOTES_KEY)
...
def mark_as_seen(note: str):
	note: Note = ragapp.get_doc("Note", note)
	note.mark_seen_by(ragapp.session.user)
...
def get_permission_query_conditions(user):
	...
	return f"(`tabNote`.owner = {ragapp.db.escape(user)} or `tabNote`.public = 1)"
...
def get_unseen_notes():
	...
	def _get_unseen_notes():
		...
		nsb = ragapp.qb.DocType("Note Seen By").as_("nsb")
		...
		return (
			...
			.where(
				...
				& (note.expire_notification_on > ragapp.utils.now())
				& (
					ParameterizedValueWrapper(ragapp.session.user).notin(
						SubQuery(ragapp.qb.from_(nsb).select(nsb.user).where(nsb.parent == note.name))
					)
				)
			)
		).run(as_dict=1)
	...
	return (
		ragapp.cache.get_value(
			f"{UNSEEN_NOTES_KEY}{ragapp.session.user}",
