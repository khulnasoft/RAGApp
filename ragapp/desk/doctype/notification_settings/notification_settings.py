...
class NotificationSettings(Document):
	...
	if TYPE_CHECKING:
		from ragapp.desk.doctype.notification_subscribed_document.notification_subscribed_document import (
	...
	def on_update(self):
		from ragapp.desk.notifications import clear_notification_config

		clear_notification_config(ragapp.session.user)


def is_notifications_enabled(user):
	enabled = ragapp.db.get_value("Notification Settings", user, "enabled")
	if enabled is None:
		return True
	return enabled
...
def is_email_notifications_enabled(user):
	enabled = ragapp.db.get_value("Notification Settings", user, "enable_email_notifications")
	if enabled is None:
		return True
	return enabled
...
def is_email_notifications_enabled_for_type(user, notification_type):
	...
	fieldname = "enable_email_" + ragapp.scrub(notification_type)
	enabled = ragapp.db.get_value("Notification Settings", user, fieldname, ignore=True)
	if enabled is None:
		return True
...
def create_notification_settings(user):
	if not ragapp.db.exists("Notification Settings", user):
		_doc = ragapp.new_doc("Notification Settings")
		_doc.name = user
...
def toggle_notifications(user: str, enable: bool = False, ignore_permissions=False):
	try:
		settings = ragapp.get_doc("Notification Settings", user)
...
def get_subscribed_documents():
	...
	try:
		if ragapp.db.exists("Notification Settings", ragapp.session.user):
			doc = ragapp.get_doc("Notification Settings", ragapp.session.user)
...
def get_permission_query_conditions(user):
	...
	return f"""(`tabNotification Settings`.name = {ragapp.db.escape(user)})"""
...
def has_permission(doc, ptype="read", user=None):
	...
	if "System Manager" in ragapp.get_roles(user):
...
def set_seen_value(value, user):
	...
	ragapp.db.set_value("Notification Settings", user, "seen", value, update_modified=False)
