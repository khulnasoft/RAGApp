...
import ragapp
from ragapp.core.doctype.scheduled_job_type.scheduled_job_type import ScheduledJobType
from ragapp.model.document import Document
from ragapp.utils.background_jobs import get_queue, get_queue_list, get_redis_conn
from ragapp.utils.caching import redis_cache
from ragapp.utils.data import add_to_date
from ragapp.utils.scheduler import get_scheduler_status, get_scheduler_tick
	...
	def suppress_exception(func: Callable):
		...
		def wrapper(*args, **kwargs):
			try:
				...
				ragapp.log(ragapp.get_traceback())
				# nosemgrep
				ragapp.msgprint(
					f"System Health check step {ragapp.bold(step)} failed: {e}", alert=True, indicator="red"
				)
...
class SystemHealthReport(Document):
	...
	if TYPE_CHECKING:
		from ragapp.desk.doctype.system_health_report_errors.system_health_report_errors import (
			SystemHealthReportErrors,
		)
		from ragapp.desk.doctype.system_health_report_failing_jobs.system_health_report_failing_jobs import (
			SystemHealthReportFailingJobs,
		)
		from ragapp.desk.doctype.system_health_report_queue.system_health_report_queue import (
			SystemHealthReportQueue,
		)
		from ragapp.desk.doctype.system_health_report_tables.system_health_report_tables import (
			SystemHealthReportTables,
		)
		from ragapp.desk.doctype.system_health_report_workers.system_health_report_workers import (
	...
	def fetch_background_jobs(self):
		...
		self.test_job_id = ragapp.enqueue("ragapp.ping", at_front=True).id
		self.background_jobs_check = "queued"
		self.scheduler_status = get_scheduler_status().get("status")
		workers = ragapp.get_all("RQ Worker")
	...
	def fetch_scheduler(self):
		...
		failing_jobs = ragapp.db.multisql(
			{
		...
		for job_type in ragapp.get_all(
			"Scheduled Job Type",
			filters={"stopped": 0, "last_execution": ("<", threshold)},
			fields="*",
			order_by="last_execution asc",
		):
			job_type: ScheduledJobType = ragapp.get_doc(doctype="Scheduled Job Type", **job_type)
	...
	def fetch_email_stats(self):
		...
		self.total_outgoing_emails = ragapp.db.count("Email Queue", filters)
		self.pending_emails = ragapp.db.count("Email Queue", {"status": "Not Sent", **filters})
		self.failed_emails = ragapp.db.count("Email Queue", {"status": "Error", **filters})
		self.unhandled_emails = ragapp.db.count("Unhandled Email", filters)
		self.handled_emails = ragapp.db.count(
			"Communication",
	...
	def fetch_errors(self):
		...
		filters = {"creation": (">", threshold), "modified": (">", threshold)}
		self.total_errors = ragapp.db.count("Error Log", filters)

		top_errors = ragapp.db.sql(
	...
	def fetch_database_details(self):
		from ragapp.core.report.database_storage_usage_by_tables.database_storage_usage_by_tables import (
			execute as db_report,
		)

		_cols, data = db_report()
		self.database = ragapp.db.db_type
		self.db_storage_usage = sum(table.size for table in data)
		for row in data[:5]:
			self.append("top_db_tables", row)
		self.database_version = ragapp.db.sql("select version()")[0][0]

		if ragapp.db.db_type == "mariadb":
			self.bufferpool_size = ragapp.db.sql("show variables like 'innodb_buffer_pool_size'")[0][1]
			self.binary_logging = ragapp.db.sql("show variables like 'log_bin'")[0][1]

	@health_check("Cache")
	def fetch_cache_details(self):
		self.cache_keys = len(ragapp.cache.get_keys(""))
		self.cache_memory_usage = ragapp.cache.execute_command("INFO", "MEMORY").get("used_memory_human")

	@health_check("Storage")
	def fetch_storage_details(self):
		from ragapp.desk.page.backups.backups import get_context
	...
	def fetch_user_stats(self):
		...
		self.total_users = ragapp.db.count("User", {"enabled": 1})
		self.new_users = ragapp.db.count("User", {"enabled": 1, "creation": (">", threshold)})
		self.failed_logins = ragapp.db.count(
			"Activity Log",
			{
				"operation": "login",
				"status": "Failed",
		...
		self.active_sessions = ragapp.db.count("Sessions")
...
def get_job_status(job_id: str | None = None):
	ragapp.only_for("System Manager")
	try:
		return ragapp.get_doc("RQ Job", job_id).status
	except Exception:
...
def _get_directory_size(*path):
	folder = os.path.abspath(ragapp.get_site_path(*path))
