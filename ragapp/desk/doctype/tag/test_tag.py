import ragapp
from ragapp.desk.doctype.tag.tag import add_tag
from ragapp.desk.reportview import get_stats
...
class TestTag(IntegrationTestCase):
	def setUp(self) -> None:
		ragapp.db.delete("Tag")
		ragapp.db.sql("UPDATE `tabDocType` set _user_tags=''")

	def test_tag_count_query(self):
		self.assertDictEqual(
			get_stats('["_user_tags"]', "DocType"),
			{"_user_tags": [["No Tags", ragapp.db.count("DocType")]]},
		)
		...
		self.assertDictEqual(
			get_stats('["_user_tags"]', "DocType"),
			{"_user_tags": [["Standard", 2], ["No Tags", ragapp.db.count("DocType") - 2]]},
		)
