...
import ragapp
from ragapp.core.doctype.doctype.doctype import clear_permissions_cache
from ragapp.model.db_query import DatabaseQuery
from ragapp.permissions import add_permission, reset_perms
...
class TestToDo(IntegrationTestCase):
	def test_delete(self):
		todo = ragapp.get_doc(doctype="ToDo", description="test todo", assigned_by="Administrator").insert()

		ragapp.db.delete("Deleted Document")
		todo.delete()

		deleted = ragapp.get_doc(
	...
	def test_fetch(self):
		todo = ragapp.get_doc(doctype="ToDo", description="test todo", assigned_by="Administrator").insert()
		self.assertEqual(
			todo.assigned_by_full_name, ragapp.db.get_value("User", todo.assigned_by, "full_name")
		)

	def test_fetch_setup(self):
		ragapp.db.delete("ToDo")

		todo_meta = ragapp.get_meta("ToDo")
	...
	def test_doc_read_access(self):
		...
		test_user = ragapp.get_doc("User", "test4@example.com")
...
def create_new_todo(description, assigned_by):
	...
	return ragapp.get_doc(todo).insert()
