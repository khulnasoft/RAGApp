...
from ragapp.utils import get_fullname, parse_addr
...
class ToDo(Document):
	...
	def validate(self):
		...
		if self.is_new():
			if self.assigned_by == self.allocated_to:
				assignment_message = ragapp._("{0} self assigned this task: {1}").format(
					get_fullname(self.assigned_by), self.description
				)
			else:
				assignment_message = ragapp._("{0} assigned {1}: {2}").format(
			...
			if self.get_db_value("status") != self.status:
				if self.allocated_to == ragapp.session.user:
					removal_message = ragapp._("{0} removed their assignment.").format(
						get_fullname(ragapp.session.user)
					)
				else:
					removal_message = ragapp._("Assignment of {0} removed by {1}").format(
						get_fullname(self.allocated_to), get_fullname(ragapp.session.user)
					)
	...
	def add_assign_comment(self, text, comment_type):
		...
		ragapp.get_doc(self.reference_type, self.reference_name).add_comment(comment_type, text)

	def delete_communication_links(self):
		# unlink todo from linked comments
		return ragapp.db.delete("Communication Link", {"link_doctype": self.doctype, "link_name": self.name})
	...
	def update_in_reference(self):
		...
		try:
			assignments = ragapp.db.get_values(
				"ToDo",
				{
			...
			if ragapp.get_meta(self.reference_type).issingle:
			...
			if ragapp.db.is_table_missing(e) and ragapp.flags.in_install:
				# no table
				return

			elif ragapp.db.is_missing_column(e):
	...
	def get_owners(cls, filters=None):
		...
		rows = ragapp.get_all(cls.DocType, filters=filters or {}, fields=["allocated_to"])
...
def on_doctype_update():
	ragapp.db.add_index("ToDo", ["reference_type", "reference_name"])
...
def get_permission_query_conditions(user):
	...
	todo_roles = ragapp.permissions.get_doctype_roles("ToDo")
	todo_roles = set(todo_roles) - set(AUTOMATIC_ROLES)

	if any(check in todo_roles for check in ragapp.get_roles(user)):
		return None
	else:
		return """(`tabToDo`.allocated_to = {user} or `tabToDo`.assigned_by = {user})""".format(
			user=ragapp.db.escape(user)
		)


def has_permission(doc, ptype="read", user=None):
	user = user or ragapp.session.user
	todo_roles = ragapp.permissions.get_doctype_roles("ToDo", ptype)
	todo_roles = set(todo_roles) - set(AUTOMATIC_ROLES)
...
def new_todo(description):
	ragapp.get_doc({"doctype": "ToDo", "description": description}).insert()
