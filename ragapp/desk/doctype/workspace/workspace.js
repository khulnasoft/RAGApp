...
ragapp.ui.form.on("Workspace", {
	setup: function () {
		ragapp.meta.get_field("Workspace Link", "only_for").no_default = true;
	},
	...
	refresh: function (frm) {
		...
		let url = `/app/${
			frm.doc.public
				? ragapp.router.slug(frm.doc.title)
				: "private/" + ragapp.router.slug(frm.doc.title)
		}`;
		frm.sidebar
		...
		if (
			(frm.doc.for_user && frm.doc.for_user !== ragapp.session.user) ||
			(frm.doc.public && !ragapp.user.has_role("Workspace Manager"))
		) {
