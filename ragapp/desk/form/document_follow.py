...
from ragapp.utils import get_url_to_form


@ragapp.whitelist()
def update_follow(doctype: str, doc_name: str, following: bool):
	if following:
		return (follow_document(doctype, doc_name, ragapp.session.user) and True) or False
	else:
		return unfollow_document(doctype, doc_name, ragapp.session.user)
...
def follow_document(doctype, doc_name, user):
	...
	if not ragapp.get_meta(doctype).track_changes:
		ragapp.toast(_("Can't follow since changes are not tracked."))
		return False

	if user == "Administrator":
		ragapp.toast(_("Administrator can't follow"))
		return False

	if not ragapp.db.get_value("User", user, "document_follow_notify", ignore=True, cache=True):
		ragapp.toast(_("Document follow is not enabled for this user."))
		return False

	if not is_document_followed(doctype, doc_name, user):
		doc = ragapp.new_doc("Document Follow")
		doc.update({"ref_doctype": doctype, "ref_docname": doc_name, "user": user})
		doc.save()
		ragapp.toast(_("Following document {0}").format(doc_name))
		return doc
...
def unfollow_document(doctype, doc_name, user):
	...
	if doc:
		ragapp.delete_doc("Document Follow", doc[0].name, force=True)
		ragapp.toast(_("Un-following document {0}").format(doc_name))
		return False
	return False
...
def get_user_list(frequency):
	...
	User = DocType("User")
	return (
		ragapp.qb.from_(DocumentFollow)
		.join(User)
...
def get_version(doctype, doc_name, frequency, user):
	...
	if version:
		for v in version:
			change = ragapp.parse_json(v.data)
			time = ragapp.utils.format_datetime(v.modified, "hh:mm a")
			timeline_items = []
			if change.changed:
...
def get_comments(doctype, doc_name, frequency, user):
	...
	for comment in comments:
		...
		time = ragapp.utils.format_datetime(comment.modified, "hh:mm a")
		timeline.append(
			{
...
def _get_filters(frequency, user):
	...
	if frequency == "Weekly":
		filters += [
			["modified", ">", ragapp.utils.add_days(ragapp.utils.nowdate(), -7)],
			["modified", "<", ragapp.utils.nowdate()],
		]

	elif frequency == "Daily":
		filters += [
			["modified", ">", ragapp.utils.add_days(ragapp.utils.nowdate(), -1)],
			["modified", "<", ragapp.utils.nowdate()],
		]

	elif frequency == "Hourly":
		filters += [
			["modified", ">", ragapp.utils.add_to_date(ragapp.utils.now_datetime(), hours=-1)],
			["modified", "<", ragapp.utils.now_datetime()],
		]
