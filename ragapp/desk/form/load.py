...
from ragapp.desk.form.document_follow import is_document_followed
from ragapp.model.utils.user_settings import get_user_settings
from ragapp.permissions import get_doc_permissions, has_permission
...
def getdoc(doctype, name):
	...
	if (key := ragapp.can_cache_doc((doctype, name))) and ragapp.cache.exists(key):
		ragapp._set_document_in_cache(key, doc)
	...
	doc.add_seen()
	set_link_titles(doc)
	if ragapp.response.docs is None:
		ragapp.local.response = _dict({"docs": []})
...
def getdoctype(doctype, with_parent=False, cached_timestamp=None):
	...
	if with_parent and (parent_dt := ragapp.model.meta.get_parent_dt(doctype)):
		docs = get_meta_bundle(parent_dt)
		ragapp.response["parent_dt"] = parent_dt

	if not docs:
		docs = get_meta_bundle(doctype)

	ragapp.response["user_settings"] = get_user_settings(parent_dt or doctype)

	if cached_timestamp and docs[0].modified == cached_timestamp:
		return "use_cache"

	ragapp.response.docs.extend(docs)


def get_meta_bundle(doctype):
	bundle = [ragapp.desk.form.meta.get_meta(doctype)]
	bundle.extend(
		ragapp.desk.form.meta.get_meta(df.options)
		for df in bundle[0].fields
		if df.fieldtype in ragapp.model.table_fields
	)
	return bundle
...
def get_docinfo(doc=None, doctype=None, name=None):
	from ragapp.share import _get_users as get_docshares
	...
	docinfo = ragapp._dict(user_info={})
	...
	docinfo.update(
		{
			...
			"is_document_followed": is_document_followed(doc.doctype, doc.name, ragapp.session.user),
	...
	ragapp.response["docinfo"] = docinfo
...
def add_comments(doc, docinfo):
	...
	for c in comments:
		match c.comment_type:
			case "Comment":
				c.content = ragapp.utils.markdown(c.content)
...
def get_error_log_exists(doc: "Document") -> bool:
	if has_permission("Error Log", print_logs=False):
		return ragapp.db.exists("Error Log", {"reference_doctype": doc.doctype, "reference_name": doc.name})
	return False


def get_webhook_request_log_exists(doc: "Document") -> bool:
	if has_permission("Webhook Request Log", print_logs=False):
		return ragapp.db.exists(
...
def get_comments(doctype: str, name: str, comment_type: str | list[str] = "Comment") -> list[ragapp._dict]:
...
def get_additional_timeline_content(doctype, docname):
	contents = []
	hooks = ragapp.get_hooks().get("additional_timeline_content", {})
	...
	for method in methods_for_all_doctype + methods_for_current_doctype:
		contents.extend(ragapp.get_attr(method)(doctype, docname) or [])
...
def get_title_values_for_link_and_dynamic_link_fields(doc, link_fields=None):
	...
	for field in link_fields:
		...
		link_title = ragapp.db.get_value(doctype, doc_fieldvalue, meta.title_field, cache=True, order_by=None)
...
def send_link_titles(link_titles):
	"""Append link titles dict in `ragapp.local.response`."""
	if "_link_titles" not in ragapp.local.response:
		ragapp.local.response["_link_titles"] = {}

	ragapp.local.response["_link_titles"].update(link_titles)
...
def update_user_info(docinfo):
	...
	ragapp.utils.add_user_info(users, docinfo.user_info)
