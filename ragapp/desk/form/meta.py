...
from ragapp.build import scrub_html_template
from ragapp.model.meta import Meta
from ragapp.model.utils import render_include
from ragapp.modules import get_module_path, load_doctype_module, scrub
from ragapp.utils import get_cli_path, get_html_format
...
def get_meta(doctype, cached=True) -> "FormMeta":
	...
	cached = cached and not ragapp.conf.developer_mode
	if cached:
		meta = ragapp.cache.hget("doctype_form_meta", doctype)
		if not meta:
			# Cache miss - explicitly get meta from DB to avoid
			meta = FormMeta(doctype, cached=False)
			ragapp.cache.hset("doctype_form_meta", doctype, meta)
	else:
...
class FormMeta(Meta):
	def __init__(self, doctype, *, cached=True):
		self.__dict__.update(ragapp.get_meta(doctype, cached=cached).__dict__)
		self.load_assets()
	...
	def add_code(self):
		...
		system_country = ragapp.get_system_settings("country")
	...
	def _add_code(self, path, fieldname):
		...
		if js:
			...
			asset_path = path.replace(cli_path, "")
			comment = f"\n\n/* Adding {asset_path} */\n\n"
...
"""
	...
	def add_search_fields(self):
		...
		for df in self.get("fields", {"fieldtype": "Link", "options": ["!=", "[Select]"]}):
			if df.options:
				try:
					search_fields = ragapp.get_meta(df.options).search_fields
	...
	def _show_missing_doctype_msg(self, df):
		...
		msg = _("Field {0} is referring to non-existing doctype {1}.").format(
			ragapp.bold(df.fieldname), ragapp.bold(df.options)
		)
		...
		ragapp.throw(msg, title=_("Missing DocType"))

	def add_linked_document_type(self):
		for df in self.get("fields", {"fieldtype": "Link"}):
			if df.options:
				try:
					df.linked_document_type = ragapp.get_meta(df.options).document_type
				except ragapp.DoesNotExistError:
					self._show_missing_doctype_msg(df)

	def load_print_formats(self):
		print_formats = ragapp.db.sql(
	...
	def load_workflows(self):
		...
		if workflow_name and ragapp.db.exists("Workflow", workflow_name):
			workflow = ragapp.get_doc("Workflow", workflow_name)
			workflow_docs.append(workflow)

			workflow_docs.extend(ragapp.get_doc("Workflow State", d.state) for d in workflow.get("states"))
		...
		if not self.custom:
			...
			if hasattr(module, "form_grid_templates"):
				for key, path in module.form_grid_templates.items():
					templates[key] = get_html_format(ragapp.get_app_path(app, path))
	...
	def load_workspaces(self):
		Shortcut = ragapp.qb.DocType("Workspace Shortcut")
		Workspace = ragapp.qb.DocType("Workspace")
		shortcut = (
			ragapp.qb.from_(Shortcut)
		...
		if shortcut:
			...
			Link = ragapp.qb.DocType("Workspace Link")
			link = (
				ragapp.qb.from_(Link)
				.select(Link.parent)
				.inner_join(Workspace)
...
def get_code_files_via_hooks(hook, name):
	...
	for app_name in ragapp.get_installed_apps():
		code_hook = ragapp.get_hooks(hook, default={}, app_name=app_name)
		if not code_hook:
			continue
		...
		for file in files:
			path = ragapp.get_app_path(app_name, *file.strip("/").split("/"))
