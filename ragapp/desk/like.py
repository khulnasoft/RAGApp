...
def _toggle_like(doctype, name, add, user=None):
	...
	try:
		liked_by = ragapp.db.get_value(doctype, name, "_liked_by")
		...
		if add == "Yes":
			if user not in liked_by:
				...
				if ragapp.get_cached_value("User", user, "follow_liked_documents"):
		...
		if ragapp.get_meta(doctype).issingle:
			ragapp.db.set_single_value(doctype, "_liked_by", json.dumps(liked_by), update_modified=False)
		else:
			ragapp.db.set_value(doctype, name, "_liked_by", json.dumps(liked_by), update_modified=False)
