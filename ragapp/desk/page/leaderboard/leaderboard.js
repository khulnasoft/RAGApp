ragapp.pages["leaderboard"].on_page_load = (wrapper) => {
	ragapp.leaderboard = new Leaderboard(wrapper);

	$(wrapper).bind("show", () => {
		// Get which leaderboard to show
		let doctype = ragapp.get_route()[1];
		ragapp.leaderboard.show_leaderboard(doctype);
	});
};
...
class Leaderboard {
	...
	get_leaderboard_config() {
		...
		ragapp
			.xcall("ragapp.desk.page.leaderboard.leaderboard.get_leaderboard_config")
			.then((config) => {
				...
				const _initial_doctype = ragapp.get_route()[1] || this.doctypes[0];
	...
	setup_leaderboard_fields() {
		this.company_select = this.page.add_field({
			...
			fieldtype: "Link",
			options: "Company",
			default: ragapp.defaults.get_default("company"),
			reqd: 1,
			change: (e) => {
				this.make_request();
			},
		...
		this.type_select = this.page.add_select(
			...
			this.options.selected_filter.map((d) => {
				return { label: __(ragapp.model.unscrub(d)), value: d };
			})
		);
	...
	create_date_range_field() {
		let timespan_field = $(this.parent).find(
			`.ragapp-control[data-original-title="${__("Timespan")}"]`
		);
		this.date_range_field = $(`<div class="from-date-field"></div>`)
			.insertAfter(timespan_field)
			.hide();

		let date_field = ragapp.ui.form.make_control({
			df: {
				fieldtype: "DateRange",
				fieldname: "selected_date_range",
				placeholder: __("Date Range"),
				default: [ragapp.datetime.month_start(), ragapp.datetime.now_date()],
	...
	render_selected_doctype() {
		this.$sidebar_list.on("click", "li", (e) => {
			...
			this.company_select.set_value(
				ragapp.defaults.get_default("company") || this.company_select.get_value()
			);
			...
			ragapp.set_route("leaderboard", this.options.selected_doctype);
			this.make_request();
		});
	}
	...
	make_request() {
		ragapp.model.with_doctype(this.options.selected_doctype, () => {
	...
	get_leaderboard(notify) {
		...
		if (!company && !this.leaderboard_config[this.options.selected_doctype].company_disabled) {
			notify(this, null);
			ragapp.show_alert(__("Please select Company"));
			return;
		}
		ragapp
			...
			.then((r) => {
				...
				ragapp.utils.make_chart(".leaderboard-graph", custom_options);
	...
	get_leaderboard_data(me, res) {
		if (res && res.message.length) {
			...
			ragapp.utils.setup_search($(me.parent), ".list-item-container", ".list-id");
		} else {
			me.$graph_area.hide();
	...
	render_list_header() {
		const _selected_filter = this.options.selected_filter.map((i) => ragapp.model.unscrub(i));
		const fields = ["rank", "name", this.options.selected_filter_item];
		const filters = fields
			.map((filter) => {
				const col = __(ragapp.model.unscrub(filter));
	...
	render_message() {
		...
		return `<div class="leaderboard-empty-state ${display_class}">
  			<div class="no-result text-center">
  				<img src="/assets/ragapp/images/ui-states/search-empty-state.svg"
  					alt="Empty State"
  					class="null-state"
  				>
	...
	get_item_html(item, index) {
		...
		const link = `/app/${ragapp.router.slug(this.options.selected_doctype)}/${item.name}`;
	...
	get_sidebar_item(item, icon) {
		let icon_html = icon ? ragapp.utils.icon(icon, "md") : "";
	...
	get_date_range() {
		...
		let current_date = ragapp.datetime.now_date();
		let date_range_map = {
			"this week": [ragapp.datetime.week_start(), ragapp.datetime.week_end()],
			"this month": [ragapp.datetime.month_start(), ragapp.datetime.month_end()],
			"this quarter": [ragapp.datetime.quarter_start(), ragapp.datetime.quarter_end()],
			"this year": [ragapp.datetime.year_start(), ragapp.datetime.year_end()],
			"last week": [ragapp.datetime.add_days(current_date, -7), current_date],
			"last month": [ragapp.datetime.add_months(current_date, -1), current_date],
			"last quarter": [ragapp.datetime.add_months(current_date, -3), current_date],
			"last year": [ragapp.datetime.add_months(current_date, -12), current_date],
			"all time": null,
			"select date range": this.selected_date_range || [
				ragapp.datetime.month_start(),
				current_date,
			],
		};
