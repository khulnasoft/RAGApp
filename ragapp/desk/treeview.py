...
def get_all_nodes(doctype, label, parent, tree_method, **filters):
	...
	tree_method = ragapp.get_attr(tree_method)

	ragapp.is_whitelisted(tree_method)
...
def get_children(doctype, parent="", include_disabled=False, **filters):
	if isinstance(include_disabled, str):
		include_disabled = ragapp.sbool(include_disabled)
...
def _get_children(doctype, parent="", ignore_permissions=False, include_disabled=False):
	parent_field = "parent_" + ragapp.scrub(doctype)
	filters = [[f"ifnull(`{parent_field}`,'')", "=", parent], ["docstatus", "<", 2]]
	if ragapp.db.has_column(doctype, "disabled") and not include_disabled:
...
@ragapp.whitelist()
def add_node():
	args = make_tree_args(**ragapp.form_dict)
	doc = ragapp.get_doc(args)
