...
def validate_route_conflict(doctype, name):
	...
	for _doctype in ["Page", "Workspace", "DocType"]:
		all_names.extend(
			[slug(d) for d in ragapp.get_all(_doctype, pluck="name") if (doctype != _doctype and d != name)]
		)

	if slug(name) in all_names:
		ragapp.msgprint(ragapp._("Name already taken, please set a new name"))
		raise ragapp.NameError
...
def pop_csv_params(form_dict):
	...
	from ragapp.utils.data import cint, cstr
...
def provide_binary_file(filename: str, extension: str, content: bytes) -> None:
	...
	ragapp.response["filecontent"] = content
	ragapp.response["filename"] = f"{_(filename)}.{extension}"
