...
def sendmail_to_system_managers(subject, content):
	ragapp.sendmail(recipients=get_system_managers(), subject=subject, content=content)
...
def get_contact_list(txt, page_length=20, extra_filters: str | None = None) -> list[dict]:
	...
	if extra_filters:
		extra_filters = ragapp.parse_json(extra_filters)
	...
	# https://github.com/khulnasoft/ragapp/blob/6c6a89bcdd9454060a1333e23b855d0505c9ebc2/ragapp/public/js/ragapp/form/controls/autocomplete.js#L29-L35
	return [
		ragapp._dict(
			value=d.email_id,
			label=d.email_id,
...
def get_communication_doctype(doctype, txt, searchfield, start, page_len, filters):
	user_perms = ragapp.utils.user.UserPermissions(ragapp.session.user)
	...
	com_doctypes = []
	if len(txt) < 2:
		for name in ragapp.get_hooks("communication_doctypes"):
			try:
		...
		com_doctypes = [
			d[0] for d in ragapp.db.get_values("DocType", {"issingle": 0, "istable": 0, "hide_toolbar": 0})
		]
