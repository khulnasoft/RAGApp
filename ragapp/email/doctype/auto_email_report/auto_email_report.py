...
from ragapp.desk.query_report import build_xlsx_data
...
from ragapp.utils.csvutils import to_csv
...
class AutoEmailReport(Document):
	...
	def autoname(self):
		...
		if ragapp.db.exists("Auto Email Report", self.name):
	...
	def sender_email(self):
		return ragapp.db.get_value("Email Account", self.sender, "email_id")
	...
	def validate_report_count(self):
		count = ragapp.db.count("Auto Email Report", {"user": self.user, "enabled": 1})

		max_reports_per_user = (
			cint(ragapp.local.conf.max_reports_per_user)  # kept for backward compatibilty
			or cint(ragapp.get_system_settings("max_auto_email_report_per_user"))
			or 20
		)
		...
		if count > max_reports_per_user + (-1 if self.flags.in_insert else 0):
			...
			ragapp.throw(msg, title=_("Report limit reached"))
		...
		if self.format not in valid_report_formats:
			ragapp.throw(
				_("{0} is not a valid report format. Report format should one of the following {1}").format(
					ragapp.bold(self.format), ragapp.bold(", ".join(valid_report_formats))
				)
			)

	def validate_mandatory_fields(self):
		# Check if all Mandatory Report Filters are filled by the User
		filters = ragapp.parse_json(self.filters) if self.filters else {}
		filter_meta = ragapp.parse_json(self.filter_meta) if self.filter_meta else {}
		throw_list = [
	...
	def get_report_content(self):
		...
		report = ragapp.get_doc("Report", self.report)

		self.filters = ragapp.parse_json(self.filters) if self.filters else {}
		...
		# add serial numbers
		columns.insert(0, ragapp._dict(fieldname="idx", label="", width="30px"))
		...
		if self.format == "HTML":
			...
			ragapp.throw(_("Invalid Output Format"))

	def get_html_table(self, columns=None, data=None):
		date_time = global_date_format(now()) + " " + format_time(now())
		report_doctype = ragapp.db.get_value("Report", self.report, "ref_doctype")

		return ragapp.render_template(
			"ragapp/templates/emails/auto_email_report.html",
			{
				"title": self.name,
	...
	def prepare_dynamic_filters(self):
		self.filters = ragapp.parse_json(self.filters)
	...
	def send(self):
		if self.filter_meta and not self.filters:
			ragapp.throw(_("Please set filters value in Report Filter table."))
...
def download(name):
	...
	auto_email_report = ragapp.get_doc("Auto Email Report", name)
	auto_email_report.check_permission()
	data = auto_email_report.get_report_content()

	if not data:
		ragapp.msgprint(_("No Data"))
		return

	ragapp.local.response.filecontent = data
	ragapp.local.response.type = "download"
	ragapp.local.response.filename = auto_email_report.get_file_name()
...
def send_daily():
	...
	enabled_reports = ragapp.get_all(
		"Auto Email Report", filters={"enabled": 1, "frequency": ("in", ("Daily", "Weekdays", "Weekly"))}
	)

	for report in enabled_reports:
		auto_email_report = ragapp.get_doc("Auto Email Report", report.name)
...
def send_monthly():
	...
	for report in ragapp.get_all("Auto Email Report", {"enabled": 1, "frequency": "Monthly"}):
		ragapp.get_doc("Auto Email Report", report.name).send()
	...
	for row in data:
		...
		for col in columns:
			...
			if col.fieldtype == "Link":
				...
				doc = None
				if doc_name and col.get("parent") and not ragapp.get_meta(col.parent).istable:
					if ragapp.db.exists(col.parent, doc_name):
						doc = ragapp.get_doc(col.parent, doc_name)

				# Pass the Document to get the currency based on docfield option
				row[col.fieldname] = ragapp.format_value(row[col.fieldname], col, doc=doc)
	return columns, data
