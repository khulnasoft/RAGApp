...
import ragapp
import ragapp.desk.form.document_follow as document_follow
from ragapp.desk.form.assign_to import add
from ragapp.desk.form.document_follow import get_document_followed_by_user
from ragapp.desk.form.utils import add_comment
from ragapp.desk.like import toggle_like
from ragapp.query_builder import DocType
from ragapp.query_builder.functions import Cast_
from ragapp.share import add as share
...
class TestDocumentFollow(IntegrationTestCase):
	...
	def tearDown(self):
		...
		ragapp.db.delete("Document Follow")
	...
	doc = ragapp.get_doc(
		{
			...
			"doc.starts_on": ragapp.utils.now(),
			"doc.ends_on": ragapp.utils.add_days(ragapp.utils.now(), 5),
...
def get_user(document_follow=None):
	...
	if ragapp.db.exists("User", "test@docsub.com"):
		doc = ragapp.delete_doc("User", "test@docsub.com")
	doc = ragapp.new_doc("User")
