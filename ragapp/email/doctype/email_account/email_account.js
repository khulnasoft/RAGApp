...
function oauth_access(frm) {
	ragapp.model.with_doc("Connected App", frm.doc.connected_app, () => {
		const connected_app = ragapp.get_doc("Connected App", frm.doc.connected_app);
		return ragapp.call({
			doc: connected_app,
...
function set_default_max_attachment_size(frm) {
	if (frm.doc.__islocal && !frm.doc["attachment_limit"]) {
		ragapp.call({
			method: "ragapp.core.api.file.get_max_file_size",
			callback: function (r) {
				if (!r.exc) {
	...
	switch (frm.doc.service) {
		...
		case "Ragapp Mail":
			frm.set_df_property(
				"api_secret",
				"description",
				'To know more visit <a href="https://github.com/khulnasoft/mail"> here'
			);
			break;
	}
}
ragapp.ui.form.on("Email Account", {
	service: function (frm) {
		$.each(ragapp.email_defaults[frm.doc.service], function (key, value) {
			frm.set_value(key, value);
		});
		if (!frm.doc.use_imap) {
			$.each(ragapp.email_defaults_pop[frm.doc.service], function (key, value) {
				frm.set_value(key, value);
			});
		}
	...
	onload: function (frm) {
		...
		frm.set_query(
			"append_to",
			"ragapp.email.doctype.email_account.email_account.get_append_to"
		);
		frm.set_query("append_to", "imap_folder", function () {
			return {
				query: "ragapp.email.doctype.email_account.email_account.get_append_to",
			};
		});
	...
	refresh: function (frm) {
		...
		if (ragapp.route_flags.delete_user_from_locals && ragapp.route_flags.linked_user) {
			delete ragapp.route_flags.delete_user_from_locals;
			delete locals["User"][ragapp.route_flags.linked_user];
		}

		if (!frm.is_dirty() && frm.doc.enable_incoming) {
			frm.add_custom_button(__("Pull Emails"), () => {
				ragapp.dom.freeze(__("Pulling emails..."));
				frm.call({
					method: "pull_emails",
					...
					if (!(r._server_messages && r._server_messages.length)) {
						ragapp.show_alert({ message: __("Emails Pulled"), indicator: "green" });
					}
				});
			});
		}
	...
	validate_ragapp_mail_settings: function (frm) {
		if (frm.doc.service == "Ragapp Mail") {
			ragapp.call({
				doc: frm.doc,
				method: "validate_ragapp_mail_settings",
			});
		}
	},
	...
	show_oauth_authorization_message(frm) {
		if (
			...
			ragapp.call({
				method: "ragapp.integrations.doctype.connected_app.connected_app.has_token",
				args: {
	...
	domain: ragapp.utils.debounce((frm) => {
		if (frm.doc.domain) {
			ragapp.call({
	...
	email_sync_option: function (frm) {
		...
		if (frm.doc.email_sync_option == "ALL") {
			...
			ragapp.confirm(msg, null, function () {
	...
	warn_autoreply_on_incoming: function (frm) {
		if (frm.doc.enable_incoming && frm.doc.enable_auto_reply && frm.doc.__islocal) {
			...
			ragapp.confirm(msg, null, function () {
				...
				ragapp.show_alert({ message: __("Disabled Auto Reply"), indicator: "blue" });
			});
		}
	},
});
