...
import ragapp
from ragapp import _, are_emails_muted, safe_encode
from ragapp.desk.form import assign_to
from ragapp.email.doctype.email_domain.email_domain import EMAIL_DOMAIN_FIELDS
from ragapp.email.ragappmail import RagappMail
from ragapp.email.receive import EmailServer, InboundMail, SentEmailInInboxError
from ragapp.email.smtp import SMTPServer
from ragapp.email.utils import get_port
from ragapp.model.document import Document
from ragapp.utils import cint, comma_or, cstr, parse_addr, validate_email_address
from ragapp.utils.background_jobs import enqueue, get_jobs
from ragapp.utils.jinja import render_template
...
def cache_email_account(cache_name):
	def decorator_cache_email_account(func):
		...
		def wrapper_cache_email_account(*args, **kwargs):
			if not hasattr(ragapp.local, cache_name):
				setattr(ragapp.local, cache_name, {})

			cached_accounts = getattr(ragapp.local, cache_name)
...
class EmailAccount(Document):
	...
	if TYPE_CHECKING:
		from ragapp.email.doctype.imap_folder.imap_folder import IMAPFolder
		...
		service: DF.Literal[
			"", "Ragapp Mail", "GMail", "Sendgrid", "SparkPost", "Yahoo Mail", "Outlook.com", "Yandex.Mail"
		]
	...
	def validate(self):
		...
		if self.login_id_is_different:
			if not self.login_id:
				ragapp.throw(_("Login Id is required"))
		else:
			self.login_id = None

		if self.service == "Ragapp Mail":
			self.use_imap = 0
			self.always_use_account_email_id_as_sender = 1

			if self.auth_method == "Basic" or self.get_oauth_token():
				self.validate_ragapp_mail_settings()

		# validate the imap settings
		if self.enable_incoming and self.use_imap and len(self.imap_folder) <= 0:
			ragapp.throw(_("You need to set one IMAP folder for {0}").format(ragapp.bold(self.email_id)))

		if ragapp.local.flags.in_patch or ragapp.local.flags.in_test:
			return
		...
		if (
			not ragapp.local.flags.in_install
			and not self.awaiting_password
			and not self.service == "Ragapp Mail"
		):
			if validate_oauth or self.password or self.smtp_server in ("127.0.0.1", "localhost"):
				...
				if self.enable_incoming or (self.enable_outgoing and not self.no_smtp_authentication):
					if not use_oauth:
						ragapp.throw(_("Password is required or select Awaiting Password"))

		if self.notify_if_unreplied:
			if not self.send_notification_to:
				ragapp.throw(_("{0} is mandatory").format(self.meta.get_label("send_notification_to")))
			...
			for folder in self.imap_folder:
				if folder.append_to:
					...
					if folder.append_to not in valid_doctypes:
						ragapp.throw(_("Append To can be one of {0}").format(comma_or(valid_doctypes)))

	@ragapp.whitelist()
	def validate_ragapp_mail_settings(self):
		if self.service == "Ragapp Mail":
			ragapp_mail_client = self.get_ragapp_mail_client()
			ragapp_mail_client.validate(for_inbound=self.enable_incoming, for_outbound=self.enable_outgoing)

	def validate_smtp_conn(self):
		if not self.smtp_server:
			ragapp.throw(_("SMTP Server is required"))
	...
	def before_save(self):
		...
		if not self.enable_incoming and self.default_incoming:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					ragapp.bold(_("Default Incoming")),
					ragapp.bold(_("Enable Incoming")),
				)
			)
		if not self.enable_outgoing and self.default_outgoing:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					ragapp.bold(_("Default Outgoing")),
					ragapp.bold(_("Enable Outgoing")),
				)
			)
		if messages:
			if len(messages) == 1:
	...
	def there_must_be_only_one_default(self):
		...
		for field in ("default_incoming", "default_outgoing"):
			...
			for email_account in ragapp.get_all("Email Account", filters={field: 1}):
				if email_account.name == self.name:
					continue

				email_account = ragapp.get_doc("Email Account", email_account.name)
				email_account.set(field, 0)
				email_account.save()

	@ragapp.whitelist()
	def get_domain_values(self, domain: str):
		return ragapp.db.get_value("Email Domain", domain, EMAIL_DOMAIN_FIELDS, as_dict=True)
	...
	def get_incoming_server(self, in_receive=False, email_sync_rule="UNSEEN"):
		...
		if not args.get("host"):
			ragapp.throw(_("{0} is required").format("Email Server"))

		email_server = EmailServer(ragapp._dict(args))
	...
	def check_email_server_connection(self, email_server, in_receive):
		...
		try:
			...
			if in_receive:
				...
				description = ragapp.message_log.pop() if ragapp.message_log else "Socket Error"
	...
	def _password(self):
		raise_exception = not (
			self.auth_method == "OAuth" or self.no_smtp_authentication or ragapp.flags.in_test
		)
	...
	def from_record(cls, record):
		email_account = ragapp.new_doc(cls.DOCTYPE)
		email_account.update(record)
		return email_account

	@classmethod
	def find(cls, name):
		return ragapp.get_doc(cls.DOCTYPE, name)

	@classmethod
	def find_one_by_filters(cls, **kwargs) -> "EmailAccount":
		name = ragapp.db.get_value(cls.DOCTYPE, kwargs)
	...
	def get_account_details_from_site_config(cls):
		if not ragapp.conf.get("mail_server"):
			return {}
		...
		field_to_conf_name_map = {
			...
			"name": {"conf_names": ("email_sender_name",), "default": "Ragapp"},
		...
		for doc_field_name, d in field_to_conf_name_map.items():
			...
			value = [ragapp.conf.get(k) for k in conf_names if ragapp.conf.get(k)]
	...
	def get_ragapp_mail_client(self):
		return self._ragapp_mail_client

	@functools.cached_property
	def _ragapp_mail_client(self):
		if self.auth_method == "OAuth":
			if access_token := self.get_access_token():
				return RagappMail(self.ragapp_mail_site, self.email_id, access_token=access_token)

			ragapp.throw(
				_("Please Authorize OAuth for Email Account {0}").format(
					ragapp.bold(self.email_account_name)
				),
				title=_("Ragapp Mail OAuth Error"),
			)
		else:
			return RagappMail(
				self.ragapp_mail_site, self.email_id, self.api_key, self.get_password("api_secret")
			)
	...
	def handle_incoming_connect_error(self, description):
		if self.get_failed_attempts_count() > 5:
			...
			ragapp.enqueue(self._disable_broken_incoming_account, description=description)
		else:
			self.set_failed_attempts_count(self.get_failed_attempts_count() + 1)

	def _disable_broken_incoming_account(self, description):
		if ragapp.flags.in_test:
			return
	...
	def set_failed_attempts_count(self, value):
		ragapp.cache.set_value(f"{self.name}:email-account-failed-attempts", value)

	def get_failed_attempts_count(self):
		return cint(ragapp.cache.get_value(f"{self.name}:email-account-failed-attempts"))
	...
	def receive(self):
		...
		for mail in inbound_mails:
			try:
				...
				try:
					...
					if self.use_imap:
						self.handle_bad_emails(mail.uid, mail.raw_message, ragapp.get_traceback())
					exceptions.append(ragapp.get_traceback())
				except Exception:
					ragapp.db.rollback()
				else:
					ragapp.db.commit()
		...
		if exceptions:
			raise Exception(ragapp.as_json(exceptions))
	...
	def get_inbound_mails(self) -> list[InboundMail]:
		...
		try:
			if self.service == "Ragapp Mail":
				ragapp_mail_client = self.get_ragapp_mail_client()
				messages = ragapp_mail_client.pull_raw(last_synced_at=self.last_synced_at)
	...
	def handle_bad_emails(self, uid, raw, reason):
		...
		if cint(self.use_imap):
			...
			unhandled_email = ragapp.get_doc(
				{
					"raw": raw_str,
					"uid": uid,
					"reason": reason,
	...
	def send_auto_reply(self, communication, email):
		...
		from ragapp.core.doctype.communication.email import (
		...
		if self.enable_auto_reply:
			...
			ragapp.sendmail(
				...
				or ragapp.get_template("templates/emails/auto_reply.html").render(communication.as_dict()),
	...
	def on_trash(self):
		...
		Communication = ragapp.qb.DocType("Communication")
		ragapp.qb.update(Communication).set(Communication.email_account, "").where(
	...
	def after_rename(self, old, new, merge=False):
		ragapp.db.set_value("Email Account", new, "email_account_name", new)
	...
	def check_automatic_linking_email_account(self):
		if self.enable_automatic_linking:
			if not self.enable_incoming:
				ragapp.throw(_("Automatic Linking can be activated only if Incoming is enabled."))

			if ragapp.db.exists(
				"Email Account",
				{"enable_automatic_linking": 1, "name": ("!=", self.name)},
			):
				ragapp.throw(_("Automatic Linking can be activated only for one Email Account."))
	...
	def get_oauth_token(self):
		if self.auth_method == "OAuth":
			connected_app = ragapp.get_doc("Connected App", self.connected_app)
...
def get_append_to(doctype=None, txt=None, searchfield=None, start=None, page_len=None, filters=None):
	...
	email_append_to_list = [
		dt.name for dt in ragapp.get_all("DocType", filters=filters, fields=["name", "email_append_to"])
	]
	# Set Email Append To DocTypes set via Customize Form
	email_append_to_list.extend(
		dt.doc_type
		for dt in ragapp.get_list(
			"Property Setter",
...
def notify_unreplied():
	...
	for email_account in ragapp.get_all(
		"Email Account",
		"name",
		...
		if append_to:
			...
			for comm in ragapp.get_all(
				...
				comm = ragapp.get_doc("Communication", comm.name)

				if ragapp.db.get_value(comm.reference_doctype, comm.reference_name, "status") == "Open":
...
def pull(now=False):
	...
	from ragapp.integrations.doctype.connected_app.connected_app import has_token

	doctype = ragapp.qb.DocType("Email Account")
	email_accounts = (
	...
	for email_account in email_accounts:
		...
		if now:
			...
			queued_jobs = get_jobs(site=ragapp.local.site, key="job_name")[ragapp.local.site]
...
def pull_emails(email_account: str) -> None:
	...
	ragapp.has_permission("Email Account", "read", throw=True)
	...
	if job_name not in queued_jobs:
		...
		ragapp.msgprint(_("Emails are already being pulled from this account."))


def pull_from_email_account(email_account):
	"""Runs within a worker process"""
	email_account = ragapp.get_doc("Email Account", email_account)
...
def setup_user_email_inbox(email_account, awaiting_password, email_id, enable_outgoing, used_oauth):
	...
	from ragapp.core.doctype.user.user import ask_pass_update
	...
	user_names = ragapp.db.get_values("User", {"email": email_id}, as_dict=True)
	if not user_names:
		return
	...
	if update_user_email_settings:
		UserEmail = ragapp.qb.DocType("User Email")
		ragapp.qb.update(UserEmail).set(UserEmail.awaiting_password, (awaiting_password or 0)).set(
		...
		users = " and ".join([ragapp.bold(user.get("name")) for user in user_names])
		ragapp.msgprint(_("Enabled email inbox for user {0}").format(users))
	ask_pass_update()
...
def remove_user_email_inbox(email_account):
	...
	for user in users:
		doc = ragapp.get_doc("User", user.get("name"))
...
def set_email_password(email_account, password):
	account = ragapp.get_doc("Email Account", email_account)
