...
class EmailDomain(Document):
	...
	def validate(self):
		...
		if ragapp.local.flags.in_patch or ragapp.local.flags.in_test or ragapp.local.flags.in_install:
			return
	...
	def on_update(self):
		...
		for email_account in ragapp.get_all("Email Account", filters={"domain": self.name}):
			try:
