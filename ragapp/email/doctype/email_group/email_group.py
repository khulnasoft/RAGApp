...
from ragapp.utils import parse_addr, validate_email_address
...
class EmailGroup(Document):
	...
	def onload(self):
		singles = [d.name for d in ragapp.get_all("DocType", "name", {"issingle": 1})]
		self.get("__onload").import_types = [
			{"value": d.parent, "label": f"{d.parent} ({d.label})"}
			for d in ragapp.get_all("DocField", ("parent", "label"), {"options": "Email"})
	...
	def import_from(self, doctype):
		...
		for user in ragapp.get_all(doctype, [email_field, unsubscribed_field or "name"]):
			with contextlib.suppress(ragapp.UniqueValidationError, ragapp.InvalidEmailAddressError):
		...
		ragapp.msgprint(_("{0} subscribers added").format(added))
	...
	def on_trash(self):
		for d in ragapp.get_all("Email Group Member", "name", {"email_group": self.name}):
			ragapp.delete_doc("Email Group Member", d.name, force=True)


@ragapp.whitelist()
def import_from(name, doctype):
	nlist = ragapp.get_doc("Email Group", name)
...
def add_subscribers(name, email_list):
	...
	template = ragapp.db.get_value("Email Group", name, "welcome_email_template")
	welcome_email = ragapp.get_doc("Email Template", template) if template else None
	...
	for email in email_list:
		...
		if parsed_email:
			if not ragapp.db.get_value("Email Group Member", {"email_group": name, "email": parsed_email}):
				ragapp.get_doc(
					{"doctype": "Email Group Member", "email_group": name, "email": parsed_email}
				).insert(ignore_permissions=ragapp.flags.ignore_permissions)
			...
			ragapp.msgprint(_("{0} is not a valid Email Address").format(email))

	ragapp.msgprint(_("{0} subscribers added").format(count))

	return ragapp.get_doc("Email Group", name).update_total_subscribers()
...
def send_welcome_email(welcome_email, email, email_group):
	...
	message = ragapp.render_template(welcome_email.response_, args)
	ragapp.sendmail(email, subject=welcome_email.subject, message=message)
