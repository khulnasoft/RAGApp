...
import ragapp
from ragapp import _, safe_encode, task
from ragapp.core.utils import html2text
from ragapp.database.database import savepoint
from ragapp.email.doctype.email_account.email_account import EmailAccount
from ragapp.email.email_body import add_attachment, get_email, get_formatted_html
from ragapp.email.ragappmail import RagappMail
from ragapp.email.queue import get_unsubcribed_url, get_unsubscribe_message
from ragapp.email.smtp import SMTPServer
from ragapp.model.document import Document
from ragapp.query_builder import DocType, Interval
...
from ragapp.utils.verified_command import get_signed_params
...
class EmailQueue(Document):
	...
	if TYPE_CHECKING:
		from ragapp.email.doctype.email_queue_recipient.email_queue_recipient import EmailQueueRecipient
	...
	def prevent_email_queue_delete(self):
		if ragapp.session.user != "Administrator":
			ragapp.throw(_("Only Administrator can delete Email Queue"))

	def get_duplicate(self, recipients):
		values = self.as_dict()
		del values["name"]
		duplicate = ragapp.get_doc(values)
	...
	def new(cls, doc_data, ignore_permissions=False) -> "EmailQueue":
		...
		doc = ragapp.new_doc(cls.DOCTYPE)
		doc.update(data)
	...
	def update_db(self, commit=False, **kwargs):
		ragapp.db.set_value(self.DOCTYPE, self.name, kwargs)
		if commit:
			ragapp.db.commit()
	...
	def update_status(self, status, commit=False, **kwargs):
		...
		if self.communication:
			communication_doc = ragapp.get_doc("Communication", self.communication)
	...
	def get_email_account(self, raise_error=False):
		if self.email_account:
			return ragapp.get_cached_doc("Email Account", self.email_account)
	...
	def can_send_now(self):
		if (
			ragapp.are_emails_muted()
			or not self.is_to_be_sent()
			or cint(ragapp.db.get_default("suspend_email_queue")) == 1
		):
			return False

		return True

	def send(self, smtp_server_instance: SMTPServer = None, ragapp_mail_client: RagappMail = None):
		"""Send emails to recipients."""
		if not self.can_send_now():
			return

		with SendMailContext(self, smtp_server_instance, ragapp_mail_client) as ctx:
			...
			for recipient in self.recipients:
				...
				elif not ragapp.flags.in_test or ragapp.flags.testing_email:
					if ctx.email_account_doc.service == "Ragapp Mail":
						is_newsletter = self.reference_doctype == "Newsletter"
						ctx.ragapp_mail_client.send_raw(
							sender=self.sender,
			...
			if ragapp.flags.in_test and not ragapp.flags.testing_email:
				ragapp.flags.sent_mail = message
				return
	...
	def clear_old_logs(days=30):
		...
		"""
		days = days or 31
		email_queue = ragapp.qb.DocType("Email Queue")
		email_recipient = ragapp.qb.DocType("Email Queue Recipient")

		# Delete queue table
		(
			ragapp.qb.from_(email_queue).delete().where(email_queue.creation < (Now() - Interval(days=days)))
		).run()
		...
		(
			ragapp.qb.from_(email_recipient)
			.delete()
			.where(email_recipient.creation < (Now() - Interval(days=days)))
		).run()


from ragapp.deprecation_dumpster import send_mail as _send_mail
...
class SendMailContext:
	def __init__(
		...
		ragapp_mail_client: RagappMail = None,
	):
		self.queue_doc: EmailQueue = queue_doc
		self.smtp_server: SMTPServer = smtp_server_instance
		self.ragapp_mail_client: RagappMail = ragapp_mail_client
	...
	def fetch_outgoing_server(self):
		...
		if self.email_account_doc.service == "Ragapp Mail":
			if not self.ragapp_mail_client:
				self.ragapp_mail_client = self.email_account_doc.get_ragapp_mail_client()
	...
	def __exit__(self, exc_type, exc_val, exc_tb):
		if exc_type:
			update_fields = {"error": ragapp.get_traceback()}
	...
	def get_tracker_str(self, recipient_email) -> str:
		...
		if self.queue_doc.get("email_read_tracker_url"):
			...
			tracker_url = f"{get_url()}/api/method/ragapp.core.doctype.communication.email.mark_email_as_seen?name={self.queue_doc.communication}"
	...
	def include_attachments(self, message):
		...
		for attachment in attachments:
			...
			if file_filters:
				_file = ragapp.get_doc("File", file_filters)
				...
				print_format_file = ragapp.attach_print(**attachment)
	...
	def _store_file(self, file_name, content):
		if not ragapp.get_system_settings("store_attached_pdf_document"):
			return

		file_data = ragapp._dict(file_name=file_name, is_private=1)
		...
		if ragapp.db.exists("File", file_data):
			return

		file = ragapp.new_doc("File", **file_data)
...
def retry_sending(queues: str | list[str]):
	if not ragapp.has_permission("Email Queue", throw=True):
		return
	...
	email_queue = ragapp.qb.DocType("Email Queue")
	ragapp.qb.update(email_queue).set(email_queue.status, "Not Sent").set(email_queue.modified, now()).set(
		email_queue.modified_by, ragapp.session.user
...
def toggle_sending(enable):
	...
	ragapp.db.set_default("suspend_email_queue", 0 if sbool(enable) else 1)


def on_doctype_update():
	"""Add index in `tabCommunication` for `(reference_doctype, reference_name)`"""
	ragapp.db.add_index("Email Queue", ("status", "send_after", "priority", "creation"), "index_bulk_flush")

	ragapp.db.add_index("Email Queue", ["message_id(140)"])


def get_email_retry_limit():
	return cint(ragapp.db.get_system_setting("email_retry_limit")) or 3
...
class QueueBuilder:
	...
	def __init__(
		...
		:param unsubscribe_method: URL method for unsubscribe. Default is `/api/method/ragapp.email.queue.unsubscribe`.
	...
	def unsubscribe_method(self):
		return self._unsubscribe_method or "/api/method/ragapp.email.queue.unsubscribe"
	...
	def get_unsubscribed_user_emails(self):
		...
		if len(all_ids) > 0:
			unsubscribed = (
				ragapp.qb.from_(EmailUnsubscribe)
	...
	def process(self, send_now=False) -> EmailQueue | None:
		...
		if not queue_separately:
			...
			if send_now and len(final_recipients) >= 1000:
				...
				send_now = False
			for recipients in ragapp.utils.create_batch(final_recipients, 1000):
				ragapp.enqueue(
					self.send_emails,
					queue_data=queue_data,
					final_recipients=recipients,
					job_name=ragapp.utils.get_job_name(
						"send_bulk_emails_for", self.reference_doctype, self.reference_name
					),
					now=ragapp.flags.in_test or send_now,
					queue="long",
				)
	...
	def send_emails(self, queue_data, final_recipients):
		...
		ragapp_mail_client = None
		smtp_server_instance = None
		for r in final_recipients:
			recipients = list(set([r, *self.final_cc(), *self.bcc]))
			q = EmailQueue.new({**queue_data, **{"recipients": recipients}}, ignore_permissions=True)
			if not ragapp_mail_client and not smtp_server_instance:
				email_account = q.get_email_account(raise_error=True)

				if email_account.service == "Ragapp Mail":
					ragapp_mail_client = email_account.get_ragapp_mail_client()
				else:
					smtp_server_instance = email_account.get_smtp_server()

			with suppress(Exception):
				q.send(smtp_server_instance=smtp_server_instance, ragapp_mail_client=ragapp_mail_client)
	...
	def as_dict(self, include_recipients=True):
		...
		except ragapp.InvalidEmailAddressError:
