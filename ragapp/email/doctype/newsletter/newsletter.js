...
ragapp.ui.form.on("Newsletter", {
	refresh(frm) {
		let doc = frm.doc;
		let can_write = ragapp.boot.user.can_write.includes(doc.doctype);
		...
		if (frm.is_new() && !doc.sender_email) {
			let { fullname, email } = ragapp.user_info(doc.owner);
	...
	send_emails(frm) {
		ragapp.dom.freeze(__("Queuing emails..."));
		frm.call("send_emails").then(() => {
			frm.refresh();
			ragapp.dom.unfreeze();
			ragapp.show_alert(
				__("Queued {0} emails", [ragapp.utils.shorten_number(frm.doc.total_recipients)])
			);
		});
	},

	schedule_send_dialog(frm) {
		let hours = ragapp.utils.range(24);
	...
	update_schedule_message(frm) {
		if (!frm.doc.email_sent && frm.doc.schedule_send) {
			let datetime = ragapp.datetime.global_date_format(frm.doc.schedule_send);
