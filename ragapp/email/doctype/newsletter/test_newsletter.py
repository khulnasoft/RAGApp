...
import ragapp
from ragapp.email.doctype.newsletter.exceptions import (
	NewsletterAlreadySentError,
	NoRecipientFoundError,
)
from ragapp.email.doctype.newsletter.newsletter import (
	Newsletter,
...
from ragapp.utils import add_days, getdate
...
class TestNewsletterMixin:
	...
	def tearDown(self):
		...
		for newsletter in newsletters:
			...
			ragapp.delete_doc("Newsletter", newsletter)
			ragapp.db.delete("Newsletter Email Group", {"parent": newsletter})
			newsletters.remove(newsletter)

	def setup_email_group(self):
		if not ragapp.db.exists("Email Group", "_Test Email Group"):
			ragapp.get_doc({"doctype": "Email Group", "title": "_Test Email Group"}).insert()
		...
		for email in emails:
			...
			try:
				...
				ragapp.db.set_value(doctype, email_filters, "unsubscribed", 0)
	...
	def get_newsletter(**kwargs) -> "Newsletter":
		...
		similar_newsletters = ragapp.get_all(doctype, newsletter_content, pluck="name")

		for similar_newsletter in similar_newsletters:
			ragapp.delete_doc(doctype, similar_newsletter)

		newsletter = ragapp.get_doc({"doctype": doctype, **newsletter_content, **kwargs})
...
class TestNewsletter(TestNewsletterMixin, IntegrationTestCase):
	def test_send(self):
		...
		email_queue_list = [ragapp.get_doc("Email Queue", e.name) for e in ragapp.get_all("Email Queue")]
	...
	def test_unsubscribe(self):
		...
		group = ragapp.get_all("Newsletter Email Group", filters={"parent": name}, fields=["email_group"])
	...
	def test_send_scheduled_email_error_handling(self):
		...
		job_path = "ragapp.email.doctype.newsletter.newsletter.Newsletter.queue_all"
		m = MagicMock(side_effect=ragapp.OutgoingEmailError)

		with self.assertRaises(ragapp.OutgoingEmailError):
	...
	def test_retry_partially_sent_newsletter(self):
		...
		# retry
		newsletter.send_emails()
		self.assertEqual(ragapp.db.count("Email Queue"), 4)
