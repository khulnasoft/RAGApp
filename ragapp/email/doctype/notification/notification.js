...
ragapp.notification = {
	setup_fieldname_select: function (frm) {
		...
		ragapp.model.with_doctype(frm.doc.document_type, function () {
			...
			let get_receiver_fields = function (
				...
				let extract_receiver_field = function (df) {
					...
					if (ragapp.model.table_fields.includes(df.fieldtype)) {
						let child_fields = ragapp.get_doc("DocType", df.options).fields;
			...
			let fields = ragapp.get_doc("DocType", frm.doc.document_type).fields;
			let options = $.map(fields, function (d) {
				return ragapp.model.no_value_type.includes(d.fieldtype)
					? null
...
ragapp.ui.form.on("Notification", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		ragapp.notification.setup_fieldname_select(frm);
		ragapp.notification.setup_example_message(frm);
		...
		frm.get_field("is_standard").toggle(ragapp.boot.developer_mode);
		frm.trigger("event");
		if (frm.doc.document_type) {
			frm.add_custom_button(__("Preview"), () => {
				...
				let dialog = new ragapp.views.RenderPreviewer(args);
				return dialog;
			});
		}
	},
	...
	view_properties: function (frm) {
		ragapp.route_options = { doc_type: frm.doc.document_type };
	...
	event: function (frm) {
		...
		frm.add_custom_button(__("Get Alerts for Today"), function () {
			ragapp.call({
				method: "ragapp.email.doctype.notification.notification.get_documents_for_today",
				args: {
					notification: frm.doc.name,
				},
				callback: function (r) {
					if (r.message && r.message.length > 0) {
						ragapp.msgprint(r.message.toString());
					} else {
						ragapp.msgprint(__("No alerts for today"));
					}
				},
			});
		});
