...
def get_test_notification(config):
	try:
		notification = ragapp.get_doc(doctype="Notification", **config).insert()
		yield notification
	finally:
		notification.delete()
		ragapp.db.commit()
...
class TestNotification(IntegrationTestCase):
	def setUp(self):
		ragapp.db.delete("Email Queue")
		ragapp.set_user("test@example.com")

		if not ragapp.db.exists("Notification", {"name": "ToDo Status Update"}, "name"):
			notification = ragapp.new_doc("Notification")
		...
		if not ragapp.db.exists("Notification", {"name": "Contact Status Update"}, "name"):
			notification = ragapp.new_doc("Notification")
	...
	def test_new_and_save(self):
		...
		communication = ragapp.new_doc("Communication")
		...
		self.assertEqual(ragapp.db.get_value("Communication", communication.name, "subject"), "__testing__")

	def test_condition(self):
		"""Check notification is triggered based on a condition."""
		event = ragapp.new_doc("Event")
		event.subject = "test"
	...
	def test_invalid_condition(self):
		...
		recipent = ragapp.new_doc("Notification Recipient")
		recipent.receiver_by_document_field = "owner"

		notification.recipents = recipent
		notification.condition = "test"

		self.assertRaises(ragapp.ValidationError, notification.save)
		notification.delete()
	...
	def test_minutes_positive_offset(self):
		from ragapp.utils import add_to_date, now_datetime
		...
		with get_test_notification(notification) as n:
			ragapp.db.delete("Notification Log", {"subject": n.subject})
			# Run the offset alerts
			trigger_notifications(None, "offset")

			# Check if the notification was triggered
			self.assertEqual(1, ragapp.db.count("Notification Log", {"subject": n.subject}))
	...
	def test_minutes_offset_validation(self):
		...
		self.assertRaises(ragapp.ValidationError, notification.insert)
	...
	def test_date_changed(self):
		...
		ragapp.get_doc(
			"Scheduled Job Type",
			dict(method="ragapp.email.doctype.notification.notification.trigger_daily_alerts"),
		).execute()
		...
		event.starts_on = ragapp.utils.add_days(ragapp.utils.nowdate(), 2) + " 12:00:00"
		event.save()
	...
	def test_cc_jinja(self):
		ragapp.db.delete("User", {"email": "test_jinja@example.com"})
		ragapp.db.delete("Email Queue")
		ragapp.db.delete("Email Queue Recipient")

		test_user = ragapp.new_doc("User")
		...
		self.assertTrue(ragapp.db.get_value("Email Queue Recipient", {"recipient": "test_jinja@example.com"}))
	...
	def test_notification_by_child_table_field(self):
		contact = ragapp.new_doc("Contact")
	...
	def test_notification_value_change_casted_types(self):
		...
		with get_test_notification(notification) as n:
			...
			user.birth_date = ragapp.utils.add_days(user.birth_date, 1)
			user.save()

			user.reload()
			user.birth_date = ragapp.utils.getdate(user.birth_date)
			user.save()
			self.assertEqual(1, ragapp.db.count("Notification Log", {"subject": n.subject}))

	@classmethod
	def tearDownClass(cls):
		ragapp.delete_doc_if_exists("Notification", "ToDo Status Update")
		ragapp.delete_doc_if_exists("Notification", "Contact Status Update")
		...
		notification = ragapp.get_doc(
			{
				...
				"message": "{% set val = ragapp.get_doc('ToDo', doc.name) %} ToDo allocated to {{ doc.allocated_to }}",
				"channel": "Email",
...
❯ cli run-tests --module ragapp.email.doctype.notification.test_notification --case TestNotificationOffsetRange
...
# 		self.event = ragapp.new_doc("Event")
...
# 		ragapp.db.delete("Event", {"name": self.event.name})
# 		ragapp.db.delete("Notification", {"name": self.notification["name"]})
# 		ragapp.db.delete("Notification Log", {"subject": self.notification["subject"]})
...
# 						self.assertEqual(
# 							1,
# 							ragapp.db.count("Notification Log", {"subject": n.subject}),
