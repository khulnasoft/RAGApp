...
from ragapp.utils import cint, cstr, get_url, now_datetime
from ragapp.utils.data import getdate
...
def get_emails_sent_this_month(email_account=None):
	...
	return ragapp.db.count("Email Queue", filters=filters)
...
def get_emails_sent_today(email_account=None):
	...
	return ragapp.db.sql(q, q_args)[0][0]


def get_unsubscribe_message(unsubscribe_message: str, expose_recipients: str) -> "ragapp._dict[str, str]":
	...
	return ragapp._dict(html=html, text=text)
...
def unsubscribe(doctype, name, email):
	...
	if not ragapp.flags.in_test and not verify_request():
		return
...
def flush():
	...
	if ragapp.are_emails_muted():
		msgprint(_("Emails are muted"))

	if cint(ragapp.db.get_default("suspend_email_queue")) == 1:
		return
	...
	for row in email_queue_batch:
		try:
			email_queue: EmailQueue = ragapp.get_doc("Email Queue", row.name)
			email_queue.send()
		except Exception:
			ragapp.get_doc("Email Queue", row.name).log_error()
			...
			if (
				...
				ragapp.throw(_("Email Queue flushing aborted due to too many failures."))


def get_queue():
	batch_size = cint(ragapp.conf.email_queue_batch_size) or 500
