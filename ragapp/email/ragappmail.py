...
from ragapp.ragappclient import RagappClient, RagappOAuth2Client
from ragapp.utils import convert_utc_to_system_timezone, get_datetime, get_system_timezone


class RagappMail:
	"""Class to interact with the Ragapp Mail API."""
	...
	def get_client(
		...
		api_secret: str | None = None,
		access_token: str | None = None,
	) -> RagappClient | RagappOAuth2Client:
		"""Returns a RagappClient or RagappOAuth2Client instance."""

		if hasattr(ragapp.local, "ragapp_mail_clients"):
			if client := ragapp.local.ragapp_mail_clients.get(mailbox):
				return client
		else:
			ragapp.local.ragapp_mail_clients = {}

		client = (
			RagappOAuth2Client(url=site, access_token=access_token)
			if access_token
			else RagappClient(url=site, api_key=api_key, api_secret=api_secret)
		)
		ragapp.local.ragapp_mail_clients[mailbox] = client
	...
	def request(
		...
		"""Makes a request to the Ragapp Mail API."""
	...
	def send_raw(
		...
		"""Sends an email using the Ragapp Mail API."""
	...
	def pull_raw(self, limit: int = 50, last_synced_at: str | None = None) -> dict[str, str | list[str]]:
		"""Pulls emails from the mailbox using the Ragapp Mail API."""
		...
		headers = {"X-Site": ragapp.utils.get_url()}
