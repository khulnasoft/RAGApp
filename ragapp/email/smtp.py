...
from ragapp.utils import cint, cstr, get_traceback


class InvalidEmailCredentials(ragapp.ValidationError):
	pass
...
class SMTPServer:
	def __init__(
		...
		if not self.server:
			ragapp.msgprint(
				...
				raise_exception=ragapp.OutgoingEmailError,
			)
	...
	def session(self):
		...
		try:
			...
			if not _session:
				ragapp.msgprint(
					_("Could not connect to outgoing email server"), raise_exception=ragapp.OutgoingEmailError
				)
			...
			if self.use_oauth:
				...
				if res[0] != 235:
					ragapp.msgprint(res[1], raise_exception=ragapp.OutgoingEmailError)
	...
	def _enqueue_connection_closure(self):
		if ragapp.request and hasattr(ragapp.request, "after_response"):
			ragapp.request.after_response.add(self.quit)
		elif ragapp.job:
			ragapp.job.after_job.add(self.quit)
		elif not ragapp.flags.in_test:
			# Console?
			import atexit
