...
def merge_location_features_in_one(coords):
	...
	for element in coords:
		geojson_loc = ragapp.parse_json(element["location"])
		if not geojson_loc:
			continue
...
def return_location(doctype, filters_sql):
	...
	if filters_sql:
		try:
			coords = ragapp.db.sql(
				f"""SELECT name, location FROM `tab{doctype}`  WHERE {filters_sql}""", as_dict=True
			)
		except ragapp.db.InternalError:
			ragapp.msgprint(ragapp._("This Doctype does not contain location fields"), raise_exception=True)
			return
	else:
		coords = ragapp.get_all(doctype, fields=["name", "location"])
	return coords
...
def return_coordinates(doctype, filters_sql):
	...
	if filters_sql:
		try:
			...
			ragapp.msgprint(
				ragapp._("This Doctype does not contain latitude and longitude fields"), raise_exception=True
			)
			return
	else:
		coords = ragapp.get_all(doctype, fields=["name", "latitude", "longitude"])
	return coords
...
def get_coords_conditions(doctype, filters=None):
	...
	if not ragapp.has_permission(doctype):
		ragapp.throw(ragapp._("Not Permitted"), ragapp.PermissionError)
