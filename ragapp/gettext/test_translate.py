from ragapp.gettext.translate import (
	generate_pot,
...
class TestTranslate(IntegrationTestCase):
	...
	def test_write_catalog(self):
		po_path = get_po_path("ragapp", "test")
		po_path.unlink(missing_ok=True)

		catalog = new_catalog("ragapp", "test")
		write_catalog("ragapp", catalog, "test")

		self.assertTrue(po_path.exists())
		self.assertIn("msgid", po_path.read_text())

	def test_write_binary(self):
		mo_path = get_mo_path("ragapp", "test")
		mo_path.unlink(missing_ok=True)

		catalog = new_catalog("ragapp", "test")
		write_binary("ragapp", catalog, "test")

		self.assertTrue(mo_path.exists())

	def test_get_method_map(self):
		method_map = get_method_map("ragapp")
	...
	def test_gitignore(self):
		...
		is_gitignored = get_is_gitignored_function_for_app("ragapp")

		file_name = "ragapp/public/dist/test_translate_test_gitignore.js"
		file_path = ragapp.get_app_source_path("ragapp", file_name)
		self.assertTrue(is_gitignored("ragapp/public/node_modules"))
		self.assertTrue(is_gitignored("ragapp/public/dist"))
		self.assertTrue(is_gitignored("ragapp/public/dist/sub"))
		self.assertTrue(is_gitignored(file_name))
		self.assertTrue(is_gitignored(file_path))
		self.assertFalse(is_gitignored("ragapp/public/dist2"))
		self.assertFalse(is_gitignored("ragapp/public/dist2/sub"))
		...
		self.assertTrue(get_is_gitignored_function_for_app(None)("ragapp/public/dist"))
