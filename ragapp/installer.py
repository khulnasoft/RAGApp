...
import ragapp
from ragapp.defaults import _clear_cache
from ragapp.utils import cint, is_git_url
from ragapp.utils.dashboard import sync_dashboards
...
def _is_scheduler_enabled(site) -> bool:
	...
	try:
		ragapp.init(site)
		ragapp.connect()
		enable_scheduler = cint(ragapp.db.get_single_value("System Settings", "enable_scheduler"))
	except Exception:
		pass
	finally:
		ragapp.db.close()
...
def _new_site(
	...
	"""Install a new Ragapp site"""

	from ragapp.utils import scheduler
	...
	if not db_name:
		db_name = f"_{ragapp.generate_hash(length=16)}"
	...
	if rollback_callback:
		rollback_callback.add(lambda: shutil.rmtree(ragapp.get_site_path()))

	with filelock("cli_new_site", timeout=1):
		install_db(
		...
		apps_to_install = ["ragapp"] + (ragapp.conf.get("install_apps") or []) + (list(install_apps or []))
	...
	scheduler_status = "disabled" if ragapp.utils.scheduler.is_scheduler_disabled() else "enabled"
...
def install_db(
	...
	from ragapp.database import bootstrap_database, drop_user_and_database, setup_database
	...
	ragapp.flags.in_install_db = True

	if root_login:
		ragapp.flags.root_login = root_login

	if root_password:
		ragapp.flags.root_password = root_password
	...
	ragapp.conf.admin_password = ragapp.conf.admin_password or admin_password

	remove_missing_apps()

	ragapp.db.create_auth_table()
	ragapp.db.create_global_search_table()
	ragapp.db.create_user_settings_table()

	ragapp.flags.in_install_db = False
...
def find_org(org_repo: str) -> tuple[str, str]:
	...
	find_org()
	ref -> https://github.com/khulnasoft/cli/blob/develop/cli/utils/__init__.py#L390
	...
	from ragapp.exceptions import InvalidRemoteException

	for org in ["ragapp", "erpnext"]:
...
def fetch_details_from_tag(_tag: str) -> tuple[str, str, str]:
	...
	ref -> https://github.com/khulnasoft/cli/blob/develop/cli/utils/__init__.py#L403
...
def parse_app_name(name: str) -> str:
	...
	ref -> https://github.com/khulnasoft/cli/blob/develop/cli/app.py#L114
...
def install_app(name, verbose=False, set_as_patched=True, force=False):
	from ragapp.core.doctype.scheduled_job_type.scheduled_job_type import sync_jobs
	from ragapp.model.sync import sync_for
	from ragapp.modules.utils import sync_customizations
	from ragapp.utils.fixtures import sync_fixtures

	ragapp.flags.in_install = name
	ragapp.flags.ignore_in_install = False

	ragapp.clear_cache()
	app_hooks = ragapp.get_hooks(app_name=name)
	...
	if name not in ragapp.get_all_apps():
	...
	for before_install in app_hooks.before_install or []:
		out = ragapp.get_attr(before_install)()
		if out is False:
			return

	for fn in ragapp.get_hooks("before_app_install"):
		ragapp.get_attr(fn)(name)
	...
	ragapp.get_doc("Portal Settings", "Portal Settings").sync_menu()

	if set_as_patched:
		set_all_patches_as_completed(name)

	for after_install in app_hooks.after_install or []:
		ragapp.get_attr(after_install)()

	for fn in ragapp.get_hooks("after_app_install"):
		ragapp.get_attr(fn)(name)
	...
	for after_sync in app_hooks.after_sync or []:
		ragapp.get_attr(after_sync)()  #

	ragapp.flags.in_install = False
...
def add_to_installed_apps(app_name, rebuild_website=True):
	...
	if app_name not in installed_apps:
		...
		ragapp.db.set_global("installed_apps", json.dumps(installed_apps))
		ragapp.db.commit()
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	app_hooks = ragapp.get_hooks(app_name=app_name)

	# dont allow uninstall app if not installed unless forced
	if not force:
		if app_name not in ragapp.get_installed_apps():
	...
	for app in ragapp.get_installed_apps():
		if app != app_name:
			hooks = ragapp.get_hooks(app_name=app)
	...
	if not (dry_run or no_backup):
		from ragapp.utils.backups import scheduled_backup

		print("Backing up...")
		scheduled_backup(ignore_files=True)

	ragapp.flags.in_uninstall = True

	for before_uninstall in app_hooks.before_uninstall or []:
		ragapp.get_attr(before_uninstall)()

	for fn in ragapp.get_hooks("before_app_uninstall"):
		ragapp.get_attr(fn)(app_name)

	modules = ragapp.get_all("Module Def", filters={"app_name": app_name}, pluck="name")
	...
	if not dry_run:
		...
		ragapp.get_single("Installed Applications").update_versions()
		ragapp.db.commit()

	for after_uninstall in app_hooks.after_uninstall or []:
		ragapp.get_attr(after_uninstall)()

	for fn in ragapp.get_hooks("after_app_uninstall"):
		ragapp.get_attr(fn)(app_name)

	click.secho(f"Uninstalled App {app_name} from Site {site}", fg="green")
	ragapp.flags.in_uninstall = False
...
def _delete_modules(modules: list[str], dry_run: bool) -> list[str]:
	...
	for module_name in modules:
		...
		for doctype in ragapp.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					ragapp.delete_doc("DocType", doctype.name, ignore_on_trash=True, force=True)
				else:
		...
		if not dry_run:
			ragapp.delete_doc("Module Def", module_name, ignore_on_trash=True, force=True)
...
def _delete_linked_documents(module_name: str, doctype_linkfield_map: dict[str, str], dry_run: bool) -> None:
	...
	for doctype, fieldname in doctype_linkfield_map.items():
		for record in ragapp.get_all(doctype, filters={fieldname: module_name}, pluck="name"):
			print(f"* removing {doctype} '{record}'...")
			if not dry_run:
				ragapp.delete_doc(doctype, record, ignore_on_trash=True, force=True)
...
def _get_module_linked_doctype_field_map() -> dict[str, str]:
	...
	linked_doctypes = ragapp.get_all(
		"DocField",
		filters={"fieldtype": "Link", "options": "Module Def"},
		fields=["parent", "fieldname"],
	)
	existing_linked_doctypes = [d for d in linked_doctypes if ragapp.db.exists("DocType", d.parent)]
...
def _delete_doctypes(doctypes: list[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			ragapp.delete_doc("DocType", doctype, ignore_on_trash=True, force=True)
			ragapp.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{doctype}`")
...
def set_all_patches_as_completed(app):
	from ragapp.modules.patch_handler import get_patches_from_app

	patches = get_patches_from_app(app)
	for patch in patches:
		ragapp.get_doc({"doctype": "Patch Log", "patch": patch}).insert(ignore_permissions=True)
	ragapp.db.commit()


def init_singles():
	singles = ragapp.get_all("DocType", filters={"issingle": True}, pluck="name")
	for single in singles:
		if ragapp.db.get_singles_dict(single):
			continue

		try:
			doc = ragapp.new_doc(single)
			doc.flags.ignore_mandatory = True
			doc.flags.ignore_validate = True
			doc.save()
		except (ImportError, ragapp.DoesNotExistError):
...
def make_conf(
	...
	sites_path = ragapp.local.sites_path
	ragapp.destroy()
...
def make_site_config(
	...
	ragapp.create_folder(os.path.join(ragapp.local.site_path))
...
def update_site_config(key, value, validate=True, site_config_path=None):
	...
	from ragapp.config import clear_site_config_cache
	from ragapp.utils.synchronization import filelock
...
def _update_config_file(key: str, value, config_file: str):
	...
	if hasattr(ragapp.local, "conf"):
		ragapp.local.conf[key] = value


def get_site_config_path():
	return os.path.join(ragapp.local.site_path, "site_config.json")
...
def make_site_dirs():
	for dir_path in [
		...
		path = ragapp.get_site_path(dir_path)
		os.makedirs(path, exist_ok=True)


def add_module_defs(app, ignore_if_duplicate=False):
	modules = ragapp.get_module_list(app)
	for module in modules:
		d = ragapp.new_doc("Module Def")
		d.app_name = app
...
def remove_missing_apps():
	...
	apps = ("ragapp_subscription", "shopping_cart")
	installed_apps = json.loads(ragapp.db.get_global("installed_apps") or "[]")
	for app in apps:
...
def extract_files(site_name, file_path):
	...
	from ragapp.utils import get_cli_relative_path

	file_path = get_cli_relative_path(file_path)

	# Need to do ragapp.init to maintain the site locals
	ragapp.init(site_name)
	abs_site_path = os.path.abspath(ragapp.get_site_path())
...
def is_downgrade(sql_file_path, verbose=False):
	"""Check if input db backup will get downgraded on current cli
	...
	current_version = Version(ragapp.__version__)
	...
	if verbose and is_downgrade:
		print(f"Your site is currently on Ragapp {current_version} and your backup is {backup_version}.")

	return is_downgrade


def get_old_backup_version(sql_file_path: str) -> Version | None:
	"""Return the ragapp version used to create the specified database dump.

	This methods supports older versions of Ragapp wich used a different format.
	"""
	header = get_db_dump_header(sql_file_path).split("\n")
	if match := re.search(r"Ragapp (\d+\.\d+\.\d+)", header[0]):
...
def get_backup_version(sql_file_path: str) -> Version | None:
	...
	header = get_db_dump_header(sql_file_path).split("\n")
	metadata = ""
	if "begin ragapp metadata" in header[0]:
		for line in header[1:]:
			if "end ragapp metadata" in line:
				break
			metadata += line.replace("--", "").strip() + "\n"
		parser = configparser.ConfigParser()
		parser.read_string(metadata)
		return Version(parser["ragapp"]["version"])
...
def partial_restore(sql_file_path, verbose=False):
	if ragapp.conf.db_type == "mariadb":
		from ragapp.database.mariadb.setup_db import import_db_from_sql
	elif ragapp.conf.db_type == "postgres":
		import warnings

		from ragapp.database.postgres.setup_db import import_db_from_sql
...
def validate_database_sql(path: str, _raise: bool = True) -> None:
	...
	if os.path.getsize(path):
		...
		try:
			ragapp.utils.execute_in_shell(f"{executable} -m1 __Auth {path}", check_exit_code=True)
			return
		except Exception:
	...
	if _raise:
		raise ragapp.InvalidDatabaseFile
