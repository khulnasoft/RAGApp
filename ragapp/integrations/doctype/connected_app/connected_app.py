...
if any((os.getenv("CI"), ragapp.conf.developer_mode, ragapp.conf.allow_tests)):
...
class ConnectedApp(Document):
	...
	if TYPE_CHECKING:
		from ragapp.integrations.doctype.oauth_scope.oauth_scope import OAuthScope
		from ragapp.integrations.doctype.query_parameters.query_parameters import QueryParameters
	...
	def validate(self):
		base_url = ragapp.utils.get_url()
		callback_path = (
			"/api/method/ragapp.integrations.doctype.connected_app.connected_app.callback/" + self.name
		)
	...
	def initiate_web_application_flow(self, user=None, success_uri=None):
		...
		if not token_cache:
			token_cache = ragapp.new_doc("Token Cache")
	...
	def get_user_token(self, user=None, success_uri=None):
		...
		ragapp.local.response["location"] = redirect
		return redirect
	...
	def get_token_cache(self, user):
		...
		if ragapp.db.exists("Token Cache", token_cache_name):
			token_cache = ragapp.get_doc("Token Cache", token_cache_name)
...
@ragapp.whitelist(methods=["GET"], allow_guest=True)
def callback(code=None, state=None):
	...
	if ragapp.session.user == "Guest":
		ragapp.local.response["type"] = "redirect"
		ragapp.local.response["location"] = "/login?" + urlencode({"redirect-to": ragapp.request.url})
		return

	path = ragapp.request.path[1:].split("/")
	if len(path) != 4 or not path[3]:
		ragapp.throw(_("Invalid Parameters."))

	connected_app = ragapp.get_doc("Connected App", path[3])
	token_cache = ragapp.get_doc("Token Cache", connected_app.name + "-" + ragapp.session.user)

	if state != token_cache.state:
		ragapp.throw(_("Invalid token state! Check if the token has been created by the OAuth user."))
	...
	ragapp.local.response["type"] = "redirect"
	ragapp.local.response["location"] = token_cache.get("success_uri") or connected_app.get_url()


@ragapp.whitelist()
def has_token(connected_app, connected_user=None):
	app = ragapp.get_doc("Connected App", connected_app)
	token_cache = app.get_token_cache(connected_user or ragapp.session.user)
