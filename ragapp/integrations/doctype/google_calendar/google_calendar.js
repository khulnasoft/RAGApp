...
ragapp.ui.form.on("Google Calendar", {
	refresh: function (frm) {
		...
		ragapp.realtime.on("import_google_calendar", (data) => {
			if (data.progress) {
		...
		if (frm.doc.refresh_token) {
			frm.add_custom_button(__("Sync Calendar"), function () {
				...
				ragapp
					.call({
						method: "ragapp.integrations.doctype.google_calendar.google_calendar.sync",
						args: {
	...
	authorize_google_calendar_access: function (frm) {
		...
		ragapp.call({
			method: "ragapp.integrations.doctype.google_calendar.google_calendar.authorize_access",
			args: {
				g_calendar: frm.doc.name,
