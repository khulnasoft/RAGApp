...
class GoogleContacts(Document):
	...
	def validate(self):
		if not ragapp.db.get_single_value("Google Settings", "enable"):
			ragapp.throw(_("Enable Google API in Google Settings."))

	def get_access_token(self):
		if not self.refresh_token:
			button_label = ragapp.bold(_("Allow Google Contacts Access"))
...
def authorize_access(g_contact, reauthorize=False, code=None):
	...
	"""
	contact = ragapp.get_doc("Google Contacts", g_contact)
...
def get_google_contacts_object(g_contact):
	...
	account = ragapp.get_doc("Google Contacts", g_contact)
	oauth_obj = GoogleOAuth("contacts")
...
def sync(g_contact=None):
	...
	google_contacts = ragapp.get_list("Google Contacts", filters=filters)
...
def sync_contacts_from_google_contacts(g_contact):
	...
	ragapp.db.set_value("Google Contacts", account.name, "last_sync_on", ragapp.utils.now_datetime())

	for idx, connection in enumerate(results):
		ragapp.publish_realtime(
			"import_google_contacts", dict(progress=idx + 1, total=len(results)), user=ragapp.session.user
		)
		# Work-around to fix
		# https://github.com/khulnasoft/ragapp/issues/22648
...
def insert_contacts_to_google_contacts(doc, method=None):
	...
	"""
	if (
		not ragapp.db.exists("Google Contacts", {"name": doc.google_contacts})
	...
	try:
		...
		ragapp.db.set_value("Contact", doc.name, "google_contacts_id", contact.get("resourceName"))
	except HttpError as err:
		ragapp.msgprint(
...
def update_contacts_to_google_contacts(doc, method=None):
	...
	try:
		...
		ragapp.msgprint(_("Contact Synced with Google Contacts."))
	except HttpError as err:
		ragapp.msgprint(
