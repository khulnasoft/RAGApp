...
import ragapp
from ragapp import _, safe_encode
from ragapp.model.document import Document
from ragapp.twofactor import authenticate_for_2factor, confirm_otp_token, should_run_2fa
...
class LDAPSettings(Document):
	...
	if TYPE_CHECKING:
		from ragapp.integrations.doctype.ldap_group_mapping.ldap_group_mapping import LDAPGroupMapping
	...
	def connect_to_ldap(self, base_dn, password, read_only=True) -> ldap3.Connection:
		try:
			...
			ragapp.throw(msg, title=_("LDAP Not Installed"))
		except LDAPInvalidCredentialsResult:
			ragapp.throw(_("Invalid username or password"))
		except Exception as ex:
			ragapp.throw(_(str(ex)))

	@staticmethod
	def get_ldap_client_settings() -> dict:
		# return the settings to be used on the client side.
		result = {"enabled": False}
		ldap = ragapp.get_cached_doc("LDAP Settings")
		if ldap.enabled:
			result["enabled"] = True
			result["method"] = "ragapp.integrations.doctype.ldap_settings.ldap_settings.login"
		return result
	...
	def create_or_update_user(self, user_data: dict, groups: list | None = None):
		...
		if ragapp.db.exists("User", user_data["email"]):
			user = ragapp.get_doc("User", user_data["email"])
		...
		if self.default_user_type == "System User":
			...
			role = ragapp.db.get_value("User Type", user.user_type, "role")
	...
	def authenticate(self, username: str, password: str):
		if not self.enabled:
			ragapp.throw(_("LDAP is not enabled."))
		...
		try:
			...
			ragapp.throw(_("Please use a valid LDAP search filter"), title=_("Misconfigured"))
		...
		if conn.search(
			...
			if conn.entries and conn.entries[0]:
				...
				if conn.modify(entry_dn, changes=changes):
					if logout_sessions:
						from ragapp.sessions import clear_sessions

						clear_sessions(user=user, force=True)
					ragapp.msgprint(_("Password changed successfully."))
				else:
					ragapp.throw(_("Failed to change password."))
			else:
				ragapp.throw(_("No Entry for the User {0} found within LDAP!").format(user))
		else:
			ragapp.throw(_("No LDAP User found for email: {0}").format(user))
...
def login():
	# LDAP LOGIN LOGIC
	args = ragapp.form_dict
	ldap: LDAPSettings = ragapp.get_doc("LDAP Settings")

	user = ldap.authenticate(ragapp.as_unicode(args.usr), ragapp.as_unicode(args.pwd))

	ragapp.local.login_manager.user = user.name
	if should_run_2fa(user.name):
		authenticate_for_2factor(user.name)
		if not confirm_otp_token(ragapp.local.login_manager):
			return False
