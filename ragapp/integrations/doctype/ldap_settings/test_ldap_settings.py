...
import ragapp
from ragapp.exceptions import MandatoryError, ValidationError
...
class LDAP_TestCase:
	...
	def mock_ldap_connection(f):
		...
		def wrapped(self, *args, **kwargs):
			with mock.patch(
				"ragapp.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.connect_to_ldap",
	...
	def clean_test_users():
		with contextlib.suppress(Exception):
			ragapp.get_doc("User", "posix.user1@unit.testing").delete()
		with contextlib.suppress(Exception):
			ragapp.get_doc("User", "posix.user2@unit.testing").delete()
		with contextlib.suppress(Exception):
			ragapp.get_doc("User", "website_ldap_user@test.com").delete()

	@classmethod
	def setUpClass(cls):
		cls.clean_test_users()
		# Save user data for restoration in tearDownClass()
		cls.user_ldap_settings = ragapp.get_doc("LDAP Settings")
		...
		user = ragapp.get_doc(cls.user1doc)
		...
		user = ragapp.get_doc(cls.user2doc)
		user.insert(ignore_permissions=True)
	...
	def tearDownClass(cls):
		with contextlib.suppress(Exception):
			ragapp.get_doc("LDAP Settings").delete()
	...
	def test_connect_to_ldap(self):
		...
		prevent_connection_parameters = {
			"mode": {
				"IP_V4_ONLY": "Locks the user to IPv4 without ragapp providing a way to configure",
				"IP_V6_ONLY": "Locks the user to IPv6 without ragapp providing a way to configure",
			},
			"auto_bind": {
	...
	def test_update_user_fields(self):
		...
		test_user = ragapp.get_doc("User", test_user_data["email"])
		self.test_class.update_user_fields(test_user, test_user_data)
		updated_user = ragapp.get_doc("User", test_user_data["email"])
		...
		self.assertIn(self.test_class.default_role, ragapp.get_roles(updated_user.name))
	...
	def test_create_website_user(self):
		...
		new_user = ragapp.get_doc("User", new_test_user_data["email"])
	...
	def test_sync_roles(self):
		if self.TEST_LDAP_SERVER.lower() == "openldap":
			test_user_data = {
				"posix.user1": [
					...
					"default_role",
					"ragapp_default_all",
					"ragapp_default_guest",
					"ragapp_default_desk_user",
				],
				"posix.user2": [
					"Users",
					"Group3",
		...
		role_to_group_map = {
			...
			"All": "ragapp_default_all",
			"Guest": "ragapp_default_guest",
			"Desk User": "ragapp_default_desk_user",
		}

		# re-create user1 to ensure clean
		ragapp.get_doc("User", "posix.user1@unit.testing").delete()
		user = ragapp.get_doc(self.user1doc)
		user.insert(ignore_permissions=True)

		for test_user in test_user_data:
			test_user_doc = ragapp.get_doc("User", f"{test_user}@unit.testing")
			test_user_roles = ragapp.get_roles(f"{test_user}@unit.testing")

			self.assertTrue(
				len(test_user_roles) == 2, "User should only be a part of the All and Guest roles"
			)  # check default ragapp roles

			self.test_class.sync_roles(test_user_doc, test_user_data[test_user])  # update user roles

			ragapp.get_doc("User", f"{test_user}@unit.testing")
			updated_user_roles = ragapp.get_roles(f"{test_user}@unit.testing")
	...
	def test_create_or_update_user(self):
		...
		ragapp.get_doc("User", f"{test_user}@unit.testing").delete()

		with self.assertRaises(
			ragapp.exceptions.DoesNotExistError
		):  # ensure user deleted so function can be tested
			ragapp.get_doc("User", f"{test_user}@unit.testing")

		with mock.patch(
			"ragapp.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.update_user_fields"
		) as update_user_fields_method:
			with mock.patch(
				"ragapp.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.sync_roles"
	...
	def test_authenticate(self):
		with mock.patch(
			"ragapp.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.fetch_ldap_groups"
		...
		for username, password in enumerate(invalid_users):
			with self.assertRaises(ragapp.exceptions.ValidationError) as display_massage:
