...
from ragapp.integrations.doctype.social_login_key.social_login_key import BaseUrlNotSetError
from ragapp.tests import IntegrationTestCase, UnitTestCase
from ragapp.utils import set_request
from ragapp.utils.oauth import login_via_oauth2
...
class TestSocialLoginKey(IntegrationTestCase):
	def setUp(self) -> None:
		...
		ragapp.delete_doc("User", TEST_GITHUB_USER, force=True)
		super().setUp()
		ragapp.set_user("Guest")

	def test_adding_ragapp_social_login_provider(self):
		ragapp.set_user("Administrator")
	...
	def test_normal_signup_and_github_login(self):
		...
		if not ragapp.db.exists("User", TEST_GITHUB_USER):
			user = ragapp.new_doc("User", email=TEST_GITHUB_USER, first_name="GitHub Login")
		...
		self.assertEqual(ragapp.session.user, TEST_GITHUB_USER)
	...
	def test_force_disabled_signups(self):
		...
		with patch.object(OAuth2Service, "get_auth_session", return_value=mock_session):
			login_via_oauth2("github", "iwriu", {"token": "ewrwerwer"})
		self.assertEqual(ragapp.session.user, "Guest")
...
def make_social_login_key(**kwargs):
	...
	return ragapp.get_doc(kwargs)


def create_or_update_social_login_key():
	# used in other tests (connected app, oauth20)
	try:
		social_login_key = ragapp.get_doc("Social Login Key", "ragapp")
	except ragapp.DoesNotExistError:
		social_login_key = ragapp.new_doc("Social Login Key")
	social_login_key.get_social_login_provider("Ragapp", initialize=True)
	social_login_key.base_url = ragapp.utils.get_url()
...
def create_github_social_login_key():
	if ragapp.db.exists("Social Login Key", "github"):
		return ragapp.get_doc("Social Login Key", "github")
	else:
