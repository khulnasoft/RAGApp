...
ragapp.webhook = {
	set_fieldname_select: (frm) => {
		if (frm.doc.webhook_doctype) {
			ragapp.model.with_doctype(frm.doc.webhook_doctype, () => {
				// get doctype fields
				let fields = $.map(
					ragapp.get_doc("DocType", frm.doc.webhook_doctype).fields,
					(d) => {
						if (
							ragapp.model.no_value_type.includes(d.fieldtype) &&
							!ragapp.model.table_fields.includes(d.fieldtype)
						) {
							return null;
						} else {
							return {
				...
				// add meta fields
				for (let field of ragapp.model.std_fields) {
...
ragapp.ui.form.on("Webhook", {
	refresh: (frm) => {
		ragapp.webhook.set_fieldname_select(frm);
		frm.set_query(
			"background_jobs_queue",
			"ragapp.integrations.doctype.webhook.webhook.get_all_queues"
		);
	...
	request_structure: (frm) => {
		ragapp.webhook.set_request_headers(frm);
	},
...
ragapp.ui.form.on("Webhook Data", {
	fieldname: (frm, cdt, cdn) => {
		...
		if (!df.length) {
			...
			df = ragapp.model.std_fields.filter((field) => field.fieldname == row.fieldname);
		}
