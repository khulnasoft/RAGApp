...
import ragapp
from ragapp.utils import get_request_site_address

CALLBACK_METHOD = "/api/method/ragapp.integrations.google_oauth.callback"
_SCOPES = {
...
_DOMAIN_CALLBACK_METHODS = {
	"mail": "ragapp.email.oauth.authorize_google_access",
	"contacts": "ragapp.integrations.doctype.google_contacts.google_contacts.authorize_access",
	"drive": "ragapp.integrations.doctype.google_drive.google_drive.authorize_access",
	"indexing": "ragapp.website.doctype.website_settings.google_indexing.authorize_access",
}
...
class GoogleOAuth:
	...
	def __init__(self, domain: str, validate: bool = True):
		self.google_settings = ragapp.get_single("Google Settings")
	...
	def validate_google_settings(self):
		...
		if not self.google_settings.enable:
			ragapp.throw(ragapp._("Please enable {} before continuing.").format(google_settings))

		if not (self.google_settings.client_id and self.google_settings.client_secret):
			ragapp.throw(ragapp._("Please update {} before continuing.").format(google_settings))
...
def handle_response(
	...
	if "error" in response:
		ragapp.log_error(ragapp._(error_title), ragapp._(response.get("error_description", error_message)))

		if raise_err:
			ragapp.throw(ragapp._(error_title), GoogleAuthenticationError, ragapp._(error_message))
...
@ragapp.whitelist(methods=["GET"])
def callback(state: str, code: str | None = None, error: str | None = None) -> None:
	"""Common callback for google integrations.
	Invokes functions using `ragapp.get_attr` and also adds required (keyworded) arguments
	along with committing and redirecting us back to ragapp site."""
	...
	if not error:
		if (domain := state.pop("domain")) in _DOMAIN_CALLBACK_METHODS:
			...
			ragapp.get_attr(_DOMAIN_CALLBACK_METHODS[domain])(**state)
	...
	ragapp.local.response["location"] = f"{redirect}?{failure_query_param if error else success_query_param}"
