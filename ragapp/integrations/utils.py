...
import ragapp
from ragapp.utils import get_request_session
...
def make_request(method: str, url: str, auth=None, headers=None, data=None, json=None, params=None):
	...
	try:
		...
		response = ragapp.flags.integration_request = s.request(
		...
		if ragapp.flags.integration_request_doc:
			ragapp.flags.integration_request_doc.log_error()
		else:
			ragapp.log_error()
		raise exc
...
def create_request_log(
	...
	**kwargs,
	...
	integration_request = ragapp.get_doc(
		{
...
def get_json(obj):
	return obj if isinstance(obj, str) else ragapp.as_json(obj, indent=1)
