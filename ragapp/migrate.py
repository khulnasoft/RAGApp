# Copyright (c) 2022, Ragapp Technologies Pvt. Ltd. and Contributors
# License: MIT. See LICENSE

import contextlib
import functools
import json
import os
from textwrap import dedent

import ragapp
import ragapp.model.sync
import ragapp.modules.patch_handler
import ragapp.translate
from ragapp.cache_manager import clear_global_cache
from ragapp.core.doctype.language.language import sync_languages
from ragapp.core.doctype.navbar_settings.navbar_settings import sync_standard_items
from ragapp.core.doctype.scheduled_job_type.scheduled_job_type import sync_jobs
from ragapp.database.schema import add_column
from ragapp.deferred_insert import save_to_db as flush_deferred_inserts
from ragapp.desk.notifications import clear_notifications
from ragapp.modules.patch_handler import PatchType
from ragapp.modules.utils import sync_customizations
from ragapp.search.website_search import build_index_for_all_routes
from ragapp.utils.connections import check_connection
from ragapp.utils.dashboard import sync_dashboards
from ragapp.utils.data import cint
from ragapp.utils.fixtures import sync_fixtures
from ragapp.website.utils import clear_website_cache

CLI_START_MESSAGE = dedent(
	"""
	Cannot run cli migrate without the services running.
	If you are running cli in development mode, make sure that cli is running:

	$ cli start

	Otherwise, check the server logs and ensure that all the required services are running.
	"""
)


def atomic(method):
	@functools.wraps(method)
	def wrapper(*args, **kwargs):
		try:
			ret = method(*args, **kwargs)
			ragapp.db.commit()
			return ret
		except Exception as e:
			# database itself can be gone while attempting rollback.
			# We should preserve original exception in this case.
			with contextlib.suppress(Exception):
				ragapp.db.rollback()
			raise e

	return wrapper


class SiteMigration:
	"""Migrate all apps to the current version, will:
	- run before migrate hooks
	- run patches
	- sync doctypes (schema)
	- sync dashboards
	- sync jobs
	- sync fixtures
	- sync customizations
	- sync languages
	- sync web pages (from /www)
	- run after migrate hooks
	"""

	def __init__(self, skip_failing: bool = False, skip_search_index: bool = False) -> None:
		self.skip_failing = skip_failing
		self.skip_search_index = skip_search_index

	def setUp(self):
		"""Complete setup required for site migration"""
		ragapp.flags.touched_tables = set()
		self.touched_tables_file = ragapp.get_site_path("touched_tables.json")
		ragapp.clear_cache()
		add_column(doctype="DocType", column_name="migration_hash", fieldtype="Data")
		clear_global_cache()

		if os.path.exists(self.touched_tables_file):
			os.remove(self.touched_tables_file)

		self.lower_lock_timeout()
		with contextlib.suppress(Exception):
			self.kill_idle_connections()
		ragapp.flags.in_migrate = True

	def tearDown(self):
		"""Run operations that should be run post schema updation processes
		This should be executed irrespective of outcome
		"""
		ragapp.translate.clear_cache()
		clear_website_cache()
		clear_notifications()

		with open(self.touched_tables_file, "w") as f:
			json.dump(list(ragapp.flags.touched_tables), f, sort_keys=True, indent=4)

		if not self.skip_search_index:
			print(f"Queued rebuilding of search index for {ragapp.local.site}")
			ragapp.enqueue(build_index_for_all_routes, queue="long")

		ragapp.publish_realtime("version-update")
		ragapp.flags.touched_tables.clear()
		ragapp.flags.in_migrate = False

	@atomic
	def pre_schema_updates(self):
		"""Executes `before_migrate` hooks"""
		for app in ragapp.get_installed_apps():
			for fn in ragapp.get_hooks("before_migrate", app_name=app):
				ragapp.get_attr(fn)()

	@atomic
	def run_schema_updates(self):
		"""Run patches as defined in patches.txt, sync schema changes as defined in the {doctype}.json files"""
		ragapp.modules.patch_handler.run_all(
			skip_failing=self.skip_failing, patch_type=PatchType.pre_model_sync
		)
		ragapp.model.sync.sync_all()
		ragapp.modules.patch_handler.run_all(
			skip_failing=self.skip_failing, patch_type=PatchType.post_model_sync
		)

	@atomic
	def post_schema_updates(self):
		"""Execute pending migration tasks post patches execution & schema sync
		This includes:
		* Sync `Scheduled Job Type` and scheduler events defined in hooks
		* Sync fixtures & custom scripts
		* Sync in-Desk Module Dashboards
		* Sync customizations: Custom Fields, Property Setters, Custom Permissions
		* Sync Ragapp's internal language master
		* Flush deferred inserts made during maintenance mode.
		* Sync Portal Menu Items
		* Sync Installed Applications Version History
		* Execute `after_migrate` hooks
		"""
		print("Syncing jobs...")
		sync_jobs()

		print("Syncing fixtures...")
		sync_fixtures()
		sync_standard_items()

		print("Syncing dashboards...")
		sync_dashboards()

		print("Syncing customizations...")
		sync_customizations()

		print("Syncing languages...")
		sync_languages()

		print("Flushing deferred inserts...")
		flush_deferred_inserts()

		print("Removing orphan doctypes...")
		ragapp.model.sync.remove_orphan_doctypes()

		print("Syncing portal menu...")
		ragapp.get_single("Portal Settings").sync_menu()

		print("Updating installed applications...")
		ragapp.get_single("Installed Applications").update_versions()

		print("Executing `after_migrate` hooks...")
		for app in ragapp.get_installed_apps():
			for fn in ragapp.get_hooks("after_migrate", app_name=app):
				ragapp.get_attr(fn)()

	def required_services_running(self) -> bool:
		"""Return True if all required services are running. Return False and print
		instructions to stdout when required services are not available.
		"""
		service_status = check_connection(redis_services=["redis_cache"])
		are_services_running = all(service_status.values())

		if not are_services_running:
			for service in service_status:
				if not service_status.get(service, True):
					print(f"Service {service} is not running.")
			print(CLI_START_MESSAGE)

		return are_services_running

	def lower_lock_timeout(self):
		"""Lower timeout for table metadata locks, default is 1 day, reduce it to 5 minutes.

		This is required to avoid indefinitely waiting for metadata lock.
		"""
		if ragapp.db.db_type != "mariadb":
			return
		ragapp.db.sql("set session lock_wait_timeout = %s", 5 * 60)

	def kill_idle_connections(self, idle_limit=30):
		"""Assuming migrate has highest priority, kill everything else.

		If someone has connected to mariadb using DB console or ipython console and then acquired
		certain locks we won't be able to migrate."""
		if ragapp.db.db_type != "mariadb":
			return

		processes = ragapp.db.sql("show full processlist", as_dict=1)
		connection_id = ragapp.db.sql("select connection_id()")[0][0]
		for process in processes:
			sleeping = process.get("Command") == "Sleep"
			sleeping_since = cint(process.get("Time")) or 0
			pid = process.get("Id")

			if (
				pid
				and pid != connection_id
				and process.db == ragapp.conf.db_name
				and sleeping
				and sleeping_since > idle_limit
			):
				try:
					ragapp.db.sql(f"kill {pid}")
					print(f"Killed inactive database connection with PID {pid}")
				except Exception as e:
					# We might not have permission to do this.
					print(f"Failed to kill inactive database connection with PID {pid}: {e}")

	def run(self, site: str):
		"""Run Migrate operation on site specified. This method initializes
		and destroys connections to the site database.
		"""
		from ragapp.utils.synchronization import filelock

		if site:
			ragapp.init(site)
			ragapp.connect()

		if not self.required_services_running():
			raise SystemExit(1)

		with filelock("cli_migrate", timeout=1):
			self.setUp()
			try:
				self.pre_schema_updates()
				self.run_schema_updates()
				self.post_schema_updates()
			finally:
				self.tearDown()
				ragapp.destroy()
