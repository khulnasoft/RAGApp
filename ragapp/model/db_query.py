...
from ragapp.database.utils import DefaultOrderBy, FallBackDateTimeStr, NestedSetHierarchy
from ragapp.model import get_permitted_fields, optional_fields
from ragapp.model.meta import get_table_columns
from ragapp.model.utils import is_virtual_doctype
from ragapp.model.utils.user_settings import get_user_settings, update_user_settings
from ragapp.query_builder.utils import Column
from ragapp.types import Filters, FilterSignature, FilterTuple
...
from ragapp.utils.data import DateTimeLikeObject, get_datetime, getdate, sbool
...
class DatabaseQuery:
	def __init__(self, doctype, user=None):
		...
		self.or_conditions = []
		self.fields = None
		self.user = user or ragapp.session.user
	...
	def get_meta(self, doctype: str):
		if doctype not in self._metas:
			self._metas[doctype] = ragapp.get_meta(doctype)
	...
	def execute(
		...
		*,
		...
		if as_list and not isinstance(self.fields, (Sequence | str)) and len(self.fields) > 1:
			ragapp.throw(_("Fields must be a list or tuple when as_list is enabled"))
		...
		if is_virtual_doctype(self.doctype):
			...
			return ragapp.call(controller.get_list, args=kwargs, **kwargs)
	...
	def build_and_run(self):
		...
		if ragapp.db.db_type == "postgres" and args.order_by and args.group_by:
	...
	def prepare_args(self):
		...
		for child in self.tables[1:]:
			...
			args.tables += f" {self.join} {child} on ({child}.parenttype = {ragapp.db.escape(self.doctype)} and {child}.parent = {parent_name})"
	...
	def sanitize_fields(self):
		...
		def _raise_exception():
			ragapp.throw(_("Use of sub-query or function is restricted"), ragapp.DataError)
		...
		for field in self.fields:
			...
			if SUB_QUERY_PATTERN.match(field):
				...
				if function in blacklisted_functions:
					ragapp.throw(
						_("Use of function {0} in field is restricted").format(function), exc=ragapp.DataError
					)
			...
			if self.strict:
				if STRICT_FIELD_PATTERN.match(field):
					ragapp.throw(_("Illegal SQL Query"))
	...
	def _set_permission_map(self, doctype: str, parent_doctype: str | None = None):
		ptype = "select" if ragapp.only_has_select_perm(doctype) else "read"
		ragapp.has_permission(
			doctype,
			ptype=ptype,
	...
	def get_table_columns(self):
		...
		except ragapp.db.TableMissingError:
			if self.ignore_ddl:
				return None
			else:
				raise
	...
	def apply_fieldlevel_read_permissions(self):
		...
		Note: Does not apply to `ragapp.model.core_doctype_list`
		...
		Example:
		        ...
		        - Query: fields=["*"]
		        - Result: fields=["title", ...] // will also include Ragapp's meta field like `name`, `owner`, etc.
		"""
		from ragapp.desk.reportview import extract_fieldnames
		...
		for i, field in enumerate(self.fields):
			...
			# labels / pseudo columns or ragapp internals
			...
			# handle child / joined table fields
				...
				if wrap_grave_quotes(table) in self.query_tables:
					...
					raise ragapp.PermissionError(ch_doctype)
	...
	def prepare_filter_condition(self, ft: FilterTuple) -> str:
		...
		from ragapp.boot import get_additional_filters_from_hooks
		...
		if f.operator.lower() in NestedSetHierarchy:
			...
			lft, rgt = "", ""
			if f.value:
				lft, rgt = ragapp.db.get_value(ref_doctype, f.value, ["lft", "rgt"]) or (0, 0)
			...
			fallback = "''"
			value = [ragapp.db.escape((cstr(v)).strip(), percent=False) for v in nodes]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
		...
		if f.operator.lower() in ("in", "not in"):
			...
			if value is None:
				...
				fallback = "''"
				value = [ragapp.db.escape((cstr(v) or "").strip(), percent=False) for v in values]
				if len(value):
					value = f"({', '.join(value)})"
				else:
					value = "('')"
			...
			if f.operator in (">", "<", ">=", "<=") and (f.fieldname in ("creation", "modified")):
				...
				value = ragapp.db.format_date(f.value)
				fallback = "'0001-01-01'"

			elif (df and df.fieldtype == "Datetime") or isinstance(f.value, datetime.datetime):
				value = ragapp.db.format_datetime(f.value)
				fallback = f"'{FallBackDateTimeStr}'"
			...
			if isinstance(f.value, Column):
				...
				quote = '"' if ragapp.conf.db_type == "postgres" else "`"
				value = f"{tname}.{quote}{f.value.name}{quote}"

			# escape value
			elif escape and isinstance(value, str):
				value = f"{ragapp.db.escape(value, percent=False)}"
		...
		if (
			...
			if f.operator.lower() == "like" and ragapp.conf.get("db_type") == "postgres":
				f.operator = "ilike"
	...
	def build_match_conditions(self, as_condition=True) -> str | list:
		...
		if not self.user:
			self.user = ragapp.session.user

		if not self.tables:
			self.extract_tables()

		role_permissions = ragapp.permissions.get_role_permissions(self.doctype_meta, user=self.user)
		if (
			...
			only_if_shared = True
			self.shared = ragapp.share.get_shared(self.doctype, self.user)
			if not self.shared:
				ragapp.throw(_("No permission to read {0}").format(_(self.doctype)), ragapp.PermissionError)
			else:
			...
			if requires_owner_constraint(role_permissions):
				...
				self.match_conditions.append(
					f"`tab{self.doctype}`.`owner` = {ragapp.db.escape(self.user, percent=False)}"
				)

			# add user permission only if role has read perm
			elif role_permissions.get("read") or role_permissions.get("select"):
				# get user permissions
				user_permissions = ragapp.permissions.get_user_permissions(self.user)
	...
	def get_share_condition(self):
		return (
			...
			+ f" in ({', '.join(ragapp.db.escape(s, percent=False) for s in self.shared)})"
		)
	...
	def add_user_permissions(self, user_permissions):
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				docs = []
				if ragapp.get_system_settings("apply_strict_user_permissions"):
					condition = ""
				else:
				...
				if docs:
					values = ", ".join(ragapp.db.escape(doc, percent=False) for doc in docs)
	...
	def get_permission_query_conditions(self) -> str:
		conditions = []
		hooks = ragapp.get_hooks("permission_query_conditions", {})
		condition_methods = hooks.get(self.doctype, []) + hooks.get("*", [])
		for method in condition_methods:
			if c := ragapp.call(ragapp.get_attr(method), self.user, doctype=self.doctype):
				conditions.append(c)

		if permission_script_name := get_server_script_map().get("permission_query", {}).get(self.doctype):
			script = ragapp.get_doc("Server Script", permission_script_name)
	...
	def validate_order_by_and_group_by(self, parameters: str):
		...
		if "select" in _lower and "from" in _lower:
			ragapp.throw(_("Cannot use sub-query in order by"))
		...
		for field in parameters.split(","):
			...
			if full_field_name:
				...
				if tbl not in self.tables:
					if tbl.startswith("`"):
						tbl = tbl[4:-1]
					ragapp.throw(_("Please select atleast 1 column from {0} to sort/group").format(tbl))

			if function in blacklisted_sql_functions:
				ragapp.throw(_("Cannot use {0} in order/group by").format(field))
...
def check_parent_permission(parent, child_doctype):
	if parent:
		...
		if child_doctype and not (
			ragapp.db.exists("DocField", {"parent": parent, "options": child_doctype})
			or ragapp.db.exists("Custom Field", {"dt": parent, "options": child_doctype})
		):
			raise ragapp.PermissionError

		if ragapp.permissions.has_permission(parent):
			return
...
def has_any_user_permission_for_doctype(doctype, user, applicable_for):
	user_permissions = ragapp.permissions.get_user_permissions(user=user)
...
def get_between_date_filter(value, df=None):
	...
	from_date = ragapp.utils.nowdate()
	to_date = ragapp.utils.nowdate()
	...
	if fieldtype == "Datetime":
		cond = f"'{ragapp.db.format_datetime(from_date)}' AND '{ragapp.db.format_datetime(to_date)}'"
	else:
		cond = f"'{ragapp.db.format_date(from_date)}' AND '{ragapp.db.format_date(to_date)}'"
...
def get_additional_filter_field(additional_filters_config, f, value):
	...
	f = ragapp._dict(ragapp.get_attr(additional_filter["get_field"])())
	if f.query_value:
		for option in f.options:
			option = ragapp._dict(option)
