...
from ragapp import _, get_module_path
from ragapp.desk.doctype.tag.tag import delete_tags_for_document
from ragapp.model.docstatus import DocStatus
from ragapp.model.dynamic_links import get_dynamic_link_map
from ragapp.model.naming import revert_series_if_last
from ragapp.model.utils import is_virtual_doctype
from ragapp.utils.file_manager import remove_all
from ragapp.utils.global_search import delete_for_document
from ragapp.utils.password import delete_all_passwords_for
...
def delete_doc(
	...
	# get from form
	if not doctype:
		doctype = ragapp.form_dict.get("dt")
		name = ragapp.form_dict.get("dn")
	...
	for name in names or []:
		if is_virtual:
			ragapp.get_doc(doctype, name).delete()
			continue
		...
		if doctype == "DocType":
			if for_reload:
				...
				doc = ragapp.get_doc(doctype, name)
				if not (doc.custom or ragapp.conf.developer_mode or ragapp.flags.in_patch or force):
					ragapp.throw(_("Standard DocType can not be deleted."))

				update_flags(doc, flags, ignore_permissions)
				check_permission_and_not_submitted(doc)
				# delete custom table fields using this doctype.
				ragapp.db.delete(
					"Custom Field", {"options": name, "fieldtype": ("in", ragapp.model.table_fields)}
				)
				ragapp.db.delete("__global_search", {"doctype": name})

			delete_from_table(doctype, name, ignore_doctypes, None)

			if (
				ragapp.conf.developer_mode
				and not doc.custom
				and not (
					for_reload
			...
			try:
				ragapp.db.get_value(doctype, name, for_update=True, wait=False)
			except ragapp.QueryTimeoutError:
				ragapp.throw(
					_(
						"This document can not be deleted right now as it's being modified by another user. Please try again after some time."
					),
					exc=ragapp.QueryTimeoutError,
				)
			doc = ragapp.get_doc(doctype, name)
			...
			if not for_reload:
				...
				if not force:
					...
					except ragapp.LinkExistsError as e:
			...
			if not for_reload:
				...
				ragapp.enqueue(
					"ragapp.model.delete_doc.delete_dynamic_links",
					doctype=doc.doctype,
					name=doc.name,
...
def add_to_deleted_document(doc):
	...
	if doc.doctype != "Deleted Document" and ragapp.flags.in_install != "ragapp":
		ragapp.get_doc(
...
def delete_from_table(doctype: str, name: str, ignore_doctypes: list[str], doc):
	if doctype != "DocType" and doctype == name:
		ragapp.db.delete("Singles", {"doctype": name})
	else:
		ragapp.db.delete(doctype, {"name": name})
	if doc:
		child_doctypes = [
			d.options for d in doc.meta.get_table_fields() if ragapp.get_meta(d.options).is_virtual == 0
		]
		...
		child_doctypes = ragapp.get_all(
			"DocField",
			fields="options",
			filters={"fieldtype": ["in", ragapp.model.table_fields], "parent": doctype},
			pluck="options",
		)

	child_doctypes_to_delete = set(child_doctypes) - set(ignore_doctypes)
	for child_doctype in child_doctypes_to_delete:
		ragapp.db.delete(child_doctype, {"parenttype": doctype, "parent": name})
...
def check_permission_and_not_submitted(doc):
	# permission
	if not doc.flags.ignore_permissions and ragapp.session.user != "Administrator":
		if doc.doctype == "DocType" and not doc.custom:
			ragapp.throw(_("Only the Administrator can delete a standard DocType."))
		else:
...
def check_if_doc_is_linked(doc, method="Delete"):
	...
	"""
	from ragapp.model.rename_doc import get_link_fields
	...
	if method == "Delete":
		ignored_doctypes.update(ragapp.get_hooks("ignore_links_on_delete"))
	...
	for lf in link_fields:
		...
		if issingle:
			if ragapp.db.get_single_value(link_dt, link_field) == doc.name:
		...
		for item in ragapp.db.get_values(link_dt, {link_field: doc.name}, fields, as_dict=True):
...
def check_if_doc_is_dynamically_linked(doc, method="Delete"):
	"""Raise `ragapp.LinkExistsError` if the document is dynamically linked"""
	for df in get_dynamic_link_map().get(doc.doctype, []):
		ignore_linked_doctypes = doc.get("ignore_linked_doctypes") or []

		if df.parent in ragapp.get_hooks("ignore_links_on_delete") or (
		...
		if meta.issingle:
			...
			for refdoc in ragapp.db.sql(
				...
				if (method == "Delete" and not DocStatus(refdoc.docstatus).is_cancelled()) or (
					...
					if reference_doctype in ragapp.get_hooks("ignore_links_on_delete") or (
...
def clear_timeline_references(link_doctype, link_name):
	ragapp.db.delete("Communication Link", {"link_doctype": link_doctype, "link_name": link_name})
...
def delete_controllers(doctype, module):
	...
	dir_path = os.path.join(module_path, "doctype", ragapp.scrub(doctype))
