...
import ragapp
from ragapp import _, is_whitelisted, msgprint
from ragapp.core.doctype.file.utils import relink_mismatched_files
from ragapp.core.doctype.server_script.server_script_utils import run_server_script_for_doc_event
from ragapp.desk.form.document_follow import follow_document
from ragapp.integrations.doctype.webhook import run_webhooks
from ragapp.model import optional_fields, table_fields
from ragapp.model.base_document import BaseDocument, get_controller
from ragapp.model.docstatus import DocStatus
from ragapp.model.naming import set_new_name, validate_name
from ragapp.model.utils import is_virtual_doctype, simple_singledispatch
from ragapp.model.workflow import set_workflow_state_on_action, validate_workflow
from ragapp.types import DF, DocRef
from ragapp.utils import compare, cstr, date_diff, file_lock, flt, get_table_name, now
from ragapp.utils.data import get_absolute_url, get_datetime, get_timedelta, getdate
from ragapp.utils.global_search import update_global_search
...
def get_doc(*args, **kwargs) -> "Document":
	"""Return a `ragapp.model.Document` object.
...
def read_only_guard(func):
	...
	def wrapper(self, *args, **kwargs):
		if getattr(ragapp.local, "read_only_depth", 0) > 0:
			...
			if getattr(ragapp.local, "read_only_context", None):
				error_msg += f" ({ragapp.local.read_only_context})"
			raise ragapp.DatabaseModificationError(error_msg)
		return func(self, *args, **kwargs)
...
def read_only_document(context=None):
	...
	if not hasattr(ragapp.local, "read_only_depth"):
		ragapp.local.read_only_depth = 0

	ragapp.local.read_only_depth += 1
	if context:
		ragapp.local.read_only_context = context

	try:
		yield
	finally:
		ragapp.local.read_only_depth -= 1
		if ragapp.local.read_only_depth == 0:
			if hasattr(ragapp.local, "read_only_context"):
				del ragapp.local.read_only_context
			del ragapp.local.read_only_depth
...
class Document(BaseDocument, DocRef):
	...
	def load_from_db(self) -> "Self":
		...
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			single_doc = ragapp.db.get_singles_dict(self.doctype, for_update=self.flags.for_update)
			if not single_doc:
				single_doc = ragapp.new_doc(self.doctype, as_dict=True)
			...
			if not d:
				ragapp.throw(
					_("{0} {1} not found").format(_(self.doctype), self.name), ragapp.DoesNotExistError
				)
	...
	def load_children_from_db(self):
		for df in self._get_table_fields():
			...
			# During ragapp is installed, the property "is_virtual" is not available in tabDocType, so
			# we need to filter those cases for the access to ragapp.db.get_value() as it would crash otherwise.
	...
	def check_permission(self, permtype="read", permlevel=None):
		"""Raise `ragapp.PermissionError` if not permitted"""
		if not self.has_permission(permtype):
			self.raise_no_permission_to(permtype)

	def has_permission(self, permtype="read", *, debug=False, user=None) -> bool:
		"""
		Call `ragapp.permissions.has_permission` if `ignore_permissions` flag isn't truthy
		...
		return ragapp.permissions.has_permission(self.doctype, permtype, self, debug=debug, user=user)

	def raise_no_permission_to(self, perm_type):
		"""Raise `ragapp.PermissionError`."""
		ragapp.flags.error_message = _(
			"You need the '{0}' permission on {1} {2} to perform this action."
		).format(
			_(perm_type),
			ragapp.bold(_(self.doctype)),
			self.name or "",
		)
		raise ragapp.PermissionError
	...
	def insert(
		...
		if not (ragapp.flags.in_migrate or ragapp.local.flags.in_install or ragapp.flags.in_setup_wizard):
			if ragapp.get_cached_value("User", ragapp.session.user, "follow_created_documents"):
				follow_document(self.doctype, self.name, ragapp.session.user)
		return self

	def check_if_locked(self):
		if self.creation and self.is_locked:
			raise ragapp.DocumentLockedError
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Self":
		...
		self.flags.ignore_version = ragapp.flags.in_test if ignore_version is None else ignore_version
	...
	def validate_amended_from(self):
		if ragapp.db.get_value(self.doctype, self.get("amended_from"), "docstatus") != 2:
			message = _(
				"{0} cannot be amended because it is not cancelled. Please cancel the document before creating an amendment."
			).format(ragapp.utils.get_link_to_form(self.doctype, self.get("amended_from")))
			ragapp.throw(message, title=_("Amendment Not Allowed"))
	...
	def set_new_name(self, force=False, set_name=None, set_child_names=True):
		"""Calls `ragapp.naming.set_new_name` for parent and child docs."""
	...
	def update_single(self, d):
		...
		ragapp.db.delete("Singles", {"doctype": self.doctype})
		...
		if self.doctype in ragapp.db.value_cache:
			del ragapp.db.value_cache[self.doctype]

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = ragapp.session.user
		...
		if self.is_new() and not (
			ragapp.flags.in_install or ragapp.flags.in_patch or ragapp.flags.in_migrate
		):
		...
		ragapp.flags.currently_saving.append((self.doctype, self.name))
	...
	def _validate_non_negative(self):
		def get_msg(df):
			if self.get("parentfield"):
				return "{} {} #{}: {} {}".format(
					...
					ragapp.bold(_(df.label, context=df.parent)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), ragapp.bold(_(df.label, context=df.parent))
				)
		...
		for df in self.meta.get(
			...
			if flt(self.get(df.fieldname)) < 0:
				msg = get_msg(df)
				ragapp.throw(msg, ragapp.NonNegativeError, title=_("Negative Value"))
				...
				if fail:
					ragapp.throw(
						_("Value cannot be changed for {0}").format(
							ragapp.bold(self.meta.get_label(field.fieldname))
						),
						exc=ragapp.CannotChangeConstantError,
					)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields():
			all_fields += ragapp.get_meta(table_field.options).fields or []
		...
		for table_field in self.meta.get_table_fields():
			for df in ragapp.get_meta(table_field.options).fields or []:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or ragapp.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = ragapp.get_meta(df.options).get_high_permlevel_fields()
			if high_permlevel_fields:
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = ragapp.get_meta(self.parenttype).permissions
		else:
	...
	def _set_defaults(self):
		...
		if self.is_new():
			new_doc = ragapp.new_doc(self.doctype, as_dict=True)
			self.update_if_missing(new_doc)

		# children
		for df in self.meta.get_table_fields():
			new_doc = ragapp.new_doc(df.options, parent_doc=self, parentfield=df.fieldname, as_dict=True)
	...
	def check_if_latest(self):
		...
		if cstr(previous.modified) != cstr(self._original_modified):
			ragapp.msgprint(
				...
				raise_exception=ragapp.TimestampMismatchError,
			)
	...
	def check_docstatus_transition(self, to_docstatus):
		...
		if to_docstatus == DocStatus.draft():
			if self.docstatus.is_draft():
				...
				raise ragapp.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise ragapp.ValidationError(_("Invalid docstatus"), self.docstatus)
			...
			raise ragapp.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		if ragapp.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise ragapp.MandatoryError(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			ragapp.throw(_("Could not find {0}").format(msg), ragapp.LinkValidationError)

		if cancelled_links:
			msg = ", ".join(each[2] for each in cancelled_links)
			ragapp.throw(_("Cannot link cancelled document: {0}").format(msg), ragapp.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			method == "onload"
			or (ragapp.flags.in_import and ragapp.flags.mute_emails)
			or ragapp.flags.in_patch
		...
		from ragapp.email.doctype.notification.notification import evaluate_alert
		...
		if self.flags.notifications is None:
			...
			self.flags.notifications = ragapp.cache.hget("notifications", self.doctype, _get_notifications)
	...
	def _rename(self, name: str, merge: bool = False, force: bool = False, validate_rename: bool = True):
		"""Rename the document. Triggers ragapp.rename_doc, then reloads."""
	...
	def discard(self):
		...
		if not self.docstatus == DocStatus.draft():
			raise ragapp.ValidationError(_("Only draft documents can be discarded"), self.docstatus)
	...
	def load_doc_before_save(self, *, raise_exception: bool = False):
		...
		try:
			self._doc_before_save = ragapp.get_doc(self.doctype, self.name, for_update=True)
	...
	def run_post_save_methods(self):
		...
		if (self.doctype, self.name) in ragapp.flags.currently_saving:
			ragapp.flags.currently_saving.remove((self.doctype, self.name))

	def clear_cache(self):
		ragapp.clear_document_cache(self.doctype, self.name)
	...
	def reset_seen(self):
		...
		if (
			...
			ragapp.db.set_value(
				self.doctype, self.name, "_seen", json.dumps([ragapp.session.user]), update_modified=False
			)

	def notify_update(self):
		"""Publish realtime that the current document is modified"""
		if ragapp.flags.in_patch:
			return
		...
		if not self.meta.get("read_only") and not self.meta.get("issingle") and not self.meta.get("istable"):
			data = {"doctype": self.doctype, "name": self.name, "user": ragapp.session.user}
			ragapp.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		:param commit: default False. run ragapp.db.commit()
		"""
		...
		if update_modified and (self.doctype, self.name) not in ragapp.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", ragapp.session.user)
	...
	def db_get(self, fieldname):
		...
		return ragapp.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from ragapp.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def hook(f):
		...
		def compose(fn, *hooks):
			def runner(self, method, *args, **kwargs):
				...
				for f in hooks:
					try:
						ragapp.db._disable_transaction_control += 1
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			ragapp.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or ragapp.EmptyTableError
			)
	...
	def round_floats_in(self, doc, fieldnames=None):
		...
		rounding_method = ragapp.get_system_settings("rounding_method")
		...
		return ragapp.get_doc(
			{
				...
				"comment_email": comment_email or ragapp.session.user,
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen and not ragapp.flags.read_only and not self.meta.issingle:
			_seen = self.get("_seen") or []
			_seen = ragapp.parse_json(_seen)
			...
			if user not in _seen:
				...
				ragapp.local.flags.commit = True
	...
	def add_viewed(self, user=None, force=False, unique_views=False):
		...
		if unique_views and ragapp.db.exists(
	...
	def get_document_share_key(self, expires_on=None, no_expiry=False):
		...
		if existing_key:
			doc = ragapp.get_doc("Document Share Key", existing_key)
		else:
			doc = ragapp.new_doc("Document Share Key")
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", ragapp._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", ragapp._dict())
	...
	def queue_action(self, action, **kwargs):
		...
		return enqueue(
			"ragapp.model.document.execute_action",
	...
	def lock(self, timeout=None):
		...
		ragapp.local.locked_documents.append(self)

	def unlock(self):
		"""Delete the lock file for this document"""
		file_lock.delete_lock(self.get_signature())
		if self in ragapp.local.locked_documents:
			ragapp.local.locked_documents.remove(self)
		...
		if date_diff(to_date, from_date) < 0:
			...
			if self.meta.istable:
				table_row = _("{0} row #{1}: ").format(
					_(ragapp.unscrub(self.parentfield)),
					self.idx,
				)
			...
			ragapp.throw(
				...
				+ _("{0} must be after {1}").format(
					ragapp.bold(_(self.meta.get_label(to_date_field))),
					ragapp.bold(_(self.meta.get_label(from_date_field))),
				),
				ragapp.exceptions.InvalidDates,
			)

	def get_assigned_users(self):
		assigned_users = ragapp.get_all(
			"ToDo",
			fields=["allocated_to"],
			filters={
	...
	def add_tag(self, tag):
		...
		from ragapp.desk.doctype.tag.tag import DocTags
	...
	def deferred_insert(self) -> None:
		...
		from ragapp.deferred_insert import deferred_insert
...
def execute_action(__doctype, __name, __action, **kwargs):
	...
	doc = ragapp.get_doc(__doctype, __name)
	doc.unlock()
	try:
		...
		# add a comment (?)
		if ragapp.message_log:
			msg = ragapp.message_log[-1].get("message")
		else:
			msg = "<pre><code>" + ragapp.get_traceback() + "</pre></code>"
...
def bulk_insert(
	...
	doctype_meta = ragapp.get_meta(doctype)
	...
	for child_table in doctype_meta.get_table_fields():
		valid_column_map[child_table.options] = ragapp.get_meta(child_table.options).get_valid_columns()
...
def _document_values_generator(
	...
	for doc in documents:
		...
		doc.owner = doc.modified_by = ragapp.session.user
...
def unlock_document(doctype: str, name: str):
	ragapp.get_doc(doctype, name).unlock()
	ragapp.msgprint(ragapp._("Document Unlocked"), alert=True)
