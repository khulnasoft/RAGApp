...
def get_dynamic_link_map(for_delete=False):
	...
	"""
	if getattr(ragapp.local, "dynamic_link_map", None) is None or ragapp.flags.in_test:
		# Build from scratch
		dynamic_link_map = {}
		...
		ragapp.local.dynamic_link_map = dynamic_link_map
	return ragapp.local.dynamic_link_map
...
def get_dynamic_links():
	...
	for query in dynamic_link_queries:
		df += ragapp.db.sql(query, as_dict=True)
	return df
...
def fetch_distinct_link_doctypes(doctype: str, fieldname: str):
	...
	doctypes = ragapp.cache.get_value(key)

	if doctypes is None:
		doctypes = ragapp.db.sql(f"""select distinct `{fieldname}` from `tab{doctype}`""", pluck=True)
		ragapp.cache.set_value(key, doctypes, expires_in_sec=12 * 60 * 60)
...
def invalidate_distinct_link_doctypes(doctype: str, fieldname: str, linked_doctype: str):
	...
	if linked_doctype not in doctypes:
		...
		ragapp.db.after_commit.add(lambda: ragapp.cache.delete_value(key))
