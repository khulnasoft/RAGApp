...
from ragapp.model.base_document import (
...
from ragapp.types import DocRef
from ragapp.utils import cast, cint, cstr
...
def get_meta(doctype: str | dict | DocRef, cached=True) -> "_Meta":
	...
	if cached and (
		...
		if meta := ragapp.client_cache.get_value(key):
			return meta

	meta = Meta(doctype)
	key = f"doctype_meta::{meta.name}"
	ragapp.client_cache.set_value(key, meta)
	return meta
...
def clear_meta_cache(doctype: str = "*"):
	...
	if doctype == "*":
		ragapp.cache.delete_keys(key)
	else:
		ragapp.client_cache.delete_value(key)
...
def get_table_columns(doctype):
	return ragapp.db.get_table_columns(doctype)


def load_doctype_from_file(doctype):
	fname = ragapp.scrub(doctype)
	with open(ragapp.get_app_path("ragapp", "core", "doctype", fname, fname + ".json")) as f:
...
class Meta(Document):
	...
	standard_set_once_fields = (
		ragapp._dict(fieldname="creation", fieldtype="Datetime"),
		ragapp._dict(fieldname="owner", fieldtype="Data"),
	)
	...
	def get_global_search_fields(self):
		...
		if getattr(self, "show_name_in_global_search", None):
			fields.append(ragapp._dict(fieldtype="Data", fieldname="name", label="Name"))
	...
	def _valid_columns(self):
		table_exists = ragapp.db.table_exists(self.name)
	...
	def _valid_fields(self):
		if (ragapp.flags.in_install or ragapp.flags.in_migrate) and self.name in self.special_doctypes:
	...
	def add_custom_fields(self):
		if not ragapp.db.table_exists("Custom Field"):
			return

		custom_fields = ragapp.db.get_values(
			"Custom Field",
	...
	def apply_property_setters(self):
		...
		"""
		if not ragapp.db.table_exists("Property Setter"):
			return

		property_setters = ragapp.db.get_values(
			"Property Setter",
	...
	def get_fields_to_check_permissions(self, user_permission_doctypes):
		...
		if self.name in user_permission_doctypes:
			fields.append(ragapp._dict({"label": "Name", "fieldname": "name", "options": self.name}))
	...
	def get_permitted_fieldnames(
		...
		*,
		...
		if not permission_type:
			permission_type = "select" if ragapp.only_has_select_perm(self.name, user=user) else "read"
		...
		if 0 not in permlevel_access and permission_type in ("read", "select"):
			if ragapp.share.get_shared(self.name, user, rights=[permission_type], limit=1):
	...
	def get_permissions(self, parenttype=None):
		if self.istable and parenttype:
			...
			permissions = ragapp.get_meta(parenttype).permissions
		else:
	...
	def get_dashboard_data(self):
		...
		implemented in other Ragapp applications via hooks.
		"""
		data = ragapp._dict()
		if not self.custom:
			try:
				...
				if hasattr(module, "get_data"):
					data = ragapp._dict(module.get_data())
			except ImportError:
				pass
		...
		if not self.custom:
			for hook in ragapp.get_hooks("override_doctype_dashboards", {}).get(self.name, []):
				data = ragapp._dict(ragapp.get_attr(hook)(data=data))
	...
	def get_web_template(self, suffix=""):
		...
		module_name = ragapp.scrub(self.module)
		doctype = ragapp.scrub(self.name)
		template_path = ragapp.get_module_path(
...
def is_single(doctype):
	try:
		return ragapp.db.get_value("DocType", doctype, "issingle")
	except IndexError:
		raise Exception("Cannot determine whether {} is single".format(doctype))


def get_parent_dt(dt):
	if not ragapp.is_table(dt):
		return ""

	return (
		ragapp.db.get_value(
			"DocField",
			{"fieldtype": ("in", ragapp.model.table_fields), "options": dt},
			"parent",
		)
		or ""
	)
...
def set_fieldname(field_id, fieldname):
	ragapp.db.set_value("DocField", field_id, "fieldname", fieldname)
...
def get_field_currency(df, doc=None):
	...
	if not getattr(ragapp.local, "field_currency", None):
		ragapp.local.field_currency = ragapp._dict()

	if not (
		ragapp.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname)
		or (
			doc.get("parent")
			and ragapp.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
		)
	):
		...
		if ":" in cstr(df.get("options")):
			...
			if len(split_opts) == 3 and doc.get(split_opts[1]):
				currency = ragapp.get_cached_value(split_opts[0], doc.get(split_opts[1]), split_opts[2])
		else:
			...
			if doc.get("parenttype"):
				if currency:
					...
					if ragapp.get_meta(doc.parenttype).has_field(df.get("options")):
						# only get_value if parent has currency field
						currency = ragapp.db.get_value(doc.parenttype, doc.parent, df.get("options"))

		if currency:
			ragapp.local.field_currency.setdefault((doc.doctype, ref_docname), ragapp._dict()).setdefault(
				df.fieldname, currency
			)

	return ragapp.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or (
		doc.get("parent") and ragapp.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
	)


def get_field_precision(df, doc=None, currency=None):
	"""get precision based on DocField options and fieldvalue in doc"""
	from ragapp.locale import get_number_format

	if df.precision:
		precision = cint(df.precision)

	elif df.fieldtype == "Currency":
		precision = cint(ragapp.db.get_default("currency_precision"))
		if not precision:
			number_format = get_number_format()
			precision = number_format.precision
	else:
		precision = cint(ragapp.db.get_default("float_precision")) or 3
...
def get_default_df(fieldname):
	if fieldname in (default_fields + child_table_fields):
		if fieldname in ("creation", "modified"):
			return ragapp._dict(fieldname=fieldname, fieldtype="Datetime")

		elif fieldname in ("idx", "docstatus"):
			return ragapp._dict(fieldname=fieldname, fieldtype="Int")

		elif fieldname in ("owner", "modified_by"):
			return ragapp._dict(fieldname=fieldname, fieldtype="Link", options="User")

		return ragapp._dict(fieldname=fieldname, fieldtype="Data")
...
def trim_tables(doctype=None, dry_run=False, quiet=False):
	...
	for doctype in ragapp.get_all("DocType", filters=filters, pluck="name"):
		try:
...
def trim_table(doctype, dry_run=True):
	ragapp.cache.hdel("table_columns", f"tab{doctype}")
	ignore_fields = default_fields + optional_fields + child_table_fields
	columns = ragapp.db.get_table_columns(doctype)
	fields = ragapp.get_meta(doctype, cached=False).get_fieldnames_with_value()
	...
	if columns_to_remove and not dry_run:
		...
		ragapp.db.sql_ddl(f"ALTER TABLE `tab{doctype}` {columns_to_remove}")
