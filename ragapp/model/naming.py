...
from ragapp.model import log_types
from ragapp.query_builder import DocType
from ragapp.utils import cint, cstr, now_datetime
...
class InvalidNamingSeriesError(ragapp.ValidationError):
	pass


class InvalidUUIDValue(ragapp.ValidationError):
	pass
...
class NamingSeries:
	...
	def validate(self):
		if "." not in self.series:
			ragapp.throw(
				_("Invalid naming series {}: dot (.) missing").format(ragapp.bold(self.series)),
		...
		if not NAMING_SERIES_PATTERN.match(self.series):
			ragapp.throw(
				...
				).format(ragapp.bold(self.series)),
				exc=InvalidNamingSeriesError,
			)
	...
	def get_prefix(self) -> str:
		...
		if prefix is None:
			ragapp.throw(_("Invalid Naming Series: {}").format(self.series))
	...
	def update_counter(self, new_count: int) -> None:
		...
		Series = ragapp.qb.DocType("Series")
		prefix = self.get_prefix()

		# Initialize if not present in DB
		if ragapp.db.get_value("Series", prefix, "name", order_by="name") is None:
			ragapp.qb.into(Series).insert(prefix, 0).columns("name", "current").run()

		(ragapp.qb.update(Series).set(Series.current, cint(new_count)).where(Series.name == prefix)).run()

	def get_current_value(self) -> int:
		prefix = self.get_prefix()
		return cint(ragapp.db.get_value("Series", prefix, "current", order_by="name"))
...
def set_new_name(doc):
	...
	if autoname.lower() not in ("prompt", "uuid") and not ragapp.flags.in_import:
		doc.name = None

	if is_autoincremented(doc.doctype, meta):
		doc.name = ragapp.db.get_next_sequence_val(doc.doctype)
		return
	...
	if meta.autoname == "UUID":
		if not doc.name:
			...
			try:
				...
				ragapp.throw(_("Invalid value specified for UUID: {}").format(doc.name), InvalidUUIDValue)
		return
...
def set_name_from_naming_options(autoname, doc):
	...
	if _autoname.startswith("field:"):
		...
		if not doc.name:
			...
			ragapp.throw(_("{0} is required").format(doc.meta.get_label(fieldname)))
...
def set_naming_from_document_naming_rule(doc):
	...
	"""
	from ragapp.model.base_document import DOCTYPES_FOR_DOCTYPE
	...
	document_naming_rules = ragapp.cache_manager.get_doctype_map(
	...
	for d in document_naming_rules:
		ragapp.get_cached_doc("Document Naming Rule", d.name).apply(doc)
		if doc.name:
			break
...
def set_name_by_naming_series(doc):
	...
	if not doc.naming_series:
		ragapp.throw(ragapp._("Naming Series mandatory"))
...
def _generate_random_string(length=10):
	"""Better version of ragapp.generate_hash for naming.
...
def parse_naming_series(
	...
	for e in parts:
		...
		if e.startswith("#"):
			...
			part = ragapp.get_attr(method[0])(doc, e)
		else:
			part = e
...
def has_custom_parser(e):
	...
	return ragapp.get_hooks("naming_series_variables", {}).get(e)
...
def getseries(key, digits):
	# series created ?
	# Using ragapp.qb as ragapp.get_values does not allow order_by=None
	series = DocType("Series")
	current = (ragapp.qb.from_(series).where(series.name == key).for_update().select("current")).run()

	if current and current[0][0] is not None:
		current = current[0][0]
		# yes, update it
		ragapp.db.sql("UPDATE `tabSeries` SET `current` = `current` + 1 WHERE `name`=%s", (key,))
		current = cint(current) + 1
	else:
		# no, create it
		ragapp.db.sql("INSERT INTO `tabSeries` (`name`, `current`) VALUES (%s, 1)", (key,))
		current = 1
...
def revert_series_if_last(key, name, doc=None):
	...
	*Example:*
	...
	series = DocType("Series")
	current = (ragapp.qb.from_(series).where(series.name == prefix).for_update().select("current")).run()

	if current and current[0][0] == count:
		ragapp.db.sql("UPDATE `tabSeries` SET `current` = `current` - 1 WHERE `name`=%s", prefix)


def get_default_naming_series(doctype: str) -> str | None:
	"""get default value for `naming_series` property"""
	naming_series_options = ragapp.get_meta(doctype).get_naming_series_options()
...
def validate_name(doctype: str, name: int | str):
	if not name:
		ragapp.throw(_("No Name Specified for {0}").format(doctype))
	...
	if isinstance(name, int):
		if is_autoincremented(doctype):
			...
			ragapp.db.set_next_sequence_val(doctype, name, is_val_used=True)
			return name

		ragapp.throw(_("Invalid name type (integer) for varchar name column"), ragapp.NameError)

	if name.startswith("New " + doctype):
		ragapp.throw(
			_("There were some errors setting the name, please contact the administrator"), ragapp.NameError
		)
	name = name.strip()

	if not ragapp.get_meta(doctype).get("issingle") and (doctype == name) and (name != "DocType"):
		ragapp.throw(_("Name of {0} cannot be {1}").format(doctype, name), ragapp.NameError)

	special_characters = "<>"
	if re.findall(f"[{special_characters}]+", name):
		message = ", ".join(f"'{c}'" for c in special_characters)
		ragapp.throw(_("Name cannot contain special characters like {0}").format(message), ragapp.NameError)
...
def append_number_if_name_exists(doctype, value, fieldname="name", separator="-", filters=None):
	...
	exists = ragapp.db.exists(doctype, filters)
	...
	if exists:
		last = ragapp.db.sql(
			...
			WHERE `{fieldname}` {ragapp.db.REGEX_CHARACTER} %s
...
def _set_amended_name(doc):
	...
	if ragapp.db.get_value(doc.doctype, doc.amended_from, "amended_from"):
...
def _prompt_autoname(autoname, doc):
	...
	if not doc.name:
		ragapp.throw(_("Please set the document name"))
