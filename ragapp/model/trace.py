"""
Traced Fields for Ragapp

This module provides utilities for creating traced fields in Ragapp documents,
...
- Seamlessly integrate with Ragapp's document model

Example of standard usage:
		from ragapp.model.trace import TracedDocument, traced_field
...
See ragapp.tests.classes.context_managers for a context manager built into test classes.
"""
...
class TracedValue:
	"""
	A descriptor class for creating traced fields in Ragapp documents.

	This class allows for monitoring and validating changes to specific fields
	in a Ragapp document. It can enforce forbidden values and apply custom
	validation logic.
	...
	def __set__(self, obj, value):
		...
		if value in self.forbidden_values:
			if ragapp.flags.in_test:
				ragapp.throw(f"{self.field_name} cannot be set to {value}", AssertionError)
			else:
				ragapp.throw(f"{self.field_name} cannot be set to {value}")
		...
		if self.custom_validation:
			try:
				...
				if ragapp.flags.in_test:
					ragapp.throw(str(e), AssertionError)
				else:
					ragapp.throw(str(e))
...
class TracedDocument(Document):
	"""
	A base class for Ragapp documents with traced fields.

	This class extends Ragapp's Document class to provide support for
	traced fields created with TracedValue.

	Attributes:
	        Inherits all attributes from ragapp.model.document.Document
	"""
...
def traced_field(*args, **kwargs):
	...
	This function simplifies the creation of traced fields in Ragapp documents.
...
from ragapp.deprecation_dumpster import model_trace_traced_field_context as traced_field_context
