...
def set_default(doc, key):
	...
	if not doc.is_default:
		ragapp.db.set(doc, "is_default", 1)
...
def set_field_property(filters, key, value):
	...
	docs = [
		ragapp.get_doc("DocType", d.parent)
		for d in ragapp.get_all("DocField", fields=["parent"], filters=filters)
	]
...
class InvalidIncludePath(ragapp.ValidationError):
	pass
	...
	for _ in range(5):
		if "{% include" in content:
			...
			for path in paths:
				...
				with open(ragapp.get_app_path(app, app_path), encoding="utf-8") as f:
					include = f.read()
...
def is_virtual_doctype(doctype: str):
	if ragapp.flags.in_install or ragapp.flags.in_migrate:
		if ragapp.db.has_column("DocType", "is_virtual"):
			return ragapp.db.get_value("DocType", doctype, "is_virtual")
	else:
		return getattr(ragapp.get_meta(doctype), "is_virtual", False)
...
def is_single_doctype(doctype: str) -> bool:
	...
	if ragapp.flags.in_install or ragapp.flags.in_migrate:
		...
		return getattr(ragapp.get_meta(doctype), "issingle", False)
