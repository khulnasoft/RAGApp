...
class VirtualDoctype(Protocol):
	...
	- DocType controller has to inherit from `ragapp.model.document.Document` class
	...
	@staticmethod
	def get_list(**kwargs) -> list[ragapp._dict]:
...
def validate_controller(doctype: str) -> None:
	try:
		...
		ragapp.msgprint(_("Failed to import virtual doctype {}, is controller file present?").format(doctype))
		return
		...
		if not isinstance(method, staticmethod):
			ragapp.msgprint(
				_("Virtual DocType {} requires a static method called {} found {}").format(
					ragapp.bold(doctype), ragapp.bold(m), ragapp.bold(_as_str(method))
				),
