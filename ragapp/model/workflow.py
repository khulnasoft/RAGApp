...
if TYPE_CHECKING:
	...
	from ragapp.workflow.doctype.workflow.workflow import Workflow


class WorkflowStateError(ragapp.ValidationError):
	pass


class WorkflowTransitionError(ragapp.ValidationError):
	pass


class WorkflowPermissionError(ragapp.ValidationError):
	pass


def get_workflow_name(doctype):
	workflow_name = ragapp.cache.hget("workflow", doctype)
	if workflow_name is None:
		workflow_name = ragapp.db.get_value("Workflow", {"document_type": doctype, "is_active": 1}, "name")
		ragapp.cache.hset("workflow", doctype, workflow_name or "")
...
def get_transitions(
	...
	if not isinstance(doc, Document):
		doc = ragapp.get_doc(ragapp.parse_json(doc))
		doc.load_from_db()
	...
	if not current_state:
		if raise_exception:
			...
			ragapp.throw(_("Workflow State not set"), WorkflowStateError)
...
def get_workflow_safe_globals():
	# access to ragapp.db.get_value, ragapp.db.get_list, and date time utils.
	return dict(
		ragapp=ragapp._dict(
			db=ragapp._dict(get_value=ragapp.db.get_value, get_list=ragapp.db.get_list),
			session=ragapp.session,
			utils=ragapp._dict(
				now_datetime=ragapp.utils.now_datetime,
				add_to_date=ragapp.utils.add_to_date,
				get_datetime=ragapp.utils.get_datetime,
...
def is_transition_condition_satisfied(transition, doc) -> bool:
	if not transition.condition:
		...
		return ragapp.safe_eval(transition.condition, get_workflow_safe_globals(), dict(doc=doc.as_dict()))
...
def apply_workflow(doc, action):
	...
	if not transition:
		ragapp.throw(_("Not a valid Workflow Action"), WorkflowTransitionError)

	if not has_approval_access(user, doc, transition):
		ragapp.throw(_("Self approval is not allowed"))
	...
	if doc.docstatus.is_draft() and new_docstatus == DocStatus.draft():
		...
		ragapp.throw(_("Illegal Document Status for {0}").format(next_state.state))
...
def validate_workflow(doc):
	...
	if current_state != next_state:
		bold_current = ragapp.bold(current_state)
		bold_next = ragapp.bold(next_state)
...
def get_workflow(doctype) -> "Workflow":
	return ragapp.get_cached_doc("Workflow", get_workflow_name(doctype))
...
def get_workflow_field_value(workflow_name, field):
	return ragapp.get_cached_value("Workflow", workflow_name, field)
...
def bulk_workflow_approval(docnames, doctype, action):
	...
	if len(docnames) < 20:
		...
		ragapp.msgprint(_("Bulk {0} is enqueued in background.").format(action), alert=True)
		ragapp.enqueue(
		...
		ragapp.throw(_("Bulk approval only support up to 500 documents."), title=_("Too Many Documents"))
...
def _bulk_workflow_action(docnames, doctype, action):
	...
	for idx, docname in enumerate(docnames, 1):
		...
		try:
			...
			apply_workflow(ragapp.get_doc(doctype, docname), action)
			ragapp.db.commit()
		except Exception as e:
			if not ragapp.message_log:
			...
			if not message_dict:
				if ragapp.message_log:
					messages = ragapp.get_message_log()
					for message in messages:
						ragapp.message_log.pop()
...
def print_workflow_log(messages, title, doctype, indicator):
	if messages.keys():
		...
		for doc in messages.keys():
			if len(messages[doc]):
				html = f"<details><summary>{ragapp.utils.get_link_to_form(doctype, doc)}</summary>"
	...
	try:
		for i, doc in enumerate(docs, 1):
			...
			actions = [
				...
				if has_approval_access(ragapp.session.user, doc, t)
			]
			if not actions:
				return []
...
def set_workflow_state_on_action(doc, workflow_name, action):
	workflow = ragapp.get_doc("Workflow", workflow_name)
