...
def run_all(skip_failing: bool = False, patch_type: PatchType | None = None) -> None:
	...
	executed = set(ragapp.get_all("Patch Log", filters={"skipped": 0}, fields="patch", pluck="patch"))
	...
	for patch in ragapp.flags.final_patches:
...
def get_all_patches(patch_type: PatchType | None = None) -> list[str]:
	if patch_type and not isinstance(patch_type, PatchType):
		ragapp.throw(f"Unsupported patch type specified: {patch_type}")
...
def get_patches_from_app(app: str, patch_type: PatchType | None = None) -> list[str]:
	...
	"""
	patches_file = ragapp.get_app_path(app, "patches.txt")
	...
	try:
		...
		if not patch_type or patch_type == PatchType.pre_model_sync:
			return ragapp.get_file_items(patches_file)
...
def parse_as_configfile(patches_file: str, patch_type: PatchType | None = None) -> list[str]:
	...
	if patch_type.value in parser.sections():
		...
		ragapp.throw(ragapp._("Patch type {} not found in patches.txt").format(patch_type))


def reload_doc(args):
	import ragapp.modules

	run_single(method=ragapp.modules.reload_doc, methodargs=args)


def run_single(patchmodule=None, method=None, methodargs=None, force=False):
	from ragapp import conf
...
def execute_patch(patchmodule: str, method=None, methodargs=None):
	...
	print(
		f"Executing {patchmodule or methodargs} in {ragapp.local.site} ({ragapp.db.cur_db_name}){docstring}"
	)
	...
	try:
		if patchmodule:
			if patchmodule.startswith("finally:"):
				...
				ragapp.flags.final_patches.append(patchmodule)
			else:
				if has_patch_file:
					_patch()
				else:
...
def update_patch_log(patchmodule, skipped=False):
	...
	patch = ragapp.get_doc({"doctype": "Patch Log", "patch": patchmodule})
...
def executed(patchmodule):
	...
	return ragapp.db.get_value("Patch Log", {"patch": patchmodule, "skipped": 0})


def _patch_mode(enable):
	"""stop/start execution till patch is run"""
	ragapp.local.flags.in_patch = enable
	ragapp.db.commit()
