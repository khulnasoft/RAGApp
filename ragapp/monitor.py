...
def start(transaction_type="request", method=None, kwargs=None):
	if ragapp.conf.monitor:
		ragapp.local.monitor = Monitor(transaction_type, method, kwargs)


def stop(response=None):
	if hasattr(ragapp.local, "monitor"):
		ragapp.local.monitor.dump(response)
...
def add_data_to_monitor(**kwargs) -> None:
	...
	if hasattr(ragapp.local, "monitor"):
		ragapp.local.monitor.add_custom_data(**kwargs)


def get_trace_id() -> str | None:
	"""Get unique ID for current transaction."""
	if monitor := getattr(ragapp.local, "monitor", None):
		return monitor.data.uuid


def log_file():
	return os.path.join(ragapp.utils.get_cli_path(), "logs", "monitor.json.log")
...
class Monitor:
	...
	def collect_request_meta(self):
		self.data.request = ragapp._dict(
			{
				"ip": ragapp.local.request_ip,
				"method": ragapp.request.method,
				"path": ragapp.request.path,
			}
		)

		if request_id := ragapp.request.headers.get("X-Ragapp-Request-Id"):
			self.data.uuid = request_id

	def collect_job_meta(self, method, kwargs):
		self.data.job = ragapp._dict({"method": method, "scheduled": False, "wait": 0})
		...
		try:
			...
			if self.data.transaction_type == "request":
				...
				if hasattr(ragapp.local, "rate_limiter"):
					limiter = ragapp.local.rate_limiter
	...
	def store(self):
		...
		length = ragapp.cache.rpush(MONITOR_REDIS_KEY, serialized)
		if cint(length) > MONITOR_MAX_ENTRIES:
			ragapp.cache.ltrim(MONITOR_REDIS_KEY, 1, -1)
...
def flush():
	try:
		...
		if logs:
			logs = list(map(ragapp.safe_decode, logs))
			with open(log_file(), "a") as f:
				f.write("\n".join(logs))
				f.write("\n")
			# Remove fetched entries from cache
			ragapp.cache.ltrim(MONITOR_REDIS_KEY, len(logs) - 1, -1)
	except Exception:
		traceback.print_exc()
