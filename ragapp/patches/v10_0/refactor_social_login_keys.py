...
def execute():
	...
	ragapp.reload_doc("integrations", "doctype", "social_login_key", force=True)

	if not ragapp.db.exists("DocType", "Social Login Keys"):
		return

	social_login_keys = ragapp.get_doc("Social Login Keys", "Social Login Keys")
	if social_login_keys.get("facebook_client_id") or social_login_keys.get("facebook_client_secret"):
		facebook_login_key = ragapp.new_doc("Social Login Key")
	...
	if social_login_keys.get("ragapp_server_url"):
		ragapp_login_key = ragapp.new_doc("Social Login Key")
		ragapp_login_key.get_social_login_provider("Ragapp", initialize=True)
		ragapp_login_key.social_login_provider = "Ragapp"
		ragapp_login_key.base_url = social_login_keys.get("ragapp_server_url")
		ragapp_login_key.client_id = social_login_keys.get("ragapp_client_id")
		ragapp_login_key.client_secret = social_login_keys.get("ragapp_client_secret")
		if not (ragapp_login_key.client_secret and ragapp_login_key.client_id and ragapp_login_key.base_url):
			ragapp_login_key.enable_social_login = 0
		ragapp_login_key.save()

	if social_login_keys.get("github_client_id") or social_login_keys.get("github_client_secret"):
		github_login_key = ragapp.new_doc("Social Login Key")
	...
	if social_login_keys.get("google_client_id") or social_login_keys.get("google_client_secret"):
		google_login_key = ragapp.new_doc("Social Login Key")
	...
	ragapp.delete_doc("DocType", "Social Login Keys")


def run_patch():
	ragapp.reload_doc("core", "doctype", "user", force=True)
	ragapp.reload_doc("core", "doctype", "user_social_login", force=True)

	users = ragapp.get_all("User", fields=["*"], filters={"name": ("not in", ["Administrator", "Guest"])})

	for user in users:
		idx = 0
		if user.ragapp_userid:
			insert_user_social_login(user.name, user.modified_by, "ragapp", idx, userid=user.ragapp_userid)
			idx += 1
...
def insert_user_social_login(user, modified_by, provider, idx, userid=None, username=None):
	...
	creation_time = ragapp.utils.get_datetime_str(ragapp.utils.get_datetime())
	values = [
		ragapp.generate_hash(length=10),
		creation_time,
		creation_time,
		user,
		modified_by,
	...
	query = """INSERT INTO `tabUser Social Login` (`{source_cols}`)
		VALUES ({values})
	""".format(source_cols="`, `".join(source_cols), values=", ".join([ragapp.db.escape(d) for d in values]))
