...
def execute():
	...
	reports = ragapp.get_all("Report", {"report_type": "Report Builder"})

	for report_name in reports:
		settings = ragapp.db.get_value("Report", report_name, "json")
		if not settings:
			continue

		settings = ragapp._dict(json.loads(settings))
		...
		ragapp.db.set_value("Report", report_name, "json", json.dumps(settings))
