...
def execute():
	ragapp.reload_doctype("Event")
	# Rename "Cancel" to "Cancelled"
	ragapp.db.sql("""UPDATE tabEvent set event_type='Cancelled' where event_type='Cancel'""")
	...
	for event in events:
		if event.ref_type and event.ref_name:
			try:
				e = ragapp.get_doc("Event", event.name)
				e.append(
					"event_participants",
				...
				ragapp.log_error(ragapp.get_traceback())
