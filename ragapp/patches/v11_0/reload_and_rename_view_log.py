...
def execute():
	if ragapp.db.table_exists("View log"):
		...
		ragapp.db.sql("CREATE TABLE `ViewLogTemp` AS SELECT * FROM `tabView log`")

		# deleting old View log table
		ragapp.db.sql("DROP table `tabView log`")
		ragapp.delete_doc("DocType", "View log")

		# reloading view log doctype to create `tabView Log` table
		ragapp.reload_doc("core", "doctype", "view_log")

		# Move the data to newly created `tabView Log` table
		ragapp.db.sql("INSERT INTO `tabView Log` SELECT * FROM `ViewLogTemp`")
		ragapp.db.commit()

		# Delete temporary table
		ragapp.db.sql("DROP table `ViewLogTemp`")
	else:
		ragapp.reload_doc("core", "doctype", "view_log")
