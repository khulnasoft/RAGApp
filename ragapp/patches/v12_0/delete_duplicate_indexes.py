...
def execute():
	...
	all_tables = ragapp.db.get_tables()
	final_deletion_map = ragapp._dict()

	for table in all_tables:
		indexes_to_keep_map = ragapp._dict()
		indexes_to_delete = []
		index_info = ragapp.db.sql(
		...
		for index in index_list:
			try:
				if is_clustered_index(table_name, index):
					continue
				ragapp.db.sql_ddl(f"ALTER TABLE `{table_name}` DROP INDEX `{index}`")
