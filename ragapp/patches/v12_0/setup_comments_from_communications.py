...
def execute():
	ragapp.reload_doctype("Comment")

	if ragapp.db.count("Communication", filters=dict(communication_type="Comment")) > 20000:
		ragapp.db.auto_commit_on_many_writes = True

	for comment in ragapp.get_all("Communication", fields=["*"], filters=dict(communication_type="Comment")):
		new_comment = ragapp.new_doc("Comment")
	...
	# clean up
	ragapp.db.delete("Communication", {"communication_type": "Comment"})
