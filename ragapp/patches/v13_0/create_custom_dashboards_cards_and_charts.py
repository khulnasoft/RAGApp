...
from ragapp.utils.dashboard import get_dashboards_with_link


def execute():
	if (
		not ragapp.db.table_exists("Dashboard Chart")
		or not ragapp.db.table_exists("Number Card")
		or not ragapp.db.table_exists("Dashboard")
	):
		return

	ragapp.reload_doc("desk", "doctype", "dashboard_chart")
	ragapp.reload_doc("desk", "doctype", "number_card")
	ragapp.reload_doc("desk", "doctype", "dashboard")
...
def get_modified_docs(doctype):
	return ragapp.get_all(doctype, filters={"owner": "Administrator", "modified_by": ["!=", "Administrator"]})


def rename_modified_doc(docname, doctype):
	new_name = docname + " Custom"
	try:
		ragapp.rename_doc(doctype, docname, new_name)
