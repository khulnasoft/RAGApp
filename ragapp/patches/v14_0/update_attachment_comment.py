...
def execute():
	...
	for name, content in ragapp.get_all(
		...
		ragapp.db.set_value("Comment", name, "content", content, update_modified=False)
	...
	for name, content in ragapp.get_all(
		...
		if content and content.startswith("Removed "):
			ragapp.db.set_value("Comment", name, "content", content[8:], update_modified=False)
