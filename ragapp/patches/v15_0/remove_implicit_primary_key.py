import ragapp
from ragapp.model.naming import is_autoincremented
...
def execute():
	...
	for doctype in possible_log_types:
		if (
			ragapp.db.exists("DocType", doctype)
			and _is_implicit_int_pk(doctype)
			and not is_autoincremented(doctype)
		):
			ragapp.db.change_column_type(
				doctype,
				"name",
				type=f"varchar({ragapp.db.VARCHAR_LEN})",
				nullable=True,
			)
...
def _is_implicit_int_pk(doctype: str) -> bool:
	...
	if ragapp.db.db_type == "mariadb":
		...
		values = (ragapp.db.cur_db_name,)

	col_type = ragapp.db.sql(query, values)
