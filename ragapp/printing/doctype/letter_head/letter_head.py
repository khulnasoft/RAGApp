...
from ragapp.utils import flt, is_image
...
class LetterHead(Document):
	...
	def validate_disabled_and_default(self):
		if self.disabled and self.is_default:
			ragapp.throw(_("Letter Head cannot be both disabled and default"))

		if not self.is_default and not self.disabled:
			if not ragapp.db.exists("Letter Head", dict(is_default=1)):
				self.is_default = 1
	...
	def set_image_as_html(
		...
		if not self.get(field) or not is_image(self.get(field)):
			ragapp.msgprint(failure_msg, alert=True, indicator="orange")
			return
...
{dimension}="{dimension_value}" style="{dimension}: {dimension_value}px;">
		...
		ragapp.msgprint(success_msg, alert=True)
	...
	def set_as_default(self):
		...
		if self.is_default:
			ragapp.db.sql("update `tabLetter Head` set is_default=0 where name != %s", self.name)

			set_default("letter_head", self.name)

			# update control panel - so it loads new letter directly
			ragapp.db.set_default("default_letter_head_content", self.content)
		else:
			ragapp.defaults.clear_default("letter_head", self.name)
			ragapp.defaults.clear_default("default_letter_head_content", self.content)
