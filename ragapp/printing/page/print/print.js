ragapp.pages["print"].on_page_load = function (wrapper) {
	ragapp.ui.make_app_page({
		parent: wrapper,
	});

	let print_view = new ragapp.ui.form.PrintView(wrapper);

	$(wrapper).bind("show", () => {
		const route = ragapp.get_route();
		const doctype = route[1];
		const docname = route.slice(2).join("/");
		if (!ragapp.route_options || !ragapp.route_options.frm) {
			ragapp.model.with_doc(doctype, docname, () => {
				let frm = { doctype: doctype, docname: docname };
				frm.doc = ragapp.get_doc(doctype, docname);
				ragapp.model.with_doctype(doctype, () => {
					frm.meta = ragapp.get_meta(route[1]);
					print_view.show(frm);
				});
			});
		} else {
			print_view.frm = ragapp.route_options.frm.doctype
				? ragapp.route_options.frm
				: ragapp.route_options.frm.frm;
			ragapp.route_options.frm = null;
			print_view.show(print_view.frm);
		}
	});
};

ragapp.ui.form.PrintView = class {
	constructor(wrapper) {
	...
	make() {
		this.print_wrapper = this.page.main.empty().html(
			`<div class="print-preview-wrapper"><div class="print-preview">
				${ragapp.render_template("print_skeleton_loading")}
		...
		this.print_settings = ragapp.model.get_doc(":Print Settings", "Print Settings");
		this.setup_menu();
		this.setup_toolbar();
		this.setup_sidebar();
	...
	add_sidebar_item(df, is_dynamic) {
		...
		let field = ragapp.ui.form.make_control({
			df: df,
	...
	setup_menu() {
		...
		if (ragapp.model.can_create("Print Format")) {
	...
	show(frm) {
		...
		// print designer link
		if (Object.keys(ragapp.boot.versions).includes("print_designer")) {
			...
			this.page.add_inner_message(`
			<a style="line-height: 2.4" href="https://ragappcloud.com/marketplace/apps/print_designer?utm_source=framework-desk&utm_medium=print-view&utm_campaign=try-link">
				${__("Try the new Print Designer")}
			</a>
			`);
		}
	...
	set_breadcrumbs() {
		ragapp.breadcrumbs.add(this.frm.meta.module, this.frm.doctype);
	}
	...
	setup_additional_settings() {
		...
		ragapp
			.xcall("ragapp.printing.page.print.print.get_print_settings_to_show", {
	...
	edit_print_format() {
		...
		if (is_standard_but_editable) {
			ragapp.set_route("Form", "Print Format", print_format.name);
			return;
		}
		if (is_custom_format) {
			if (print_format.print_format_builder_beta) {
				ragapp.set_route("print-format-builder-beta", print_format.name);
			} else {
				ragapp.set_route("print-format-builder", print_format.name);
			}
			return;
		}
		...
		ragapp.prompt(
			...
			(data) => {
				...
				ragapp.set_route("print-format-builder");
	...
	setup_keyboard_shortcuts() {
		this.wrapper.find(".print-toolbar a.btn-default").each((i, el) => {
			ragapp.ui.keys.get_shortcut_group(this.frm.page).add($(el));
		});
	}
	...
	set_default_print_language() {
		let print_format = this.get_print_format();
		this.lang_code =
			this.frm.doc.language || print_format.default_print_language || ragapp.boot.lang;
	...
	preview() {
		...
		this.get_print_html((out) => {
			...
			const print_height_inches = ragapp.dom.pixel_to_inches(print_height);
	...
	setup_print_format_dom(out, $print_format) {
		...
		let base_url = ragapp.urllib.get_base_url();
		let print_css = ragapp.assets.bundled_asset(
			"print.bundle.css",
			ragapp.utils.is_rtl(this.lang_code)
		);
		this.$print_format_body
			.find("html")
			.attr("dir", ragapp.utils.is_rtl(this.lang_code) ? "rtl" : "ltr");
	...
	go_to_form_view() {
		...
		ragapp.set_route("Form", this.frm.doctype, this.frm.docname);
	}
			...
			if (me.is_raw_printing()) {
				me.get_raw_commands(function (out) {
					ragapp.ui.form
						...
						.then(ragapp.ui.form.qz_success)
						.catch((err) => {
							ragapp.ui.form.qz_fail(err);
						});
				});
			} else {
				ragapp.show_alert(
	...
	print_by_server() {
		...
		if (localStorage.getItem("network_printer")) {
			ragapp.call({
				method: "ragapp.utils.print_format.print_by_server",
				args: {
	...
	network_printer_setting_dialog(callback) {
		ragapp.call({
			method: "ragapp.printing.doctype.network_printer_settings.network_printer_settings.get_network_printer_settings",
			callback: function (r) {
				if (r.message) {
	...
	async is_wkhtmltopdf_valid() {
		const is_valid = await ragapp.xcall("ragapp.utils.pdf.is_wkhtmltopdf_valid");
	...
	render_pdf() {
		...
		if (print_format.print_format_builder_beta) {
			...
			let w = window.open(`/api/method/ragapp.utils.weasyprint.download_pdf?${params}`);
			if (!w) {
				ragapp.msgprint(__("Please enable pop-ups"));
				return;
			}
		} else {
			this.is_wkhtmltopdf_valid();
			this.render_page("/api/method/ragapp.utils.print_format.download_pdf?");
		}
	}
	...
	get_print_html(callback) {
		...
		this._req = ragapp.call({
			method: "ragapp.www.printview.get_html_and_style",
			args: {
				doc: this.frm.doc,
	...
	get_raw_commands(callback) {
		...
		ragapp.call({
			method: "ragapp.www.printview.get_rendered_raw_commands",
			args: {
				doc: this.frm.doc,
				print_format: this.selected_format(),
				_lang: this.lang_code,
	...
	set_style(style) {
		ragapp.dom.set_style(style || ragapp.boot.print_css, "print-style");
	}
	...
	printer_setting_dialog() {
		...
		this.printer_list = [];
		ragapp.ui.form.qz_get_printer_list().then((data) => {
						...
						fields: [
							{
								...
								fieldname: "print_format",
								default: 0,
								options: ragapp.meta.get_print_formats(this.frm.doctype),
								read_only: 0,
								in_list_view: 1,
								label: __("Print Format"),
							},
			...
			if (!(this.printer_list && this.printer_list.length)) {
				ragapp.throw(__("No Printer is Available."));
			}
		});
	}
};
