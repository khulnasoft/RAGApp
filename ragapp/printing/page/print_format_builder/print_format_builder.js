ragapp.pages["print-format-builder"].on_page_load = function (wrapper) {
	ragapp.print_format_builder = new ragapp.PrintFormatBuilder(wrapper);
	ragapp.breadcrumbs.add("Setup", "Print Format");
};

ragapp.pages["print-format-builder"].on_page_show = function (wrapper) {
	var route = ragapp.get_route();
	if (route.length > 1) {
		ragapp.model.with_doc("Print Format", route[1], function () {
			ragapp.print_format_builder.print_format = ragapp.get_doc("Print Format", route[1]);
			ragapp.print_format_builder.refresh();
		});
	} else if (ragapp.route_options) {
		if (ragapp.route_options.make_new) {
			let { doctype, name, based_on, beta } = ragapp.route_options;
			ragapp.route_options = null;
			ragapp.print_format_builder.setup_new_print_format(doctype, name, based_on, beta);
		} else {
			ragapp.print_format_builder.print_format = ragapp.route_options.doc;
			ragapp.route_options = null;
			ragapp.print_format_builder.refresh();
		}
	}
...
ragapp.PrintFormatBuilder = class PrintFormatBuilder {
	constructor(parent) {
		this.parent = parent;
		this.make();
		this.refresh();
	...
	make() {
		this.page = ragapp.ui.make_app_page({
			parent: this.parent,
	...
	show_start() {
		this.page.main.html(ragapp.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format() {
		...
		var me = this;
		this.print_format_input = ragapp.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function () {
			...
			if (!name) return;
			ragapp.model.with_doc("Print Format", name, function (doc) {
				ragapp.set_route("print-format-builder", name);
			});
		});
	}
	start_new_print_format() {
		var me = this;
		this.doctype_input = ragapp.ui.form.make_control({
		...
		this.name_input = ragapp.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function () {
			...
			if (!(doctype && name)) {
				ragapp.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format(doctype, name, based_on, beta) {
		ragapp.call({
			method: "ragapp.printing.page.print_format_builder.print_format_builder.create_custom_format",
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on,
	...
	setup_print_format() {
		var me = this;
		ragapp.model.with_doctype(this.print_format.doc_type, function (doctype) {
			me.meta = ragapp.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function () {
				ragapp.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	}
	setup_sidebar() {
		...
		this.page.sidebar.html(
			$(ragapp.render_template("print_format_builder_sidebar", { fields: fields }))
		);
	...
	render_layout() {
		...
		this.prepare_data();
		$(
			ragapp.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function (i, f) {
			...
			if (!f.name && f.fieldname) {
				...
				if (f.fieldname === "_custom_html") {
					...
					f = $.extend(
						ragapp.meta.get_docfield(me.print_format.doc_type, f.fieldname) || {},
						f
					);
				}
			}
			...
			if (f.fieldtype === "Section Break") {
				...
				set_column();
			} else if (!ragapp.model.layout_fields.includes(f.fieldtype)) {
	...
	init_visible_columns(f) {
		f.visible_columns = [];
		$.each(ragapp.get_meta(f.options).fields, function (i, _f) {
			if (
			...
			onAdd: function (evt) {
				...
				if (!$item.hasClass("print-format-builder-field")) {
					...
					if (fieldname === "_custom_html") {
						...
						field = ragapp.meta.get_docfield(me.print_format.doc_type, fieldname);
					}

					var html = ragapp.render_template("print_format_builder_field", {
						field: field,
						me: me,
					});
	...
	setup_add_section() {
		...
		this.page.main.find(".print-format-builder-add-section").on("click", function () {
			...
			var $section = $(
				ragapp.render_template("print_format_builder_section", {
					section: section,
					me: me,
				})
	...
	setup_column_selector() {
		...
		this.page.main.on("click", ".select-columns", function () {
			...
			var doc_fields = ragapp.get_meta(doctype).fields;
			...
			// render checkboxes
			$(
				ragapp.render_template("print_format_builder_column_selector", {
					fields: fields,
	...
	save_print_format() {
		...
		// save format_data
		ragapp.call({
			method: "ragapp.client.set_value",
			args: {
			...
			callback: function (r) {
				...
				ragapp.show_alert({ message: __("Saved"), indicator: "green" });
			},
		});
	}
};
