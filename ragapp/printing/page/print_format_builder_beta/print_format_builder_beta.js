ragapp.pages["print-format-builder-beta"].on_page_load = function (wrapper) {
	...
	if (ragapp.boot.developer_mode) {
		...
		ragapp.hot_update.push(() => load_print_format_builder_beta(wrapper));
	}
};

ragapp.pages["print-format-builder-beta"].on_page_show = function (wrapper) {
...
function load_print_format_builder_beta(wrapper) {
	...
	if (route.length > 1) {
		ragapp.require("print_format_builder.bundle.js").then(() => {
			ragapp.print_format_builder = new ragapp.ui.PrintFormatBuilder({
				wrapper: $parent,
				page: wrapper.page,
		...
		let d = new ragapp.ui.Dialog({
			...
			primary_action({ action, doctype, print_format, print_format_name }) {
				if (action === "Edit") {
					ragapp.set_route("print-format-builder-beta", print_format);
					...
					ragapp.db
						...
						.then((doc) => {
							ragapp.set_route("print-format-builder-beta", doc.name);
						})
						.finally(() => {
