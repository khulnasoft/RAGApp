const ragappCloudBaseEndpoint = "https://ragappcloud.com";

$(document).ready(function () {
	if (
		ragapp.boot.fc_communication_secret &&
		ragapp.boot.setup_complete === 1 &&
		!ragapp.is_mobile() &&
		ragapp.user.has_role("System Manager")
	) {
		ragapp.call({
			method: "ragapp.integrations.ragapp_providers.ragappcloud_billing.current_site_info",
			callback: (r) => {
				...
				if (response.trial_end_date) {
					...
					$(".login-to-fc").on("click", function () {
						initiateRequestForLoginToRagappCloud();
					});
				}
			},
		});
...
function initiateRequestForLoginToRagappCloud() {
	ragapp.confirm(__("Are you sure you want to login to Ragapp Cloud dashboard?"), () => {
		requestLoginToFC();
	});
}

function requestLoginToFC(freezing_msg = "Initating login to Ragapp Cloud...") {
	ragapp.call({
		method: "ragapp.integrations.ragapp_providers.ragappcloud_billing.send_verification_code",
		freeze: true,
		...
		error: function (r) {
			ragapp.throw(__("Failed to login to Ragapp Cloud. Please try again"));
		},
	});
}
...
function showFCLoginDialog(email) {
	if (!window.fc_login_dialog) {
		var d = new ragapp.ui.Dialog({
			title: __("Login to Ragapp Cloud"),
	...
	function verifyCode() {
		...
		ragapp.call({
			method: "ragapp.integrations.ragapp_providers.ragappcloud_billing.verify_and_login",
			args: {
			...
			callback: function (r) {
				...
				if (message.login_token) {
					...
					window.open(
						`${ragappCloudBaseEndpoint}/api/method/press.api.developer.saas.login_to_fc?token=${message.login_token}`,
						"_blank"
					);
					ragapp.msgprint({
						title: __("Ragapp Cloud Login Successful"),
						indicator: "green",
						message: `<p>${__(
							"You will be redirected to Ragapp Cloud soon."
						)}</p><p>${__(
							"If you haven't been redirected,"
						)} <a href="${ragappCloudBaseEndpoint}/api/method/press.api.developer.saas.login_to_fc?token=${
							message.login_token
...
function addLoginToFCDropdownItem() {
	$(".dropdown-navbar-user .dropdown-menu .dropdown-item:nth-child(2)").after(
		`<div class="dropdown-item login-to-fc" target="_blank">Login to Ragapp Cloud</div>`
	);
}
