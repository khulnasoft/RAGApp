...
function remove_field() {
	if (store.is_customize_form && props.field.df.is_custom_field == 0) {
		ragapp.msgprint(__("Cannot delete standard field. You can hide it if you want"));
...
function duplicate_field() {
	...
	duplicate_field.df.owner = ragapp.session.user;
...
function make_dialog(frm) {
	frm.dialog = new ragapp.ui.Dialog({
	...
	if (frm.doctype === "Customize Form") {
		...
		ragapp.db.exists("Property Setter", property_setter_id).then((exits) => {
			if (exits) {
				frm.dialog.set_secondary_action_label(__("Reset To Default"));
				frm.dialog.set_secondary_action(() => {
					ragapp.call({
						method: "ragapp.custom.doctype.customize_form.customize_form.get_link_filters_from_doc_without_customisations",
						args: {
...
function edit_filters() {
	...
	ragapp.model.with_doctype(field_doctype, () => {
		frm.dialog.show();
		add_existing_filter(frm, props.field.df);
	});
}
			...
			<template #label>
				<div class="field-label">
					...
					<div
						...
						v-html="ragapp.utils.icon('help', 'sm')"
					/>
				</div>
			</template>
			<template #actions>
				<div class="field-actions" :hidden="store.read_only">
					<button
						...
						@click="edit_filters"
					>
						<div v-html="ragapp.utils.icon('filter', 'sm')" />
					</button>
					<AddFieldButton ref="add_field_ref" :column="column" :field="field">
						<div v-html="ragapp.utils.icon('add', 'sm')" />
					</AddFieldButton>
					<button
					...
					>
						<div v-html="ragapp.utils.icon('move', 'sm')" />
					</button>
					<button
						class="btn btn-xs btn-icon"
					...
					>
						<div v-html="ragapp.utils.icon('duplicate', 'sm')" />
					</button>
					<button
					...
					>
						<div v-html="ragapp.utils.icon('external-link', 'sm')" />
					</button>
					<button
						class="btn btn-xs btn-icon"
					...
					>
						<div v-html="ragapp.utils.icon('remove', 'sm')" />
					</button>
				</div>
			</template>
		</component>
