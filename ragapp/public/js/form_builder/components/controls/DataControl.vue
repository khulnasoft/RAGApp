...
if (props.df.fieldtype === "Datetime") {
	let time_zone_text = ragapp.boot.time_zone
		? ragapp.boot.time_zone.user
		: ragapp.sys_defaults.time_zone;
...
<template>
	<div class="control ragapp-control" :class="{ editable: slots.label }">
		<!-- label -->
		...
		<div
			...
			v-html="ragapp.utils.icon('folder-normal', 'md')"
		/>
		<!-- phone selector icon -->
		<div
			v-if="df.fieldtype == 'Phone'"
			class="selected-phone no-value"
			v-html="ragapp.utils.icon('down', 'sm')"
		/>
	</div>
</template>
