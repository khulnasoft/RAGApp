...
let table_columns = computedAsync(async () => {
	...
	if (!doctype) return [];
	if (!ragapp.get_meta(doctype)) {
		await load_doctype_model(doctype);
	}
	let child_doctype = ragapp.get_meta(doctype);
...
function open_new_child_doctype_dialog() {
	...
	ragapp.model.with_doctype("DocType").then(() => {
		ragapp.listview_settings["DocType"].new_doctype_dialog({ is_child: 1, is_custom });
	});
}
</script>
...
<template>
	<div class="control editable">
		...
		<div class="grid-empty text-center">
			<img
				src="/assets/ragapp/images/ui-states/grid-empty-state.svg"
