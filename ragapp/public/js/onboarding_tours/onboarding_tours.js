ragapp.ui.OnboardingTour = class OnboardingTour {
	constructor() {
		this.driver_steps = [];
	...
	init_driver() {
		this.driver = new ragapp.Driver({
			className: "ragapp-driver",
			allowClose: false,
			padding: 10,
			...
			opacity: 0.5,
			onHighlighted: (step) => {
				ragapp.ui.next_form_tour = step.options.step_info?.next_form_tour;
				const wait_for_node = setInterval(() => {
					...
					if (step.popover.closeBtnNode) {
						step.popover.closeBtnNode.onclick = () => {
							...
							!ragapp.boot.user.onboarding_status[this.tour.name] &&
								(ragapp.boot.user.onboarding_status[this.tour.name] = {});
							ragapp.boot.user.onboarding_status[this.tour.name].is_complete = true;
							if (!this.driver.hasNextStep()) {
								ragapp.boot.user.onboarding_status[
									this.tour.name
								].all_steps_completed = true;
							}

							ragapp.call({
								method: "ragapp.desk.doctype.form_tour.form_tour.update_user_status",
								args: {
									value: JSON.stringify(ragapp.boot.user.onboarding_status),
				...
				if ($input) ragapp.utils.sleep(200).then(() => $input.focus());
			},
		});
	}

	async init({ tour_name, start_step }) {
		this.tour = await ragapp.db.get_doc("Form Tour", tour_name);
		this.init_driver();
		this.build_steps();
	...
	get_step(step_info, on_next) {
		...
		return {
			...
			popover: {
				title,
				description,
				position: ragapp.router.slug(position || "Bottom"),
			},
			onNext: on_next,
			step_info: step_info,
			showButtons,
...
ragapp.ui.init_onboarding_tour = () => {
	...
	typeof ragapp.boot.onboarding_tours == "undefined" && ragapp.boot.onboarding_tours == [];
	typeof ragapp.boot.user.onboarding_status == "undefined" &&
		ragapp.boot.user.onboarding_status == {};
	let route = ragapp.router.current_route;
	if (route?.[0] === "") return;
	...
	if (route[0] != "dashboard-view") {
		ragapp.boot.onboarding_tours &&
			ragapp.boot.onboarding_tours.forEach((tour) => {
	...
	matching_tours = matching_tours.filter((tour) => {
		if (ragapp.boot.user.onboarding_status[tour[0]]?.is_complete == true) return false;
		return true;
	});
	matching_tours = matching_tours.map((tour) => {
		if (ragapp.boot.user.onboarding_status[tour[0]]?.steps_complete != undefined) {
			tour.push(ragapp.boot.user.onboarding_status[tour[0]].steps_complete);
		}
		return tour;
	});
	if (matching_tours.length == 0) return;
	let current_tour = matching_tours.find(
		(tour) => tour[0] == ragapp.ui.currentTourInstance?.tour?.name
	);
	let next_tour = matching_tours.find((tour) => tour[0] == ragapp.ui.next_form_tour);
	if (current_tour) {
		...
		ragapp.ui.next_form_tour = undefined;
	} else {
	...
	if (!tour_name) return;
	if (ragapp.ui.currentTourInstance?.driver) {
		ragapp.ui.currentTourInstance.driver_steps = [];
		ragapp.ui.currentTourInstance.driver.reset(true);
		ragapp.ui.currentTourInstance.update_driver_steps();
	}
	const tour = (ragapp.ui.currentTourInstance = new ragapp.ui.OnboardingTour());
