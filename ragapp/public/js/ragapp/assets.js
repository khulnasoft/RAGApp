...
// depends on ragapp.versions to manage versioning

ragapp.require = function (items, callback) {
	if (typeof items === "string") {
		items = [items];
	}
	items = items.map((item) => ragapp.assets.bundled_asset(item));

	return new Promise((resolve) => {
		ragapp.assets.execute(items, () => {
			resolve();
			callback && callback();
		});
	});
...
class AssetManager {
	...
	init_local_storage() {
		...
		if (ragapp.boot) localStorage.metadata_version = ragapp.boot.metadata_version;
	}
	...
	execute(items, callback) {
		...
		const version_string =
			ragapp.boot.developer_mode || window.dev_server ? Date.now() : window._version_number;
...
ragapp.assets = new AssetManager();
