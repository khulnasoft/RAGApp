...
ragapp.realtime.on("build_event", (data) => {
	if (data.success) {
			...
			for (let file of changed_files) {
				if (file.includes(".bundle.")) {
					...
					if (parts.length === 2) {
						...
						ragapp.assets._executed = ragapp.assets._executed.filter(
		...
		// update assets json
		ragapp.call("ragapp.sessions.get_boot_assets_json").then((r) => {
			if (r.message) {
				ragapp.boot.assets_json = r.message;

				if (ragapp.hot_update) {
					ragapp.hot_update.forEach((callback) => {
						callback();
					});
				}
			}
