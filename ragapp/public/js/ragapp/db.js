...
ragapp.db = {
	get_list: function (doctype, args) {
		...
		return new Promise((resolve) => {
			ragapp.call({
				method: "ragapp.desk.reportview.get_list",
				args: args,
				type: "GET",
				callback: function (r) {
					resolve(r.message);
	...
	exists: function (doctype, nameOrFilters) {
		return new Promise((resolve) => {
			...
			if (typeof nameOrFilters === "string") {
				...
				ragapp.db.get_value(doctype, { name: nameOrFilters }, "name").then((r) => {
					r.message && r.message.name ? resolve(true) : resolve(false);
				});
			} else if (typeof nameOrFilters === "object") {
				ragapp.db.count(doctype, { filters: nameOrFilters, limit: 1 }).then((count) => {
					resolve(count > 0);
				});
			}
		});
	...
	get_single_value: (doctype, field) => {
		return new Promise((resolve) => {
			ragapp
				.call({
					method: "ragapp.client.get_single_value",
					args: { doctype, field },
					type: "GET",
				})
	...
	insert: function (doc) {
		return ragapp.xcall("ragapp.client.insert", { doc });
	},
	delete_doc: function (doctype, name) {
		return new Promise((resolve) => {
			ragapp.call("ragapp.client.delete", { doctype, name }, (r) => resolve(r.message));
		});
	},
	count: function (doctype, args = {}, cache = false) {
		...
		return ragapp.xcall(
			"ragapp.desk.reportview.get_count",
			{
				doctype,
				filters,
				fields,
