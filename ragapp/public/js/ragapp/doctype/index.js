...
*/
ragapp.model.DocTypeController = class DocTypeController extends ragapp.ui.form.Controller {
	setup() {
		// setup formatters for fieldtype
		ragapp.meta.docfield_map[
	...
	show_db_utilization() {
		...
		ragapp
			.xcall("ragapp.core.doctype.doctype.doctype.get_row_size_utilization", {
				doctype,
			})
			.then((r) => {
				if (r < 50.0) return;
	...
	setup_fetch_from_fields(doc, doctype, docname) {
		...
		let row = ragapp.get_doc(doctype, docname);
		...
		function update_fieldname_options() {
			...
			ragapp.model.with_doctype(link_doctype, () => {
				let fields = ragapp.meta
