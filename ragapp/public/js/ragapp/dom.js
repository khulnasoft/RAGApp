...
ragapp.dom = {
	...
	get_unique_id: function () {
		const id = "unique-" + ragapp.dom.id_count;
		ragapp.dom.id_count++;
		return id;
	},
	set_unique_id: function (ele) {
		...
		var id = "unique-" + ragapp.dom.id_count;
		$ele.attr("id", id);
		ragapp.dom.id_count++;
		return id;
	},
	...
	add: function (parent, newtag, className, cs, innerHTML, onclick) {
		if (parent && parent.substr) parent = ragapp.dom.by_id(parent);
	...
	freeze: function (msg, css_class) {
		...
		ragapp.dom.freeze_count++;
	},
	unfreeze: function () {
		if (!ragapp.dom.freeze_count) return; // anything open?
		ragapp.dom.freeze_count--;
		if (!ragapp.dom.freeze_count) {
	...
	scroll_to_section(section_name) {
		setTimeout(() => {
			...
			if (section.length) {
				...
				ragapp.ui.scroll(section.parent().parent());
			}
		}, 200);
	},
...
ragapp.ellipsis = function (text, max) {
	if (!max) max = 20;
	text = cstr(text);
	if (text.length > max) {
...
ragapp.run_serially = function (tasks) {
...
ragapp.load_image = (src, onload, onerror, preprocess = () => {}) => {
...
ragapp.timeout = (seconds) => {
	return new Promise((resolve) => {
		setTimeout(() => resolve(), seconds * 1000);
	});
};

ragapp.scrub = ragapp.slug = function (text, spacer = "_") {
	return text.replace(/ /g, spacer).toLowerCase();
};

ragapp.unscrub = function (txt) {
	return ragapp.model.unscrub(txt);
};

ragapp.get_data_pill = (
	label,
	target_id = null,
	remove_action = null,
	image = null,
	...
	let color = "",
		style = "";
	if (colored) {
		color = ragapp.get_palette(label);
	}
	...
	if (remove_action) {
		let remove_btn = $(`
			<span class="remove-btn cursor-pointer">
				${ragapp.utils.icon("close", "sm", "es-icon")}
			</span>
		`);
...
ragapp.get_modal = function (title, content) {
				...
				<div class="modal-header">
					...
					<div class="modal-actions">
						<button class="btn btn-modal-minimize btn-link hide">
							${ragapp.utils.icon("collapse")}
						</button>
						<button class="btn btn-modal-close btn-link" data-dismiss="modal">
							${ragapp.utils.icon("close-alt", "sm", "close-alt")}
						</button>
					</div>
				</div>
...
ragapp.is_online = function () {
	if (ragapp.boot.developer_mode == 1) {
...
ragapp.create_shadow_element = function (wrapper, html, css, js) {
	let random_id = "custom-block-" + ragapp.utils.get_random(5).toLowerCase();
	...
	class CustomBlock extends HTMLElement {
		constructor() {
			...
			div.innerHTML = ragapp.dom.remove_script_and_style(html);

			// link global desk css
			let link = document.createElement("link");
			link.rel = "stylesheet";
			link.href = ragapp.assets.bundled_asset("desk.bundle.css");
