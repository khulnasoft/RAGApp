	...
	<div class="tree-node" :class="{ opened: node.open }">
		...
		>
			...
			<a
				...
				v-html="ragapp.utils.icon('external-link', 'sm')"
				@click.stop
			/>
		</span>
		<div v-if="node.file_url && ragapp.utils.is_image_file(node.file_url)">
...
let icon = computed(() => {
	let icons = {
		open: ragapp.utils.icon("folder-open", "md"),
		closed: ragapp.utils.icon("folder-normal", "md"),
		leaf: ragapp.utils.icon("primitive-dot", "xs"),
		search: ragapp.utils.icon("search"),
	};
...
let open_file = (filename) => {
	return ragapp.utils.get_form_link("File", filename);
};
