ragapp.ui.form.Control = class BaseControl {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	...
	make_wrapper() {
		this.$wrapper = $("<div class='ragapp-control'></div>").appendTo(this.parent);
	...
	get_status(explain) {
		...
		var status = ragapp.perm.get_field_display_status(
			this.df,
			ragapp.model.get_doc(this.doctype, this.docname),
		...
		let value = ragapp.model.get_value(this.doctype, this.docname, this.df.fieldname);
		...
		if (
			...
			!this.only_input &&
			is_null(value) &&
			cint(ragapp.boot.sysdefaults.hide_empty_read_only_fields) &&
	...
	show_translatable_button(value) {
		...
		if (
			...
			!ragapp.model.can_write("Translation") ||
			!value
		)
			return;

		// Disable translation in website
		if (!ragapp.views || !ragapp.views.TranslationManager) return;
		...
		$(translation_btn)
			...
			.on("click", () => {
				if (!this.doc.__islocal) {
					new ragapp.views.TranslationManager({
						df: this.df,
