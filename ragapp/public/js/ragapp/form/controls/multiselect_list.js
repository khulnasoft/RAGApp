ragapp.ui.form.ControlMultiSelectList = class ControlMultiSelectList extends (
	ragapp.ui.form.ControlData
) {
	...
	set_selectable_items(options) {
		let html = options
			.map((option) => {
				...
				<div class="multiselect-check">${ragapp.utils.icon("tick", "xs")}</div>
			</li>`;
			})
			.join("");
		if (!html) {
