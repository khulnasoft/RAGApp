ragapp.ui.form.ControlPassword = class ControlPassword extends ragapp.ui.form.ControlData {
	...
	make_input() {
		...
		this.toggle_password.on("click", () => {
			if (this.$input.attr("type") === "password") {
				...
				this.toggle_password.html(ragapp.utils.icon("hide", "sm"));
			} else {
				this.$input.attr("type", "password");
				this.toggle_password.html(ragapp.utils.icon("unhide", "sm"));
			}
		});
	...
	get_password_strength(value) {
		...
		ragapp.call({
			type: "POST",
			method: "ragapp.core.doctype.user.user.test_password_strength",
			args: {
				new_password: value || "",
			},
			callback: function (r) {
