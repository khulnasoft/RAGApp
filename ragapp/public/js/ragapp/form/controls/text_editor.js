...
ragapp.ui.form.ControlTextEditor = class ControlTextEditor extends ragapp.ui.form.ControlCode {
	make_wrapper() {
		super.make_wrapper();
	}
	...
	bind_events() {
		this.quill.on(
			"text-change",
			ragapp.utils.debounce((delta, oldDelta, source) => {
		...
		$(this.quill.root).on("keydown", (e) => {
			const key = ragapp.ui.keys && ragapp.ui.keys.get_key(e);
	...
	get_mention_options() {
		...
		return {
			...
			source: ragapp.utils.debounce(async function (search_term, renderList) {
				let method =
					me.mention_search_method || "ragapp.desk.search.get_names_for_mentions";
				let values = await ragapp.xcall(method, {
					search_term,
				});
			...
			renderItem(item) {
				...
				return `${value} ${item.is_group ? ragapp.utils.icon("users") : ""}`;
			},
		};
	}
	...
	parse(value) {
		...
		return ragapp.dom.remove_script_and_style(value);
	}
