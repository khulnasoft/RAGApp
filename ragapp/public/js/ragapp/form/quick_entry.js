...
ragapp.quick_edit = function (doctype, name) {
	if (!name) name = doctype; // single
	ragapp.db.get_doc(doctype, name).then((doc) => {
		ragapp.ui.form.make_quick_entry(doctype, null, null, doc);
	});
};

ragapp.ui.form.make_quick_entry = (doctype, after_insert, init_callback, doc, force) => {
	var trimmed_doctype = doctype.replace(/ /g, "");
	var controller_name = "QuickEntryForm";

	if (ragapp.ui.form[trimmed_doctype + "QuickEntryForm"]) {
		controller_name = trimmed_doctype + "QuickEntryForm";
	}

	ragapp.quick_entry = new ragapp.ui.form[controller_name](
		doctype,
		after_insert,
		init_callback,
		doc,
	...
	return ragapp.quick_entry.setup();
};

ragapp.ui.form.QuickEntryForm = class QuickEntryForm extends ragapp.ui.Dialog {
	...
	check_quick_entry_doc() {
		if (!this.doc) {
			this.doc = ragapp.model.get_new_doc(this.doctype, null, null, true);
		}
	}
	...
	render_dialog() {
		...
		this.onhide = () => (ragapp.quick_entry = null);
		this.show();
			...
			if (data) {
				...
				me.script_manager.trigger("validate").then(() => {
					me.insert().then(() => {
						...
						me.dialog.hide();
						setTimeout(function () {
							ragapp.show_alert({ message: messagetxt, indicator: "green" }, 3);
						}, 500);
					});
				});
			}
	...
	insert() {
		...
		return new Promise((resolve) => {
			me.update_doc();
			ragapp.call({
				method: "ragapp.client.save",
				args: {
					doc: me.dialog.doc,
				},
				callback: function (r) {
					if (
						ragapp.model.is_submittable(me.doctype) &&
						!ragapp.model.has_workflow(me.doctype)
					) {
						ragapp.run_serially([
							() => (me.dialog.working = true),
							() => {
	...
	process_after_insert(r) {
		...
		} else if (ragapp._from_link) {
			ragapp.ui.form.update_calling_link(this.doc);
	...
	open_form_if_not_list() {
		...
		if (route && !(route[0] === "List" && route[1] === doc.doctype)) {
			ragapp.run_serially([() => ragapp.set_route("Form", doc.doctype, doc.name)]);
		}
	}
	...
	open_doc(set_hooks) {
		...
		if (set_hooks && this.after_insert) {
			ragapp.route_options = ragapp.route_options || {};
			ragapp.route_options.after_save = (frm) => {
				this.after_insert(frm);
			};
		}
		this.doc.__run_link_triggers = false;
		ragapp.set_route("Form", this.doctype, this.doc.name);
	}
