...
ragapp.provide("ragapp.ui.form.handlers");
...
ragapp.ui.form.get_event_handler_list = function (doctype, fieldname) {
	if (!ragapp.ui.form.handlers[doctype]) {
		ragapp.ui.form.handlers[doctype] = {};
	}
	if (!ragapp.ui.form.handlers[doctype][fieldname]) {
		ragapp.ui.form.handlers[doctype][fieldname] = [];
	}
	return ragapp.ui.form.handlers[doctype][fieldname];
};

ragapp.ui.form.on = ragapp.ui.form.on_change = function (doctype, fieldname, handler) {
	var add_handler = function (fieldname, handler) {
		var handler_list = ragapp.ui.form.get_event_handler_list(doctype, fieldname);
...
ragapp.ui.form.off = function (doctype, fieldname, handler) {
	var handler_list = ragapp.ui.form.get_event_handler_list(doctype, fieldname);
...
ragapp.ui.form.trigger = function (doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
};

ragapp.ui.form.ScriptManager = class ScriptManager {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	trigger(event_name, doctype, name) {
		...
		this.frm.selected_doc = ragapp.get_doc(doctype, name);
	...
	has_handler(event_name) {
		...
		return (
			ragapp.ui.form.handlers[this.frm.doctype] &&
			ragapp.ui.form.handlers[this.frm.doctype][event_name]
		);
	}
	...
	get_handlers(event_name, doctype) {
		...
		if (ragapp.ui.form.handlers[doctype] && ragapp.ui.form.handlers[doctype][event_name]) {
			$.each(ragapp.ui.form.handlers[doctype][event_name], function (i, fn) {
	...
	setup() {
		...
		$.each(this.frm.fields, function (i, field) {
			...
			if (ragapp.model.table_fields.includes(field.df.fieldtype)) {
				$.each(
					ragapp.meta.get_docfields(field.df.options, me.frm.docname),
					function (i, df) {
						setup_add_fetch(df);
					}
				);
		...
		// css
		doctype.__css && ragapp.dom.set_style(doctype.__css);

		this.trigger("setup");
	}

	log_error(caller, e) {
		ragapp.show_alert({ message: __("Error in Client Script."), indicator: "error" });
