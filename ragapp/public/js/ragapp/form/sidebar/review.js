...
ragapp.ui.form.Review = class Review {
	constructor({ parent, frm }) {
		this.parent = parent;
		this.frm = frm;
		this.points = ragapp.boot.points;
	...
	update_points() {
		return ragapp
			.xcall("ragapp.social.doctype.energy_point_log.energy_point_log.get_energy_points", {
				user: ragapp.session.user,
			})
			.then((data) => {
				ragapp.boot.points = data;
				this.points = data;
			});
	}
	...
	show_review_dialog() {
		...
		const review_dialog = new ragapp.ui.Dialog({
			title: __("Add Review"),
			fields: [
				{
					fieldname: "to_user",
			...
			primary_action: (values) => {
				...
				if (values.points > this.points.review_points) {
					return ragapp.msgprint(__("You do not have enough points"));
				}
				ragapp
					.xcall("ragapp.social.doctype.energy_point_log.energy_point_log.review", {
						doc: {
	...
	setup_detail_popover(el, data) {
		let subject = "";
		let fullname = ragapp.user.full_name(data.user);
		let timestamp = `<span class="text-muted">${ragapp.datetime.comment_when(
			data.creation
		)}</span>`;
			...
			content: () => {
				return `
					<div class="text-medium">
						...
						<div class="subject">
							${ragapp.utils.icon("review")}
							${subject}

							<p class="mt-1">
								<!-- ${ragapp.avatar("shivam@example.com", "avatar-xs")} -->
								<span>${ragapp.user.full_name(data.owner)}</span> - ${timestamp}
							</p>
						</div>
					</div>
				`;
