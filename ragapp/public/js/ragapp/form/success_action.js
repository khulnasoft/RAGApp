...
ragapp.provide("ragapp.success_action");

ragapp.ui.form.SuccessAction = class SuccessAction {
	constructor(form) {
		this.form = form;
		this.load_setting();
	}
	...
	load_setting() {
		this.setting = ragapp.boot.success_action.find(
	...
	show_alert() {
		ragapp.db.get_list(this.form.doctype, { limit: 2 }).then((result) => {
	...
	get default_actions() {
		return {
			new: {
				label: __("New"),
				action: (frm) => ragapp.new_doc(frm.doctype),
			},
			print: {
				label: __("Print"),
			...
			list: {
				label: __("View All"),
				action: (frm) => {
					ragapp.set_route("List", frm.doctype);
				},
			},
		};
	}
