...
ragapp.views.ListGroupBy = class ListGroupBy {
	constructor(opts) {
		$.extend(this, opts);
		this.make_wrapper();
	...
	render_group_by_items() {
		let get_item_html = (fieldname) => {
			...
			if (fieldname === "assigned_to") {
				...
				label = ragapp.meta.get_label(this.doctype, fieldname);
				let docfield = ragapp.meta.get_docfield(this.doctype, fieldname);
				if (!docfield) {
					return;
				}
			...
			return `<div class="group-by-field list-link">
						...
						href="#" onclick="return false;">
							...
							<span>${ragapp.utils.icon("select", "xs")}</span>
						</a>
	...
	setup_search($dropdown) {
		ragapp.utils.setup_search($dropdown, ".group-by-item", ".group-by-value", "data-name");
	}
	...
	get_group_by_count(field) {
		...
		return ragapp.call("ragapp.desk.listview.get_group_by_count", args).then((r) => {
			let field_counts = r.message || [];
			field_counts = field_counts.filter((f) => f.count !== 0);
			let current_user = field_counts.find((f) => f.name === ragapp.session.user);
			field_counts = field_counts.filter(
				(f) => !["Guest", "Administrator", ragapp.session.user].includes(f.name)
			);
			// Set ragapp.session.user on top of the list
	...
	get_dropdown_html(field, fieldtype, applied = false) {
		...
		if (field.name == null) {
			label = __("Not Set");
		} else if (field.name === ragapp.session.user) {
			label = __("Me");
		...
		let applied_html = applied
			? `<span class="applied"> ${ragapp.utils.icon("tick", "xs")} </span>`
			: "";
