...
ragapp.views.ListViewSelect = class ListViewSelect {
	constructor(opts) {
		$.extend(this, opts);
	...
	set_current_view() {
		...
		const view_name = ragapp.utils.to_title_case(route[2] || "");
		if (route.length > 2 && ragapp.views.view_modes.includes(view_name)) {
	...
	set_route(view, calendar_name) {
		...
		if (search_params) {
			ragapp.route_options = Object.fromEntries(search_params);
		}
	...
	setup_views() {
		const views = {
			...
			Calendar: {
				condition: ragapp.views.calendar[this.doctype],
			...
			Inbox: {
				condition: this.doctype === "Communication" && ragapp.boot.email_accounts.length,
				action: () => this.set_route("inbox"),
				current_view_handler: () => {
					const accounts = this.get_email_accounts();
					let default_action;
					if (has_common(ragapp.user_roles, ["System Manager", "Administrator"])) {
						default_action = {
							label: __("New Email Account"),
							action: () => ragapp.new_doc("Email Account"),
						};
					}
			...
			Tree: {
				condition:
					ragapp.treeview_settings[this.doctype] ||
					ragapp.get_meta(this.doctype).is_tree,
		...
		ragapp.views.view_modes.forEach((view) => {
	...
	setup_kanban_switcher(kanbans) {
		...
		if (can_create) {
			this.page.add_custom_menu_item(
				...
				() => ragapp.views.KanbanView.show_kanban_dialog(this.doctype),
				true
			);
		}
	}
	...
	get_page_name() {
		return ragapp.utils.to_title_case(ragapp.get_route().slice(-1)[0] || "");
	}
	...
	get_reports() {
		...
		var reports =
			Object.values(ragapp.boot.user.all_reports).sort((a, b) =>
	...
	setup_kanban_boards() {
			...
			ragapp.db.get_value(
				...
				(board) => {
					if (!$.isEmptyObject(board)) {
						ragapp.set_route("list", doctype, "kanban", board.name);
					} else {
						ragapp.views.KanbanView.show_kanban_dialog(doctype);
					}
				}
			);
		}
		...
		const last_opened_kanban =
			ragapp.model.user_settings[this.doctype]["Kanban"]?.last_kanban_board;
		if (!last_opened_kanban) {
			fetch_kanban_board(this.doctype);
		} else {
			ragapp.db.exists("Kanban Board", last_opened_kanban).then((exists) => {
				if (exists) {
					ragapp.set_route("list", this.doctype, "kanban", last_opened_kanban);
				} else {
					fetch_kanban_board(this.doctype);
				}
			});
	...
	get_calendars() {
		...
		return ragapp.db
			...
			.then((result) => {
				...
				if (ragapp.views.calendar[this.doctype]) {
	...
	get_email_accounts() {
		let accounts_to_add = [];
		let accounts = ragapp.boot.email_accounts;
	...
	slug() {
		return ragapp.router.slug(ragapp.router.doctype_layout || this.doctype);
	}
};
