...
$.extend(ragapp.model, {
	...
	init: function () {
		...
		ragapp.realtime.on("doc_update", function (data) {
			var doc = locals[data.doctype] && locals[data.doctype][data.name];

			if (doc) {
				// current document is dirty, show message if its not me
				if (
					ragapp.get_route()[0] === "Form" &&
					cur_frm.doc.doctype === doc.doctype &&
					cur_frm.doc.name === doc.name
				) {
					if (data.modified !== cur_frm.doc.modified && !ragapp.ui.form.is_saving) {
						if (!cur_frm.is_dirty()) {
					...
					if (!doc.__unsaved) {
						...
						ragapp.model.remove_from_locals(doc.doctype, doc.name);
					} else {
	...
	is_value_type: function (fieldtype) {
		...
		return ragapp.model.no_value_type.indexOf(fieldtype) === -1;
	},

	is_non_std_field: function (fieldname) {
		return ![...ragapp.model.std_fields_list, ...ragapp.model.child_table_field_list].includes(
			fieldname
		);
	},
	...
	get_std_field: function (fieldname, ignore = false) {
		var docfield = $.map(
			[].concat(ragapp.model.std_fields).concat(ragapp.model.std_fields_table),
			function (d) {
		...
		if (!docfield.length) {
			...
			if (ignore) {
				...
				ragapp.msgprint(__("Unknown Column: {0}", [fieldname]));
			}
		}
		return docfield[0];
	},
	...
	set_in_localstorage: function (doctype, docs) {
		try {
			...
			ragapp.model.clear_local_storage();
			localStorage["_doctype:" + doctype] = JSON.stringify(docs);
		}
	},
	...
	with_doctype: function (doctype, callback, async) {
		if (locals.DocType[doctype]) {
			...
			let cached_docs = ragapp.model.get_from_localstorage(doctype);
			...
			return ragapp.call({
				method: "ragapp.desk.form.load.getdoctype",
				type: "GET",
				args: {
					doctype: doctype,
					with_parent: 1,
				...
				callback: function (r) {
					if (r.exc) {
						ragapp.msgprint(__("Unable to load: {0}", [__(doctype)]));
						throw "No doctype";
					}
					if (r.message == "use_cache") {
						ragapp.model.sync(meta);
					} else {
						ragapp.model.set_in_localstorage(doctype, r.docs);
						meta = r.docs[0];
					}
					ragapp.model.init_doctype(meta);

					if (r.user_settings) {
						// remember filters and other settings from last view
						ragapp.model.user_settings[doctype] = JSON.parse(r.user_settings);
						ragapp.model.user_settings[doctype].updated_on = moment().toString();
					}
					callback && callback(r);
				},
			});
	...
	init_doctype: function (meta) {
		if (meta.name === "DocType") {
			...
			ragapp.meta.__doctype_meta = JSON.parse(JSON.stringify(meta));
		}
		...
		if (meta.__templates) {
			$.extend(ragapp.templates, meta.__templates);
		}
	},
	...
	with_doc: function (doctype, name, callback) {
		return new Promise((resolve) => {
			...
			if (
				...
				resolve(ragapp.get_doc(doctype, name));
			} else {
				return ragapp.call({
					method: "ragapp.desk.form.load.getdoc",
					type: "GET",
					args: {
						doctype: doctype,
						name: name,
	...
	get_docinfo: function (doctype, name) {
		return (ragapp.model.docinfo[doctype] && ragapp.model.docinfo[doctype][name]) || null;
	},

	set_docinfo: function (doctype, name, key, value) {
		if (ragapp.model.docinfo[doctype] && ragapp.model.docinfo[doctype][name]) {
			ragapp.model.docinfo[doctype][name][key] = value;
		}
	},

	get_shared: function (doctype, name) {
		return ragapp.model.get_docinfo(doctype, name).shared;
	},

	get_server_module_name: function (doctype) {
		var dt = ragapp.model.scrub(doctype);
		var module = ragapp.model.scrub(locals.DocType[doctype].module);
		var app = ragapp.boot.module_app[module];
	...
	can_create: function (doctype) {
		return ragapp.boot.user.can_create.indexOf(doctype) !== -1;
	},

	can_select: function (doctype) {
		if (ragapp.boot.user) {
			return ragapp.boot.user.can_select.indexOf(doctype) !== -1;
		}
	},

	can_read: function (doctype) {
		if (ragapp.boot.user) {
			return ragapp.boot.user.can_read.indexOf(doctype) !== -1;
		}
	},

	can_write: function (doctype) {
		return ragapp.boot.user.can_write.indexOf(doctype) !== -1;
	},

	can_get_report: function (doctype) {
		return ragapp.boot.user.can_get_report.indexOf(doctype) !== -1;
	},

	can_delete: function (doctype) {
		if (!doctype) return false;
		return ragapp.boot.user.can_delete.indexOf(doctype) !== -1;
	},

	can_submit: function (doctype) {
		if (!doctype) return false;
		return ragapp.boot.user.can_submit.indexOf(doctype) !== -1;
	},

	can_cancel: function (doctype) {
		if (!doctype) return false;
		return ragapp.boot.user.can_cancel.indexOf(doctype) !== -1;
	},

	has_workflow: function (doctype) {
		return ragapp.get_list("Workflow", { document_type: doctype, is_active: 1 }).length;
	},
	...
	is_single: function (doctype) {
		if (!doctype) return false;
		return ragapp.boot.single_types.indexOf(doctype) != -1;
	},
	...
	can_import: function (doctype, frm, meta = null) {
		...
		if (ragapp.user_roles.includes("System Manager")) return true;

		if (frm) return frm.perm[0].import === 1;
		return ragapp.boot.user.can_import.indexOf(doctype) !== -1;
	},
	...
	can_export: function (doctype, frm) {
		...
		return ragapp.boot.user.can_export.indexOf(doctype) !== -1;
	},

	can_print: function (doctype, frm) {
		if (frm) return frm.perm[0].print === 1;
		return ragapp.boot.user.can_print.indexOf(doctype) !== -1;
	},

	can_email: function (doctype, frm) {
		if (frm) return frm.perm[0].email === 1;
		return ragapp.boot.user.can_email.indexOf(doctype) !== -1;
	},

	can_share: function (doctype, frm) {
		let disable_sharing = cint(ragapp.sys_defaults.disable_document_sharing);

		if (disable_sharing && ragapp.session.user !== "Administrator") {
			return false;
		}
		...
		return ragapp.boot.user.can_share.indexOf(doctype) !== -1;
	},

	has_value: function (dt, dn, fn) {
		// return true if property has value
		var val = locals[dt] && locals[dt][dn] && locals[dt][dn][fn];
		var df = ragapp.meta.get_docfield(dt, fn, dn);

		let ret;
		if (ragapp.model.table_fields.includes(df.fieldtype)) {
			ret = false;
	...
	get_list: function (doctype, filters) {
		...
		return ragapp.utils.filter_dict(docsdict, filters);
	},

	get_value: function (doctype, filters, fieldname, callback) {
		if (callback) {
			ragapp.call({
				method: "ragapp.client.get_value",
				args: {
					doctype: doctype,
					fieldname: fieldname,
					filters: filters,
			...
			if (
				...
				var l = ragapp.get_list(doctype, filters);
	...
	set_value: function (
		...
		$.each(to_update, (key, value) => {
			if (doc && doc[key] !== value) {
				...
				doc[key] = value;
				tasks.push(() => ragapp.model.trigger(key, value, doc, skip_dirty_trigger));
			} else {
	...
	on: function (doctype, fieldname, fn) {
		...
		*/
		/* example: ragapp.model.on("Customer", "age", function(fieldname, value, doc) {
		  if(doc.age < 16) {
		   	ragapp.msgprint("Warning, Customer must atleast be 16 years old.");
		    raise "CustomerAgeError";
		  }
		}) */
		ragapp.provide("ragapp.model.events." + doctype);
		if (!ragapp.model.events[doctype][fieldname]) {
			ragapp.model.events[doctype][fieldname] = [];
		}
		ragapp.model.events[doctype][fieldname].push(fn);
	},
		...
		function enqueue_events(events) {
			...
			for (const fn of events) {
				...
				tasks.push(() => {
					...
					// or use the default promise ragapp.after_ajax
					if (return_value && return_value.then) {
						return return_value;
					} else {
						return ragapp.after_server_call();
					}
				});
			}
		}

		if (ragapp.model.events[doc.doctype]) {
			enqueue_events(ragapp.model.events[doc.doctype][fieldname]);
			enqueue_events(ragapp.model.events[doc.doctype]["*"]);
		}
	...
	get_doc: function (doctype, name) {
		...
		if ($.isPlainObject(name)) {
			var doc = ragapp.get_list(doctype, name);
	...
	get_children: function (doctype, parent, parentfield, filters) {
		...
		if ($.isPlainObject(doctype)) {
			...
			doc = ragapp.get_doc(doctype, parent);
		}

		var children = doc[parentfield] || [];
		if (filters) {
			return ragapp.utils.filter_dict(children, filters);
		} else {
			return children;
		}
	},
	...
	remove_from_locals: function (doctype, name) {
		...
		if (ragapp.views.formview[doctype]) {
			delete ragapp.views.formview[doctype].frm.opendocs[name];
		}
	},
	...
	get_no_copy_list: function (doctype) {
		...
		var docfields = ragapp.get_meta(doctype).fields || [];
	...
	delete_doc: function (doctype, docname, callback) {
		let title = docname;
		const title_field = ragapp.get_meta(doctype).title_field;
		if (ragapp.get_meta(doctype).autoname == "hash" && title_field) {
			const value = ragapp.model.get_value(doctype, docname, title_field);
			if (value) {
				title = `${value} (${docname})`;
			}
		}
		ragapp.confirm(__("Permanently delete {0}?", [title.bold()]), function () {
			return ragapp.call({
				method: "ragapp.client.delete",
				args: {
					doctype: doctype,
					name: docname,
				},
				...
				callback: function (r, rt) {
					if (!r.exc) {
						ragapp.utils.play_sound("delete");
						ragapp.model.clear_doc(doctype, docname);
	...
	round_floats_in: function (doc, fieldnames) {
		...
		if (!fieldnames) {
			fieldnames = ragapp.meta.get_fieldnames(doc.doctype, doc.parent, {
	...
	validate_missing: function (doc, fieldname) {
		if (!doc[fieldname]) {
			ragapp.throw(
				__("Please specify") +
					": " +
					__(ragapp.meta.get_label(doc.doctype, fieldname, doc.parent || doc.name))
			);
		}
	},
	...
	is_numeric_field: function (fieldtype) {
		...
		return ragapp.model.numeric_fieldtypes.includes(fieldtype);
	},

	set_default_views_for_doctype(doctype, frm) {
		ragapp.model.with_doctype(doctype, () => {
			let meta = ragapp.get_meta(doctype);
			let default_views = ["List", "Report", "Dashboard", "Kanban"];

			if (meta.is_calendar_and_gantt && ragapp.views.calendar[doctype]) {
			...
			if (doctype === "Communication" && ragapp.boot.email_accounts.length) {
...
// legacy
ragapp.get_doc = ragapp.model.get_doc;
ragapp.get_children = ragapp.model.get_children;
ragapp.get_list = ragapp.model.get_list;
