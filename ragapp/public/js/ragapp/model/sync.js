...
Object.assign(ragapp.model, {
	docinfo: {},
	sync: function (r) {
		/* docs:
			extract docs, docinfo (attachments, comments, assignments)
			from incoming request and set in `locals` and `ragapp.model.docinfo`
		*/
		var isPlain;
		...
		if (r.docs) {
			for (var i = 0, l = r.docs.length; i < l; i++) {
				...
				if (locals[d.doctype] && locals[d.doctype][d.name]) {
					// update values
					ragapp.model.update_in_locals(d);
				} else {
					ragapp.model.add_to_locals(d);
				}
				...
				if (d.localname) {
					ragapp.model.rename_after_save(d, i);
				}
			}
		}

		ragapp.model.sync_docinfo(r);
		return r.docs;
	},

	rename_after_save: (d, i) => {
		ragapp.model.new_names[d.localname] = d.name;
		$(document).trigger("rename", [d.doctype, d.localname, d.name]);
		delete locals[d.doctype][d.localname];

		// update docinfo to new dict keys
		if (i === 0) {
			ragapp.model.docinfo[d.doctype][d.name] = ragapp.model.docinfo[d.doctype][d.localname];
			ragapp.model.docinfo[d.doctype][d.localname] = undefined;
		}
	},
	...
	sync_docinfo: (r) => {
		...
		if (r.docinfo) {
			...
			if (!ragapp.model.docinfo[doctype]) {
				ragapp.model.docinfo[doctype] = {};
			}
			ragapp.model.docinfo[doctype][name] = r.docinfo;

			// copy values to ragapp.boot.user_info
			Object.assign(ragapp.boot.user_info, r.docinfo.user_info);
		}
	...
	add_to_locals: function (doc) {
		...
		if (!doc.name && doc.__islocal) {
			...
			if (!doc.parentfield) ragapp.model.clear_doc(doc);

			doc.name = ragapp.model.get_new_name(doc.doctype);

			if (!doc.parentfield)
				ragapp.provide("ragapp.model.docinfo." + doc.doctype + "." + doc.name);
		}
	...
	update_in_locals: function (doc) {
		...
		for (let fieldname in doc) {
			let df = ragapp.meta.get_field(doc.doctype, fieldname);
			if (df && ragapp.model.table_fields.includes(df.fieldtype)) {
				// table
				...
				for (let i = 0; i < doc[fieldname].length; i++) {
					...
					if (local_d) {
						...
						if (!d.name) {
							...
							d.name = ragapp.model.get_new_name(doc.doctype);
						}
