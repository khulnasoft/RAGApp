ragapp.provide("ragapp.model.user_settings");

$.extend(ragapp.model.user_settings, {
	get: function (doctype) {
		return ragapp
			.call("ragapp.model.utils.user_settings.get", { doctype })
			.then((r) => JSON.parse(r.message || "{}"));
	},
	save: function (doctype, key, value) {
		if (ragapp.session.user === "Guest") return Promise.resolve();

		const old_user_settings = ragapp.model.user_settings[doctype] || {};
	...
	remove: function (doctype, key) {
		var user_settings = ragapp.model.user_settings[doctype] || {};
	...
	update: function (doctype, user_settings) {
		if (ragapp.session.user === "Guest") return Promise.resolve();
		return ragapp.call({
			method: "ragapp.model.utils.user_settings.save",
			args: {
				doctype: doctype,
				user_settings: user_settings,
			},
			callback: function (r) {
				ragapp.model.user_settings[doctype] = r.message;
			},
		});
	},
});

ragapp.get_user_settings = function (doctype, key) {
	var settings = ragapp.model.user_settings[doctype] || {};
	if (key) {
