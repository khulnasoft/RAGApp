...
ragapp.re_route = { "#login": "" };
ragapp.route_titles = {};
ragapp.route_flags = {};
ragapp.route_history = [];
ragapp.view_factory = {};
ragapp.view_factories = [];
ragapp.route_options = null;
ragapp.open_in_new_tab = false;
ragapp.route_hooks = {};
...
ragapp.router = {
	...
	setup() {
		...
		for (let doctype of ragapp.boot.user.can_read) {
			this.routes[this.slug(doctype)] = { doctype: doctype };
		}
		if (ragapp.boot.doctype_layouts) {
			for (let doctype_layout of ragapp.boot.doctype_layouts) {
	...
	async route() {
		...
		if (!ragapp.app) return;

		let sub_path = this.get_sub_path();
		if (ragapp.boot.setup_complete) {
			!ragapp.re_route["setup-wizard"] && (ragapp.re_route["setup-wizard"] = "app");
		} else if (!sub_path.startsWith("setup-wizard")) {
			ragapp.re_route["setup-wizard"] && delete ragapp.re_route["setup-wizard"];
			ragapp.set_route(["setup-wizard"]);
		}
	...
	async convert_to_standard_route(route) {
		...
		if (ragapp.workspaces[route[0]]) {
			// public workspace
			route = ["Workspaces", ragapp.workspaces[route[0]].name];
		} else if (route[0] == "private") {
			// private workspace
			let private_workspace = route[1] && `${route[1]}-${ragapp.user.name.toLowerCase()}`;
			if (!ragapp.workspaces[private_workspace]) {
				ragapp.msgprint(__("Workspace <b>{0}</b> does not exist", [route[1]]));
				return ["Workspaces"];
			}
			route = ["Workspaces", "private", ragapp.workspaces[private_workspace].name];
	...
	set_doctype_route(route) {
		...
		return ragapp.model.with_doctype(doctype_route.doctype).then(() => {
			// doctype route
			let meta = ragapp.get_meta(doctype_route.doctype);
			...
			} else if (ragapp.model.is_single(doctype_route.doctype)) {
	...
	set_history() {
		ragapp.route_history.push(this.current_route);
		ragapp.ui.hide_open_dialog();
	},

	async set_active_sidebar_item() {
		ragapp.app.sidebar.set_active_workspace_item();
	},
	...
	render_page() {
		...
		const factory = ragapp.utils.to_title_case(route[0]);

		if (route[1] && ragapp.views[factory + "Factory"]) {
			route[0] = factory;
			// has a view generator, generate!
			if (!ragapp.view_factory[factory]) {
				ragapp.view_factory[factory] = new ragapp.views[factory + "Factory"]();
			}

			ragapp.view_factory[factory].show();
		} else {
			// show page
			const route_name = ragapp.utils.xss_sanitise(route[0]);
			if (ragapp.views.pageview) {
				ragapp.views.pageview.show(route_name);
			}
		}
	},

	re_route(sub_path) {
		if (ragapp.re_route[sub_path] !== undefined) {
			...
			const re_route_val = this.get_sub_path(ragapp.re_route[sub_path]);
			if (re_route_val === this.current_sub_path) {
				window.history.back();
			} else {
				ragapp.set_route(re_route_val);
			}
	...
	set_title(sub_path) {
		if (ragapp.route_titles[sub_path]) {
			ragapp.utils.set_title(ragapp.route_titles[sub_path]);
		}
	},

	set_route() {
		// set the route (push state) with given arguments
		// example 1: ragapp.set_route('a', 'b', 'c');
		// example 2: ragapp.set_route(['a', 'b', 'c']);
		// example 3: ragapp.set_route('a/b/c');
		...
		return new Promise((resolve) => {
			...
			sub_path += ragapp.route_hash || "";
			ragapp.route_hash = null;
			if (ragapp.open_in_new_tab) {
				localStorage["route_options"] = JSON.stringify(ragapp.route_options);
				window.open(sub_path, "_blank");
				ragapp.open_in_new_tab = false;
			} else {
				try {
					const route_options = ragapp.route_options || {};
	...
	make_url(params) {
		...
		let private_home = `home-${ragapp.user.name.toLowerCase()}`;
		let default_workspace = ragapp.router.slug(ragapp.boot.user.default_workspace?.name || "");

		let workspace =
			ragapp.workspaces[default_workspace] ||
			ragapp.workspaces[private_home] ||
			ragapp.workspaces["home"] ||
			Object.values(ragapp.workspace_map).find((w) => w.app === ragapp.current_app) ||
			Object.values(ragapp.workspaces)[0];

		if (workspace) {
			return (
				"/app/" + (workspace.public ? "" : "private/") + ragapp.router.slug(workspace.name)
			);
		}
	...
	push_state(path, query_params = "") {
		if (window.location.pathname !== path || window.location.search !== query_params) {
			...
			const method = ragapp.route_flags.replace_route ? "replaceState" : "pushState";
	...
	set_route_options_from_url() {
		// set query parameters as ragapp.route_options
		let query_string = window.location.search;

		if (!ragapp.route_options) {
			ragapp.route_options = {};
		}

		if (localStorage.getItem("route_options")) {
			ragapp.route_options = JSON.parse(localStorage.getItem("route_options"));
...
ragapp.get_route = () => ragapp.router.current_route;
ragapp.get_route_str = () => ragapp.router.current_route.join("/");
ragapp.set_route = function () {
	return ragapp.router.set_route.apply(ragapp.router, arguments);
};

ragapp.get_prev_route = function () {
	if (ragapp.route_history && ragapp.route_history.length > 1) {
		return ragapp.route_history[ragapp.route_history.length - 2];
	} else {
		return [];
	}
};

ragapp.set_re_route = function () {
	var tmp = ragapp.router.get_sub_path();
	ragapp.set_route.apply(null, arguments);
	ragapp.re_route[tmp] = ragapp.router.get_sub_path();
};

ragapp.has_route_options = function () {
	return Boolean(Object.keys(ragapp.route_options || {}).length);
};

ragapp.utils.make_event_emitter(ragapp.router);
