...
ragapp.provide("ragapp.realtime");
...
class RealTimeClient {
	...
	get_host(port = 9000) {
		...
		if (window.dev_server) {
			...
			port = ragapp.boot.socketio_port || port.toString() || "9000";
			if (parts.length > 2) {
				host = parts[0] + ":" + parts[1];
			}
			host = host + ":" + port;
		}
		return host + `/${ragapp.boot.sitename}`;
	}
	...
	doc_subscribe(doctype, docname) {
		...
		ragapp.flags.doc_subscribe = true;

		// throttle to 1 per sec
		setTimeout(function () {
			ragapp.flags.doc_subscribe = false;
		}, 1000);
...
ragapp.realtime = new RealTimeClient();

// backward compatibility
ragapp.socketio = ragapp.realtime;
