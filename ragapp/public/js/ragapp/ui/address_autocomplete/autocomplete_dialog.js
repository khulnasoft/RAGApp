...
ragapp.ui.AddressAutocompleteDialog = class AddressAutocompleteDialog {
	constructor(opts) {
	...
	_get_dialog() {
		...
		const dialog = new ragapp.ui.Dialog({
			...
			fields: [
				{
					...
					reqd: 1,
					get_query:
						"ragapp.integrations.doctype.geolocation_settings.geolocation_settings.autocomplete",
					onchange: () => {
						// Disable "Create Address" button if mandatory fields are missing
						ragapp.model.with_doctype("Address", () => {
							const address = this.parse_selected_value();
							const mandatory_fields = ragapp
								.get_meta("Address")
								.fields.filter(
									(field) =>
										field.reqd &&
			...
			primary_action: () => {
				...
				ragapp.db.insert(address).then((doc) => {
			...
			secondary_action: () => {
				...
				const address = this.parse_selected_value();
				ragapp.new_doc("Address", address);
			},
		});
