ragapp.provide("ragapp.ui.keys");

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
ragapp.ui.keys.shortcut_groups = shortcut_groups;

ragapp.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new ragapp.ui.keys.AltShortcutGroup());
	}
...
ragapp.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on("keydown", (e) => {
		let key = (ragapp.ui.keys.key_map[e.which] || "").toLowerCase();
...
ragapp.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = ragapp.ui.keys.standard_shortcuts
			.filter((s) => !s.page)
