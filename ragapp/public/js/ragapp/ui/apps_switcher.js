ragapp.ui.AppsSwitcher = class AppsSwitcher {
	constructor(sidebar) {
	...
	create_app_data_map() {
		ragapp.boot.app_data_map = {};
		for (var app of ragapp.boot.app_data) {
			ragapp.boot.app_data_map[app.app_name] = app;
	...
	add_private_app() {
		...
		ragapp.boot.app_data_map["private"] = app;
	...
	setup_select_app() {
		this.app_switcher_menu.find(".app-item").on("click", (e) => {
			...
			if (item.attr("data-app-name") == "settings") {
				ragapp.quick_edit("Workspace Settings");
				return;
			}
			if (route.startsWith("/app/private")) {
				this.set_current_app("private");
				let ws = Object.values(ragapp.workspace_map).find((ws) => ws.public === 0);
				route += "/" + ragapp.router.slug(ws.title);
		...
		this.add_app_item(
			{
				...
				app_route: "/",
				app_logo_url: "/assets/ragapp/images/web.svg",
			},
	...
	add_settings_select() {
		...
		this.add_app_item({
			...
			app_logo_url: "/assets/ragapp/images/settings-gear.svg",
		});
	...
	set_current_app(app) {
		...
		let app_data = ragapp.boot.app_data_map[app] || ragapp.boot.app_data_map["ragapp"];
		...
		if (ragapp.current_app === app) return;
		ragapp.current_app = app;

		// re-render the sidebar
		ragapp.app.sidebar.make_sidebar();
	}
};
