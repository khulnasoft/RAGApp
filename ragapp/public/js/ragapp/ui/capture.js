// ragapp.ui.Capture
// Author - Achilles Rasquinha <achilles@ragapp.khulnasoft.com>
 ...
 *
 * @example
 * ragapp._.get_data_uri(video)
 * // returns "data:image/pngbase64,..."
 */
ragapp._.get_data_uri = (element) => {
...
/**
 * @description Ragapp's Capture object.
 *
 * @example
 * const capture = ragapp.ui.Capture()
 * capture.show()
 *
 ...
 */
ragapp.ui.Capture = class {
	constructor(options = {}) {
		this.options = ragapp.ui.Capture.OPTIONS;
	...
	set_options(options) {
		this.options = { ...ragapp.ui.Capture.OPTIONS, ...options };
	...
	show() {
		...
		if (cint(ragapp.boot.sysdefaults.force_web_capture_mode_for_uploads)) {
			this.show_for_desktop();
		} else if (ragapp.is_mobile()) {
	...
	build_dialog() {
		...
		me.$template = $(ragapp.ui.Capture.TEMPLATE);
	...
	show_for_desktop() {
		...
		this.render_stream()
			...
			.catch((err) => {
				if (me.options.error) {
					ragapp.show_alert(ragapp.ui.Capture.ERR_MESSAGE, 3);
				}
	...
	setup_take_photo_action() {
		...
		this.dialog.set_primary_action(__("Take Photo"), () => {
			const data_url = ragapp._.get_data_uri(me.video);
...
ragapp.ui.Capture.OPTIONS = {
	title: __("Camera"),
	animate: false,
	error: false,
};
ragapp.ui.Capture.ERR_MESSAGE = __("Unable to load camera.");
ragapp.ui.Capture.TEMPLATE = `
<div class="ragapp-capture">
	<div class="embed-responsive embed-responsive-16by9 fc-stream-container">
		<video class="fc-stream embed-responsive-item">${ragapp.ui.Capture.ERR_MESSAGE}</video>
	</div>
