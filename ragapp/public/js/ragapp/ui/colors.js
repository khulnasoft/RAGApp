...
ragapp.ui.color = {
	get: function (color_name, shade) {
		...
		return ragapp.ui.color_map;
	},
	get_color: function (color_name) {
		const color_names = Object.keys(ragapp.ui.color_map);
		if (color_names.includes(color_name)) {
			return ragapp.ui.color_map[color_name];
		} else {
	...
	all: function () {
		return Object.values(ragapp.ui.color_map).reduce((acc, curr) => acc.concat(curr), []);
	},
	names: function () {
		return Object.keys(ragapp.ui.color_map);
	},
	...
	get_color_name: function (hex) {
		for (const key in ragapp.ui.color_map) {
			const colors = ragapp.ui.color_map[key];
...
ragapp.ui.color_map = ragapp.ui.color.get_color_map();
