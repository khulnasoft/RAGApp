...
ragapp.ui.open_dialogs = [];

ragapp.ui.Dialog = class Dialog extends ragapp.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		...
		this.$wrapper
			.on("hide.bs.modal", function () {
				...
				ragapp.ui.form.get_open_grid_form?.()?.hide_form();

				if (ragapp.ui.open_dialogs[ragapp.ui.open_dialogs.length - 1] === me) {
					ragapp.ui.open_dialogs.pop();
					if (ragapp.ui.open_dialogs.length) {
						window.cur_dialog =
							ragapp.ui.open_dialogs[ragapp.ui.open_dialogs.length - 1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function () {
				...
				ragapp.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.hide_scrollbar(true);
				me.on_page_show && me.on_page_show();
				$(document).trigger("ragapp.ui.Dialog:shown");
	...
	toggle_minimize() {
		...
		this.get_minimize_btn().html(ragapp.utils.icon(icon));
