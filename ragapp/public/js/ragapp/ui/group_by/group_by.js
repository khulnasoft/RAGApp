...
ragapp.ui.GroupBy = class {
	...
	init_group_by_popover() {
		...
		const group_by_template = $(
			ragapp.render_template("group_by", {
			...
			if (!this.aggregate_on_html.length) {
				...
				for (let doctype in this.all_fields) {
					...
					doctype_fields.forEach((field) => {
						...
						if (ragapp.model.is_numeric_field(field.fieldtype)) {
							let field_label = field.label || ragapp.model.unscrub(field.fieldname);
							let option_text =
								doctype == this.doctype
			...
			$(`<div class="group-by-selector">
				<button class="btn btn-default btn-sm group-by-button ellipsis">
					<span class="group-by-icon button-icon">
						${ragapp.utils.icon("es-line-folder-alt")}
					</span>
	...
	get_group_by_docfield() {
		...
		if (this.aggregate_function === "count") {
			...
			docfield = Object.assign(
				{},
				ragapp.meta.docfield_map[this.aggregate_on_doctype][this.aggregate_on_field]
			);
	...
	get_group_by_fields() {
		...
		const standard_fields = ragapp.model.std_fields.filter(
		...
		const standard_fields_filter = (df) =>
			!ragapp.model.no_value_type.includes(df.fieldtype) && !df.report_hide;
