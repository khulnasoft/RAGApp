...
ragapp.provide("ragapp.ui.keys.handlers");

ragapp.ui.keys.setup = function () {
	$(window).on("keydown", function (e) {
		var key = ragapp.ui.keys.get_key(e);
		if (ragapp.ui.keys.handlers[key]) {
			var out = null;
			for (var i = 0, l = ragapp.ui.keys.handlers[key].length; i < l; i++) {
				var handler = ragapp.ui.keys.handlers[key][i];
...
ragapp.ui.keys.standard_shortcuts = standard_shortcuts;
ragapp.ui.keys.add_shortcut = ({
	...
	ragapp.ui.keys.off(shortcut, page);
	// attach new handler
	ragapp.ui.keys.on(shortcut, handler);
...
ragapp.ui.keys.show_keyboard_shortcut_dialog = () => {
	if (ragapp.ui.keys.is_dialog_shown) return;
		...
		let html = shortcuts
			...
			.map((shortcut) => {
				let shortcut_label = shortcut.shortcut
					.split("+")
					.map(ragapp.utils.to_title_case)
					.join("+");
				if (ragapp.utils.is_mac()) {
	...
	let dialog = new ragapp.ui.Dialog({
		...
		on_hide() {
			ragapp.ui.keys.is_dialog_shown = false;
		},
	});
	...
	dialog.show();
	ragapp.ui.keys.is_dialog_shown = true;
};

ragapp.ui.keys.get_key = function (e) {
	var keycode = e.keyCode || e.which;
	var key = ragapp.ui.keys.key_map[keycode] || String.fromCharCode(keycode);
...
ragapp.ui.keys.on = function (key, handler) {
	if (!ragapp.ui.keys.handlers[key]) {
		ragapp.ui.keys.handlers[key] = [];
	}
	ragapp.ui.keys.handlers[key].push(handler);
};

ragapp.ui.keys.off = function (key, page) {
	let handlers = ragapp.ui.keys.handlers[key];
	if (!handlers || handlers.length === 0) return;
	ragapp.ui.keys.handlers[key] = handlers.filter((h) => {
...
ragapp.ui.keys.add_shortcut({
	...
	action: function (e) {
		...
		ragapp.app.trigger_primary_action();
		e.preventDefault();
		return false;
	},
...
ragapp.ui.keys.on("escape", function (e) {
	handle_escape_key();
});

ragapp.ui.keys.on("esc", function (e) {
	handle_escape_key();
});

ragapp.ui.keys.on("enter", function (e) {
	if (window.cur_dialog && cur_dialog.confirm_dialog) {
		cur_dialog.get_primary_btn().trigger("click");
	}
});

ragapp.ui.keys.on("ctrl+down", function (e) {
	const grid_row = ragapp.ui.form.get_open_grid_form();
...
ragapp.ui.keys.on("ctrl+up", function (e) {
	const grid_row = ragapp.ui.form.get_open_grid_form();
...
"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter, i) => {
	ragapp.ui.keys.key_map[65 + i] = letter;
});
