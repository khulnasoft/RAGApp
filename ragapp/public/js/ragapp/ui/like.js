...
ragapp.ui.is_liked = function (doc) {
	return ragapp.ui.get_liked_by(doc).includes(ragapp.session.user);
};

ragapp.ui.get_liked_by = function (doc) {
	return doc._liked_by ? JSON.parse(doc._liked_by) : [];
};

ragapp.ui.toggle_like = function ($btn, doctype, name, callback) {
	const add = $btn.hasClass("not-liked") ? "Yes" : "No";
	// disable click
	$btn.css("pointer-events", "none");

	ragapp.call({
		method: "ragapp.desk.like.toggle_like",
		quiet: true,
		args: {
			doctype: doctype,
			name: name,
		...
		callback: function (r) {
			...
			if (doc) {
				let liked_by = ragapp.ui.get_liked_by(doc);

				if (add === "Yes" && !liked_by.includes(ragapp.session.user)) {
					liked_by.push(ragapp.session.user);
				}

				if (add === "No" && liked_by.includes(ragapp.session.user)) {
					liked_by = liked_by.filter((user) => user !== ragapp.session.user);
				}
...
ragapp.ui.click_toggle_like = function () {
	console.warn("`ragapp.ui.click_toggle_like` is deprecated and has no effect.");
};

ragapp.ui.setup_like_popover = ($parent, selector) => {
	if (ragapp.dom.is_touchscreen()) {
		return;
	}
			...
			content: () => {
				...
				liked_by.forEach((user) => {
					...
					liked_by_list.append(`
						<li data-user=${user}>${ragapp.avatar(user, "avatar-xs")}
							<span>${ragapp.user.full_name(user)}</span>
						</li>
					`);
				});
				...
				liked_by_list.children("li").click((ev) => {
					...
					ragapp.set_route(link_base + user);
				});
