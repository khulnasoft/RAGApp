...
ragapp.ui.ThemeSwitcher = class ThemeSwitcher {
	constructor() {
		this.setup_dialog();
		this.refresh();
	}
	...
	bind_events() {
		this.dialog.$wrapper.on("keydown", (e) => {
			...
			const key = ragapp.ui.keys.get_key(e);
			let increment_by;
	...
	toggle_theme(theme) {
		...
		ragapp.show_alert(__("Theme Changed"), 3);

		ragapp.xcall("ragapp.core.doctype.user.user.switch_theme", {
			theme: toTitle(theme),
		});
	}
...
ragapp.ui.add_system_theme_switch_listener = () => {
	ragapp.ui.dark_theme_media_query.addEventListener("change", () => {
		ragapp.ui.set_theme();
	});
};

ragapp.ui.dark_theme_media_query = window.matchMedia("(prefers-color-scheme: dark)");

ragapp.ui.set_theme = (theme) => {
	const root = document.documentElement;
	let theme_mode = root.getAttribute("data-theme-mode");
	if (!theme) {
		if (theme_mode === "automatic") {
			theme = ragapp.ui.dark_theme_media_query.matches ? "dark" : "light";
		}
	}
