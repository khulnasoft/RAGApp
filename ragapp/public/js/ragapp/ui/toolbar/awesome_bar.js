...
ragapp.provide("ragapp.tags");

ragapp.search.AwesomeBar = class AwesomeBar {
	setup(element) {
		var me = this;
		...
		var awesomplete = new Awesomplete(input, {
			...
			item: function (item, term) {
				...
				let target = "#";
				if (d.route) {
					target = ragapp.router.make_url(
						ragapp.router.convert_from_standard_route(
							ragapp.router.get_route_from_arguments(
			...
			ragapp.utils.debounce(function (e) {
				...
				if (txt && txt.length > 1) {
					...
					me.options = me.options.concat(
						me.deduplicate(ragapp.search.utils.get_recent_pages(txt || ""))
					);
					me.options = me.options.concat(ragapp.search.utils.get_frequent_links());
				}
				me.add_help();
		...
		$input.on("awesomplete-select", function (e) {
			...
			if (item.route_options) {
				ragapp.route_options = item.route_options;
			}
		...
		ragapp.search.utils.setup_recent();
	}
	...
	add_help() {
		this.options.push({
			...
			onclick: function () {
				...
				ragapp.msgprint(txt, __("Search Help"));
			},
		});
	}
	...
	build_options(txt) {
		var options = ragapp.search.utils
			.get_creatables(txt)
			.concat(
				ragapp.search.utils.get_search_in_list(txt),
				ragapp.search.utils.get_doctypes(txt),
				ragapp.search.utils.get_reports(txt),
				ragapp.search.utils.get_pages(txt),
				ragapp.search.utils.get_workspaces(txt),
				ragapp.search.utils.get_dashboards(txt),
				ragapp.search.utils.get_recent_pages(txt || ""),
				ragapp.search.utils.get_executables(txt),
				ragapp.search.utils.get_marketplace_apps(txt)
			);
		if (txt.charAt(0) === "#") {
			options = ragapp.tags.utils.get_tags(txt);
		}
	...
	make_global_search(txt) {
		...
		// 		<span class="ellipsis">Search for ${ragapp.utils.xss_sanitise(txt).bold()}</span>
		// 		<kbd>↵</kbd>
		// 	</span>
		// `);

		// search_text.click(() => {
		// 	ragapp.searchdialog.search.init_search(txt, "global_search");
		// });
		...
		this.options.push({
			label: `
				<span class="flex justify-between text-medium">
					<span class="ellipsis">${__("Search for {0}", [ragapp.utils.xss_sanitise(txt).bold()])}</span>
					<kbd>↵</kbd>
				</span>
			`,
	...
	make_search_in_current(txt) {
		...
		if (route[0] === "List" && txt.indexOf(" in") === -1) {
			...
			const doctype = ragapp.container.page?.list_view?.doctype;
			if (!doctype) return;
		...
		if (first == parseInt(first) || first === "(" || first === "=") {
			...
			try {
				...
				this.options.push({
					...
					default: "Calculator",
					onclick: function () {
						ragapp.msgprint(formatted_value, __("Result"));
					},
				});
			} catch (e) {
				// pass
	...
	make_random(txt) {
		if (txt.toLowerCase().includes("random")) {
			this.options.push({
				...
				value: ragapp.utils.get_random(16),
				onclick: function () {
					ragapp.msgprint(ragapp.utils.get_random(16), __("Result"));
				},
			});
		}
	}
