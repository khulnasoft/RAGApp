...
ragapp.tags.utils = {
	get_tags: function (txt) {
		...
		if (!ragapp.tags.tags) {
			ragapp.tags.utils.fetch_tags();
			return [];
		}

		ragapp.tags.tags.forEach((tag) => {
			const search_result = ragapp.search.utils.fuzzy_search(txt, tag, true);
			if (search_result.score) {
				out.push({
					...
					onclick() {
						...
						ragapp.searchdialog.search.init_search("#".concat(tag), "tags");
					},
				});
			}
		});
	...
	fetch_tags() {
		ragapp.call({
			method: "ragapp.desk.doctype.tag.tag.get_tags_list_for_awesomebar",
			callback: function (r) {
				if (r && r.message) {
					ragapp.tags.tags = $.extend([], r.message);
				}
			},
		});
	},
	...
	get_tag_results: function (tag) {
		...
		return new Promise(function (resolve) {
			ragapp.call({
				method: "ragapp.desk.doctype.tag.tag.get_documents_for_tag",
				args: {
					tag: tag,
				},
				callback: function (r) {
