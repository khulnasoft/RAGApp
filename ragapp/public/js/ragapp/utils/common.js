...
ragapp.avatar = function (
	...
	if (user) {
		// desk
		user_info = ragapp.user_info(user);
	} else {
		// website
		let full_name = title || ragapp.get_cookie("full_name");
		user_info = {
			image: image_url === null ? ragapp.get_cookie("user_image") : image_url,
			fullname: full_name,
			abbr: ragapp.get_abbr(full_name),
			color: ragapp.get_palette(full_name),
		};
	}
...
ragapp.get_avatar = function (css_class, title, image_url = null, remove_color, data_attributes) {
	if (!css_class) {
	...
	if (image_url) {
		...
		let abbr = ragapp.get_abbr(title);
		let style = "";
		if (!remove_color) {
			let color = ragapp.get_palette(title);
...
ragapp.avatar_group = function (users, limit = 4, options = {}) {
	...
	let html = display_users
		.map((user) =>
			ragapp.avatar(user, "avatar-small " + css_class, null, null, false, options.filterable)
		)
		.join("");
		...
		html = `
			...
			<span class="avatar avatar-small ${css_class}">
				<div class="avatar-frame standard-image avatar-extra-count"
					title="${extra_users.map((u) => ragapp.user_info(u).fullname).join(", ")}">
					+${extra_users.length}
				</div>
			</span>
		`;
		...
		avatar_action_html = `
			<span class="avatar avatar-small">
				<div class="avatar-frame avatar-action">
					${ragapp.utils.icon(options.action_icon, "sm")}
				</div>
			</span>
		`;
	}
...
ragapp.ui.scroll = function (element, animate, additional_offset) {
...
ragapp.get_palette = function (txt) {
	if (!txt) return ragapp.palette[8]; // breaks when undefined
	var idx = cint((parseInt(md5(txt).substr(4, 2), 16) + 1) / 5.33);
	return ragapp.palette[idx % 8];
};

ragapp.get_abbr = function (txt, max_length) {
	if (!txt) return "";
	var abbr = "";
...
ragapp.gravatars = {};
ragapp.get_gravatar = function (email_id, size = 0) {
	var param = size ? "s=" + size : "d=retro";
	if (!ragapp.gravatars[email_id]) {
		// TODO: check if gravatar exists
		ragapp.gravatars[email_id] =
			"https://secure.gravatar.com/avatar/" + md5(email_id) + "?" + param;
	}
	return ragapp.gravatars[email_id];
};
...
ragapp.get_cookie = function getCookie(name) {
	return ragapp.get_cookies()[name];
};

ragapp.get_cookies = function getCookies() {
...
ragapp.is_mobile = function () {
	return $(document).width() < 768;
};

ragapp.is_large_screen = function () {
	return $(document).height() > 1180;
};

ragapp.utils.xss_sanitise = function (string, options) {
...
ragapp.utils.sanitise_redirect = (url) => {
	const is_external = (() => {
		return (url) => {
			function domain(url) {
	...
	url = ragapp.utils.strip_url(url);

	return is_external(url)
		? ""
		: sanitise_javascript(ragapp.utils.xss_sanitise(url, { strategies: ["js"] }));
};

ragapp.utils.strip_url = (url) => {
...
ragapp.utils.new_auto_repeat_prompt = function (frm) {
	const fields = [
		{
			fieldname: "frequency",
			fieldtype: "Select",
		...
		},
		{
			...
			label: __("Start Date"),
			reqd: 1,
			default: ragapp.datetime.nowdate(),
		},
		{
			fieldname: "end_date",
			fieldtype: "Date",
	...
	ragapp.prompt(
		...
		function (values) {
			ragapp.call({
				method: "ragapp.automation.doctype.auto_repeat.auto_repeat.make_auto_repeat",
				args: {
...
ragapp.utils.get_page_view_count = function (route) {
	return ragapp.call("ragapp.website.doctype.web_page_view.web_page_view.get_page_view_count", {
		path: route,
	});
};
