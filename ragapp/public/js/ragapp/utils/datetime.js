...
ragapp.provide("ragapp.datetime");

ragapp.defaultDateFormat = "YYYY-MM-DD";
ragapp.defaultTimeFormat = "HH:mm:ss";
ragapp.defaultDatetimeFormat = ragapp.defaultDateFormat + " " + ragapp.defaultTimeFormat;
moment.defaultFormat = ragapp.defaultDateFormat;

ragapp.provide("ragapp.datetime");

$.extend(ragapp.datetime, {
	convert_to_user_tz: function (date, format) {
		...
		let date_obj = null;
		if (ragapp.boot.time_zone && ragapp.boot.time_zone.system && ragapp.boot.time_zone.user) {
			date_obj = moment
				.tz(date, ragapp.boot.time_zone.system)
				.clone()
				.tz(ragapp.boot.time_zone.user);
		} else {
			date_obj = moment(date);
		}

		return format === false ? date_obj : date_obj.format(ragapp.defaultDatetimeFormat);
	},
	...
	convert_to_system_tz: function (date, format) {
		...
		if (ragapp.boot.time_zone && ragapp.boot.time_zone.system && ragapp.boot.time_zone.user) {
			date_obj = moment
				.tz(date, ragapp.boot.time_zone.user)
				.clone()
				.tz(ragapp.boot.time_zone.system);
		} else {
			date_obj = moment(date);
		}
	...
	is_system_time_zone: function () {
		if (ragapp.boot.time_zone && ragapp.boot.time_zone.system && ragapp.boot.time_zone.user) {
			return (
				moment().tz(ragapp.boot.time_zone.system).utcOffset() ===
				moment().tz(ragapp.boot.time_zone.user).utcOffset()
			);
		}
	...
	is_timezone_same: function () {
		return ragapp.datetime.is_system_time_zone();
	},

	str_to_obj: function (d) {
		return moment(d, ragapp.defaultDatetimeFormat)._d;
	},
	...
	obj_to_user: function (d) {
		return moment(d).format(ragapp.datetime.get_user_date_fmt().toUpperCase());
	},
	...
	get_user_time_fmt: function () {
		return (ragapp.sys_defaults && ragapp.sys_defaults.time_format) || "HH:mm:ss";
	},

	get_user_date_fmt: function () {
		return (ragapp.sys_defaults && ragapp.sys_defaults.date_format) || "yyyy-mm-dd";
	},
	...
	str_to_user: function (val, only_time = false, only_date = false) {
		if (!val) return "";
		const user_date_fmt = ragapp.datetime.get_user_date_fmt().toUpperCase();
		const user_time_fmt = ragapp.datetime.get_user_time_fmt();
		let user_format = user_time_fmt;

		if (only_time) {
			let date_obj = moment(val, ragapp.defaultTimeFormat);
			return date_obj.format(user_format);
		} else if (only_date) {
			let date_obj = moment(val, ragapp.defaultDateFormat);
			return date_obj.format(user_date_fmt);
		} else {
			let date_obj = moment.tz(val, ragapp.boot.time_zone.system);
			if (typeof val !== "string" || val.indexOf(" ") === -1) {
				user_format = user_date_fmt;
			} else {
				user_format = user_date_fmt + " " + user_time_fmt;
			}
			return date_obj.clone().tz(ragapp.boot.time_zone.user).format(user_format);
		}
	},

	get_datetime_as_string: function (d) {
		let time_format = ragapp?.boot?.sysdefaults?.time_format || ragapp.defaultTimeFormat;
		let datetime_format = ragapp.defaultDateFormat + " " + time_format;
		return moment(d).format(datetime_format);
	},

	user_to_str: function (val, only_time = false) {
		var user_time_fmt = ragapp.datetime.get_user_time_fmt();
		if (only_time) {
			return moment(val, user_time_fmt).format(ragapp.defaultTimeFormat);
		}

		var user_fmt = ragapp.datetime.get_user_date_fmt().toUpperCase();
	...
	user_to_obj: function (d) {
		return ragapp.datetime.str_to_obj(ragapp.datetime.user_to_str(d));
	},
	...
	now_date: function (as_obj = false) {
		return ragapp.datetime._date(ragapp.defaultDateFormat, as_obj);
	},

	now_time: function (as_obj = false) {
		return ragapp.datetime._date(ragapp.defaultTimeFormat, as_obj);
	},

	now_datetime: function (as_obj = false) {
		return ragapp.datetime._date(ragapp.defaultDatetimeFormat, as_obj);
	},

	system_datetime: function (as_obj = false) {
		return ragapp.datetime._date(ragapp.defaultDatetimeFormat, as_obj, true);
	},

	_date: function (format, as_obj = false, system_time = false) {
		let time_zone = ragapp.boot.time_zone?.system || ragapp.sys_defaults.time_zone;
		...
		if (!system_time) {
			time_zone = ragapp.boot.time_zone?.user || time_zone;
		}
		let date = moment.tz(time_zone);

		return as_obj ? ragapp.datetime.moment_to_date_obj(date) : date.format(format);
	},
	...
	nowdate: function () {
		return ragapp.datetime.now_date();
	},
	...
	validate: function (d) {
		return moment(
			d,
			[ragapp.defaultDateFormat, ragapp.defaultDatetimeFormat, ragapp.defaultTimeFormat],
			true
		).isValid();
	},

	get_first_day_of_the_week_index() {
		const first_day_of_the_week = ragapp.sys_defaults.first_day_of_the_week || "Sunday";
