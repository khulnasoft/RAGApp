function prettyDate(date, mini) {
	...
	if (typeof date == "string") {
		date = ragapp.datetime.convert_to_user_tz(date);
		date = new Date(
			(date || "")
				.replace(/-/g, "/")
				.replace(/[TZ]/g, " ")
	...
	let diff =
		(new Date(ragapp.datetime.now_datetime().replace(/-/g, "/")).getTime() - date.getTime()) /
		1000;
...
window.comment_when = function (datetime, mini) {
	var timestamp = ragapp.datetime.str_to_user ? ragapp.datetime.str_to_user(datetime) : datetime;
	return (
		'<span class="ragapp-timestamp ' +
		(mini ? " mini" : "") +
		'" data-timestamp="' +
		datetime +
		'" title="' +
...
ragapp.datetime.comment_when = comment_when;
ragapp.datetime.prettyDate = prettyDate;

ragapp.datetime.refresh_when = function () {
	if (jQuery) {
		$(".ragapp-timestamp").each(function () {
