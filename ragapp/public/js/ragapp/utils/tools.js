...
ragapp.provide("ragapp.tools");

ragapp.tools.downloadify = function (data, roles, title) {
	if (roles && roles.length && !has_common(roles, roles)) {
		ragapp.msgprint(
			__("Export not allowed. You need {0} role to export.", [ragapp.utils.comma_or(roles)])
		);
		return;
	}

	var filename = title + ".csv";
	var csv_data = ragapp.tools.to_csv(data);
...
ragapp.markdown = function (txt) {
	if (!ragapp.md2html) {
		ragapp.md2html = new showdown.Converter({ tables: true });
	}
	...
	return ragapp.md2html.makeHtml(txt);
};

ragapp.tools.to_csv = function (data) {
	var res = [];
