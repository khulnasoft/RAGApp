...
ragapp.breadcrumbs = {
	...
	add(module, doctype, type) {
		...
		this.all[ragapp.breadcrumbs.current_page()] = obj;
		this.update();
	},

	current_page() {
		return ragapp.get_route_str();
	},

	update() {
		var breadcrumbs = this.all[ragapp.breadcrumbs.current_page()];
		...
		if (
			...
			ragapp.workspace_map[breadcrumbs.workspace]?.app &&
			ragapp.workspace_map[breadcrumbs.workspace]?.app != ragapp.current_app
		) {
			let app = ragapp.workspace_map[breadcrumbs.workspace].app;
	...
	get last_route() {
		return ragapp.route_history.slice(-2)[0];
	},
	...
	set_workspace_breadcrumb(breadcrumbs) {
		...
		if (
			...
			(breadcrumbs.module_info.blocked ||
				!ragapp.visible_modules.includes(breadcrumbs.module_info.module))
		) {
			return;
		}

		this.append_breadcrumb_element(
			`/app/${ragapp.router.slug(breadcrumbs.workspace)}`,
	...
	set_workspace(breadcrumbs) {
		...
		if (this.last_route?.[0] == "Workspaces") {
			...
			if (
				breadcrumbs.module &&
				ragapp.boot.module_wise_workspaces[breadcrumbs.module]?.includes(last_workspace)
			) {
				breadcrumbs.workspace = last_workspace;
			}
		} else {
			// choose from __workspaces
			const doctype_meta = ragapp.get_meta(breadcrumbs.doctype);
			...
			if (breadcrumbs.module) {
				...
				breadcrumbs.module_info = ragapp.get_module(breadcrumbs.module);

				// set workspace
				if (
					breadcrumbs.module_info &&
					ragapp.boot.module_wise_workspaces[breadcrumbs.module]
				) {
					breadcrumbs.workspace =
						ragapp.boot.module_wise_workspaces[breadcrumbs.module][0];
				}
			}
		}
	},
	...
	set_list_breadcrumb(breadcrumbs) {
		...
		const doctype_meta = ragapp.get_meta(doctype);
		if (
			(doctype === "User" && !ragapp.user.has_role("System Manager")) ||
			doctype_meta?.issingle
		) {
			// no user listview for non-system managers and single doctypes
		} else {
			let route;
			const doctype_route = ragapp.router.slug(ragapp.router.doctype_layout || doctype);
			if (doctype_meta?.is_tree) {
				let view = ragapp.model.user_settings[doctype].last_view || "Tree";
	...
	set_form_breadcrumb(breadcrumbs, view) {
		const doctype = breadcrumbs.doctype;
		let docname = ragapp.get_route().slice(2).join("/");
		let docname_title;
		...
		let form_route = `/app/${ragapp.router.slug(doctype)}/${encodeURIComponent(docname)}`;
		...
		if (view === "form") {
			...
			last_crumb.click((event) => {
				...
				ragapp.utils.copy_to_clipboard(last_crumb.text());
			});
		}
	},
	...
	set_dashboard_breadcrumb(breadcrumbs) {
		const doctype = breadcrumbs.doctype;
		const docname = ragapp.get_route()[1];
		let dashboard_route = `/app/${ragapp.router.slug(doctype)}/${docname}`;
		$(`<li><a href="${dashboard_route}">${__(docname)}</a></li>`).appendTo(this.$breadcrumbs);
	},

	setup_modules() {
		if (!ragapp.visible_modules) {
			ragapp.visible_modules = $.map(ragapp.boot.allowed_workspaces, (m) => {
				return m.module;
			});
		}
	},
	...
	rename(doctype, old_name, new_name) {
		...
		delete ragapp.breadcrumbs.all[old_route_str];
		this.update();
	},
