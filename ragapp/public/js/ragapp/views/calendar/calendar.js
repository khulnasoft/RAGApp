...
ragapp.provide("ragapp.views.calendar");
ragapp.provide("ragapp.views.calendars");

ragapp.views.CalendarView = class CalendarView extends ragapp.views.ListView {
	static load_last_view() {
		const route = ragapp.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = ragapp.get_user_settings(doctype)["Calendar"] || {};
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.calendar_settings = ragapp.views.Calendar[this.doctype] || {};
			this.calendar_name = ragapp.get_route()[3];
		});
	}
	...
	render() {
		...
		this.load_lib
			...
			.then((options) => {
				this.calendar = new ragapp.views.Calendar(options);
			});
	}
	...
	get_calendar_preferences() {
		...
		return new Promise((resolve) => {
			if (calendar_name === "default") {
				Object.assign(options, ragapp.views.calendar[this.doctype]);
				resolve(options);
			} else {
				ragapp.model.with_doc("Calendar View", calendar_name, () => {
					const doc = ragapp.get_doc("Calendar View", calendar_name);
					if (!doc) {
						ragapp.show_alert(
						...
						ragapp.set_route("List", this.doctype, "Calendar", "default");
						return;
					}
...
ragapp.views.Calendar = class Calendar {
	constructor(options) {
		$.extend(this, options);
	...
	make_page() {
		...
		$.each(ragapp.boot.calendars, function (i, doctype) {
			if (ragapp.model.can_read(doctype)) {
				me.page.add_menu_item(__(doctype), function () {
					ragapp.set_route("List", doctype, "Calendar");
				});
			}
		});
	...
	make() {
		...
		this.footnote_area = ragapp.utils.set_footnote(
			this.footnote_area,
			this.$wrapper,
		...
		this.fullCalendar = new ragapp.FullCalendar(this.$cal[0], this.cal_options);
	...
	set_css() {
		...
		this.$wrapper
			...
			.attr("class", "")
			.html(ragapp.utils.icon("left"));
		this.$wrapper
			.find(`.fc-next-button span`)
			.attr("class", "")
			.html(ragapp.utils.icon("right"));
		if (this.$wrapper.find(".fc-today-button svg").length == 0)
			this.$wrapper.find(".fc-today-button").prepend(ragapp.utils.icon("today"));
	...
	get_system_datetime(date) {
		return ragapp.datetime.convert_to_system_tz(date, true);
	}
	setup_options(defaults) {
		var me = this;
		defaults.meridiem = "false";
		this.cal_options = {
			plugins: ragapp.FullCalendar.Plugins,
			initialView: defaults.initialView || "dayGridMonth",
			locale: ragapp.boot.lang,
			firstDay: 1,
			headerToolbar: {
			...
			events: function (info, successCallback, failureCallback) {
				return ragapp.call({
					method: me.get_events_method || "ragapp.desk.calendar.get_events",
					type: "GET",
			...
			eventClick: function (info) {
				...
				if (ragapp.model.can_read(doctype)) {
					ragapp.set_route("Form", doctype, info.event.id);
				}
			},
			...
			select: function (info) {
				...
				var event = ragapp.model.get_new_doc(me.doctype);
				...
				ragapp.set_route("Form", me.doctype, event.name);
			},
	...
	prepare_events(events) {
		...
		return (events || []).map((d) => {
			d.id = d.name;
			d.editable = ragapp.model.can_write(d.doctype || me.doctype);
			...
			if (d.convertToUserTz) {
				d.start = ragapp.datetime.convert_to_user_tz(d.start);
				d.end = ragapp.datetime.convert_to_user_tz(d.end);
			}

			// show event on single day if start or end date is invalid
			if (!ragapp.datetime.validate(d.start) && d.end) {
				d.start = ragapp.datetime.add_days(d.end, -1);
			}

			if (d.start && !ragapp.datetime.validate(d.end)) {
				d.end = ragapp.datetime.add_days(d.start, 1);
			}

			me.prepare_colors(d);

			d.title = ragapp.utils.html2text(d.title);
	...
	prepare_colors(d) {
		...
		if (this.get_css_class) {
			...
			if (color_name.startsWith("#")) {
				color_name = ragapp.ui.color.validate_hex(color_name) ? color_name : "blue";
			}

			d.backgroundColor = ragapp.ui.color.get(color_name, "extra-light");
			d.textColor = ragapp.ui.color.get(color_name, "dark");
		} else {
			color = d.color;
			if (!ragapp.ui.color.validate_hex(color) || !color) {
				color = ragapp.ui.color.get("blue", "extra-light");
			}
			d.backgroundColor = color;
			d.textColor = ragapp.ui.color.get_contrast_color(color);
		}
		return d;
	}
	update_event(event, revertFunc) {
		var me = this;
		ragapp.model.remove_from_locals(me.doctype, event.id);
		return ragapp.call({
			method: me.update_event_method || "ragapp.desk.calendar.update_event",
			args: me.get_update_args(event),
			callback: function (r) {
				if (r.exc) {
					ragapp.show_alert(__("Unable to update event"));
					revertFunc();
				}
			},
			error: function () {
