...
ragapp.views.FileView = class FileView extends ragapp.views.ListView {
	static load_last_view() {
		const route = ragapp.get_route();
		if (route.length === 2) {
			const view_user_settings = ragapp.get_user_settings("File", "File");
			ragapp.set_route(
				"List",
				"File",
				view_user_settings.last_folder || ragapp.boot.home_folder
			);
			return true;
		}
	...
	setup_view() {
		...
		this.page.add_button(__("Toggle Grid View"), () => {
			ragapp.views.FileView.grid_view = !ragapp.views.FileView.grid_view;
			this.refresh();
		});
	}
	...
	get_args() {
		...
		if (ragapp.views.FileView.grid_view) {
			Object.assign(args, {
		...
		return [
			{
				label: __("Home"),
				action: () => {
					ragapp.set_route("List", "File", "Home");
				},
			},
			{
				label: __("New Folder"),
					...
					ragapp.prompt(
						...
						(values) => {
							if (values.value.indexOf("/") > -1) {
								ragapp.throw(__("Folder name should not include '/' (slash)"));
							}
							const data = {
							...
							ragapp.call({
								method: "ragapp.core.api.file.create_new_folder",
								args: data,
							});
						},
			...
			},
			{
				...
				action: () => {
					new ragapp.ui.FileUploader({
						...
						on_success: (file) => {
							ragapp.show_alert(__("Unzipping files..."));
							ragapp
								.call("ragapp.core.api.file.unzip_file", {
									name: file.name,
								})
								.then((r) => {
									if (r.message) {
										ragapp.show_alert(__("Unzipped {0} files", [r.message]));
									}
								});
						},
					});
	...
	add_file_action_buttons() {
		this.$cut_button = this.page
			.add_button(__("Cut"), () => {
				ragapp.file_manager.cut(this.get_checked_items(), this.current_folder);
				this.$checks.parents(".file-wrapper").addClass("cut");
			})
			.hide();

		this.$paste_btn = this.page
			.add_button(__("Paste"), () => ragapp.file_manager.paste(this.current_folder))
			.hide();

		this.page.add_actions_menu_item(__("Export as zip"), () => {
			let docnames = this.get_checked_items(true);
			if (docnames.length) {
				open_url_post("/api/method/ragapp.core.api.file.zip_files", {
	...
	set_fields() {
		this.fields = this.meta.fields
			.filter((df) => ragapp.model.is_value_type(df.fieldtype) && !df.hidden)
			.map((df) => df.fieldname)
	...
	prepare_datum(d) {
		...
		if (d.is_folder) {
			...
			type = "folder";
		} else if (ragapp.utils.is_image_file(d.file_name)) {
			icon_class = "image";
			type = "image";
		} else {
			icon_class = "file";
	...
	before_render() {
		super.before_render();
		ragapp.model.user_settings.save("File", "grid_view", ragapp.views.FileView.grid_view);
	...
	get_header_html() {
		...
		let header_selector_html = !ragapp.views.FileView.grid_view
			? `<input class="level-item list-check-all hidden-xs" type="checkbox" title="${__(
					"Select All"
			  )}">`
			: "";

		let header_columns_html = !ragapp.views.FileView.grid_view
	...
	get_creation_date(file) {
		...
		let created_on;
		if (date === ragapp.datetime.now_date()) {
			created_on = comment_when(file.creation);
		} else {
			created_on = ragapp.datetime.str_to_user(date);
		}
		return created_on;
	}
	...
	get_left_html(file) {
		...
		const file_size = file.file_size ? ragapp.form.formatters.FileSize(file.file_size) : "";
		...
		return `
			<div class="list-row-col ellipsis list-subject level">
				...
				<span class="level-item  ellipsis" title="${ragapp.utils.escape_html(file.file_name)}">
					<a class="ellipsis" href="${route_url}" title="${ragapp.utils.escape_html(file.file_name)}">
						${file.subject_html}
					</a>
				</span>
			</div>
		...
		this.$result.on("drop", (e) => {
			...
			if (dataTransfer.files && dataTransfer.files.length > 0) {
				...
				if ($el.parents(".folders").length !== 0) {
					...
					ragapp.file_manager.paste(folder_name);
					ragapp.show_alert(`File ${file_name} moved to ${folder_name}`);
				}
			}
		});
	}
	...
	toggle_cut_paste_buttons() {
		const hide_paste_btn =
			!ragapp.file_manager.can_paste ||
			ragapp.file_manager.old_folder === this.current_folder;
...
ragapp.views.FileView.grid_view = ragapp.get_user_settings("File").grid_view || false;
