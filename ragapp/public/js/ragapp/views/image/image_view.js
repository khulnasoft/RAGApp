/**
 * ragapp.views.ImageView
 */
ragapp.provide("ragapp.views");

ragapp.views.ImageView = class ImageView extends ragapp.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = ragapp.utils.escape_html(title);
	...
	get_attached_images() {
		return ragapp
			.call({
				method: "ragapp.core.api.file.get_attached_images",
				args: {
	...
	setup_gallery() {
		var me = this;
		this.gallery = new ragapp.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
	...
	get required_libs() {
		return [
			"assets/ragapp/node_modules/photoswipe/src/photoswipe.css",
			"photoswipe.bundle.js",
		];
	}
};

ragapp.views.GalleryView = class GalleryView {
	constructor(opts) {
		$.extend(this, opts);
		var me = this;
		me.prepare();
	...
	prepare() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = ragapp.render_template("photoswipe_dom");
	...
	_show(docname) {
		...
		// init
		this.pswp = new ragapp.PhotoSwipe(options);
		this.pswp.init();
	}
};
