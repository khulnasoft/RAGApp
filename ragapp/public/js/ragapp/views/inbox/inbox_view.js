/**
 * ragapp.views.InboxView
 */

ragapp.provide("ragapp.views");

ragapp.views.InboxView = class InboxView extends ragapp.views.ListView {
	static load_last_view() {
		const route = ragapp.get_route();
		if (!route[3] && ragapp.boot.email_accounts.length) {
			let email_account;
			if (ragapp.boot.email_accounts[0].email_id == "All Accounts") {
				email_account = "All Accounts";
			} else {
				email_account = ragapp.boot.email_accounts[0].email_account;
			}
			ragapp.set_route("List", "Communication", "Inbox", email_account);
			return true;
		...
		function is_valid(email_account) {
			return ragapp.boot.email_accounts.find((d) => d.email_account === email_account);
		}
	}
	...
	setup_defaults() {
		...
		this.email_account = ragapp.get_route()[3];
	...
	get_seen_class(doc) {
		return Boolean(doc.seen) || JSON.parse(doc._seen || "[]").includes(ragapp.session.user)
			? ""
			: "bold";
	}
	...
	get_meta_html(email) {
		...
		if (email.reference_doctype && email.reference_doctype !== this.doctype) {
			link = `<a class="text-muted grey"
				href="${ragapp.utils.get_form_link(email.reference_doctype, email.reference_name)}"
	...
	get_inbox_filters() {
		...
		if (email_account === "Sent") {
			...
			filters = default_filters.concat([
				...
				["Communication", "email_account", "in", ragapp.boot.all_accounts, true],
			]);
		} else {
			var op = "=";
			if (email_account == "All Accounts") {
				op = "in";
				email_account = ragapp.boot.all_accounts;
			}
	...
	get_no_result_message() {
		...
		} else if (!email_account && !ragapp.boot.email_accounts.length) {
		...
		const html = ragapp.model.can_create(args.doctype)
			? `<p>${args.msg}</p>
			<p>
	...
	make_new_doc() {
		if (!this.email_account && !ragapp.boot.email_accounts.length) {
			ragapp.route_options = {
				email_id: ragapp.session.user_email,
			};
			ragapp.new_doc("Email Account");
		} else {
