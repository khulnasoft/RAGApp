...
ragapp.provide("ragapp.interaction_settings");

ragapp.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		ragapp.model.with_doctype("Event", () => {
			let categories = ragapp.meta
	...
	setup_attach() {
		...
		attach.find(".add-more-attachments a").on("click", () => new ragapp.ui.FileUploader(args));
		this.render_attach();
	}
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)) {
			interaction_values["owner"] = ragapp.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = ragapp.session.user;
		}
		return ragapp.call({
	...
	add_attachments(doc, attachments) {
		ragapp.call({
			method: "ragapp.utils.file_manager.add_attachments",
			args: {
				doctype: doc.doctype,
				name: doc.name,
