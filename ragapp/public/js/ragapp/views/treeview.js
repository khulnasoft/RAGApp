...
ragapp.provide("ragapp.treeview_settings");
ragapp.provide("ragapp.views.trees");
window.cur_tree = null;

ragapp.views.TreeFactory = class TreeFactory extends ragapp.views.Factory {
	make(route) {
		ragapp.model.with_doctype(route[1], function () {
			var options = {
				doctype: route[1],
				meta: ragapp.get_meta(route[1]),
			};

			if (
				!ragapp.treeview_settings[route[1]] &&
				!ragapp.meta.get_docfield(route[1], "is_group")
			) {
				ragapp.msgprint(__("Tree view is not available for {0}", [route[1]]));
				return false;
			}
			$.extend(options, ragapp.treeview_settings[route[1]] || {});
			ragapp.views.trees[options.doctype] = new ragapp.views.TreeView(options);
		});
	}
	...
	on_show() {
		...
		let treeview = ragapp.views.trees[route[1]];
...
ragapp.views.TreeView = class TreeView {
	constructor(opts) {
		var me = this;
		...
		this.get_tree_nodes = me.opts.get_tree_nodes || "ragapp.desk.treeview.get_children";
	...
	get_permissions() {
		this.can_read = ragapp.model.can_read(this.doctype);
		this.can_create =
			ragapp.boot.user.can_create.indexOf(this.doctype) !== -1 ||
			ragapp.boot.user.in_create.indexOf(this.doctype) !== -1;
		this.can_write = ragapp.model.can_write(this.doctype);
		this.can_delete = ragapp.model.can_delete(this.doctype);
	}
	make_page() {
		var me = this;
		if (!this.opts || !this.opts.do_not_make_page) {
			this.parent = ragapp.container.add_page(this.page_name);
			$(this.parent).addClass("treeview");
			ragapp.ui.make_app_page({ parent: this.parent, single_column: true });
			this.page = this.parent.page;
			ragapp.container.change_to(this.page_name);
			ragapp.breadcrumbs.add(
			...
			this.page.main.addClass("ragapp-card");
		} else {
			this.page = this.opts.page;
			$(this.page[0]).addClass("ragapp-card");
		}

		if (ragapp.meta.has_field(me.doctype, "disabled")) {
	...
	make_filters() {
		var me = this;
		ragapp.treeview_settings.filters = [];
		$.each(this.opts.filters || [], function (i, filter) {
			if (ragapp.route_options && ragapp.route_options[filter.fieldname]) {
				filter.default = ragapp.route_options[filter.fieldname];
			}
	...
	rebuild_tree() {
		let me = this;
		ragapp.call({
			method: "ragapp.utils.nestedset.rebuild_tree",
			args: {
				doctype: me.doctype,
			},
			callback: function (r) {
	...
	select_node(node) {
		...
		if (this.opts.view_template) {
			...
			$(
				ragapp.render_template(me.opts.view_template, {
					data: node.data,
					doctype: me.doctype,
				})
		...
		var toolbar = [
			{
				...
				click: function (node) {
					ragapp.set_route("Form", me.doctype, node.label);
				},
			},
			{
				label: __("Add Child"),
			...
			},
			{
				...
				condition: function (node) {
					...
					if (me.doctype && ragapp.get_meta(me.doctype)) {
						if (!ragapp.get_meta(me.doctype).allow_rename) allow_rename = false;
					}
					return !node.is_root && me.can_write && allow_rename;
				},
				click: function (node) {
					ragapp.model.rename_doc(me.doctype, node.label, function (new_name) {
			...
			},
			{
				...
				click: function (node) {
					ragapp.model.delete_doc(me.doctype, node.label, function () {
						node.parent.remove();
					});
				},
				btnClass: "hidden-xs",
	...
	new_node() {
		...
		if (!(node && node.expandable)) {
			ragapp.msgprint(__("Select a group node first."));
			return;
		}
		...
		d.set_primary_action(__("Create New"), function () {
			...
			d.hide();
			ragapp.dom.freeze(__("Creating {0}", [me.doctype]));

			$.extend(args, v);
			return ragapp.call({
				method: me.opts.add_tree_node || "ragapp.desk.treeview.add_node",
				args: args,
				callback: function (r) {
					if (!r.exc) {
	...
	print_tree() {
		if (!ragapp.model.can_print(this.doctype)) {
			ragapp.msgprint(__("You are not allowed to print this report"));
			return false;
		}
		var tree = $(".tree:visible").html();
		var me = this;
		ragapp.ui.get_print_settings(false, function (print_settings) {
			var title = __(me.docname || me.doctype);
			ragapp.render_tree({ title: title, tree: tree, print_settings: print_settings });
			ragapp.call({
				method: "ragapp.core.doctype.access_log.access_log.make_access_log",
				args: {
					doctype: me.doctype,
	...
	set_menu_item() {
		...
		this.menu_items = [
			{
				label: __("View List"),
				action: function () {
					ragapp.set_route("List", me.doctype);
				},
			},
			{
				label: __("Print"),
		...
		if (
			...
			ragapp.meta.has_field(me.doctype, "lft") &&
			ragapp.meta.has_field(me.doctype, "rgt")
		) {
