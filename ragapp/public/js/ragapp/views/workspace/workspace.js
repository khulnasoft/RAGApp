...
ragapp.standard_pages["Workspaces"] = function () {
	var wrapper = ragapp.container.add_page("Workspaces");
	...
	ragapp.workspace = new ragapp.views.Workspace(wrapper);
	$(wrapper).bind("show", function () {
		ragapp.workspace.show();
	});
};

ragapp.views.Workspace = class Workspace {
	constructor(wrapper) {
		this.wrapper = $(wrapper);
		this.page = wrapper.page;
		this.blocks = ragapp.workspace_block.blocks;
		...
		this.sidebar = ragapp.app.sidebar;
		this.app_switcher_menu = ragapp.app.app_switcher_menu;
		this.sidebar.setup_pages();
		this.cached_pages = $.extend(true, {}, ragapp.boot.sidebar_pages);
		this.has_access = ragapp.boot.sidebar_pages.has_access;
		this.has_create_access = ragapp.boot.sidebar_pages.has_create_access;
		this.show();
	...
	`).appendTo(this.body);
		...
		this.body.find(".btn-edit-workspace").on("click", async () => {
			...
			this.editor.isReady.then(() => {
				...
				ragapp.app.sidebar.toggle_sorting();
			});
		});
	}

	get_pages() {
		return ragapp.xcall("ragapp.desk.desktop.get_workspace_sidebar_items", null, "GET");
	}
	...
	show() {
		...
		if (!ragapp.router.current_route[0]) {
			ragapp.route_flags.replace_route = true;
			ragapp.set_route(ragapp.router.slug(page.public ? page.name : "private/" + page.name));
			return;
		}
	...
	get_data(page) {
		return ragapp
			.call({
				method: "ragapp.desk.desktop.get_desktop_page",
				args: {
			...
			.then((data) => {
				...
				return ragapp.dashboard_utils.get_dashboard_settings().then((settings) => {
					if (settings) {
	...
	get_page_to_show() {
		...
		if (ragapp.boot.user.default_workspace) {
			default_page = {
				name: ragapp.boot.user.default_workspace.name,
				public: ragapp.boot.user.default_workspace.public,
			};
		} else if (
		...
		if (this.sidebar.all_pages.length) {
			...
			if (!this._page.public) {
				...
				if (!app && this._page.module) {
					app = ragapp.boot.module_app[ragapp.router.slug(this._page.module)];
				}
				if (!app) app = "ragapp";
			}
			...
			if (this.pages && this.pages[current_page.name]) {
				...
				await ragapp.after_ajax(() => this.get_data(current_page));
			}
			...
			ragapp.app.sidebar.apps_switcher.set_current_app(app);
			this.wrapper.find(".workspace-title").html(__(this._page.title));
			this.wrapper
				.find(".workspace-icon")
				.html(ragapp.utils.icon(this._page.icon || "folder-normal", "md"));
	...
	setup_customization_buttons(page) {
		...
		this.page.set_secondary_action(__("Discard"), async () => {
			...
			await this.editor.readOnly.toggle();
			this.is_read_only = true;
			ragapp.boot.sidebar_pages = this.cached_pages;
			this.reload();
			ragapp.show_alert({ message: __("Customizations Discarded"), indicator: "info" });
		});

		if (page.name && this.has_access) {
			this.page.add_inner_button(__("Settings"), () => {
				ragapp.set_route(`workspace/${page.name}`);
			});
		}
	}
	...
	initialize_new_page() {
		...
		const d = new ragapp.ui.Dialog({
			...
			primary_action: (values) => {
				...
				let name = values.title + (values.is_public ? "" : "-" + ragapp.session.user);
				let blocks = [
					{
						type: "header",
				...
				let new_page = {
					...
					for_user: values.is_public ? "" : ragapp.session.user,
					icon: values.icon,
					indicator_color: values.indicator_color,
					parent_page: values.parent || "",
					is_editable: true,
					selected: true,
					app: ragapp.current_app,
					type: values.type,
					...
					this.editor
						...
						.then(async () => {
							...
							this.create_page(new_page).then(() => {
								...
								let route = pre_url + ragapp.router.slug(new_page.title);
	...
	create_page(new_page) {
		return new Promise((resolve) => {
			ragapp.call({
				method: "ragapp.desk.doctype.workspace.workspace.new_page",
				args: {
					new_page: new_page,
				},
				callback: (r) => {
					if (r.message) {
						...
						ragapp.boot.sidebar_pages = r.message;
						this.sidebar.setup_pages();

						if (!ragapp.boot.app_data_map["private"] && new_page.public === 0) {
	...
	initialize_editorjs(blocks) {
		this.tools = {
			...
			HeaderSize: ragapp.workspace_block.tunes["header_size"],
		};
	...
	save_page(page) {
		...
		return this.editor
			...
			.then((outputData) => {
				...
				page.content = JSON.stringify(blocks);
				ragapp.call({
					method: "ragapp.desk.doctype.workspace.workspace.save_page",
					args: {
						name: page.name,
	...
	create_page_skeleton() {
		...
		this.body.prepend(ragapp.render_template("workspace_loading_skeleton"));
	...
	register_awesomebar_shortcut() {
		"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter) => {
			...
			ragapp.ui.keys.add_shortcut({ shortcut: letter, ...default_shortcut });
			ragapp.ui.keys.add_shortcut({ shortcut: `shift+${letter}`, ...default_shortcut });
		});
	}
};
