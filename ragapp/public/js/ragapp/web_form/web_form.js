...
ragapp.provide("ragapp.web_form");
import EventEmitterMixin from "../../ragapp/event_emitter";

export default class WebForm extends ragapp.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		ragapp.web_form = this;
		ragapp.web_form.events = {};
		Object.assign(ragapp.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		ragapp.init_client_script && ragapp.init_client_script();
		this.setup_listeners();
		ragapp.web_form.events.trigger("after_load");
	...
	set_default_values() {
		...
		let values = ragapp.utils.get_query_params();
		delete values.new;
	...
	render_progress_dots() {
		...
		for (let i = 0; i <= this.page_breaks.length; i++) {
			let $dot = $(`<div class="slide-step">
				...
				<div class="slide-step-complete">${ragapp.utils.icon("tick", "xs")}</div>
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _page.find(".ragapp-control:not(.hide-control)");
	...
	save() {
		...
		// Save
		window.saving = true;
		ragapp.form_dirty = false;

		ragapp.call({
			type: "POST",
			method: "ragapp.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
				for_payment,
			...
			callback: (response) => {
				...
				if (!response.exc) {
					...
					ragapp.web_form.events.trigger("after_save");
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						ragapp.call({
							type: "POST",
							method: "ragapp.handler.upload_file",
							args: {
	...
	handle_success(data) {
		...
		if (this.success_url) {
			ragapp.utils.setup_timer(5, 0, $(".time"));
			setTimeout(() => {
