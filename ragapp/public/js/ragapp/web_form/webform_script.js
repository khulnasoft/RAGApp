...
ragapp.ready(function () {
	let web_form_doc = ragapp.web_form_doc;
	let reference_doc = ragapp.reference_doc;
	...
	function show_login_prompt() {
		if (ragapp.session.user != "Guest" || !web_form_doc.login_required) return;
		const login_required = new ragapp.ui.Dialog({
	...
	function show_form() {
		...
		$(".file-size").each(function () {
			$(this).text(ragapp.form.formatters.FileSize($(this).text()));
		});
	}
