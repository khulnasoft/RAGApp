...
export default class OnboardingWidget extends Widget {
	async refresh() {
		ragapp.utils.load_video_player();
	...
	add_step(step, index) {
		...
		let $step = $(`<a class="onboarding-step ${status}">
				<div class="step-title">
					<div class="step-index step-pending">${ragapp.utils.icon(
						"es-line-success",
						"md",
						"",
						"",
					...
					<div class="step-index step-skipped">${ragapp.utils.icon(
					...
					<div class="step-index step-complete">${ragapp.utils.icon(
						"es-solid-success",
						"md",
						"",
						"",
	...
	show_step(step) {
		...
		let actions = {
			...
			"Create Entry": (step) => {
				if (step.is_complete) {
					ragapp.set_route(`/app/List/${step.reference_document}`);
				} else {
		...
		const set_description = () => {
			let content = __(step.description)
				? ragapp.markdown(__(step.description))
				: `<h1>${__(step.title)}</h1>`;

			if (step.action === "Create Entry") {
				// add a secondary action to view list
				content += `<p>
					<a href='/app/${ragapp.router.slug(step.reference_document)}'>
		...
		const toggle_video = () => {
			...
			let plyr = new ragapp.Plyr(video[0], {
				hideControls: true,
				resetOnEnd: true,
			});
	...
	go_to_page(step) {
		...
		ragapp.set_route(step.path).then(() => {
			let message =
			...
			let msg_dialog = ragapp.msgprint({
				message: message,
				title: title,
				primary_action: {
					action: () => {
	...
	open_report(step) {
		let route = ragapp.utils.generate_route({
		...
		let current_route = ragapp.get_route();

		ragapp.set_route(route).then(() => {
			let msg_dialog = ragapp.msgprint({
				message: __(step.report_description),
				title: __(step.reference_report),
				primary_action: {
					action: () => {
						ragapp.set_route(current_route).then(() => {
							this.mark_complete(step);
						});
						msg_dialog.hide();
					},
			...
			ragapp.msg_dialog.custom_onhide = () => this.mark_complete(step);
		});
	}
	...
	show_form_tour(step) {
		let route;
		if (step.is_single) {
			route = ragapp.router.slug(step.reference_document);
		} else {
			route = `${ragapp.router.slug(step.reference_document)}/new`;
		}

		let current_route = ragapp.get_route();

		ragapp.route_hooks = {};
		ragapp.route_hooks.after_load = (frm) => {
			const on_finish = () => {
				let msg_dialog = ragapp.msgprint({
	...
	update_settings(step) {
		...
		ragapp.route_hooks.after_save = (frm) => {
			let success = false;
			let args = {};
			...
			if (success) {
				...
				args.secondary_action = {
					action: () => ragapp.set_route(current_route),
					label: __("Go Back"),
				};
			...
			ragapp.msgprint(args);
			ragapp.msg_dialog.custom_onhide = () => custom_onhide();
		};

		ragapp.set_route("Form", step.reference_document);
	}
	...
	async create_entry(step) {
		...
		if (step.is_submittable) {
			ragapp.route_hooks.after_save = () => {
				ragapp.msgprint({
					message: __("Submit this document to complete this step."),
					title: __("Document Saved"),
				});
			};
			ragapp.route_hooks.after_submit = callback;
		} else {
			ragapp.route_hooks.after_save = callback;
		}

		ragapp.set_route("Form", step.reference_document, docname);
	}

	show_quick_entry(step) {
		let current_route = ragapp.get_route_str();
		ragapp.ui.form.make_quick_entry(
			step.reference_document,
			() => {
				if (ragapp.get_route_str() != current_route) {
					let success_dialog = ragapp.msgprint({
						message: __("Let's take you back to onboarding"),
						title: __("Document Saved"),
						primary_action: {
							action: () => {
	...
	show_video(step) {
		ragapp.help.show_video(step.video_url, step.title);
		this.mark_complete(step);
	}
	...
	update_step_status(step, status, value, callback) {
		...
		ragapp
			.call("ragapp.desk.desktop.update_onboarding_step", {
				name: step.name,
				field: status,
				value: value,
			})
	...
	show_success() {
		...
		let success_state_image =
			this.success_state_image || "/assets/ragapp/images/ui-states/success-color.png";
		let documentation = "";
		if (this.docs_url) {
			documentation = __(
	...
	is_dismissed() {
		...
		if (Object.keys(dismissed).includes(this.title)) {
			...
			let today = new Date();
			let diff = ragapp.datetime.get_hour_diff(today, last_hidden);
			return diff < 24;
		}
		return false;
	}
	...
	set_actions() {
		...
		dismiss.on("click", () => {
			let dismissed = JSON.parse(localStorage.getItem("dismissed-onboarding") || "{}");
			dismissed[this.title] = ragapp.datetime.now_datetime();

			localStorage.setItem("dismissed-onboarding", JSON.stringify(dismissed));
			this.delete(true, true);
			this.widget.closest(".ce-block").hide();
			ragapp.telemetry.capture("dismissed_" + ragapp.scrub(this.title), "ragapp_onboarding");
		});
		...
		return ragapp.model
			...
			.then((onboarding_doc) => {
				if (onboarding_doc) {
					...
					const method =
						"ragapp.desk.doctype.onboarding_step.onboarding_step.get_onboarding_steps";
					return ragapp
	...
	async get_first_document(doctype) {
		...
		if (message.first_document) {
			await ragapp.db.get_list(doctype, { order_by: "creation" }).then((res) => {
