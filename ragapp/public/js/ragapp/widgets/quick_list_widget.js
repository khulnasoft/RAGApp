...
export default class QuickListWidget extends Widget {
	...
	set_actions() {
		...
		if (ragapp.model.can_create(this.document_type)) {
	...
	setup_refresh_list_button() {
		this.refresh_list = $(
			`<div class="refresh-list btn btn-xs pull-right" title="${__("Refresh List")}">
				${ragapp.utils.icon("es-line-reload", "sm")}
			</div>`
		);
	...
	setup_filter(doctype) {
		...
		this.filters = ragapp.utils.process_filter_expression(this.quick_list_filter);
	...
	setup_quick_list_item(doc) {
		const indicator = ragapp.get_indicator(doc, this.document_type);
			...
			<div class="quick-list-item">
				<div class="ellipsis left">
					...
					<div class="timestamp text-muted">
						${ragapp.datetime.prettyDate(doc.modified)}
					</div>
				</div>
			</div>
		`);
		...
		$(`<div class="right-arrow">${ragapp.utils.icon("right", "xs")}</div>`).appendTo(
			$quick_list_item
		);
		...
		$quick_list_item.click((e) => {
			...
			ragapp.set_route(`${ragapp.utils.get_form_link(this.document_type, doc.name)}`);
		});
	...
	set_body() {
		...
		ragapp.model.with_doctype(this.document_type, () => {
			let fields = ["name"];

			// get name of title field
			if (!this.title_field_name) {
				let meta = ragapp.get_meta(this.document_type);
			...
			this.has_status_field = ragapp.meta.has_field(this.document_type, "status");
			...
			let workflow_fieldname = ragapp.workflow.get_state_fieldname(this.document_type);
			workflow_fieldname && fields.push(workflow_fieldname);
			fields.push("modified");

			let add_fields = ragapp.listview_settings?.[this.document_type]?.add_fields;
			if (Array.isArray(add_fields)) {
				for (const fieldname of add_fields) {
					// Only keep fields that exist and are permitted
					if (ragapp.meta.has_field(this.document_type, fieldname)) {
			...
			let quick_list_filter = ragapp.utils.process_filter_expression(this.quick_list_filter);

			let args = {
				method: "ragapp.desk.reportview.get",
				args: {
	...
	set_footer() {
		...
		let filters = ragapp.utils.get_filter_from_json(this.quick_list_filter);
		let route = ragapp.utils.generate_route({ type: "doctype", name: this.document_type });
		this.see_all_button = $(`
