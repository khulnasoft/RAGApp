...
export const useStore = defineStore("workflow-builder-store", () => {
	...
	async function fetch() {
		await ragapp.model.clear_doc("Workflow", workflow_name.value);
		await ragapp.model.with_doc("Workflow", workflow_name.value);

		workflow_doc.value = ragapp.get_doc("Workflow", workflow_name.value);
		await ragapp.model.with_doctype(workflow_doc.value.document_type);

		if (!workflowfields.value.length) {
			await ragapp.model.with_doctype("Workflow");
			workflowfields.value = ragapp.get_meta("Workflow").fields;
		}

		if (!statefields.value.length) {
			await ragapp.model.with_doctype("Workflow Document State");
			statefields.value = ragapp.get_meta("Workflow Document State").fields;
		}

		if (!transitionfields.value.length) {
			await ragapp.model.with_doctype("Workflow Transition");
			transitionfields.value = ragapp.get_meta("Workflow Transition").fields;
		}

		if (!workflow_doc_fields.value.length) {
			let doc_type = workflow_doc.value.document_type;
			await ragapp.model.with_doctype(doc_type);
			workflow_doc_fields.value = ragapp.meta
				.get_docfields(doc_type, null, {
					fieldtype: ["not in", ragapp.model.no_value_type],
				})
				.sort((a, b) => {
					if (a.label && b.label) {
	...
	async function save_changes() {
		...
		try {
			...
			await ragapp.call("ragapp.client.save", { doc });
			ragapp.toast(__("Workflow updated successfully"));
			fetch();
		} catch (e) {
			console.error(e);
		} finally {
	...
	function setup_breadcrumbs() {
		...
		`;
		ragapp.breadcrumbs.clear();
		ragapp.breadcrumbs.$breadcrumbs.append(breadcrumbs);
	}
