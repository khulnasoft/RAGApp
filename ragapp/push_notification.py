...
import ragapp
from ragapp import sbool
from ragapp.utils.response import Response

from .ragappclient import RagappClient
...
class PushNotification:
	...
	def send_notification_to_user(
		...
		if strip_html:
			body = ragapp.utils.strip_html(body)
	...
	def is_enabled(self) -> bool:
		...
		"""
		return sbool(
			ragapp.db.get_single_value("Push Notification Settings", "enable_push_notification_relay")
		)
	...
	def _get_credential(self) -> tuple[str, str]:
		...
		"""
		notification_settings = ragapp.get_doc("Push Notification Settings")
		...
		token = ragapp.generate_hash(length=48)
		# store the token in the redis cache
		ragapp.cache().set_value(
		...
		body = {
			...
			"token": token,
			"webhook_route": "/api/method/ragapp.push_notification.auth_webhook",
		}
	...
	def _send_post_request(self, method: str, params: dict, use_authentication: bool = True):
		...
		relay_server_endpoint = ragapp.conf.get("push_relay_server_url")
		if use_authentication:
			api_key, api_secret = self._get_credential()
			client = RagappClient(relay_server_endpoint, api_key=api_key, api_secret=api_secret)
		else:
			client = RagappClient(relay_server_endpoint)
	...
	def _site_name(self) -> str:
		return urlparse(ragapp.utils.get_url()).hostname

	@property
	def _site_protocol(self) -> str:
		return urlparse(ragapp.utils.get_url()).scheme

	@property
	def _site_port(self) -> str:
		site_uri = urlparse(ragapp.utils.get_url())
...
def auth_webhook():
	url = urlparse(ragapp.utils.get_url()).hostname
	token = ragapp.cache().get_value(f"{url}:push_relay_registration_token")
	response = Response()
...
def subscribe(fcm_token: str, project_name: str) -> dict:
	success, message = PushNotification(project_name).add_token(ragapp.session.user, fcm_token)
	return {"success": success, "message": message}


@ragapp.whitelist(methods=["GET"])
def unsubscribe(fcm_token: str, project_name: str) -> dict:
	success, message = PushNotification(project_name).remove_token(ragapp.session.user, fcm_token)
	return {"success": success, "message": message}
