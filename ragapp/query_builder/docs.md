...
Check out some examples [here](https://ragapp.khulnasoft.com/docs/v14/user/en/api/query-builder)
...
```python
ragapp.db.sql("select `name` from `tabUser`")
```
...
ragapp.qb.from_("User").select(Field("name"))
...
```python
ragapp.db.get_values("User", fieldname="name", filters={})
```

This module is used to support the 3rd way of query generation in ragapp.
...
This module supports almost all the features present in db_query which powers `ragapp.get_all` and `ragapp.get_list`
...
1. Dict Query
   ...
   ```python
       ragapp.db.get_values("ToDo", fieldname="name", filters={"description": "Something Random"})
   ```

   and many other possible caveats to the dict representation such as

   ```python
      ragapp.db.get_values("User", fieldname="name",
      filters={"name": ("like", "admin%")})

      ragapp.db.get_values("ToDo", fieldname="name", filters={"description": ("in", ["somso%", "someome"])})
   ```
...
2. Misc Query
   ...
   - To support this
     ...
     ```python
     ragapp.db.get_values("ToDo", fieldname="name", filters=["description", "=", "someone"])
     ```
...
3. Criterion Query
   ...
   ragapp.db.get_values("User", fieldname="name", filters=Field("name") == "Administrator")
...
### 2. Implementing the missing features which are present in `ragapp.get_list` and `ragapp.get_all` (do we even need so much magic?)
