...
import ragapp
from ragapp.query_builder.custom import GROUP_CONCAT, MATCH, STRING_AGG, TO_TSVECTOR
from ragapp.query_builder.utils import ImportMapper, db_type_is
...
class Cast_(Function):
	def __init__(self, value, as_type, alias=None):
		if ragapp.db.db_type == "mariadb" and (
...
def _aggregate(function, dt, fieldname, filters, **kwargs):
	return (
		ragapp.qb.get_query(dt, filters=filters, fields=[function(PseudoColumn(fieldname))]).run(**kwargs)[0][
			0
		]
		or 0
	)
