"""
RagappClient is a library that helps you connect with other ragapp systems
"""
...
class RagappClient:
	def __init__(
		...
		api_key=None,
		api_secret=None,
		ragapp_authorization_source=None,
	):
		import requests
		...
		self.ragapp_authorization_source = ragapp_authorization_source
	...
	def setup_key_authentication_headers(self):
		if self.api_key and self.api_secret:
			...
			if self.ragapp_authorization_source:
				auth_source = {"Ragapp-Authorization-Source": self.ragapp_authorization_source}
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": ragapp.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return ragapp._dict(self.post_process(res))
	...
	def insert_many(self, docs):
		...
		return self.post_request({"cmd": "ragapp.client.insert_many", "docs": ragapp.as_json(docs)})
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": ragapp.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return ragapp._dict(self.post_process(res))
	...
	def bulk_update(self, docs):
		...
		return self.post_request({"cmd": "ragapp.client.bulk_update", "docs": ragapp.as_json(docs)})
	...
	def delete(self, doctype, name):
		...
		return self.post_request({"cmd": "ragapp.client.delete", "doctype": doctype, "name": name})

	def submit(self, doc):
		"""Submit remote document

		:param doc: dict or Document object to be submitted remotely"""
		return self.post_request({"cmd": "ragapp.client.submit", "doc": ragapp.as_json(doc)})
	...
	def get_value(self, doctype, fieldname=None, filters=None):
		...
		return self.get_request(
			{
				"cmd": "ragapp.client.get_value",
				"doctype": doctype,
				"fieldname": fieldname or "name",
				"filters": ragapp.as_json(filters),
			}
		)
	...
	def cancel(self, doctype, name):
		...
		return self.post_request({"cmd": "ragapp.client.cancel", "doctype": doctype, "name": name})
	...
	def migrate_doctype(self, doctype, filters=None, update=None, verbose=1, exclude=None, preprocess=None):
		...
		for doc in docs:
			...
			if doctype != "User" and not ragapp.db.exists("User", doc.get("owner")):
				ragapp.get_doc(
					{
						"doctype": "User",
						"email": doc.get("owner"),
	...
	def migrate_single(self, doctype):
		...
		doc.modified = ragapp.db.get_single_value(doctype, "modified")
	...
	def post_process(self, response):
		...
		if rjson and ("exc" in rjson) and rjson["exc"]:
			try:
				...
				exc = "RagappClient Request Failed\n\n" + exc
			except Exception:
				exc = rjson["exc"]

			raise RagappException(exc)
		if "message" in rjson:
...
class RagappOAuth2Client(RagappClient):
