...
def apply():
	rate_limit = ragapp.conf.rate_limit
	if rate_limit:
		ragapp.local.rate_limiter = RateLimiter(rate_limit["limit"], rate_limit["window"])
		ragapp.local.rate_limiter.apply()


def update():
	if hasattr(ragapp.local, "rate_limiter"):
		ragapp.local.rate_limiter.update()


def respond():
	if hasattr(ragapp.local, "rate_limiter"):
		return ragapp.local.rate_limiter.respond()
...
class RateLimiter:
	...
	def __init__(self, limit, window):
		...
		self.key = ragapp.cache.make_key(f"rate-limit-counter-{self.window_number}")
		self.counter = cint(ragapp.cache.get(self.key))
		if not self.counter:
			# This is the first request in this window
			ragapp.cache.incrby(self.key, 0)
			ragapp.cache.expire(self.key, self.window)
	...
	def reject(self):
		raise ragapp.TooManyRequestsError

	def update(self):
		self.record_request_end()
		ragapp.cache.incrby(self.key, self.duration)
...
def rate_limit(
	...
	def ratelimit_decorator(fn):
		...
		def wrapper(*args, **kwargs):
			...
			if not ragapp.request or (
				methods != "ALL" and ragapp.request.method and ragapp.request.method.upper() not in methods
			):
				return fn(*args, **kwargs)

			_limit = limit() if callable(limit) else limit

			ip = ragapp.local.request_ip if ip_based is True else None

			user_key = ragapp.form_dict.get(key, "")
			...
			if not identity:
				ragapp.throw(_("Either key or IP flag is required."))

			cache_key = ragapp.cache.make_key(f"rl:{ragapp.form_dict.cmd}:{identity}")

			value = ragapp.cache.get(cache_key)
			if not value:
				ragapp.cache.setex(cache_key, seconds, 0)

			value = ragapp.cache.incrby(cache_key, 1)
			if value > _limit:
				ragapp.throw(
					_("You hit the rate limit because of too many requests. Please try after sometime."),
					ragapp.RateLimitExceededError,
				)
