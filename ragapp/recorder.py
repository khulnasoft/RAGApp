...
from ragapp.database.database import is_query_type
...
class RecorderConfig:
	...
	def __post_init__(self):
		if not (self.record_jobs or self.record_requests):
			ragapp.throw("You must record one of jobs or requests")

	def store(self):
		ragapp.cache.set_value(RECORDER_CONFIG_FLAG, self, expires_in_sec=RECORDER_AUTO_DISABLE)

	@classmethod
	def retrieve(cls):
		return ragapp.cache.get_value(RECORDER_CONFIG_FLAG) or cls()

	@staticmethod
	def delete():
		ragapp.cache.delete_value(RECORDER_CONFIG_FLAG)


def record_sql(*args, **kwargs):
	start_time = time.monotonic()
	result = ragapp.db._sql(*args, **kwargs)
	end_time = time.monotonic()

	query = getattr(ragapp.db, "last_query", None)
	if not query or isinstance(result, str):
		# run=0, doesn't actually run the query so last_query won't be present
		return result

	stack = []
	if ragapp.local._recorder.config.capture_stack:
	...
	ragapp.local._recorder.register(data)
	return result


def get_current_stack_frames():
	from ragapp.utils.safe_exec import SERVER_SCRIPT_FILE_PREFIX
...
def post_process():
	...
	"""
	ragapp.db.rollback()
	ragapp.db.begin(read_only=True)  # Explicitly start read only transaction

	config = RecorderConfig.retrieve()
	result = list(ragapp.cache.hgetall(RECORDER_REQUEST_HASH).values())
	...
	for request in result:
		for call in request["calls"]:
			...
			if config.explain and is_query_type(formatted_query, ("select", "update", "delete")):
				...
				try:
					call["explain_result"] = ragapp.db.sql(f"EXPLAIN {formatted_query}", as_dict=True)
				except Exception:
					pass
		mark_duplicates(request)
		ragapp.cache.hset(RECORDER_REQUEST_HASH, request["uuid"], request)
...
def record(force=False):
	flag_value = ragapp.client_cache.get_value(RECORDER_INTERCEPT_FLAG)
	if flag_value or force:
		ragapp.local._recorder = Recorder(force=force)
	elif flag_value is None:
		# Explicitly set it once so next requests can use client-side cache
		ragapp.client_cache.set_value(RECORDER_INTERCEPT_FLAG, False)


def dump():
	if hasattr(ragapp.local, "_recorder"):
		ragapp.local._recorder.dump()
...
class Recorder:
	def __init__(self, force=False):
		...
		if (
			...
			and ragapp.request
			and self.config.request_filter in ragapp.request.path
		):
			self.path = ragapp.request.path
			self.cmd = ragapp.local.form_dict.cmd or ""
			self.method = ragapp.request.method
			self.headers = dict(ragapp.local.request.headers)
			self.form_dict = ragapp.local.form_dict
			self.event_type = "HTTP Request"
		elif self.config.record_jobs and ragapp.job and self.config.jobs_filter in ragapp.job.method:
			self.event_type = "Background Job"
			self.path = ragapp.job.method
			self.cmd = None
			self.method = None
			self.headers = None
			self.form_dict = None
		...
		self.uuid = ragapp.generate_hash(length=10)
	...
	def dump(self):
		...
		ragapp.cache.hset(RECORDER_REQUEST_SPARSE_HASH, self.uuid, request_data)

		request_data["calls"] = self.calls
		request_data["headers"] = self.headers
		request_data["form_dict"] = self.form_dict
		request_data["profile"] = profiler_output
		ragapp.cache.hset(RECORDER_REQUEST_HASH, self.uuid, request_data)
	...
	@staticmethod
	def _patch_sql():
		ragapp.db._sql = ragapp.db.sql
		ragapp.db.sql = record_sql

	@staticmethod
	def _unpatch_sql():
		ragapp.db.sql = ragapp.db._sql
...
def do_not_record(function):
	...
	def wrapper(*args, **kwargs):
		if hasattr(ragapp.local, "_recorder"):
			ragapp.local._recorder.cleanup()
			del ragapp.local._recorder
...
def administrator_only(function):
	...
	def wrapper(*args, **kwargs):
		if ragapp.session.user != "Administrator":
			ragapp.throw(_("Only Administrator is allowed to use Recorder"))
		return function(*args, **kwargs)
...
def status(*args, **kwargs):
	return bool(ragapp.cache.get_value(RECORDER_INTERCEPT_FLAG))
...
def start(
	...
	**kwargs,
	...
	ragapp.client_cache.set_value(RECORDER_INTERCEPT_FLAG, True)
...
def stop(*args, **kwargs):
	ragapp.client_cache.set_value(RECORDER_INTERCEPT_FLAG, False)
	ragapp.enqueue(post_process, now=ragapp.flags.in_test)
...
def get(uuid=None, *args, **kwargs):
	if uuid:
		result = ragapp.cache.hget(RECORDER_REQUEST_HASH, uuid)
	else:
		result = list(ragapp.cache.hgetall(RECORDER_REQUEST_SPARSE_HASH).values())
	return result
...
def export_data(*args, **kwargs):
	return list(ragapp.cache.hgetall(RECORDER_REQUEST_HASH).values())
...
def delete(*args, **kwargs):
	ragapp.cache.delete_value(RECORDER_REQUEST_SPARSE_HASH)
	ragapp.cache.delete_value(RECORDER_REQUEST_HASH)
...
def record_queries(func: Callable):
	...
	def wrapped(*args, **kwargs):
		record(force=True)
		ragapp.local._recorder.path = f"Function call: {func.__module__}.{func.__qualname__}"
...
def import_data(file: str) -> None:
	file_doc = ragapp.get_doc("File", {"file_url": file})
	file_content = json.loads(file_doc.get_content())
	for request in file_content:
		ragapp.cache.hset(RECORDER_REQUEST_SPARSE_HASH, request["uuid"], request)
		ragapp.cache.hset(RECORDER_REQUEST_HASH, request["uuid"], request)
