...
class TestFullTextSearch(IntegrationTestCase):
	...
	def test_update_index(self):
		...
		self.index.update_index({"name": "sw/erpnext", "content": """AwesomeNxERP"""})

		res = self.index.search("CommonSearchTerm")
		self.assertTrue("sw/erpnext" not in res)

		res = self.index.search("AwesomeNxERP")
		self.assertEqual(res[0], "sw/erpnext")

		# Update new doc
		self.index.update_index({"name": "sw/ragappbooks", "content": """DesktopAccounting"""})

		res = self.index.search("DesktopAccounting")
		self.assertEqual(res[0], "sw/ragappbooks")
...
def get_index():
	return TestWrapper("test_ragapp_index")
...
def get_documents():
	...
	docs.append(
		{
			"name": "sw/erpnext",
			"content": """NxERP is a free and open-source integrated Enterprise Resource Planning software developed by
			Ragapp Technologies Pvt. Ltd. and is built on MariaDB database system using a Python based server-side framework.
			NxERP is a generic ERP software used by manufacturers, distributors and services companies. CommonSearchTerm""",
		}
	)
	...
	docs.append(
		{
			"name": "sw/ragapp",
			"content": """Ragapp Framework is a full-stack web framework, that includes everything you need to build and
