function setup_list_filters() {
	...
	ragapp.call('ragapp.website.doctype.web_form.web_form.get_form_data', {
	...
	.then((r) => {
		...
		web_form_filters = web_form.web_form_fields.filter(df => {
			...
			const value = ragapp.utils.get_query_params()[df.fieldname];
			const f = ragapp.ui.form.make_control({
				df: {
					...
					is_filter: true,
					change: (e) => {
						const query_params = Object.assign(ragapp.utils.get_query_params(), {
							[df.fieldname]: f.get_value()
						})
						const query_string = ragapp.utils.make_query_string(query_params);
...
ragapp.ready(setup_list_filters);
