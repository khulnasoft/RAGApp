"""
Ragapp Testing Module

This module provides a comprehensive framework for running tests in Ragapp applications.
...
- TestRunner: Main class for running test suites with additional Ragapp-specific functionality
- TestResult: Custom test result class for improved output formatting and logging
- discover_all_tests: Function to discover all tests in specified Ragapp apps
...
- Integration with Ragapp's hooks and test record creation system

Usage:
This module is typically used by Ragapp's CLI commands for running tests, but can also
be used programmatically for custom test execution scenarios.

Example:
    from ragapp.testing import TestConfig, TestRunner, discover_all_tests
...
from ragapp.utils.logger import create_handler as createRagappFileHandler
...
LOGGING_CONFIG = {
	...
	"loggers": {
		f"{__name__}": {
			"handlers": [],  # only log to the ragapp handler
			"propagate": False,
		},
	},
}
...
handlers = createRagappFileHandler(__name__)
