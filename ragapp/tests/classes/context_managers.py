...
if TYPE_CHECKING:
	from ragapp.model import Document
...
def freeze_time(time_to_freeze: Any, is_utc: bool = False, *args: Any, **kwargs: Any) -> None:
	...
	from ragapp.utils.data import get_datetime, get_system_timezone
...
def patch_hooks(overridden_hooks: dict) -> None:
	...
	with patch.object(ragapp, "get_hooks", patched_hooks):
		yield
...
def change_settings(doctype, settings_dict=None, /, commit=False, **settings) -> None:
	...
	settings = ragapp.get_doc(doctype)
	...
	ragapp.db.value_cache[settings] = {}
	if commit:
		ragapp.db.commit()
	yield
	settings = ragapp.get_doc(doctype)
...
def enable_safe_exec() -> None:
	...
	conf = os.path.join(ragapp.local.sites_path, "common_site_config.json")
...
def trace_fields(
	...
	**field_configs: dict[str, list | Callable | None],
	...
	Yields:
	    Document class
	"""
	from ragapp.model.trace import traced_field
...
# NOTE: declare those who should also be made available directly ragapp.tests.* namespace
