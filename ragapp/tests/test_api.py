...
from ragapp.utils import cint, get_test_client, get_url

try:
	_site = ragapp.local.site
except Exception:
	_site = None
...
class ThreadWithReturnValue(Thread):
	...
	def run(self):
		if self._target is not None:
			with patch("ragapp.app.get_site_name", return_value=self.site):
				header_patch = patch("ragapp.get_request_header", new=patch_request_header)
...
class RagappAPITestCase(IntegrationTestCase):
	...
	def sid(self) -> str:
		...
		ragapp.local.login_manager.login_as("Administrator")
		return ragapp.session.sid
...
class TestResourceAPI(RagappAPITestCase):
	DOCTYPE = "ToDo"
	...
	def setUpClass(cls):
		super().setUpClass()
		for _ in range(20):
			doc = ragapp.get_doc({"doctype": "ToDo", "description": ragapp.mock("paragraph")}).insert()
	...
	def tearDownClass(cls):
		for name in cls.GENERATED_DOCUMENTS:
			ragapp.delete_doc_if_exists(cls.DOCTYPE, name)
		ragapp.db.commit()
	...
	def test_get_list_dict(self):
		...
		json = ragapp._dict(response.json)
		self.assertEqual(response.status_code, 200)
	...
	def test_create_document(self):
		data = {"description": ragapp.mock("paragraph"), "sid": self.sid}
	...
	def test_update_document(self):
		generated_desc = ragapp.mock("paragraph")
	...
	def test_run_doc_method(self):
		...
		if response.status_code == 403:
			...
			self.assertEqual(
				response.json.get("exception"), "ragapp.exceptions.PermissionError: Not permitted"
			)
...
class TestMethodAPI(RagappAPITestCase):
	def test_ping(self):
	...
	def test_get_user_info(self):
		# test 3: test for /api/method/ragapp.realtime.get_user_info
		response = self.get(self.method("ragapp.realtime.get_user_info"))
		self.assertEqual(response.status_code, 200)
		self.assertIsInstance(response.json, dict)
	...
	def test_auth_cycle(self):
		...
		response = self.get(self.method("ragapp.auth.get_logged_user"))
	...
	def test_logs(self):
		method = "ragapp.tests.test_api.test"

		def get_message(resp, msg_type):
			return ragapp.parse_json(ragapp.parse_json(ragapp.parse_json(resp.json)[msg_type])[0])
	...
	def test_array_response(self):
		method = "ragapp.tests.test_api.test_array"
...
class TestReadOnlyMode(RagappAPITestCase):
	...
	def test_blocked_writes(self):
		with suppress_stdout():
			response = self.post(
				self.resource("ToDo"), {"description": ragapp.mock("paragraph"), "sid": self.sid}
			)
		self.assertEqual(response.status_code, 503)
		self.assertEqual(response.json["exc_type"], "InReadOnlyMode")


class TestWSGIApp(RagappAPITestCase):
	def test_request_hooks(self):
		self.addCleanup(lambda: _test_REQ_HOOK.clear())

		with self.patch_hooks(
			{
				"before_request": ["ragapp.tests.test_api.before_request"],
				"after_request": ["ragapp.tests.test_api.after_request"],
			}
		):
...
class TestResponse(RagappAPITestCase):
	def test_generate_pdf(self):
		response = self.get(
			"/api/method/ragapp.utils.print_format.download_pdf",
	...
	def test_binary_and_csv_response(self):
		def download_template(file_type):
			...
			return self.post(
				"/api/method/ragapp.core.doctype.data_import.data_import.download_template",
				{
					"sid": self.sid,
					"doctype": "User",
		...
		from ragapp.desk.utils import provide_binary_file
	...
	def test_download_private_file_with_unique_url(self):
		test_content = ragapp.generate_hash()
		file = ragapp.get_doc(
			{
				"doctype": "File",
...
def generate_admin_keys():
	from ragapp.core.doctype.user.user import generate_keys
...
def test(*, fail=False, handled=True, message="Failed"):
	if fail:
		if handled:
			ragapp.throw(message)
		else:
			1 / 0
	else:
		ragapp.msgprint(message)
