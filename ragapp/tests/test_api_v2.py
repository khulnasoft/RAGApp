...
from ragapp.tests.test_api import RagappAPITestCase, suppress_stdout
...
class TestResourceAPIV2(RagappAPITestCase):
	version = "v2"
	DOCTYPE = "ToDo"
	...
	def test_execute_doc_method(self):
		...
		self.assertEqual(response.json["data"][0]["name"], "ragapp")
	...
	def test_delete_document_non_existing(self):
		non_existent_doc = ragapp.generate_hash(length=12)
...
class TestMethodAPIV2(RagappAPITestCase):
	version = "v2"
	...
	def test_shorthand_controller_methods(self):
		...
		expanded_response = self.get(
			self.method("ragapp.core.doctype.user.user.get_all_roles"), {"sid": self.sid}
		)
	...
	def test_add_comment(self):
		comment_txt = ragapp.generate_hash()
		response = self.post(
...
class TestDocTypeAPIV2(RagappAPITestCase):
	version = "v2"
