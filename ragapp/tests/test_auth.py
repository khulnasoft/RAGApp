...
import ragapp
from ragapp.auth import LoginAttemptTracker
from ragapp.ragappclient import AuthError, RagappClient
from ragapp.sessions import Session, get_expired_sessions, get_expiry_in_seconds
from ragapp.tests import IntegrationTestCase, UnitTestCase
from ragapp.tests.test_api import RagappAPITestCase
from ragapp.utils import get_datetime, get_site_url, now
from ragapp.utils.data import add_to_date
from ragapp.www.login import _generate_temporary_login_link
...
class TestAuth(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.HOST_NAME = ragapp.get_site_config().host_name or get_site_url(ragapp.local.site)
	...
	def tearDownClass(cls):
		ragapp.delete_doc("User", cls.test_user_email, force=True)
		ragapp.local.request_ip = None
		ragapp.form_dict.email = None
		ragapp.local.response["http_status_code"] = None

	def set_system_settings(self, k, v):
		ragapp.db.set_single_value("System Settings", k, v)
		ragapp.clear_cache()
		ragapp.db.commit()
	...
	def test_allow_login_using_mobile(self):
		...
		RagappClient(self.HOST_NAME, self.test_user_mobile, self.test_user_password)
		RagappClient(self.HOST_NAME, self.test_user_email, self.test_user_password)

		# login by username should fail
		with self.assertRaises(AuthError):
			RagappClient(self.HOST_NAME, self.test_user_name, self.test_user_password)
	...
	def test_deny_multiple_login(self):
		...
		first_login = RagappClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		first_login.get_list("ToDo")

		second_login = RagappClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		second_login.get_list("ToDo")
		with self.assertRaises(Exception):
			first_login.get_list("ToDo")

		third_login = RagappClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		with self.assertRaises(Exception):
			first_login.get_list("ToDo")
		with self.assertRaises(Exception):
			second_login.get_list("ToDo")
	...
	def test_disable_user_pass_login(self):
		RagappClient(self.HOST_NAME, self.test_user_email, self.test_user_password).get_list("ToDo")
	...
	def test_correct_cookie_expiry_set(self):
		client = RagappClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
...
class TestAllowedReferrer(UnitTestCase):
	def test_is_allowed_referrer(self):
		...
		ragapp.cache.set_value("allowed_referrers", ["https://example.com"])
		ragapp.local.request = create_request({"Referer": "https://example.com/some/path"})
		http_request = ragapp.auth.HTTPRequest()
		self.assertTrue(http_request.is_allowed_referrer())

		# Test with invalid referrer
		ragapp.local.request = create_request({"Referer": "https://malicious.com"})
		http_request = ragapp.auth.HTTPRequest()
		self.assertFalse(http_request.is_allowed_referrer())

		# Test with valid origin
		ragapp.local.request = create_request({"Origin": "https://example.com"})
		http_request = ragapp.auth.HTTPRequest()
		self.assertTrue(http_request.is_allowed_referrer())

		# Test with invalid origin
		ragapp.local.request = create_request({"Origin": "https://malicious.com"})
		http_request = ragapp.auth.HTTPRequest()
		self.assertFalse(http_request.is_allowed_referrer())

		# Clean up
		ragapp.cache.delete_value("allowed_referrers")
...
class TestSessionExpirty(RagappAPITestCase):
	def test_session_expires(self):
		sid = self.sid  # triggers login for test case login
		s: Session = ragapp.local.session_obj
