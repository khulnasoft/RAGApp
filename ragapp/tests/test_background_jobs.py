...
import ragapp
from ragapp.core.doctype.rq_job.rq_job import remove_failed_jobs
from ragapp.tests import IntegrationTestCase
from ragapp.utils.background_jobs import (
	RQ_JOB_FAILURE_TTL,
	RQ_RESULTS_TTL,
	create_job_id,
	execute_job,
...
class TestBackgroundJobs(IntegrationTestCase):
	def test_remove_failed_jobs(self):
		ragapp.enqueue(method="ragapp.tests.test_background_jobs.fail_function", queue="short")
	...
	def test_enqueue_at_front(self):
		kwargs = {
			"method": "ragapp.handler.ping",
			"queue": "short",
		}

		# give worker something to work on first so that get_position doesn't return None
		ragapp.enqueue(**kwargs)

		# test enqueue with at_front=True
		low_priority_job = ragapp.enqueue(**kwargs)
		high_priority_job = ragapp.enqueue(**kwargs, at_front=True)
	...
	def test_job_hooks(self):
		...
		with (
			...
			ragapp.init_site(locals.site),
			patch("ragapp.get_hooks", patch_job_hooks),
		):
			ragapp.connect()
...
def patch_job_hooks(event: str):
	return {
		"before_job": ["ragapp.tests.test_background_jobs.before_job"],
		"after_job": ["ragapp.tests.test_background_jobs.after_job"],
	}[event]
