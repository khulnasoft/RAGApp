...
import ragapp
from ragapp.modules.patch_handler import get_all_patches, parse_as_configfile
from ragapp.utils.boilerplate import (
	PatchCreator,
...
class TestBoilerPlate(unittest.TestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.default_hooks = ragapp._dict(
			{
				"app_name": "test_app",
				"app_title": "Test App",
		...
		cls.cli_path = ragapp.utils.get_cli_path()
		cls.apps_dir = os.path.join(cls.cli_path, "apps")
	...
	def delete_test_app(cls, app_name):
		test_app_dir = os.path.join(cls.cli_path, "apps", app_name)
	...
	def test_create_app(self):
		...
		new_app_dir = os.path.join(self.cli_path, self.apps_dir, app_name)
	...
	def test_new_patch_util(self):
		...
		patches_txt = pathlib.Path(pathlib.Path(ragapp.get_app_path("ragapp", "patches.txt")))
		...
		expected_patch = "ragapp.core.doctype.user.patches.delete_all_users"
