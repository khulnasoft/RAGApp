...
class TestClient(IntegrationTestCase):
	def test_set_value(self):
		...
		ragapp.set_value("ToDo", todo.name, "description", "test 1")
		self.assertEqual(ragapp.get_value("ToDo", todo.name, "description"), "test 1")

		ragapp.set_value("ToDo", todo.name, {"description": "test 2"})
		self.assertEqual(ragapp.get_value("ToDo", todo.name, "description"), "test 2")

	def test_delete(self):
		from ragapp.client import delete
		from ragapp.desk.doctype.note.note import Note

		note = ragapp.get_doc(
			doctype="Note",
			title=ragapp.generate_hash(length=8),
			content="test",
		...
		self.assertFalse(ragapp.db.exists("Note", note.name))
		self.assertRaises(ragapp.DoesNotExistError, delete, "Note", note.name)
		self.assertRaises(ragapp.DoesNotExistError, delete, "Note Seen By", child_row_name)
	...
	def test_http_valid_method_access(self):
		...
		ragapp.local.form_dict = ragapp._dict(
			{"doc": dict(doctype="ToDo", description="Valid http method"), "cmd": "ragapp.client.save"}
		)
		todo = execute_cmd("ragapp.client.save")
	...
	def test_http_invalid_method_access(self):
		...
		ragapp.local.form_dict = ragapp._dict(
			{"doc": dict(doctype="ToDo", description="Invalid http method"), "cmd": "ragapp.client.save"}
		)

		self.assertRaises(ragapp.PermissionError, execute_cmd, "ragapp.client.save")
	...
	def test_run_doc_method(self):
		...
		report = ragapp.get_doc(
			{
				"doctype": "Report",
				"ref_doctype": "User",
				"report_name": ragapp.generate_hash(),
		...
		execute_cmd(ragapp.local.form_dict.cmd)
		...
		self.assertRaises(ragapp.PermissionError, execute_cmd, ragapp.local.form_dict.cmd)
	...
	def test_array_values_in_request_args(self):
		...
		params = {
			...
			"sid": ragapp.session.sid,
		}
		headers = {
			"accept": "application/json",
			"content-type": "application/json",
		}
		url = get_site_url(ragapp.local.site)
		url += "/api/method/ragapp.client.get_list"
	...
	def test_client_get(self):
		from ragapp.client import get

		todo = ragapp.get_doc(doctype="ToDo", description="test").insert()
		filters = {"name": todo.name}
		filters_json = ragapp.as_json(filters)
	...
	def test_client_insert(self):
		from ragapp.client import insert

		def get_random_title():
			return f"test-{ragapp.generate_hash()}"
		...
		# cleanup
		ragapp.delete_doc("Note", note1.name)
		ragapp.delete_doc("Note", note2.name)

	def test_client_insert_many(self):
		from ragapp.client import insert, insert_many

		def get_random_title():
			return f"test-{ragapp.generate_hash(length=5)}"
		...
		self.assertEqual(ragapp.db.get_value("Note", docs[3], "title"), "not-a-random-title")
		self.assertEqual(ragapp.db.get_value("Note", docs[6], "title"), "another-note-title")
		self.assertIn(note1.name, docs)

		# cleanup
		for doc in docs:
			ragapp.delete_doc("Note", doc)
