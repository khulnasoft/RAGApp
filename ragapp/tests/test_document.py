...
import ragapp
from ragapp.app import make_form_dict
from ragapp.core.doctype.doctype.test_doctype import new_doctype
from ragapp.desk.doctype.note.note import Note
from ragapp.model.naming import make_autoname, parse_naming_series, revert_series_if_last
from ragapp.tests import IntegrationTestCase
from ragapp.utils import cint, now_datetime, set_request
...
class TestDocument(IntegrationTestCase):
	...
	def test_load_single(self):
		d = ragapp.get_doc("Website Settings", "Website Settings")
	...
	def test_insert(self):
		...
		self.assertEqual(ragapp.db.get_value("Event", d.name, "subject"), "test-doc-test-event 1")
	...
	def test_website_route_default(self):
		...
		doc = ragapp.get_doc({"doctype": parent, "child_table": [{"some_fieldname": "xasd"}]}).insert()
	...
	def test_insert_with_child(self):
		...
		d.insert()
		self.assertTrue(d.name.startswith("EV"))
		self.assertEqual(ragapp.db.get_value("Event", d.name, "subject"), "test-doc-test-event 2")
	...
	def test_update(self):
		...
		self.assertEqual(ragapp.db.get_value(d.doctype, d.name, "subject"), "subject changed")
	...
	def test_discard_transitions(self):
		...
		d.submit()
		self.assertRaises(ragapp.ValidationError, d.discard)
		d.reload()
	...
	def test_save_on_discard_throws(self):
		from ragapp.desk.doctype.event.event import Event
		...
		self.assertRaises(ragapp.ValidationError, d3.discard)
	...
	def test_mandatory(self):
		...
		ragapp.delete_doc_if_exists("User", "test_mandatory@example.com", 1)
		...
		self.assertRaises(ragapp.MandatoryError, d.insert)

		d.set("first_name", "Test Mandatory")
		d.insert()
		self.assertEqual(ragapp.db.get_value("User", d.name), d.name)

	def test_text_editor_field(self):
		try:
			ragapp.get_doc(doctype="Activity Log", subject="test", message='<img src="test.png" />').insert()
		except ragapp.MandatoryError:
			self.fail("Text Editor false positive mandatory error")

	def test_conflict_validation(self):
		d1 = self.test_insert()
		d2 = ragapp.get_doc(d1.doctype, d1.name)
		d1.save()
		self.assertRaises(ragapp.TimestampMismatchError, d2.save)

	def test_conflict_validation_single(self):
		d1 = ragapp.get_doc("Website Settings", "Website Settings")
		d1.home_page = "test-web-page-1"

		d2 = ragapp.get_doc("Website Settings", "Website Settings")
	...
	def test_permission(self):
		...
		self.assertRaises(ragapp.PermissionError, self.test_insert)
		ragapp.set_user("Administrator")

	def test_permission_single(self):
		ragapp.set_user("Guest")
		d = ragapp.get_doc("Website Settings", "Website Settings")
		self.assertRaises(ragapp.PermissionError, d.save)
		ragapp.set_user("Administrator")

	def test_link_validation(self):
		ragapp.delete_doc_if_exists("User", "test_link_validation@example.com", 1)
		...
		self.assertRaises(ragapp.LinkValidationError, d.insert)
	...
	def test_validate(self):
		...
		self.assertRaises(ragapp.ValidationError, d.validate)
		self.assertRaises(ragapp.ValidationError, d.run_method, "validate")
		self.assertRaises(ragapp.ValidationError, d.save)
	...
	def test_new_doc_with_fields(self):
		user = ragapp.new_doc("User", first_name="wizard")
		self.assertEqual(user.first_name, "wizard")

	def test_update_after_submit(self):
		d = self.test_insert()
		d.starts_on = "2014-09-09"
		self.assertRaises(ragapp.UpdateAfterSubmitError, d.validate_update_after_submit)
	...
	def test_varchar_length(self):
		...
		self.assertRaises(ragapp.CharacterLengthExceededError, d.save)
	...
	def test_naming_series(self):
		...
		for series in data:
			...
			old_current = ragapp.db.get_value("Series", prefix, "current", order_by="name")

			revert_series_if_last(series, name)
			new_current = cint(ragapp.db.get_value("Series", prefix, "current", order_by="name"))

			self.assertEqual(cint(old_current) - 1, new_current)

	def test_non_negative_check(self):
		ragapp.delete_doc_if_exists("Currency", "Ragapp Coin", 1)

		d = ragapp.get_doc(
			{"doctype": "Currency", "currency_name": "Ragapp Coin", "smallest_currency_fraction_value": -1}
		)

		self.assertRaises(ragapp.NonNegativeError, d.insert)

		d.set("smallest_currency_fraction_value", 1)
		d.insert()
		self.assertEqual(ragapp.db.get_value("Currency", d.name), d.name)
	...
	def test_get_formatted(self):
		...
		ragapp.delete_doc_if_exists("Currency", "INR", 1)
		...
		d = ragapp.get_doc({"doctype": "Test Formatted", "currency": 100000})
	...
	def test_limit_for_get(self):
		doc = ragapp.get_doc("DocType", "DocType")
	...
	def test_virtual_fields(self):
		...
		ragapp.db.delete("Custom Field", {"dt": "Note", "fieldname": "age"})
		note = ragapp.new_doc("Note")
		note.content = "some content"
		note.title = ragapp.generate_hash(length=20)
		note.insert()

		def patch_note(class_=None):
			return patch("ragapp.controllers", new={ragapp.local.site: {"Note": class_ or CustomTestNote}})

		@contextmanager
		def customize_note(with_options=False):
			options = (
				"ragapp.utils.now_datetime() - ragapp.utils.get_datetime(doc.creation)"
				if with_options
				else ""
			)
			custom_field = ragapp.get_doc(
				{
	...
	def test_error_on_saving_new_doc_with_name(self):
		...
		doc = ragapp.get_doc(
			{
				"doctype": "ToDo",
				"description": "this should raise ragapp.DoesNotExistError",
				"name": "lets-trick-doc-save",
			}
		)

		self.assertRaises(ragapp.DoesNotExistError, doc.save)
	...
	def test_validate_from_to_dates(self):
		...
		self.assertRaises(
			ragapp.exceptions.InvalidDates, doc.validate_from_to_dates, "start_date", "end_date"
		)
	...
	def test_db_set_singles(self):
		...
		c.db_set(key, val)
		changed_val = ragapp.db.get_single_value(c.doctype, key)
...
class TestDocumentWebView(IntegrationTestCase):
	def get(self, path, user="Guest"):
		...
		make_form_dict(ragapp.local.request)
	...
	def test_web_view_link_authentication(self):
		...
		# expire the key
		document_key_doc = ragapp.get_doc("Document Share Key", {"key": document_key})
	...
	def test_base_class_set_correctly_on_has_web_view_change(self):
		...
		from ragapp.modules.utils import get_doc_path, scrub

		ragapp.flags.allow_doctype_export = True

		ragapp.delete_doc_if_exists("DocType", "Test WebViewDocType", force=1)
	...
	def test_bulk_inserts(self):
		from ragapp.model.document import bulk_insert

		doctype = "Role Profile"
		child_field = "roles"
		child_doctype = ragapp.get_meta(doctype).get_field(child_field).options
		...
		def doc_generator():
			for _ in range(21):
				...
				doc.role_profile = ragapp.generate_hash()
		...
		all_docs = set(ragapp.get_all(doctype, pluck="name"))
		all_child_docs = set(
			ragapp.get_all(
