...
class TestDynamicLinks(IntegrationTestCase):
	...
	def test_delete_normal(self):
		...
		self.assertFalse(ragapp.db.exists("Email Unsubscribe", unsub.name))
	...
	def test_delete_with_comment(self):
		...
		self.assertTrue(
			ragapp.get_all("Comment", filters={"reference_doctype": "Event", "reference_name": event.name})
		)
		event.delete()
		self.assertFalse(
			ragapp.get_all("Comment", filters={"reference_doctype": "Event", "reference_name": event.name})
	...
	def test_custom_fields(self):
		...
		self.assertRaises(ragapp.LinkExistsError, unsub.delete)
		...
		ragapp.db.commit()  # undo changes done by DDL
