...
import ragapp
from ragapp.core.doctype.data_import.data_import import export_json, import_doc
from ragapp.desk.form.save import savedocs
...
class TestFixtureImport(IntegrationTestCase):
	def create_new_doctype(self, DocType: str) -> None:
		file = ragapp.get_app_path("ragapp", "custom", "fixtures", f"{DocType}.json")
	...
	def insert_dummy_data_and_export(self, DocType: str, dummy_name_list: list[str]) -> str:
		for name in dummy_name_list:
			doc = ragapp.get_doc({"doctype": DocType, "member_name": name})
			doc.insert()
	...
	def test_fixtures_import(self):
		self.assertFalse(ragapp.db.exists("DocType", "temp_doctype"))
		...
		ragapp.db.truncate("temp_doctype")
		...
		self.assertEqual(ragapp.db.count("temp_doctype"), len(dummy_name_list))

		data = ragapp.get_all("temp_doctype", "member_name")
		ragapp.db.truncate("temp_doctype")
	...
	def test_singles_fixtures_import(self):
		self.assertFalse(ragapp.db.exists("DocType", "temp_singles"))
		...
		singles_doctype = ragapp.qb.DocType("Singles")
		truncate_query = (
			ragapp.qb.from_(singles_doctype).delete().where(singles_doctype.doctype == "temp_singles")
		)
		truncate_query.run()

		import_doc(path_to_exported_fixtures)

		data = ragapp.db.get_single_value("temp_singles", "member_name")
		truncate_query.run()
