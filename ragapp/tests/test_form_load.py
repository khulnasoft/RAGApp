...
from ragapp.desk.form.load import get_docinfo, getdoc, getdoctype
from ragapp.tests import IntegrationTestCase
from ragapp.utils.file_manager import save_file
...
class TestFormLoad(IntegrationTestCase):
	def test_load(self):
		getdoctype("DocType")
		meta = next(filter(lambda d: d.name == "DocType", ragapp.response.docs))
		self.assertEqual(meta.name, "DocType")
		self.assertTrue(meta.get("__js"))

		ragapp.response.docs = []
		getdoctype("Event")
		meta = next(filter(lambda d: d.name == "Event", ragapp.response.docs))
	...
	def test_fieldlevel_permissions_in_load(self):
		blog = ragapp.get_doc(
			{
				...
				"title": f"_Test Blog Post {ragapp.utils.now()}",
				"published": 0,
			}
		)
		...
		doc = ragapp.get_doc("Blog Post", blog.name)
		doc.published = 1
		doc.save()
		...
		blog_doc.delete()
		ragapp.delete_doc(blog_post_property_setter.doctype, blog_post_property_setter.name)
	...
	def test_fieldlevel_permissions_in_load_for_child_table(self):
		...
		contact = ragapp.get_doc("Contact", "_Test Contact 1")
	...
	def test_get_doc_info(self):
		...
		get_docinfo(note)
		docinfo = ragapp.response["docinfo"]
