...
import ragapp
from ragapp.cache_manager import clear_controller_cache
from ragapp.desk.doctype.todo.todo import ToDo
...
class TestHooks(IntegrationTestCase):
	...
	def test_override_doctype_class(self):
		from ragapp import hooks

		# Set hook
		hooks.override_doctype_class = {"ToDo": ["ragapp.tests.test_hooks.CustomToDo"]}

		# Clear cache
		ragapp.client_cache.delete_value("app_hooks")
		clear_controller_cache("ToDo")

		todo = ragapp.get_doc(doctype="ToDo", description="asdf")
	...
	def test_has_permission(self):
		...
		address_has_permission_hook.append("ragapp.tests.test_hooks.custom_has_permission")
		...
		wildcard_has_permission_hook.append("ragapp.tests.test_hooks.custom_has_permission")
		...
		user = ragapp.get_doc("User", username)
		user.add_roles("System Manager")
		address = ragapp.new_doc("Address")
		...
		# Test!
		self.assertTrue(ragapp.has_permission("Address", doc=address, user=username))
		self.assertTrue(ragapp.has_permission("Note", doc=note, user=username))

		address.flags.dont_touch_me = True
		self.assertFalse(ragapp.has_permission("Address", doc=address, user=username))

		note.flags.dont_touch_me = True
		self.assertFalse(ragapp.has_permission("Note", doc=note, user=username))

	def test_ignore_links_on_delete(self):
		email_unsubscribe = ragapp.get_doc(
			{"doctype": "Email Unsubscribe", "email": "test@example.com", "global_unsubscribe": 1}
		).insert()

		event = ragapp.get_doc(
			{
				"doctype": "Event",
				"subject": "Test Event",
		...
		self.assertRaises(ragapp.LinkExistsError, email_unsubscribe.delete)
	...
	def test_fixture_prefix(self):
		...
		app = "ragapp"
		if os.path.isdir(ragapp.get_app_path(app, "fixtures")):
			shutil.rmtree(ragapp.get_app_path(app, "fixtures"))
		...
		# every call to ragapp.get_hooks loads the hooks module into cache
		...
		if ragapp._load_app_hooks.__wrapped__ in ragapp.local.request_cache.keys():
			del ragapp.local.request_cache[ragapp._load_app_hooks.__wrapped__]
		self.assertEqual([False], ragapp.get_hooks("fixture_auto_order", app_name=app))
		self.assertEqual(
			[
				{"dt": "User"},
				{"dt": "Contact"},
			...
			ragapp.get_hooks("fixtures", app_name=app),
		)
		...
		self.assertCountEqual(
			["user.json", "contact.json", "role.json"], os.listdir(ragapp.get_app_path(app, "fixtures"))
		)

		hooks.fixture_auto_order = True
		del ragapp.local.request_cache[ragapp._load_app_hooks.__wrapped__]
		self.assertEqual([True], ragapp.get_hooks("fixture_auto_order", app_name=app))

		shutil.rmtree(ragapp.get_app_path(app, "fixtures"))
		export_fixtures(app)
		self.assertCountEqual(
			["1_user.json", "2_contact.json", "3_role.json"],
			os.listdir(ragapp.get_app_path(app, "fixtures")),
		)
...
class TestAPIHooks(RagappAPITestCase):
	def test_auth_hook(self):
		with self.patch_hooks({"auth_hooks": ["ragapp.tests.test_hooks.custom_auth"]}):
			site_url = ragapp.utils.get_site_url(ragapp.local.site)
			response = self.get(
				site_url + "/api/method/ragapp.auth.get_logged_user",
...
def custom_auth():
	auth_type, token = ragapp.get_request_header("Authorization", "Bearer ").split(" ")
