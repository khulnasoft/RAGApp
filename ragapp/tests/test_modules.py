...
from ragapp.modules import export_customizations, export_module_json, get_module_path
from ragapp.modules.utils import export_doc, sync_customizations
...
class TestUtils(IntegrationTestCase):
	def setUp(self):
		self._dev_mode = ragapp.local.conf.developer_mode
		ragapp.local.conf.developer_mode = True

	def tearDown(self):
		ragapp.db.rollback()
		ragapp.local.conf.developer_mode = self._dev_mode
		ragapp.local.flags.pop("in_import", None)

	def test_export_module_json_no_export(self):
		ragapp.local.flags.in_import = True
		doc = ragapp.get_last_doc("DocType")
	...
	def test_export_module_json(self):
		doc = ragapp.get_last_doc("DocType", {"issingle": 0, "custom": 0})
		...
		with open(export_doc_path) as f:
			export_doc_before = ragapp.parse_json(f.read())

		last_modified_before = os.path.getmtime(export_doc_path)
		self.addCleanup(write_file, path=export_doc_path, content=ragapp.as_json(export_doc_before))

		ragapp.flags.in_import = False
		ragapp.conf.developer_mode = True
		...
		with open(f"{export_path}.json") as f:
			ragapp.parse_json(f.read())  # export_doc_after
	...
	def test_sync_customizations(self):
		with note_customizations() as (custom_field, property_setter):
			...
			sync_customizations(app="ragapp")
	...
	def test_reload_doc(self):
		ragapp.db.set_value("DocType", "Note", "migration_hash", "", update_modified=False)
		self.assertFalse(ragapp.db.get_value("DocType", "Note", "migration_hash"))
		ragapp.db.set_value(
			"DocField",
		...
		self.assertEqual(
			ragapp.db.get_value("DocField", {"parent": "Note", "fieldname": "title"}, "fieldtype"),
			"Text",
		)
		ragapp.reload_doctype("Note")
		self.assertEqual(
			ragapp.db.get_value("DocField", {"parent": "Note", "fieldname": "title"}, "fieldtype"),
			"Data",
		)
		self.assertTrue(ragapp.db.get_value("DocType", "Note", "migration_hash"))
	...
	def test_export_doc(self):
		...
		note.title = ragapp.generate_hash(length=10)
		note.save()
		export_doc(doctype="Note", name=note.name)
		exported_doc_path = Path(
			ragapp.get_app_path("ragapp", "desk", "note", note.name, f"{note.name}.json")
		)
	...
	def test_make_boilerplate(self):
		with temp_doctype() as doctype:
			scrubbed = ragapp.scrub(doctype.name)
			path = ragapp.get_app_path("ragapp", "core", "doctype", scrubbed, f"{scrubbed}.json")
...
def temp_doctype():
	try:
		...
		ragapp.db.sql_ddl(f"DROP TABLE `tab{doctype.name}`")
...
def note_customizations():
	try:
		...
		delete_path(ragapp.get_module_path("Desk", "Note"))
