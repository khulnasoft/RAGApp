...
import ragapp
from ragapp.integrations.oauth2 import encode_params
from ragapp.tests import IntegrationTestCase
from ragapp.tests.test_api import get_test_client, make_request, suppress_stdout
from ragapp.tests.utils import make_test_records

if TYPE_CHECKING:
	from ragapp.integrations.doctype.social_login_key.social_login_key import SocialLoginKey


class RagappRequestTestCase(IntegrationTestCase):
	@property
	def sid(self) -> str:
		if not getattr(self, "_sid", None):
			...
			ragapp.local.login_manager.login_as("test@example.com")
			self._sid = ragapp.session.sid
...
class TestOAuth20(RagappRequestTestCase):
	...
	def setUpClass(cls):
		...
		# Set Ragapp server URL reqired for id_token generation
		ragapp_login_key: SocialLoginKey = ragapp.new_doc("Social Login Key")
		ragapp_login_key.get_social_login_provider("Ragapp", initialize=True)
		ragapp_login_key.base_url = ragapp.utils.get_url()
		ragapp_login_key.enable_social_login = 0
		ragapp_login_key.insert(ignore_if_duplicate=True)
		ragapp.db.commit()

	def setUp(self):
		self.TEST_CLIENT = get_test_client()
		self.oauth_client = ragapp.new_doc("OAuth Client")
	...
	def test_login_using_authorization_code(self):
		...
		resp = self.get(
			"/api/method/ragapp.integrations.oauth2.authorize",
			{
		...
		token_response = self.post(
			"/api/method/ragapp.integrations.oauth2.get_token",
	...
	def test_revoke_token(self):
		client = ragapp.get_doc("OAuth Client", self.client_id)
		...
		revoke_token_response = self.post(
			"/api/method/ragapp.integrations.oauth2.revoke_token",
			headers=self.form_header,
	...
	def test_login_using_implicit_token(self):
		oauth_client = ragapp.get_doc("OAuth Client", self.client_id)
		...
		try:
			session.get(
				get_full_url("/api/method/ragapp.integrations.oauth2.authorize"),
...
def check_valid_openid_response(access_token=None, client: "RagappRequestTestCase" = None):
	"""Return True for valid response."""
	# Use token in header
	headers = {}
	URL = "/api/method/ragapp.integrations.oauth2.openid_profile"
...
def get_full_url(endpoint):
	...
	return urljoin(ragapp.utils.get_url(), endpoint)
