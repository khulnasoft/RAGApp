...
from ragapp.utils.password import check_password, decrypt, encrypt, passlibctx, update_password


class TestPassword(IntegrationTestCase):
	def setUp(self):
		ragapp.delete_doc("Email Account", "Test Email Account Password")
		ragapp.delete_doc("Email Account", "Test Email Account Password-new")
	...
	def make_email_account(self, name="Test Email Account Password"):
		if not ragapp.db.exists("Email Account", name):
			...
			return ragapp.get_doc("Email Account", name)
	...
	def test_hashed_password(self, user="test@example.com"):
		...
		self.assertRaises(ragapp.AuthenticationError, check_password, user, new_password)
	...
	def test_password_on_rename_user(self):
		...
		ragapp.rename_doc(doc.doctype, old_name, new_name)

		new_doc = ragapp.get_doc(doc.doctype, new_name)
		self.assertEqual(new_doc.get_password(), password)
		self.assertTrue(not get_password_list(doc))

		ragapp.rename_doc(doc.doctype, new_name, old_name)
