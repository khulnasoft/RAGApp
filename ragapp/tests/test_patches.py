...
import ragapp
from ragapp.modules import patch_handler
...
execute:ragapp.db.updatedb("Item")
execute:ragapp.function(arg="1")
...
class TestPatches(IntegrationTestCase):
	def test_patch_module_names(self):
		ragapp.flags.final_patches = []
		ragapp.flags.in_install = True
		for patchmodule in patch_handler.get_all_patches():
			if patchmodule.startswith("execute:"):
				...
				self.assertTrue(ragapp.get_attr(patchmodule.split(maxsplit=1)[0] + ".execute"))

		ragapp.flags.in_install = False

	def test_get_patch_list(self):
		pre = patch_handler.get_patches_from_app("ragapp", patch_handler.PatchType.pre_model_sync)
		post = patch_handler.get_patches_from_app("ragapp", patch_handler.PatchType.post_model_sync)
		all_patches = patch_handler.get_patches_from_app("ragapp")
	...
	def test_all_patches_are_marked_completed(self):
		all_patches = patch_handler.get_patches_from_app("ragapp")
		finished_patches = ragapp.db.count("Patch Log")
...
class TestPatchReader(IntegrationTestCase):
	def get_patches(self):
		return (
			patch_handler.get_patches_from_app("ragapp"),
			patch_handler.get_patches_from_app("ragapp", patch_handler.PatchType.pre_model_sync),
			patch_handler.get_patches_from_app("ragapp", patch_handler.PatchType.post_model_sync),
		)
...
def check_patch_files(app):
	...
	patch_dir = Path(ragapp.get_app_path(app)) / "patches"
	...
	for file in patch_dir.glob("**/*.py"):
		...
		try:
			patch_module = ragapp.get_module(module)
...
def _get_dotted_path(file: Path, app) -> str:
	app_path = Path(ragapp.get_app_path(app))
