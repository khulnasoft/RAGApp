...
- Time bound tests: Climarks are done on GHA before adding numbers
...
from ragapp.website.path_resolver import PathResolver
...
class TestPerformance(IntegrationTestCase):
	def reset_request_specific_caches(self):
		...
		ragapp.destroy()  # releases everything on ragapp.local
		ragapp.init(self.TEST_SITE)
		ragapp.connect()
		ragapp.clear_cache()

	def setUp(self) -> None:
		self.HOST = ragapp.utils.get_site_url(ragapp.local.site)

		self.reset_request_specific_caches()

	def test_meta_caching(self):
		ragapp.clear_cache()
		ragapp.get_meta("User")
	...
	def test_permitted_fieldnames(self):
		...
		doc = ragapp.new_doc("Prepared Report")
		# load permitted fieldnames once
		doc.permitted_fieldnames

		with patch("ragapp.model.base_document.get_permitted_fields") as mocked:
			doc.as_dict()
			# get_permitted_fields should not be called again
			mocked.assert_not_called()

	def test_set_value_query_count(self):
		ragapp.db.set_value("User", "Administrator", "interest", "Nothing")

		with self.assertQueryCount(1):
			ragapp.db.set_value("User", "Administrator", "interest", "Nothing")

		with self.assertQueryCount(1):
			ragapp.db.set_value("User", {"user_type": "System User"}, "interest", "Nothing")
	...
	def test_get_value_limits(self):
		...
		# Warm up code
		ragapp.db.get_values("User", filters=filters[0], limit=1)
		for filter in filters:
			with self.assertRowsRead(1):
				self.assertEqual(1, len(ragapp.db.get_values("User", filters=filter, limit=1)))
			with self.assertRowsRead(2):
				self.assertEqual(2, len(ragapp.db.get_values("User", filters=filter, limit=2)))

			self.assertEqual(
				len(ragapp.db.get_values("User", filters=filter)), ragapp.db.count("User", filter)
			)

			with self.assertRowsRead(1):
				ragapp.db.get_value("User", filters=filter)

			with self.assertRowsRead(1):
				ragapp.db.exists("User", filter)
	...
	def test_req_per_seconds_basic(self):
		...
		req_count = 1000
		client = RagappClient(self.HOST, "Administrator", self.ADMIN_PASSWORD)
	...
	def test_consistent_build_version(self):
		from ragapp.utils import get_build_version
	...
	def test_get_list_single_query(self):
		...
		user = ragapp.get_doc("User", TEST_USER)

		ragapp.set_user(TEST_USER)
		# Give full read access, no share/user perm check should be done.
		user.add_roles("System Manager")

		ragapp.get_list("User")
		with self.assertQueryCount(1):
			ragapp.get_list("User")

	def test_no_ifnull_checks(self):
		query = ragapp.get_all("DocType", {"autoname": ("is", "set")}, run=0).lower()
		self.assertNotIn("coalesce", query)
		self.assertNotIn("ifnull", query)

	def test_no_stale_ref_sql(self):
		"""ragapp.db.sql should not hold any internal references to result set.
	...
	def test_one_time_setup(self):
		...
		run = ragapp.qb._BuilderClasss.run

		ragapp.init(site, force=True)
		patched_run = ragapp.qb._BuilderClasss.run

		self.assertIs(run, patched_run, "ragapp.init should run one-time patching code just once")

	def test_cpu_allocation(self):
		from ragapp._optimizations import assign_core
...
@run_only_if(db_type_is.MARIADB)
class TestOverheadCalls(RagappAPITestCase):
	...
	def test_get_doc_overheads(self):
		sid = self.sid
		tables = len(ragapp.get_meta("User").get_table_fields())
