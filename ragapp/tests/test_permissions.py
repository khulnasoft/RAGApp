...
from ragapp.core.doctype.user_permission.user_permission import clear_user_permissions
from ragapp.core.page.permission_manager.permission_manager import add, remove, reset, update
from ragapp.desk.form.load import getdoc
from ragapp.permissions import (
	ALL_USER_ROLE,
	AUTOMATIC_ROLES,
	GUEST_ROLE,
	SYSTEM_USER_ROLE,
...
class TestPermissions(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		ragapp.clear_cache(doctype="Blog Post")
		user = ragapp.get_doc("User", "test1@example.com")
		user.add_roles("Website Manager")
		user.add_roles("System Manager")

		user = ragapp.get_doc("User", "test2@example.com")
		user.add_roles("Blogger")

		user = ragapp.get_doc("User", "test3@example.com")
		user.add_roles("Sales User")

		user = ragapp.get_doc("User", "testperm@example.com")
		user.add_roles("Website Manager")
	...
	def setUp(self):
		...
		ragapp.db.delete("User Permission")

		ragapp.set_user("test1@example.com")

	def tearDown(self):
		ragapp.set_user("Administrator")
		ragapp.db.set_value("Blogger", "_Test Blogger 1", "user", None)
	...
	def set_strict_user_permissions(ignore):
		ss = ragapp.get_doc("System Settings")
		ss.apply_strict_user_permissions = ignore
		ss.flags.ignore_mandatory = 1
		ss.save()

	def test_basic_permission(self):
		post = ragapp.get_doc("Blog Post", "-test-blog-post")
	...
	def test_select_permission(self):
		...
		ragapp.clear_cache(doctype="Blog Post")
		ragapp.set_user("test3@example.com")
		...
		self.assertRaises(ragapp.PermissionError, post.save)

		permitted_record = ragapp.get_list("Blog Post", fields="*", limit=1)[0]
		full_record = ragapp.get_all("Blog Post", fields="*", limit=1)[0]
	...
	def test_user_permissions_in_doc(self):
		...
		post1 = ragapp.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_user_permissions_in_report(self):
		...
		ragapp.set_user("test2@example.com")
		names = [d.name for d in ragapp.get_list("Blog Post", fields=["name", "blog_category"])]

		self.assertTrue("-test-blog-post-1" in names)
		self.assertFalse("-test-blog-post" in names)

	def test_default_values(self):
		doc = ragapp.new_doc("Blog Post")
	...
	def test_user_link_match_doc(self):
		blogger = ragapp.get_doc("Blogger", "_Test Blogger 1")
		blogger.user = "test2@example.com"
		blogger.save()

		ragapp.set_user("test2@example.com")

		post = ragapp.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(post.has_permission("read"))
	...
	def test_user_link_match_report(self):
		...
		names = [d.name for d in ragapp.get_list("Blog Post", fields=["name", "owner"])]
	...
	def test_not_allowed_to_set_user_permissions(self):
		...
		self.assertRaises(
			ragapp.PermissionError, add_user_permission, "Blog Post", "-test-blog-post", "test2@example.com"
		)
	...
	def test_read_if_explicit_user_permissions_are_set(self):
		...
		doc = ragapp.get_doc("Blog Post", "-test-blog-post")
		self.assertTrue(doc.has_permission("read"))

		# and not this one
		doc = ragapp.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_set_standard_fields_manually(self):
		...
		fake_owner = ragapp.db.get_value("User", {"name": ("!=", ragapp.session.user)})

		d = ragapp.new_doc("ToDo")
	...
	def test_dont_change_standard_constants(self):
		...
		user = ragapp.get_doc("User", ragapp.session.user)
		user.creation = now_datetime()
		self.assertRaises(ragapp.CannotChangeConstantError, user.save)
	...
	def test_set_only_once(self):
		blog_post = ragapp.get_meta("Blog Post")
		doc = ragapp.get_doc("Blog Post", "-test-blog-post-1")
		doc.db_set("title", "Old")
		blog_post.get_field("title").set_only_once = 1
		doc.title = "New"
		self.assertRaises(ragapp.CannotChangeConstantError, doc.save)
	...
	def test_set_only_once_child_table_rows(self):
		...
		doc = ragapp.get_doc("DocType", "Blog Post")
	...
	def test_user_permission_doctypes(self):
		...
		doc = ragapp.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(doc.has_permission("read"))
	...
	def test_insert_if_owner_with_user_permissions(self):
		...
		ragapp.delete_doc("Blog Post", "-test-blog-post-title")
		...
		self.assertRaises(ragapp.PermissionError, doc.insert)
		...
		ragapp.set_user("test2@example.com")
		doc = ragapp.get_doc(doc.doctype, doc.name)
		self.assertTrue(doc.has_permission("read"))
	...
	def test_strict_user_permissions(self):
		...
		ragapp.set_user("Administrator")
		ragapp.db.delete("Contact")
		ragapp.db.delete("Contact Email")
		ragapp.db.delete("Contact Phone")
		...
		allowed_contact = ragapp.get_doc("Contact", "_Test Contact For _Test Customer")
		other_contact = ragapp.get_doc("Contact", "_Test Contact For _Test Supplier")

		ragapp.set_user("test3@example.com")
		self.assertTrue(allowed_contact.has_permission("read"))
		self.assertTrue(other_contact.has_permission("read"))
		self.assertEqual(len(ragapp.get_list("Contact")), 2)
		...
		self.assertTrue(len(ragapp.get_list("Contact")), 1)
	...
	def test_contextual_user_permission(self):
		...
		posts = ragapp.get_all("Blog Post", fields=["name", "blogger"])
		...
		posts = ragapp.get_list("Blog Post", fields=["name", "blogger"])
	...
	def test_if_owner_permission_overrides_properly(self):
		...
		ragapp.set_user("test2@example.com")
		self.assertEqual(ragapp.get_list("Blog Post"), [])
	...
	def test_if_owner_permission_on_getdoc(self):
		...
		doclist = [d.name for d in ragapp.response.docs]
		self.assertTrue(doc.name in doclist)

		ragapp.set_user("test2@example.com")
		self.assertRaises(ragapp.PermissionError, getdoc, "Blog Post", doc.name)
	...
	def test_if_owner_permission_on_get_list(self):
		...
		ragapp.set_user("test2@example.com")
		self.assertIn(doc.name, ragapp.get_list("Blog Post", pluck="name"))
		...
		ragapp.set_user("test2@example.com")
		self.assertNotIn(doc.name, ragapp.get_list("Blog Post", pluck="name"))
	...
	def test_if_owner_permission_on_delete(self):
		...
		ragapp.set_user("testperm@example.com")
		...
		self.assertRaises(ragapp.PermissionError, ragapp.delete_doc, "Blog Post", doc.name)

		ragapp.set_user("test2@example.com")
		ragapp.delete_doc("Blog Post", "-test-blog-post-title-new-1")
	...
	def test_clear_user_permissions(self):
		...
		# reset the user
		ragapp.set_user(current_user)

	def test_child_permissions(self):
		ragapp.set_user("test3@example.com")
		self.assertIsInstance(ragapp.get_list("DefaultValue", parent_doctype="User", limit=1), list)

		# ragapp.get_list
		self.assertRaises(ragapp.PermissionError, ragapp.get_list, "DefaultValue")
		self.assertRaises(ragapp.PermissionError, ragapp.get_list, "DefaultValue", parent_doctype="ToDo")
		self.assertRaises(
			ragapp.PermissionError, ragapp.get_list, "DefaultValue", parent_doctype="DefaultValue"
		)
		...
		self.assertRaises(ragapp.PermissionError, doc.check_permission)
		...
		user = ragapp.get_doc("User", "Administrator")
		doc = user.append("defaults")
		self.assertRaises(ragapp.PermissionError, doc.check_permission)
	...
	def test_select_user(self):
		...
		for gender in ("Male", "Female"):
			if ragapp.db.exists("Gender", gender):
				continue

			ragapp.get_doc({"doctype": "Gender", "gender": gender}).insert()

		# asssign gender to test users
		ragapp.db.set_value("User", "test1@example.com", "gender", "Male")
		ragapp.db.set_value("User", "test2@example.com", "gender", "Female")
		ragapp.db.set_value("User", "test3@example.com", "gender", "Female")
		...
		ragapp.set_user("test3@example.com")
		users = ragapp.get_list("User", pluck="name")
	...
	def test_overrides_work_as_expected(self):
		...
		custom_role = ragapp.new_doc("Role", role_name=ragapp.generate_hash()).insert().name
		with self.set_user("Administrator"):
