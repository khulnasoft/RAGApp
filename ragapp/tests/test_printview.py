...
from ragapp.www.printview import get_html_and_style


class PrintViewTest(IntegrationTestCase):
	def test_print_view_without_errors(self):
		user = ragapp.get_last_doc("User")

		messages_before = ragapp.get_message_log()
		ret = get_html_and_style(doc=user.as_json(), print_format="Standard", no_letterhead=1)
		messages_after = ragapp.get_message_log()
	...
	def test_print_error(self):
		...
		self.assertRaises(ragapp.PermissionError, ragapp.attach_print, doc.doctype, doc.name)
