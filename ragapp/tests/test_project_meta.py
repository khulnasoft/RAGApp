# Copyright (c) 2024, Ragapp Technologies Pvt. Ltd. and Contributors
# License: MIT. See LICENSE
import datetime
from pathlib import Path

import ragapp
from ragapp.tests.classes.integration_test_case import IntegrationTestCase
...
class TestProjectMeta(IntegrationTestCase):
	def test_init_py_tax_paid(self):
		...
		ragapp/__init__.py has grown crazy big and keeps getting bigger. Plot the LOC over time and
		...
		init_py_size = len(Path(ragapp.__file__).read_text().splitlines())
		...
		self.assertLessEqual(
			init_py_size,
			expected,
			"""You have either added too many lines to ragapp/__init__.py or that file hasn't shrunk fast enough. Remember the Zen of Python:\n"""
