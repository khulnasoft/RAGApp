...
from ragapp.query_builder.builder import Function
from ragapp.query_builder.custom import ConstantColumn
from ragapp.query_builder.functions import (
	Cast_,
	Coalesce,
	CombineDatetime,
	Date,
...
def run_only_if(dbtype: db_type_is) -> Callable:
	return unittest.skipIf(db_type_is(ragapp.conf.db_type) != dbtype, f"Only runs for {dbtype.value}")
...
class TestCustomFunctionsMariaDB(IntegrationTestCase):
	...
	def test_constant_column(self):
		query = ragapp.qb.from_("DocType").select("name", ConstantColumn("John").as_("User"))
		self.assertEqual(query.get_sql(), "SELECT `name`,'John' `User` FROM `tabDocType`")

	def test_timestamp(self):
		note = ragapp.qb.DocType("Note")
		self.assertEqual(
	...
	def test_time(self):
		...
		select_query = ragapp.qb.from_(note).select(CombineDatetime(note.posting_date, note.posting_time))
	...
	def test_cast(self):
		...
		self.assertEqual(
			ragapp.qb.from_("red").from_(note).select("other", Cast_(note.name, "varchar")).get_sql(),
	...
	def test_round(self):
		...
		query = ragapp.qb.from_(note).select(Round(note.price))
		self.assertEqual("select round(`price`,0) from `tabnote`", str(query).lower())

		query = ragapp.qb.from_(note).select(Round(note.price, 3))
		self.assertEqual("select round(`price`,3) from `tabnote`", str(query).lower())

	def test_truncate(self):
		note = ragapp.qb.DocType("Note")
		query = ragapp.qb.from_(note).select(Truncate(note.price, 3))
...
class TestBuilderBase:
	def test_adding_tabs(self):
		self.assertEqual("tabNotes", ragapp.qb.DocType("Notes").get_sql())
		self.assertEqual("__Auth", ragapp.qb.DocType("__Auth").get_sql())
		self.assertEqual("Notes", ragapp.qb.Table("Notes").get_sql())

	def test_run_patcher(self):
		query = ragapp.qb.from_("ToDo").select("*").limit(1)
	...
	def test_agg_funcs(self):
		...
		ragapp.db.truncate(self.doctype_name)
		sample_data = {
			"doctype": self.doctype_name,
			"number": 1,
		}
		ragapp.get_doc(sample_data).insert(ignore_mandatory=True)
		sample_data["number"] = 3
		ragapp.get_doc(sample_data).insert(ignore_mandatory=True)
		sample_data["number"] = 4
		ragapp.get_doc(sample_data).insert(ignore_mandatory=True)
		self.assertEqual(ragapp.qb.max(self.doctype_name, "number"), 4)
		self.assertEqual(ragapp.qb.min(self.doctype_name, "number"), 1)
		self.assertAlmostEqual(ragapp.qb.avg(self.doctype_name, "number"), 2.666, places=2)
		self.assertEqual(ragapp.qb.sum(self.doctype_name, "number"), 8.0)
		ragapp.db.rollback()
...
class TestParameterization(IntegrationTestCase):
	def test_where_conditions(self):
		...
		query = ragapp.qb.from_(DocType).select(DocType.name).where(DocType.owner == "Administrator' --")
	...
	def test_set_conditions(self):
		DocType = ragapp.qb.DocType("DocType")
		query = ragapp.qb.update(DocType).set(DocType.value, "some_value")
	...
	def test_where_conditions_functions(self):
		DocType = ragapp.qb.DocType("DocType")
		query = (
			ragapp.qb.from_(DocType).select(DocType.name).where(Coalesce(DocType.search_fields == "subject"))
		)
	...
	def test_case(self):
		DocType = ragapp.qb.DocType("DocType")
		query = ragapp.qb.from_(DocType).select(
			Case()
	...
	def test_case_in_update(self):
		DocType = ragapp.qb.DocType("DocType")
		query = ragapp.qb.update(DocType).set(
			"parent",
			Case()
			.when(DocType.search_fields == "value", "other_value")
			.when(Coalesce(DocType.search_fields == "subject_in_function"), "true_value")
	...
	def test_named_parameter_wrapper(self):
		from ragapp.query_builder.terms import NamedParameterWrapper
...
class TestBuilderMaria(IntegrationTestCase, TestBuilderBase):
	def test_adding_tabs_in_from(self):
		self.assertEqual("SELECT * FROM `tabNotes`", ragapp.qb.from_("Notes").select("*").get_sql())
		self.assertEqual("SELECT * FROM `__Auth`", ragapp.qb.from_("__Auth").select("*").get_sql())

	def test_get_qb_type(self):
		from ragapp.query_builder import get_query_builder

		qb = get_query_builder(ragapp.db.db_type)
...
class TestBuilderPostgres(IntegrationTestCase, TestBuilderBase):
	...
	def test_replace_tables(self):
		info_schema = ragapp.qb.Schema("information_schema")
		self.assertEqual(
			'SELECT * FROM "pg_stat_all_tables"',
			ragapp.qb.from_(info_schema.tables).select("*").get_sql(),
		)

	def test_replace_fields_post(self):
		self.assertEqual("relname", ragapp.qb.Field("table_name").get_sql())
...
class TestMisc(IntegrationTestCase):
	...
	def test_union(self):
		...
		role = ragapp.qb.DocType("Role")
		users = ragapp.qb.from_(user).select(user.name)
		roles = ragapp.qb.from_(role).select(role.name)
