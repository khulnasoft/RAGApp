...
import ragapp
from ragapp.desk.search import get_names_for_mentions, search_link, search_widget
...
class TestSearch(IntegrationTestCase):
	...
	def test_validate_and_sanitize_search_inputs(self):
		...
		# should not fail if function is called via ragapp.call with extra arguments
		args = ("Random DocType", "Random", "email", "2", "10", dict())
		kwargs = {"as_dict": False}
		self.assertListEqual(ragapp.call("ragapp.tests.test_search.get_data", *args, **kwargs), [])
	...
	def test_reference_doctype(self):
		...
		results = search_link(
			...
			query="ragapp.tests.test_search.query_with_reference_doctype",
		)
		self.assertListEqual(results, [])

	def test_search_relevance(self):
		ragapp.db.set_value("Language", {"name": ("like", "e%")}, "enabled", 1)
		...
		ragapp.db.set_value("Language", "es", "idx", 10)
...
def setup_test_link_field_order(TestCase):
	...
	# Create Tree doctype
	if not ragapp.db.exists("DocType", TestCase.tree_doctype_name):
		TestCase.tree_doc = ragapp.get_doc(
			{
				"doctype": "DocType",
		...
		TestCase.tree_doc = ragapp.get_doc("DocType", TestCase.tree_doctype_name)

	# Create root for the tree doctype
	if not ragapp.db.exists(TestCase.tree_doctype_name, {"random": TestCase.parent_doctype_name}):
		ragapp.get_doc(
