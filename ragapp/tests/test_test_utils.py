...
class TestTestUtils(IntegrationTestCase):
	...
	def test_document_assertions(self):
		currency = ragapp.new_doc("Currency")
	...
	def test_thread_locals(self):
		ragapp.flags.temp_flag_to_be_discarded = True

	def test_temp_setting_changes(self):
		current_setting = ragapp.get_system_settings("logout_on_password_reset")

		with IntegrationTestCase.change_settings(
			"System Settings", {"logout_on_password_reset": int(not current_setting)}
		):
			updated_settings = ragapp.get_system_settings("logout_on_password_reset")
			self.assertNotEqual(current_setting, updated_settings)

		restored_settings = ragapp.get_system_settings("logout_on_password_reset")
...
def tearDownModule():
	...
	assert "temp_flag_to_be_discarded" not in ragapp.flags
	assert not ragapp.db.exists("Currency", "STONKS")
