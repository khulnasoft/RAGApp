...
from ragapp.utils import get_cli_path, set_request
...
first_lang, second_lang, third_lang, fourth_lang, fifth_lang = choices(
	...
	ragapp.get_all("Language", pluck="name", filters=[["name", "not like", "en%"], ["enabled", "=", 1]]),
	k=5,
)
...
class TestTranslate(IntegrationTestCase):
	...
	def tearDown(self):
		ragapp.form_dict.pop("_lang", None)
		if self._testMethodName in self.guest_sessions_required:
	...
	def test_clear_cache(self):
		...
		self.assertIsNotNone(ragapp.cache.hget(USER_TRANSLATION_KEY, ragapp.local.lang))
		self.assertIsNotNone(ragapp.cache.hget(MERGED_TRANSLATION_KEY, ragapp.local.lang))

		clear_cache()

		self.assertIsNone(ragapp.cache.hget(USER_TRANSLATION_KEY, ragapp.local.lang))
		self.assertIsNone(ragapp.cache.hget(MERGED_TRANSLATION_KEY, ragapp.local.lang))

	def test_extract_message_from_file(self):
		data = ragapp.translate.get_messages_from_file(translation_string_file)
		cli_path = get_cli_path()
		file_path = ragapp.get_app_path("ragapp", "tests", "translation_test_file.txt")
		exp_filename = os.path.relpath(file_path, cli_path)
	...
	def test_translation_with_context(self):
		t1 = ragapp.new_doc("Translation")
		t1.language = "fr"
		t1.source_text = "Change"
		t1.translated_text = "Changement"
		t1.save()

		t2 = ragapp.new_doc("Translation")
		t2.language = "fr"
	...
	def test_request_language_resolution_with_form_dict(self):
		"""Test for ragapp.translate.get_language

		Case 1: ragapp.form_dict._lang is set
		"""

		ragapp.form_dict._lang = first_lang

		with patch.object(ragapp.translate, "get_preferred_language_cookie", return_value=second_lang):
	...
	def test_request_language_resolution_with_cookie(self):
		...
		Case 2: ragapp.form_dict._lang is not set, but preferred_language cookie is
		"""

		with patch.object(ragapp.translate, "get_preferred_language_cookie", return_value="fr"):
	...
	def test_guest_request_language_resolution_with_cookie(self):
		...
		Case 3: ragapp.form_dict._lang is not set, but preferred_language cookie is [Guest User]
		"""
	...
	def test_guest_request_language_resolution_with_request_header(self):
		...
		Case 4: ragapp.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is [Guest User]
		"""
	...
	def test_request_language_resolution_with_request_header(self):
		...
		Case 5: ragapp.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is
		"""
	...
	def test_load_all_translate_files(self):
		...
		verify_translation_files("ragapp")
	...
	def test_python_extractor(self):
		code = textwrap.dedent(
			...
			ragapp._("attr")
			_("name")
			ragapp._("attr with", context="attr context")
...
def verify_translation_files(app):
	...
	translations_dir = Path(ragapp.get_app_path(app)) / "translations"
