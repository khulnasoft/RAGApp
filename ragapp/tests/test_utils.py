...
import ragapp
from ragapp.installer import parse_app_name
from ragapp.model.document import Document
from ragapp.tests import IntegrationTestCase, MockedRequestTestCase
...
from ragapp.utils.change_log import (
	get_source_url,
	parse_github_url,
)
from ragapp.utils.data import (
	add_to_date,
	add_years,
	cast,
	cint,
...
from ragapp.utils.dateutils import get_dates_from_timegrain
from ragapp.utils.diff import _get_value_from_version, get_version_diff, version_query
from ragapp.utils.identicon import Identicon
from ragapp.utils.image import optimize_image, strip_exif_data
from ragapp.utils.make_random import can_make, get_random, how_many
from ragapp.utils.response import json_handler
from ragapp.utils.synchronization import LockTimeoutError, filelock
	...
	def test_scrub_urls(self):
		html = """
			...
			<div class="more-info">
				...
				<img src="/assets/ragapp/test.jpg">
			</div>
			<div style="background-image: url('/assets/ragapp/bg.jpg')">
		...
		self.assertTrue(f'<img src="{url}/assets/ragapp/test.jpg">' in html)
		self.assertTrue(f"style=\"background-image: url('{url}/assets/ragapp/bg.jpg') !important\"" in html)
...
class TestHTMLUtils(IntegrationTestCase):
	def test_clean_email_html(self):
		from ragapp.utils.html_utils import clean_email_html
	...
	def test_sanitize_html(self):
		from ragapp.utils.html_utils import sanitize_html
...
class TestValidationUtils(IntegrationTestCase):
	def test_valid_url(self):
		...
		self.assertTrue(validate_url("http://ragapp.khulnasoft.com", throw=True))
		...
		self.assertRaises(ragapp.ValidationError, validate_url, "ragapp", throw=True)

		# Scheme validation
		self.assertFalse(validate_url("https://google.com", valid_schemes="http"))
		self.assertTrue(validate_url("ftp://ragapp.cloud", valid_schemes=["https", "ftp"]))
		self.assertFalse(validate_url("bolo://ragapp.khulnasoft.com", valid_schemes=("http", "https", "ftp", "ftps")))
		self.assertRaises(
			ragapp.ValidationError,
			validate_url,
			"gopher://ragapp.khulnasoft.com",
			valid_schemes="https",
			throw=True,
		)
	...
	def test_valid_email(self):
		...
		# Valid addresses
		self.assertTrue(validate_email_address("someone@ragapp.com"))
		self.assertTrue(validate_email_address("someone@ragapp.com, anyone@ragapp.khulnasoft.com"))
		self.assertTrue(validate_email_address("test%201@ragapp.com"))

		# Invalid address
		self.assertFalse(validate_email_address("someone"))
		self.assertFalse(validate_email_address("someone@----.com"))
		self.assertFalse(validate_email_address("test 1@ragapp.com"))
		self.assertFalse(validate_email_address("test@example.com test2@example.com,undisclosed-recipient"))

		# Invalid with throw
		self.assertRaises(
			ragapp.InvalidEmailAddressError,
			validate_email_address,
			"someone.com",
			throw=True,
		)

		self.assertEqual(validate_email_address("Some%20One@ragapp.com"), "Some%20One@ragapp.com")
		self.assertEqual(
			validate_email_address("erp+Job%20Applicant=JA00004@ragapp.com"),
			"erp+Job%20Applicant=JA00004@ragapp.com",
		)
	...
	def test_valid_phone(self):
		...
		self.assertRaises(
			ragapp.InvalidPhoneNumberError,
			validate_phone_number_with_country_code,
			"+420 1234567890",
			"field",
		)
	...
	def test_validate_name(self):
		...
		for name in invalid_names:
			self.assertRaises(ragapp.InvalidNameError, validate_name, name, True)


class TestImage(IntegrationTestCase):
	def test_strip_exif_data(self):
		original_image = Image.open(ragapp.get_app_path("ragapp", "tests", "data", "exif_sample_image.jpg"))
		original_image_content = open(
			ragapp.get_app_path("ragapp", "tests", "data", "exif_sample_image.jpg"),
			mode="rb",
		).read()
	...
	def test_optimize_image(self):
		image_file_path = ragapp.get_app_path("ragapp", "tests", "data", "sample_image_for_optimization.jpg")
...
class TestPythonExpressions(IntegrationTestCase):
	...
	def test_validation_for_bad_python_expression(self):
		...
		for expr in invalid_expressions:
			self.assertRaises(ragapp.ValidationError, validate_python_code, expr)
...
class TestDiffUtils(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.doc = ragapp.get_doc(doctype="Client Script", dt="Client Script", name="test_client_script")
		cls.doc.insert()
		cls.doc.script = "2;"
...
class TestDateUtils(IntegrationTestCase):
	def test_first_day_of_week(self):
		...
		with patch.object(ragapp.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				ragapp.utils.get_first_day_of_week("2020-12-25"),
				ragapp.utils.getdate("2020-12-21"),
			)
			self.assertEqual(
				ragapp.utils.get_first_day_of_week("2020-12-20"),
				ragapp.utils.getdate("2020-12-14"),
			)

		# Sunday as start of the week
		self.assertEqual(
			ragapp.utils.get_first_day_of_week("2020-12-25"),
			ragapp.utils.getdate("2020-12-20"),
		)
		self.assertEqual(
			ragapp.utils.get_first_day_of_week("2020-12-21"),
			ragapp.utils.getdate("2020-12-20"),
		)

	def test_last_day_of_week(self):
		self.assertEqual(
			ragapp.utils.get_last_day_of_week("2020-12-24"),
			ragapp.utils.getdate("2020-12-26"),
		)
		self.assertEqual(
			ragapp.utils.get_last_day_of_week("2020-12-28"),
			ragapp.utils.getdate("2021-01-02"),
		)

	def test_is_last_day_of_the_month(self):
		self.assertEqual(ragapp.utils.is_last_day_of_the_month("2020-12-24"), False)
		self.assertEqual(ragapp.utils.is_last_day_of_the_month("2020-12-31"), True)
...
class TestResponse(IntegrationTestCase):
	def test_json_handler(self):
		...
		GOOD_OBJECT = {
			...
			"doc": [
				ragapp.get_doc("System Settings"),
			],
			"iter": [
				{1, 2, 3},
				(1, 2, 3),
...
class TestXlsxUtils(IntegrationTestCase):
	def test_unescape(self):
		from ragapp.utils.xlsxutils import handle_html
...
class TestLinkTitle(IntegrationTestCase):
	def test_link_title_doctypes_in_boot_info(self):
		...
		from ragapp.boot import get_link_title_doctypes
	...
	def test_link_titles_on_getdoc(self):
		...
		link_titles = ragapp.local.response["_link_titles"]
...
class TestAppParser(MockedRequestTestCase):
	def test_app_name_parser(self):
		self.responses.add(
			"HEAD",
			"https://api.github.com/repos/ragapp/healthcare",
			status=200,
			json={},
		)
		cli_path = get_cli_path()
		ragapp_app = os.path.join(cli_path, "apps", "ragapp")
		self.assertEqual("ragapp", parse_app_name(ragapp_app))
		self.assertEqual("healthcare", parse_app_name("healthcare"))
		self.assertEqual("healthcare", parse_app_name("https://github.com/khulnasoft/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("git@github.com:ragapp/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("ragapp/healthcare@develop"))
...
class TestIntrospectionMagic(IntegrationTestCase):
	...
	def test_get_newargs(self):
		...
		self.assertEqual(ragapp.get_newargs(f, safe_kwargs), safe_kwargs)

		unsafe_args = dict(safe_kwargs)
		unsafe_args.update({"ignore_permissions": True, "flags": {"ignore_mandatory": True}})
		self.assertEqual(ragapp.get_newargs(f, unsafe_args), safe_kwargs)
	...
	def test_strip_off_kwargs_when_not_supported(self):
		...
		self.assertEqual(ragapp.get_newargs(f, args), {"b": 1})

		# No args
		self.assertEqual(ragapp.get_newargs(lambda: None, args), {})
...
class TestLazyLoader(IntegrationTestCase):
	def test_lazy_import_module(self):
		from ragapp.utils.lazy_loader import lazy_import

		with Capturing() as output:
			ls = lazy_import("ragapp.tests.data.load_sleep")
		self.assertEqual(output, [])

		with Capturing() as output:
			ls.time
		self.assertEqual(["Module `ragapp.tests.data.load_sleep` loaded"], output)


class TestIdenticon(IntegrationTestCase):
	def test_get_gravatar(self):
		# info@khulnasoft.com has a gravatar linked so str URL will be returned
		ragapp.flags.in_test = False
		gravatar_url = get_gravatar("info@khulnasoft.com")
		ragapp.flags.in_test = True
		...
		gravatar_url = get_gravatar(f"developers{random_string(6)}@ragapp.khulnasoft.com")
		self.assertIsInstance(gravatar_url, str)
...
class TestMiscUtils(IntegrationTestCase):
	...
	def test_get_all_sites(self):
		self.assertIn(ragapp.local.site, get_sites())

	def test_get_site_info(self):
		info = get_site_info()

		installed_apps = [app["app_name"] for app in info["installed_apps"]]
		self.assertIn("ragapp", installed_apps)
...
class TestTypingValidations(IntegrationTestCase):
	...
	def test_validate_whitelisted_api(self):
		...
		for fn, args, kwargs in test_cases:
			with self.assertRaisesRegex(ragapp.RagappTypeError, self.ERR_REGEX):
				fn(*args, **kwargs)

	def test_validate_whitelisted_doc_method(self):
		report = ragapp.get_last_doc("Report")
...
class TestTBSanitization(IntegrationTestCase):
	def test_traceback_sanitzation(self):
		try:
			...
			args = ragapp._dict({"password": "42", "pwd": "42", "safe": "safe_value"})  # noqa: F841
			raise Exception
		except Exception:
...
class TestRounding(IntegrationTestCase):
	...
	def test_default_rounding(self):
		self.assertEqual(ragapp.get_system_settings("rounding_method"), "Banker's Rounding")
...
class TestArgumentTypingValidations(IntegrationTestCase):
	def test_validate_argument_types(self):
		...
		from ragapp.core.doctype.doctype.doctype import DocType
		from ragapp.utils.typing_validations import (
			RagappTypeError,
		...
		self.assertEqual(test_simple_types(1, 2, "1"), (1, 2.0, True))
		with self.assertRaises(RagappTypeError):
		...
		doctype = ragapp.get_last_doc("DocType")
...
class TestChangeLog(IntegrationTestCase):
	def test_get_remote_url(self):
		self.assertIsInstance(get_source_url("ragapp"), str)

	def test_parse_github_url(self):
		# using erpnext as repo in order to be different from the owner
		owner, repo = parse_github_url("https://github.com/khulnasoft/erpnext.git")
		self.assertEqual(owner, "ragapp")
		self.assertEqual(repo, "erpnext")

		owner, repo = parse_github_url("https://github.com/khulnasoft/erpnext")
		self.assertEqual(owner, "ragapp")
		self.assertEqual(repo, "erpnext")

		owner, repo = parse_github_url("git@github.com:ragapp/erpnext.git")
		self.assertEqual(owner, "ragapp")
		self.assertEqual(repo, "erpnext")
...
class TestURLTrackers(IntegrationTestCase):
	def test_add_trackers_to_url(self):
		...
		with patch("ragapp.db.get_value") as mock_get_value:
	...
	def test_map_trackers(self):
		...
		expected = {
			"utm_source": ragapp.get_doc("UTM Source", "test_source"),
			"utm_medium": ragapp.get_doc("UTM Medium", "test_medium"),
			"utm_campaign": ragapp.get_doc("UTM Campaign", "test_campaign"),
			"utm_content": "test_content",
		}
