...
import ragapp
from ragapp import get_hooks
from ragapp.tests import IntegrationTestCase
from ragapp.utils import set_request
from ragapp.website.page_renderers.static_page import StaticPage
from ragapp.website.serve import get_response, get_response_content
from ragapp.website.utils import build_response, clear_website_cache, get_home_page
...
class TestWebsite(IntegrationTestCase):
	...
	def test_home_page(self):
		...
		role = ragapp.get_doc(doctype="Role", role_name="home-page-test", desk_access=0).insert(
		...
		ragapp.db.set_value("Role", "home-page-test", "home_page", "home-page-test")
		ragapp.set_user("test-user-for-home-page@example.com")
		self.assertEqual(get_home_page(), "home-page-test")

		ragapp.set_user("Administrator")
		ragapp.db.set_value("Role", "home-page-test", "home_page", "")

		# home page via portal settings
		ragapp.db.set_single_value("Portal Settings", "default_portal_home", "test-portal-home")

		ragapp.set_user("test-user-for-home-page@example.com")
		ragapp.cache.hdel("home_page", ragapp.session.user)
		self.assertEqual(get_home_page(), "test-portal-home")

		ragapp.db.set_single_value("Portal Settings", "default_portal_home", "")
		clear_website_cache()

		# home page via website settings
		ragapp.db.set_single_value("Website Settings", "home_page", "contact")
		self.assertEqual(get_home_page(), "contact")

		ragapp.db.set_single_value("Website Settings", "home_page", None)
		clear_website_cache()
		...
		with patch.object(
			...
			"get_hooks",
			patched_get_hooks(
				"get_website_user_home_page", ["ragapp.www._test._test_home_page.get_website_user_home_page"]
			),
		):
			self.assertEqual(get_home_page(), "_test/_test_folder")

		clear_website_cache()
		with patch.object(ragapp, "get_hooks", patched_get_hooks("website_user_home_page", ["login"])):
			self.assertEqual(get_home_page(), "login")

		clear_website_cache()
		with patch.object(ragapp, "get_hooks", patched_get_hooks("home_page", ["about"])):
			self.assertEqual(get_home_page(), "about")

		clear_website_cache()
		with patch.object(
			ragapp, "get_hooks", patched_get_hooks("role_home_page", {"home-page-test": ["home-page-test"]})
		):
			self.assertEqual(get_home_page(), "home-page-test")
	...
	def test_page_load(self):
		...
		html = ragapp.safe_decode(response.get_data())
	...
	def test_redirect(self):
		...
		ragapp.hooks.website_redirects = [
		...
		delattr(ragapp.hooks, "website_redirects")
		ragapp.client_cache.delete_value("app_hooks")
	...
	def test_custom_page_renderer(self):
		...
		def patched_get_hooks(*args, **kwargs):
			...
			if args and args[0] == "page_renderer":
				return_value = ["ragapp.tests.test_website.CustomPageRenderer"]
			return return_value

		with patch.object(ragapp, "get_hooks", patched_get_hooks):
	...
	def test_printview_page(self):
		ragapp.db.value_cache[("DocType", "Language", "name")] = (("Language",),)
		ragapp.set_user("Administrator")
	...
	def test_json_sidebar_data(self):
		ragapp.flags.look_for_sidebar = False
		content = get_response_content("/_test/_test_folder/_test_page")
		self.assertNotIn("Test Sidebar", content)
		clear_website_cache()
		ragapp.flags.look_for_sidebar = True
		content = get_response_content("/_test/_test_folder/_test_page")
	...
	def test_caching(self):
		# to enable caching
		ragapp.flags.force_website_cache = True
	...
	def test_safe_render(self):
		...
		self.assertNotIn("ragapp.exceptions.ValidationError: Illegal template", content)

	def test_never_render(self):
		from pathlib import Path
		from random import choices

		WWW = Path(ragapp.get_app_path("ragapp")) / "www"
	...
	def test_resolve_class(self):
		from ragapp.utils.jinja_globals import resolve_class

		context = ragapp._dict(primary=True)
		...
		self.assertEqual(
			ragapp.render_template(content, context), '<a class="btn btn-default btn-primary">Test</a>'
		)
	...
	def test_app_include(self):
		...
		with patch.object(ragapp, "get_hooks", patched_get_hooks):
			ragapp.set_user("Administrator")
			ragapp.hooks.app_include_js.append("test_app_include.js")
			ragapp.hooks.app_include_css.append("test_app_include.css")
			ragapp.conf.update({"app_include_js": ["test_app_include_via_site_config.js"]})
			ragapp.conf.update({"app_include_css": ["test_app_include_via_site_config.css"]})
