...
UI_TEST_USER = "ragapp@example.com"


def whitelist_for_tests(fn):
	if ragapp.request and not ragapp.flags.in_test and not getattr(ragapp.local, "dev_server", 0):
		ragapp.throw("Cannot run UI tests. Use a development server with `cli start`")

	return ragapp.whitelist()(fn)
...
def create_if_not_exists(doc):
	...
	for doc in docs:
		...
		filters = doc.copy()
		filters.pop("doctype")
		name = ragapp.db.exists(doc.doctype, filters)
		if not name:
			d = ragapp.get_doc(doc)
...
def prepare_webform_test():
	for note in ragapp.get_all("Note", pluck="name"):
		ragapp.delete_doc("Note", note, force=True)

	ragapp.delete_doc_if_exists("Web Form", "note")
	...
	doc = ragapp.get_doc(
		{
			...
			"communication_date": ragapp.utils.now_datetime(),
		}
	)
	doc.insert()
	return doc
...
@whitelist_for_tests
def setup_workflow():
	from ragapp.workflow.doctype.workflow.test_workflow import create_todo_workflow
...
def create_contact_phone_nos_records():
	if ragapp.get_all("Contact", {"first_name": "Test Contact"}):
		return

	doc = ragapp.new_doc("Contact")
...
def create_contact_records():
	if ragapp.get_all("Contact", {"first_name": "Test Form Contact 1"}):
		return
...
def create_multiple_todo_records():
	if ragapp.get_all("ToDo", {"description": "Multiple ToDo 1"}):
		return

	values = [(f"100{i}", f"Multiple ToDo {i}") for i in range(1, 1002)]

	ragapp.db.bulk_insert("ToDo", fields=["name", "description"], values=set(values))


def insert_contact(first_name, phone_number):
	doc = ragapp.get_doc({"doctype": "Contact", "first_name": first_name})
...
def create_form_tour():
	if ragapp.db.exists("Form Tour", {"name": "Test Form Tour"}):
		return
...
def create_web_page(title, route, single_thread):
	web_page = ragapp.db.exists("Web Page", {"route": route})
	if web_page:
		return web_page
	web_page = ragapp.get_doc({"doctype": "Web Page", "title": title, "route": route, "published": True})
	web_page.save()
	...
	web_page.append(
		"page_blocks",
		{
			...
			"web_template_values": ragapp.as_json(
				{
					"title": "Discussions",
...
def update_webform_to_multistep():
	if not ragapp.db.exists("Web Form", "update-profile-duplicate"):
		doc = ragapp.get_doc("Web Form", "edit-profile")
		_doc = ragapp.copy_doc(doc)
...
def insert_doctype_with_child_table_record(name):
	if ragapp.get_all(name, {"title": "Test Grid Search"}):
		return
...
def insert_translations():
	...
	for doc in translation:
		if not ragapp.db.exists("doc"):
			ragapp.get_doc(doc).insert()


@whitelist_for_tests
def create_blog_post():
	blog_category = ragapp.get_doc(
		{"name": "general", "doctype": "Blog Category", "title": "general"}
	).insert(ignore_if_duplicate=True)

	blogger = ragapp.get_doc(
		{
...
def create_test_user(username=None):
	...
	if ragapp.db.exists("User", name):
		return

	user = ragapp.new_doc("User")
	user.email = name
	user.first_name = "Ragapp"
	user.new_password = ragapp.local.conf.admin_password
	...
	all_roles = set(ragapp.get_all("Role", pluck="name"))
	...
	ragapp.db.set_single_value("Workspace Settings", "workspace_setup_completed", 1)


@whitelist_for_tests
def setup_tree_doctype():
	ragapp.delete_doc_if_exists("DocType", "Custom Tree", force=True)
	...
	if not ragapp.db.exists("Custom Tree", "All Trees"):
		ragapp.get_doc({"doctype": "Custom Tree", "tree": "All Trees"}).insert()


@whitelist_for_tests
def setup_image_doctype():
	ragapp.delete_doc_if_exists("DocType", "Custom Image", force=True)
...
def setup_default_view(view, force_reroute=None):
	ragapp.delete_doc_if_exists("Property Setter", "Event-main-default_view")
	ragapp.delete_doc_if_exists("Property Setter", "Event-main-force_re_route_to_default_view")
...
@whitelist_for_tests
def create_kanban():
	if not ragapp.db.exists("Custom Field", "Note-kanban"):
		ragapp.get_doc(
			{
				"is_system_generated": 0,
				"dt": "Note",
	...
	if not ragapp.db.exists("Kanban Board", "_Note _Kanban"):
		ragapp.get_doc(
			{
...
def create_todo(description):
	return ragapp.get_doc({"doctype": "ToDo", "description": description}).insert()
...
def create_admin_kanban():
	if not ragapp.db.exists("Kanban Board", "Admin Kanban"):
		ragapp.get_doc(
			{
				"doctype": "Kanban Board",
				"name": "Admin Kanban",
...
def publish_progress(duration=3, title=None, doctype=None, docname=None):
	...
	ragapp.enqueue(slow_task, duration=duration, title=title, doctype=doctype, docname=docname)
...
def slow_task(duration, title, doctype, docname):
	...
	for i in range(steps + 1):
		ragapp.publish_progress(i * 10, title=title, doctype=doctype, docname=docname)
