...
class TypeExporter:
	def __init__(self, doc):
		...
		self.imports = {"from ragapp.types import DF"}
		self.indent = "\t"
		self.controller_path = (
			Path(ragapp.get_module_path(doc.module))
			/ "doctype"
			/ scrub(self.doctype)
	...
	def _validate_code(code) -> bool:
		"""Make sure whatever code Ragapp adds dynamically is valid python."""
		try:
			ast.parse(code)
			return True
		except Exception:
			ragapp.msgprint(ragapp._("Failed to export python type hints"), alert=True)
			return False
