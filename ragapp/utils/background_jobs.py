...
from ragapp.utils import CallbackManager, cint, get_cli_id
from ragapp.utils.commands import log
...
def get_queues_timeout() -> dict[str, int]:
	...
	"""
	common_site_config = ragapp.get_conf()
...
def enqueue(
	...
	**kwargs,
	...
	duplicate calls
	:param now: if now=True, the method is executed via ragapp.call()
	...
	if deduplicate:
		if not job_id:
			ragapp.throw(_("`job_id` paramater is required for deduplication."))
		job = get_job(job_id)
		if job and job.get_status() in (JobStatus.QUEUED, JobStatus.STARTED):
			ragapp.logger().debug(f"Not queueing job {job.id} because it is in queue already")
			return
		elif job:
	...
	if not is_async and not ragapp.flags.in_test:
		from ragapp.deprecation_dumpster import deprecation_warning
	...
	call_directly = now or (not is_async and not ragapp.flags.in_test)
	if call_directly:
		return ragapp.call(method, **kwargs)

	try:
		q = get_queue(queue, is_async=is_async)
	except ConnectionError:
		if ragapp.local.flags.in_migrate:
	...
	def enqueue_call():
		return q.enqueue_call(
			...
			kwargs=queue_args,
			at_front=at_front,
			failure_ttl=ragapp.conf.get("rq_job_failure_ttl") or RQ_JOB_FAILURE_TTL,
			result_ttl=ragapp.conf.get("rq_results_ttl") or RQ_RESULTS_TTL,
			job_id=job_id,
		)

	if enqueue_after_commit:
		ragapp.db.after_commit.add(enqueue_call)
		return
...
def enqueue_doc(doctype, name=None, method=None, queue="default", timeout=300, now=False, **kwargs):
	...
	return enqueue(
		"ragapp.utils.background_jobs.run_doc_method",
		doctype=doctype,
		name=name,
		doc_method=method,
		queue=queue,
...
def run_doc_method(doctype, name, doc_method, **kwargs):
	getattr(ragapp.get_doc(doctype, name), doc_method)(**kwargs)
...
def execute_job(site, method, event, job_name, kwargs, user=None, is_async=True, retry=0):
	...
	for before_job_task in ragapp.get_hooks("before_job"):
		ragapp.call(before_job_task, method=method_name, kwargs=kwargs, transaction_type="job")

	try:
		retval = method(**kwargs)

	except (ragapp.db.InternalError, ragapp.RetryBackgroundJobError) as e:
		ragapp.db.rollback()

		if retry < 5 and (
			isinstance(e, ragapp.RetryBackgroundJobError)
			or (ragapp.db.is_deadlocked(e) or ragapp.db.is_timedout(e))
		):
			# retry the job if
			# 1213 = deadlock
			# 1205 = lock wait timeout
			# or RetryBackgroundJobError is explicitly raised
			ragapp.job.after_job.reset()
			ragapp.destroy()
			time.sleep(retry + 1)
			...
			ragapp.log_error(title=method_name)
			raise

	except Exception as e:
		ragapp.db.rollback()
		ragapp.log_error(title=method_name)
		ragapp.monitor.add_data_to_monitor(exception=e.__class__.__name__)
		ragapp.db.commit()
		...
		if not hasattr(ragapp.local, "site"):
			ragapp.init(site)
			ragapp.connect()
		for after_job_task in ragapp.get_hooks("after_job"):
			ragapp.call(after_job_task, method=method_name, kwargs=kwargs, result=retval)
		ragapp.local.job.after_job.run()
...
class RagappWorker(Worker):
	def work(self, *args, **kwargs):
		self.start_ragapp_scheduler()
	...
	def start_ragapp_scheduler(self):
		from ragapp.utils.scheduler import start_scheduler
	...
	def subscribe(self):
		...
		# ref: https://github.com/khulnasoft/caffeine/issues/46
...
def start_worker_pool(
	...
	import ragapp.database.query  # sqlparse and indirect imports
	import ragapp.query_builder  # pypika
	import ragapp.utils  # common utils
	import ragapp.utils.safe_exec
	import ragapp.utils.scheduler
	...
	pool = WorkerPool(
		...
		worker_class=RagappWorker,  # Auto starts scheduler with workerpool
	)
...
def validate_queue(queue: str, default_queue_list: list | None = None) -> None:
	...
	if queue not in default_queue_list:
		ragapp.throw(_("Queue should be one of {0}").format(", ".join(default_queue_list)))
...
def get_redis_conn(username=None, password=None):
	if not hasattr(ragapp.local, "conf"):
		raise Exception("You need to call ragapp.init")
	...
	if conf.get("use_rq_auth"):
		if username:
			...
			cred["username"] = conf.rq_username or get_cli_id()
	...
	try:
		...
		log(
			...
			"You can reset credentials using `cli create-rq-users` CLI and restart the server",
			colour="red",
		)
		raise
	except Exception as e:
		log(
			f"Please make sure that Redis Queue runs @ {ragapp.get_conf().redis_queue}. Redis reported error: {e!s}",
			colour="red",
		)
		raise
...
def get_queues(connection=None) -> list[Queue]:
	"""Get all the queues linked to the current cli."""
...
def create_job_id(job_id: str | None = None) -> str:
	...
	if not job_id:
		job_id = str(uuid4())
	return f"{ragapp.local.site}::{job_id}"
...
def truncate_failed_registry(job, connection, type, value, traceback):
	...
	from ragapp.utils import create_batch

	conf = ragapp.conf if ragapp.conf else ragapp.get_conf(site=job.kwargs.get("site"))
...
def flush_telemetry():
	...
	to push events."""
	ph = getattr(ragapp.local, "posthog", None)
	with suppress(Exception):
		ph and ph.flush()


def _start_sentry():
	sentry_dsn = os.getenv("RAGAPP_SENTRY_DSN")
	if not sentry_dsn:
		return
