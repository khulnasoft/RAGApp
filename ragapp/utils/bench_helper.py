...
def RagappClickWrapper(cls, handler):
	class Cls(cls):
...
def main():
	...
	commands.update({"get-ragapp-commands": get_ragapp_commands, "get-ragapp-help": get_ragapp_help})
	RagappClickWrapper(click.Group, handle_exception)(commands=commands)(prog_name="cli")


def get_app_groups() -> dict[str, click.Group | click.Command]:
	"""Get all app groups, put them in main group "ragapp" since cli is
	...
	return dict(
		ragapp=click.group(
			name="ragapp", commands=commands, cls=RagappClickWrapper(click.Group, handle_exception)
		)(app_group)
	)
...
def get_app_group(app: str) -> click.Group:
	if app_commands := get_app_commands(app):
		return click.group(
			name=app, commands=app_commands, cls=RagappClickWrapper(click.Group, handle_exception)
		)(app_group)
...
def app_group(ctx, site=False, force=False, verbose=False, profile=False):
	...
	if ctx.info_name == "ragapp":
		ctx.info_name = ""


def get_sites(site_arg: str) -> list[str]:
	if site_arg == "all":
		return ragapp.utils.get_sites()
	elif site_arg:
		return [site_arg]
	elif env_site := os.environ.get("RAGAPP_SITE"):
		return [env_site]
	elif default_site := ragapp.get_conf().default_site:
		return [default_site]
	# This is not supported, just added here for warning.
	elif (site := ragapp.read_file("currentsite.txt")) and site.strip():
		click.secho(
			dedent(
				f"""
...
@click.command("get-ragapp-commands")
def get_ragapp_commands():
	commands = list(get_app_commands("ragapp"))
...
@click.command("get-ragapp-help")
def get_ragapp_help():
	print(click.Context(get_app_groups()["ragapp"]).get_help())


def get_apps():
	return ragapp.get_all_apps(with_internal_apps=False, sites_path=".")
