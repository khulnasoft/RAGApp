...
def make_boilerplate(dest, app_name, no_git=False):
	...
	app_name = ragapp.scrub(app_name)
...
def _get_user_inputs(app_name):
	...
	new_app_config = {
		...
		"branch_name": {"prompt": "Branch Name", "default": get_app_branch("ragapp")},
	}
...
def copy_from_ragapp(rel_path: str, new_app_path: str):
	"""Copy files from ragapp app to new app."""
	src = Path(ragapp.get_app_path("ragapp", "..")) / rel_path
...
def _create_app_boilerplate(dest, hooks, no_git=False):
	ragapp.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, ragapp.scrub(hooks.app_title)),
		with_init=True,
	)
	ragapp.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates"), with_init=True)
	ragapp.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "www"))
	ragapp.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "pages"), with_init=True
	)
	ragapp.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "includes"))
	ragapp.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "config"), with_init=True)
	ragapp.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "css"))
	ragapp.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "js"))

	# add .gitkeep file so that public folder is committed to git
	# this is needed because if public doesn't exist, cli build doesn't symlink the apps assets
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "__init__.py"), "w") as f:
		f.write(ragapp.as_unicode(init_template))

	with open(os.path.join(dest, hooks.app_name, "pyproject.toml"), "w") as f:
		f.write(ragapp.as_unicode(pyproject_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, ".pre-commit-config.yaml"), "w") as f:
		f.write(ragapp.as_unicode(precommit_template.format(**hooks)))

	license_body = get_license_text(license_name=hooks.app_license)
	with open(os.path.join(dest, hooks.app_name, "license.txt"), "w") as f:
		f.write(ragapp.as_unicode(license_body))

	with open(
		os.path.join(dest, hooks.app_name, hooks.app_name, ragapp.scrub(hooks.app_title), ".ragapp"), "w"
	) as f:
		f.write("")

	from ragapp.deprecation_dumpster import boilerplate_modules_txt
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "hooks.py"), "w") as f:
		f.write(ragapp.as_unicode(hooks_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "patches.txt"), "w") as f:
		f.write(ragapp.as_unicode(patches_template.format(**hooks)))

	app_directory = os.path.join(dest, hooks.app_name)

	copy_from_ragapp(".editorconfig", app_directory)
	copy_from_ragapp(".eslintrc", app_directory)
	...
	with open(os.path.join(dest, hooks.app_name, "README.md"), "w") as f:
		f.write(ragapp.as_unicode(readme_template.format(**hooks)))

	if not no_git:
		with open(os.path.join(dest, hooks.app_name, ".gitignore"), "w") as f:
			f.write(ragapp.as_unicode(gitignore_template.format(app_name=hooks.app_name)))
...
class PatchCreator:
	def __init__(self):
		self.all_apps = ragapp.get_all_apps(sites_path=".", with_internal_apps=False)
	...
	def _ask_app_name(self):
		...
		self.app_dir = pathlib.Path(ragapp.get_app_path(self.app))
	...
	def _ask_patch_meta_info(self):
		...
		default_filename = ragapp.scrub(self.docstring) + ".py"
...
dynamic = ["version"]
dependencies = [
    # "ragapp~=15.0.0" # Installed and managed by cli.
]
...
typing-modules = ["ragapp.types.DF"]
...
# Desk Notifications
# ------------------
# See ragapp.core.notifications.get_notification_config
...
# 	"ragapp.desk.doctype.event.event.get_events": "{app_name}.event.get_events"
# }}
#
# each overriding function accepts a `data` argument;
# generated from the base implementation of the doctype dashboard,
# along with any modifications made in other Ragapp apps
    ...
    steps:
      ...
      - name: Setup
        run: |
          ...
          cli init --skip-redis-config-generation --skip-assets --python "$(which python)" ~/ragapp-cli
          mariadb --host 127.0.0.1 --port 3306 -u root -proot -e "SET GLOBAL character_set_server = 'utf8mb4'"
          mariadb --host 127.0.0.1 --port 3306 -u root -proot -e "SET GLOBAL collation_server = 'utf8mb4_unicode_ci'"

      - name: Install
        working-directory: /home/runner/ragapp-cli
        run: |
          cli get-app {app_name} $GITHUB_WORKSPACE
          cli setup requirements --dev
          cli new-site --db-root-password root --admin-password admin test_site
          cli --site test_site install-app {app_name}
          cli build
        env:
          CI: 'Yes'
      ...
      - name: Run Tests
        working-directory: /home/runner/ragapp-cli
        run: |
          cli --site test_site set-config allow_tests true
          cli --site test_site run-tests --app {app_name}
        env:
          TYPE: server
"""
...
# Read docs to understand patches: https://ragapp.khulnasoft.com/docs/v14/user/en/database-migrations
  ...
  linter:
    ...
    steps:
      ...
      - name: Download Semgrep rules
        run: git clone --depth 1 https://github.com/khulnasoft/semgrep-rules.git ragapp-semgrep-rules

      - name: Run Semgrep rules
        run: |
          pip install semgrep
          semgrep ci --config ./ragapp-semgrep-rules/rules --config r/python.lang.correctness
...
You can install this app using the [cli](https://github.com/khulnasoft/cli) CLI:

```bash
cd $PATH_TO_YOUR_CLI
cli get-app $URL_OF_THIS_REPO --branch {branch_name}
cli install-app {app_name}
```
...
- Linters: Runs [Ragapp Semgrep Rules](https://github.com/khulnasoft/semgrep-rules) and [pip-audit](https://pypi.org/project/pip-audit/) on every pull request.
