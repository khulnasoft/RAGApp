...
EXCLUDE_EXCEPTIONS = (
	...
	ragapp.CSRFTokenError,  # CSRF covers OAuth too
	ragapp.SecurityException,
	ragapp.InReadOnlyMode,
)
...
def log_error(title=None, message=None, reference_doctype=None, reference_name=None, *, defer_insert=False):
	...
	from ragapp.utils.sentry import capture_exception
	...
	traceback = ragapp.as_unicode(traceback or ragapp.get_traceback(with_context=True))
	...
	if ragapp.flags.read_only or defer_insert:
...
def log_error_snapshot(exception: Exception):
	...
	logger = ragapp.logger(with_more_info=True)
	...
	def decorator_raise_error_on_no_output(func):
		...
		def wrapper_raise_error_on_no_output(*args, **kwargs):
			...
			if (not response) and raise_error:
				ragapp.throw(error_message, error_type or Exception)
			return response
...
def guess_exception_source(exception: str) -> str | None:
	...
	- For ragapp.throws the exception source is possibly present after skipping ragapp.throw frames
	...
	with suppress(Exception):
		...
		if (probably_source := apps.most_common(1)) and probably_source[0][0] != "ragapp":
