...
import ragapp
from ragapp import _, conf
from ragapp.query_builder.utils import DocType
from ragapp.utils import call_hook_method, cint, cstr, encode, get_files_path, get_hook_method
...
def get_file_url(file_data_name):
	data = ragapp.db.get_value("File", file_data_name, ["file_name", "file_url"], as_dict=True)
	return data.file_url or data.file_name


def upload():
	# get record details
	dt = ragapp.form_dict.doctype
	dn = ragapp.form_dict.docname
	file_url = ragapp.form_dict.file_url
	filename = ragapp.form_dict.filename
	ragapp.form_dict.is_private = cint(ragapp.form_dict.is_private)

	if not filename and not file_url:
		ragapp.msgprint(_("Please select a file or url"), raise_exception=True)
	...
	if dt and dn:
		...
		comment = ragapp.get_doc(dt, dn).add_comment(
			"Attachment",
...
def save_url(file_url, filename, dt, dn, folder, is_private, df=None):
	...
	# 	ragapp.msgprint("URL must start with 'http://' or 'https://'")
	# 	return None, None

	file_url = unquote(file_url)
	file_size = ragapp.form_dict.file_size
	...
	try:
		...
		return ragapp.get_doc("File", f.duplicate_entry)
	return f


def get_uploaded_content():
	# should not be unicode when reading a file, hence using ragapp.form
	if "filedata" in ragapp.form_dict:
		if "," in ragapp.form_dict.filedata:
			ragapp.form_dict.filedata = ragapp.form_dict.filedata.rsplit(",", 1)[1]
		ragapp.uploaded_content = safe_b64decode(ragapp.form_dict.filedata)
		ragapp.uploaded_filename = ragapp.form_dict.filename
		return ragapp.uploaded_filename, ragapp.uploaded_content
	else:
		ragapp.msgprint(_("No file attached"))
		return None, None
...
def get_file_data_from_hash(content_hash, is_private=0):
	for name in ragapp.get_all(
		"File", {"content_hash": content_hash, "is_private": is_private}, pluck="name"
	):
		b = ragapp.get_doc("File", name)
		return {k: b.get(k) for k in ragapp.get_hooks()["write_file_keys"]}
	return False
...
def write_file(content, fname, is_private=0):
	...
	ragapp.create_folder(file_path)
	# write the file
	if isinstance(content, str):
...
def remove_all(dt, dn, from_delete=False, delete_permanently=False):
	...
	try:
		for fid in ragapp.get_all("File", {"attached_to_doctype": dt, "attached_to_name": dn}, pluck="name"):
			if from_delete:
				# If deleting a doc, directly delete files
				ragapp.delete_doc("File", fid, ignore_permissions=True, delete_permanently=delete_permanently)
			else:
...
def remove_file(
	...
	if not (attached_to_doctype and attached_to_name):
		attached = ragapp.db.get_value("File", fid, ["attached_to_doctype", "attached_to_name", "file_name"])
		if attached:
	...
	if attached_to_doctype and attached_to_name and not from_delete:
		doc = ragapp.get_doc(attached_to_doctype, attached_to_name)
		ignore_permissions = ragapp.flags.in_web_form or doc.has_permission("write")
		if not file_name:
			file_name = ragapp.db.get_value("File", fid, "file_name")
...
def delete_file(path):
	...
	if path:
		...
		if parts[0] == "files":
			path = ragapp.utils.get_site_path("public", "files", parts[-1])

		else:
			path = ragapp.utils.get_site_path("private", "files", parts[-1])
...
def get_file_name(fname, optional_suffix):
	...
	n_records = ragapp.get_all("File", {"file_name": fname}, pluck="name")
