...
def sync_fixtures(app=None):
	...
	ragapp.flags.in_fixtures = True

	for app in apps:
		import_fixtures(app)
		import_custom_scripts(app)

	ragapp.flags.in_fixtures = False


def import_fixtures(app):
	fixtures_path = ragapp.get_app_path(app, "fixtures")
	...
	for fname in fixture_files:
		...
		file_path = ragapp.get_app_path(app, "fixtures", fname)
		try:
			import_doc(file_path, sort=True)
		except (ImportError, ragapp.DoesNotExistError) as e:
...
def import_custom_scripts(app):
	...
	scripts_folder = ragapp.get_app_path(app, "fixtures", "custom_scripts")
...
def export_fixtures(app=None):
	...
	if app:
		...
		apps = ragapp.get_installed_apps()
	for app in apps:
		fixture_auto_order = bool(next(iter(ragapp.get_hooks("fixture_auto_order", app_name=app)), False))
		fixtures = ragapp.get_hooks("fixtures", app_name=app)
		prefix = None
		for index, fixture in enumerate(fixtures, start=1):
			...
			if not os.path.exists(ragapp.get_app_path(app, "fixtures")):
				os.mkdir(ragapp.get_app_path(app, "fixtures"))

			filename = ragapp.scrub(fixture)
			if prefix:
			...
			export_json(
				fixture,
				ragapp.get_app_path(app, "fixtures", filename + ".json"),
				filters=filters,
