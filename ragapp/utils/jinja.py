...
def get_jenv():
	...
	if jenv := getattr(ragapp.local, "jenv", None):
		return jenv
...
def _get_jenv():
	...
	from ragapp.utils.safe_exec import UNSAFE_ATTRIBUTES

	UNSAFE_ATTRIBUTES = UNSAFE_ATTRIBUTES - {"format", "format_map"}

	class RagappSandboxedEnvironment(SandboxedEnvironment):
	...
	# ragapp will be loaded last, so app templates will get precedence
	jenv = RagappSandboxedEnvironment(loader=get_jloader(), undefined=DebugUndefined, cache_size=32)
	set_filters(jenv)
...
def validate_template(html):
	...
	try:
		...
		ragapp.throw(f"Syntax error in template as line {e.lineno}: {e.message}")
...
def render_template(template, context=None, is_path=None, safe_render=True):
	...
	from ragapp import _, get_traceback, throw
...
def get_jloader():
	...
	ragapp.local.jloader = jloader  # backward compat
	return jloader
...
def _get_jloader():
	...
	apps = ragapp.get_hooks("template_apps")
	if not apps:
		apps = list(reversed(ragapp.get_installed_apps(_ensure_on_cli=True)))

	if "ragapp" not in apps:
		apps.append("ragapp")
...
def get_jinja_hooks():
	...
	if not getattr(ragapp.local, "site", None):
		return (None, None)
	...
	def get_obj_dict_from_paths(object_paths):
		...
		for obj_path in object_paths:
			try:
				obj = ragapp.get_module(obj_path)
			except ModuleNotFoundError:
				obj = ragapp.get_attr(obj_path)
