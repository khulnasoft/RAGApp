...
class SignupDisabledError(ragapp.PermissionError): ...


def get_oauth2_providers() -> dict[str, dict]:
	out = {}
	providers = ragapp.get_all("Social Login Key", fields=["*"])
...
def get_oauth_keys(provider: str) -> dict[str, str]:
	...
	if keys := ragapp.conf.get(f"{provider}_login"):
		return {"client_id": keys["client_id"], "client_secret": keys["client_secret"]}

	return {
		"client_id": ragapp.db.get_value("Social Login Key", provider, "client_id"),
...
def get_oauth2_authorize_url(provider: str, redirect_to: str) -> str:
	...
	state = {
		"site": ragapp.utils.get_url(),
		"token": ragapp.generate_hash(),
...
def get_redirect_uri(provider: str) -> str:
	keys = ragapp.conf.get(f"{provider}_login")
	...
	return ragapp.utils.get_url(redirect_uri)
...
def get_info_via_oauth(provider: str, code: str, decoder: Callable | None = None, id_token: bool = False):
	...
	if not (info.get("email_verified") or info.get("email")):
		ragapp.throw(_("Email not verified with {0}").format(provider.title()))
...
def login_oauth_user(
	...
	*,
	...
	if not (state and state["token"]):
		ragapp.respond_as_web_page(_("Invalid Request"), _("Token is missing"), http_status_code=417)
		return
	...
	try:
		...
		return ragapp.respond_as_web_page(
			"Signup is Disabled",
	...
	ragapp.local.login_manager.user = user
	ragapp.local.login_manager.post_login()

	# because of a GET request!
	ragapp.db.commit()

	if ragapp.utils.cint(generate_login_token):
		login_token = ragapp.generate_hash(length=32)
		ragapp.cache.set_value(f"login_token:{login_token}", ragapp.local.session.sid, expires_in_sec=120)

		ragapp.response["login_token"] = login_token

	else:
		redirect_to = state.get("redirect_to")
		redirect_post_login(
			desk_user=ragapp.local.response.get("message") == "Logged In",
			redirect_to=redirect_to,
			provider=provider,
		)
...
def get_user_record(user: str, data: dict, provider: str) -> "User":
	from ragapp.integrations.doctype.social_login_key.social_login_key import provider_allows_signup

	try:
		return ragapp.get_doc("User", user)
	except ragapp.DoesNotExistError:
		if not provider_allows_signup(provider):
			raise SignupDisabledError

	user: User = ragapp.new_doc("User")
	...
	user.update(
		{
			...
			"gender": gender,
			"enabled": 1,
			"new_password": ragapp.generate_hash(),
...
def update_oauth_user(user: str, data: dict, provider: str):
	...
	if not user.enabled:
		ragapp.respond_as_web_page(_("Not Allowed"), _("User {0} is disabled").format(user.email))
		return False
	...
	if update_user_record:
		...
		if default_role := ragapp.db.get_single_value("Portal Settings", "default_role"):
...
def redirect_post_login(desk_user: bool, redirect_to: str | None = None, provider: str | None = None):
	...
	if not redirect_to:
		...
		redirect_to = ragapp.utils.get_url(desk_uri if desk_user else "/me")

	ragapp.local.response["location"] = redirect_to
