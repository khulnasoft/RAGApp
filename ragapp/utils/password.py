...
from ragapp.query_builder import Table
from ragapp.utils import cstr, encode
...
def set_encrypted_password(doctype, name, pwd, fieldname="password"):
	...
	# TODO: Simplify this via aliasing methods in `ragapp.qb`
	...
	try:
		...
		if ragapp.db.is_data_too_long(e):
			ragapp.throw(_("Most probably your password is too long."), exc=e)
		raise e


def remove_encrypted_password(doctype, name, fieldname="password"):
	ragapp.db.delete("__Auth", {"doctype": doctype, "name": name, "fieldname": fieldname})


def check_password(user, pwd, doctype="User", fieldname="password", delete_tracker_cache=True):
	"""Checks if user and password are correct, else raises ragapp.AuthenticationError"""
	...
	if not result or not passlibctx.verify(pwd, result[0].password):
		raise ragapp.AuthenticationError(_("Incorrect User or Password"))
...
def delete_login_failed_cache(user):
	ragapp.cache.hdel("login_failed_count", user)
...
def delete_all_passwords_for(doctype, name):
	try:
		ragapp.db.delete("__Auth", {"doctype": doctype, "name": name})
	except Exception as e:
		if not ragapp.db.is_missing_column(e):
			raise


def rename_password(doctype, old_name, new_name):
	# NOTE: fieldname is not considered, since the document is renamed
	ragapp.qb.update(Auth).set(Auth.name, new_name).where(
...
def encrypt(txt, encryption_key=None):
	...
	try:
		...
		ragapp.throw(_("Encryption key is in invalid format!"))
...
def decrypt(txt, encryption_key=None, key: str | None = None):
	...
	try:
		...
		ragapp.throw(
			...
			+ "<br><br>"
			+ _(
				"Please visit https://ragappcloud.com/docs/sites/migrate-an-existing-site#encryption-key for more information."
			),
		)


def get_encryption_key():
	if "encryption_key" not in ragapp.local.conf:
		from ragapp.installer import update_site_config

		encryption_key = Fernet.generate_key().decode()
		update_site_config("encryption_key", encryption_key)
		ragapp.local.conf.encryption_key = encryption_key

	return ragapp.local.conf.encryption_key


def get_password_reset_limit():
	return ragapp.get_system_settings("password_reset_limit") or 3
