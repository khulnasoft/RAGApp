...
from ragapp.www.printview import validate_print_permission
...
def download_multi_pdf_async(
	...
	ragapp.local.response["http_status_code"] = http.HTTPStatus.CREATED
...
def _download_multi_pdf(
	...
	if not isinstance(doctype, dict):
		...
		for idx, ss in enumerate(result):
			try:
				...
				if task_id:
					ragapp.publish_realtime(task_id=task_id, message={"message": "Failed"})
		...
		if task_id is None:
			ragapp.local.response.filename = "{doctype}.pdf".format(
		...
		for doctype_name in doctype:
			...
			for doc_name in doctype[doctype_name]:
				try:
					...
					if task_id:
						ragapp.publish_realtime(task_id=task_id, message="Failed")
					ragapp.log_error(
		...
		if task_id is None:
			ragapp.local.response.filename = f"{name}.pdf"
	...
	with BytesIO() as merged_pdf:
		...
		if task_id:
			...
			_file.save()
			ragapp.publish_realtime(f"task_complete:{task_id}", message={"file_url": _file.unique_url})
		else:
			ragapp.local.response.filecontent = merged_pdf.getvalue()
			ragapp.local.response.type = "pdf"


from ragapp.deprecation_dumpster import read_multi_pdf
...
def download_pdf(
	...
	doc = doc or ragapp.get_doc(doctype, name)
	validate_print_permission(doc)

	with print_language(language):
		pdf_file = ragapp.get_print(
			doctype, name, format, doc=doc, as_pdf=True, letterhead=letterhead, no_letterhead=no_letterhead
		)

	ragapp.local.response.filename = "{name}.pdf".format(name=name.replace(" ", "-").replace("/", "-"))
	ragapp.local.response.filecontent = pdf_file
	ragapp.local.response.type = "pdf"
...
def report_to_pdf(html, orientation="Landscape"):
	...
	ragapp.local.response.filename = "report.pdf"
	ragapp.local.response.filecontent = get_pdf(html, {"orientation": orientation})
	ragapp.local.response.type = "pdf"
...
def print_by_server(
	...
	print_settings = ragapp.get_doc("Network Printer Settings", printer_setting)
	try:
		import cups
	except ImportError:
		ragapp.throw(_("You need to install pycups to use this feature!"))
	...
	try:
		...
		output = PdfWriter()
		output = ragapp.get_print(
			doctype, name, print_format, doc=doc, no_letterhead=no_letterhead, as_pdf=True, output=output
		)
		if not file_path:
			file_path = os.path.join("/", "tmp", f"ragapp-pdf-{ragapp.generate_hash()}.pdf")
		...
		if (
			...
			ragapp.throw(_("PDF generation failed"))
	except cups.IPPError:
		ragapp.throw(_("Printing failed"))
