...
import ragapp
from ragapp.utils import get_cli_id, random_string
...
class RedisQueue:
	...
	def add_user(self, username, password=None):
		...
		return ragapp._dict(user_settings) if is_created else {}

	@classmethod
	def get_connection(cls, username=None, password=None):
		conf = ragapp.get_site_config()
		if conf.redis_queue_sentinel_enabled:
			from ragapp.utils.redis_wrapper import get_sentinel_connection
	...
	def get_acl_key_rules(cls, include_key_prefix=False):
		...
		rules = ["rq:[^q]*", "rq:queues", f"rq:queue:{get_cli_id()}:*"]
	...
	def gen_acl_list(cls, set_admin_password=False):
		...
		This list contains default ACL user and the cli ACL user(used by all sites incase of ACL is enabled).
		"""
		cli_username = get_cli_id()
		cli_user_rules = cls.get_acl_key_rules(include_key_prefix=True) + cls.get_acl_command_rules()
		cli_user_rule_str = " ".join(cli_user_rules).strip()
		cli_user_password = random_string(20)
		...
		return [
			...
			f"user {cli_username} on >{cli_user_password} {cli_user_rule_str}",
		], {
			"cli": (cli_username, cli_user_password),
