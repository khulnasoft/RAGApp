...
class RedisWrapper(redis.Redis):
	...
	def __call__(self):
		"""WARNING: Added for backward compatibility to support ragapp.cache().method(...)"""
		return self
	...
	def make_key(self, key, user=None, shared=False):
		...
		return f"{ragapp.conf.db_name}|{key}".encode()
	...
	def get_value(self, key, generator=None, user=None, expires=False, shared=False, *, use_local_cache=True):
		...
		:param expires: If the key is supposed to be with an expiry, don't store it in ragapp.local
		"""
		original_key = key
		key = self.make_key(key, user, shared)

		local_cache = ragapp.local.cache
	...
	def get_keys(self, key):
		...
		try:
			...
			return [k for k in list(ragapp.local.cache) if regex.match(cstr(k))]
	...
	def hset(
		...
		**kwargs,
		...
		# set in local
		ragapp.local.cache.setdefault(_name, {})[key] = value
	...
	def hdel(
		...
		:param shared: shared ragapp key or not
		:param pipeline: A redis.client.Pipeline object, if this transaction is to be run in a pipeline
		"""
		_name = self.make_key(name, shared=shared)

		name_in_local_cache = _name in ragapp.local.cache

		if not isinstance(keys, list | tuple):
			if name_in_local_cache and keys in ragapp.local.cache[_name]:
				del ragapp.local.cache[_name][keys]
			if pipeline:
		...
		for key in keys:
			if name_in_local_cache:
				if key in ragapp.local.cache[_name]:
					del ragapp.local.cache[_name][key]
...
def setup_cache() -> RedisWrapper:
	if ragapp.conf.redis_cache_sentinel_enabled:
		sentinels = [tuple(node.split(":")) for node in ragapp.conf.get("redis_cache_sentinels", [])]
		sentinel = get_sentinel_connection(
			sentinels=sentinels,
			sentinel_username=ragapp.conf.get("redis_cache_sentinel_username"),
			sentinel_password=ragapp.conf.get("redis_cache_sentinel_password"),
			master_username=ragapp.conf.get("redis_cache_master_username"),
			master_password=ragapp.conf.get("redis_cache_master_password"),
		)
		return sentinel.master_for(
			ragapp.conf.get("redis_cache_master_service"),
			redis_class=RedisWrapper,
		)

	return RedisWrapper.from_url(ragapp.conf.get("redis_cache"))
...
class ClientCache:
	...
	This feature is internal to Ragapp Framework and is subjected to change without any notice.
	...
	Usage/Notes:
		...
		- Same keys can be accessed with `ragapp.cache` too, but that won't implement invalidation.
		...
		- Never use `ragapp.cache`'s request local cache along with client-side cache. Two
	...
	def get_doc(self, doctype: str, name: str | None = None):
		...
		key = ragapp.get_document_cache_key(doctype, name)
		return self.get_value(key, generator=lambda: ragapp.get_doc(doctype, name))
