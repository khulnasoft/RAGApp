...
import ragapp
from ragapp.utils import cint, get_cli_path, get_datetime, get_sites, now_datetime
from ragapp.utils.background_jobs import set_niceness
...
def _proctitle(message):
	setproctitle.setthreadtitle(f"ragapp-scheduler: {message}")
...
def start_scheduler() -> NoReturn:
	...
	lock_path = os.path.abspath(os.path.join(get_cli_path(), "config", "scheduler_process"))

	try:
		lock = FileLock(lock_path)
		lock.acquire(blocking=False)
	except Timeout:
		ragapp.logger("scheduler").debug("Scheduler already running")
		return
...
def enqueue_events_for_all_sites() -> None:
	...
	for site in sites:
		try:
			...
			ragapp.logger("scheduler").debug(f"Failed to enqueue events for site: {site}", exc_info=True)


def enqueue_events_for_site(site: str) -> None:
	def log_exc():
		ragapp.logger("scheduler").error(f"Exception in Enqueue Events for Site {site}", exc_info=True)
	...
	try:
		...
		ragapp.logger("scheduler").debug(f"Queued events for site {site}")
	except Exception as e:
		if ragapp.db.is_access_denied(e):
			ragapp.logger("scheduler").debug(f"Access denied for site {site}")
		log_exc()
...
def enqueue_events() -> list[str] | None:
	if schedule_jobs_based_on_activity():
		enqueued_jobs = []
		all_jobs = ragapp.get_all("Scheduled Job Type", filters={"stopped": 0}, fields="*")
		random.shuffle(all_jobs)
		for job_type in all_jobs:
			job_type = ragapp.get_doc(doctype="Scheduled Job Type", **job_type)
			try:
				if job_type.enqueue():
					enqueued_jobs.append(job_type.method)
			except CroniterBadCronError:
				ragapp.logger("scheduler").error(
					f"Invalid Job on {ragapp.local.site} - {job_type.name}", exc_info=True
				)
...
def is_scheduler_inactive(verbose=True) -> bool:
	if ragapp.local.conf.maintenance_mode:
		if verbose:
			cprint(f"{ragapp.local.site}: Maintenance mode is ON")
		return True

	if ragapp.local.conf.pause_scheduler:
		if verbose:
			cprint(f"{ragapp.local.site}: ragapp.conf.pause_scheduler is SET")
		return True
...
def is_scheduler_disabled(verbose=True) -> bool:
	if ragapp.conf.disable_scheduler:
		if verbose:
			cprint(f"{ragapp.local.site}: ragapp.conf.disable_scheduler is SET")
		return True

	scheduler_disabled = not ragapp.utils.cint(
		ragapp.db.get_single_value("System Settings", "enable_scheduler")
	)
	if scheduler_disabled:
		if verbose:
			cprint(f"{ragapp.local.site}: SystemSettings.enable_scheduler is UNSET")
	return scheduler_disabled


def toggle_scheduler(enable):
	ragapp.db.set_single_value("System Settings", "enable_scheduler", int(enable))
...
def is_dormant(check_time=None):
	...
	since = (ragapp.get_system_settings("dormant_days") or 4) * 86400
...
def _get_last_creation_timestamp(doctype):
	timestamp = ragapp.db.get_value(doctype, filters={}, fieldname="creation", order_by="creation desc")
	if timestamp:
...
def activate_scheduler():
	...
	if ragapp.local.conf.maintenance_mode:
		ragapp.throw(ragapp._("Scheduler can not be re-enabled when maintenance mode is active."))

	if is_scheduler_disabled():
		enable_scheduler()
	if ragapp.conf.pause_scheduler:
