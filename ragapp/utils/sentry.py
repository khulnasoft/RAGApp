...
from ragapp.database.database import Database, EmptyQueryValues


class RagappIntegration(Integration):
	identifier = "ragapp"
...
def set_scope(scope):
	if job := rq.get_current_job():
		...
		context = ragapp._dict({"scheduled": False, "wait": 0})
		...
		if ragapp.form_dict.cmd:
			path = f"/api/method/{ragapp.form_dict.cmd}"
		else:
			path = ragapp.request.path
	...
	scope.set_user({"id": ragapp.local.site})
	user = getattr(ragapp.session, "user", "Unidentified")
	scope.set_tag("ragapp_user", user)
	# Extract `X-Ragapp-Request-ID` to store as a separate field if its present
	if trace_id := ragapp.monitor.get_trace_id():
		scope.set_tag("ragapp_trace_id", trace_id)
	...
	try:
		...
		with hub.configure_scope() as scope:
			if (
				...
				set_scope(scope)
			if ragapp.request:
				evt_processor = _make_wsgi_event_processor(ragapp.request.environ, False)
				scope.add_event_processor(evt_processor)
				if ragapp.request.is_json:
					scope.set_context("JSON Body", ragapp.request.json)
				elif ragapp.request.form:
					scope.set_context("Form Data", ragapp.request.form)

		if client := hub.client:
			exc_info = sys.exc_info()
			if any(exc_info):
				# Don't report errors which we can't "fix" in code
				if isinstance(exc_info[1], ragapp.ValidationError | ragapp.PermissionError):
					return
		...
		ragapp.logger().error("Failed to capture exception", exc_info=True)
