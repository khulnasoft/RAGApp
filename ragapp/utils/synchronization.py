...
from ragapp.utils import get_cli_path, get_site_path
from ragapp.utils.file_lock import LockTimeoutError
...
def filelock(lock_name: str, *, timeout=30, is_global=False):
	...
	args:
	        ...
	        is_global: if set lock is global to cli

	Lock file location:
	        global - {cli_dir}/config/{name}.lock
	        site - {cli_dir}/sites/sitename/{name}.lock
	...
	if not is_global:
		...
		lock_path = os.path.abspath(os.path.join(get_cli_path(), "config", lock_filename))

	try:
		with _StrongFileLock(lock_path, timeout=timeout):
			yield
	except Timeout as e:
		ragapp.log_error(f"Filelock: Failed to aquire {lock_path}")
