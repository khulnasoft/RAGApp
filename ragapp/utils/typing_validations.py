...
from ragapp.exceptions import RagappTypeError
...
RagappPydanticConfig = ConfigDict(arbitrary_types_allowed=True)
...
def TypeAdapter(type_):
	...
	try:
		return PyTypeAdapter(type_, config=RagappPydanticConfig)
...
def transform_parameter_types(func: Callable, args: tuple, kwargs: dict):
	...
	func_signature = ragapp._cached_inspect_signature(func)
