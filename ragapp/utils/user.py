...
from ragapp.boot import get_allowed_reports
from ragapp.core.doctype.domain_settings.domain_settings import get_active_modules
from ragapp.permissions import AUTOMATIC_ROLES, get_roles, get_valid_perms
from ragapp.query_builder import DocType, Order
...
class UserPermissions:
	"""
	A user permission object can be accessed as `ragapp.get_user()`
	"""

	def __init__(self, name=""):
		self.defaults = None
		self.name = name or ragapp.session.get("user")
		self.roles = []
	...
	def setup_user(self):
		def get_user_doc():
			user = None
			try:
				user = ragapp.get_doc("User", self.name).as_dict()
			except ragapp.DoesNotExistError:
				pass
			except Exception as e:
				# install boo-boo
				if not ragapp.db.is_table_missing(e):
					raise

			return user

		if not ragapp.flags.in_install_db and not ragapp.flags.in_test:
			user_doc = ragapp.cache.hget("user_doc", self.name, get_user_doc)
			if user_doc:
				self.doc = ragapp.get_doc(user_doc)
	...
	def build_perm_map(self):
		...
		for r in get_valid_perms():
			...
			for k in ragapp.permissions.rights:
	...
	def build_permissions(self):
		...
		self.build_perm_map()
		user_shared = ragapp.share.get_shared_doctypes()
		no_list_view_link = []
		...
		if "System Manager" in self.get_roles():
			self.can_import += ragapp.get_all("DocType", {"allow_import": 1}, pluck="name")
			self.can_import += ragapp.get_all(
				"Property Setter",
				pluck="doc_type",
				filters={"property": "allow_import", "value": "1"},
			)

		ragapp.cache.hset("can_import", ragapp.session.user, self.can_import)

	def get_defaults(self):
		import ragapp.defaults

		self.defaults = ragapp.defaults.get_defaults(self.name)
	...
	def load_user(self):
		...
		if d.get("default_workspace"):
			workspace = ragapp.get_cached_doc("Workspace", d.default_workspace)
			d.default_workspace = {
				"name": workspace.name,
		...
		d.name = self.name
		d.onboarding_status = ragapp.parse_json(d.onboarding_status)
...
def get_fullname_and_avatar(user: str) -> _dict:
	first_name, last_name, avatar, name = ragapp.get_cached_value(
...
def get_system_managers(only_name: bool = False) -> list[str]:
	...
	system_managers = (
		...
		.where(
			...
			& (User.docstatus < 2)
			& (User.name.notin(ragapp.STANDARD_USERS))
		)
		.select(*fields)
...
def add_role(user: str, role: str) -> None:
	ragapp.get_doc("User", user).add_roles(role)
...
def is_website_user(username: str | None = None) -> str | None:
	return ragapp.db.get_value("User", username or ragapp.session.user, "user_type") == "Website User"
...
def is_system_user(username: str | None = None) -> str | None:
	...
	return ragapp.db.get_value(
		"User",
		{
			"email": username or ragapp.session.user,
			"enabled": 1,
			"user_type": "System User",
		},
		cache=True,
