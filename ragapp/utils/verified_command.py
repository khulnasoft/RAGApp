...
def get_secret():
	from ragapp.utils.password import get_encryption_key

	return ragapp.local.conf.get("secret") or get_encryption_key()


def verify_request():
	"""Verify if the incoming signed request if it is correct."""
	query_string = ragapp.safe_decode(
		ragapp.local.flags.signed_query_string or getattr(ragapp.request, "query_string", None)
	)
	...
	if signature_string in query_string:
		...
		valid_method = ragapp.request.method == "GET"
		valid_request_data = not (ragapp.request.form or ragapp.request.data)
...
def get_url(cmd, params, nonce=None, secret=None):
	...
	return ragapp.utils.get_url("".join(["api/method/", cmd, "?", urlencode(params)]))


def get_signature(params, nonce, secret=None):
	params = "".join(ragapp.utils.cstr(p) for p in params.values())
	if not secret:
		secret = ragapp.local.conf.get("secret") or "secret"
