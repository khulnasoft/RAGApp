...
from ragapp.utils import random_string, set_request
from ragapp.website.doctype.blog_post.blog_post import get_blog_list
...
class TestBlogPost(IntegrationTestCase):
	...
	def test_generator_not_found(self):
		pages = ragapp.get_all("Blog Post", fields=["name", "route"], filters={"published": 0}, limit=1)

		route = f"test-route-{ragapp.generate_hash(length=5)}"

		ragapp.db.set_value("Blog Post", pages[0].name, "route", route)
	...
	def test_category_link(self):
		...
		blog_page_html = ragapp.safe_decode(blog_page_response.get_data())
		...
		cached_value = ragapp.db.value_cache.get(("DocType", "Blog Post", "name"))
		ragapp.db.value_cache[("DocType", "Blog Post", "name")] = (("Blog Post",),)
		...
		category_page_html = ragapp.safe_decode(category_page_response.get_data())
		# Category page should contain the blog post title
		self.assertIn(blog.title, category_page_html)

		# Cleanup
		ragapp.db.value_cache[("DocType", "Blog Post", "name")] = cached_value
		ragapp.delete_doc("Blog Post", blog.name)
		ragapp.delete_doc("Blog Category", blog.blog_category)
	...
	def test_blog_pagination(self):
		...
		filters = ragapp._dict({"blog_category": scrub(category_title)})
		...
		for blog in blogs:
			ragapp.delete_doc(blog.doctype, blog.name)
		ragapp.delete_doc("Blog Category", blogs[0].blog_category)

	def test_caching(self):
		# to enable caching
		ragapp.flags.force_website_cache = True
		print(ragapp.session.user)
	...
	def test_like_dislike(self):
		...
		ragapp.db.delete("Comment", {"comment_type": "Like", "reference_doctype": "Blog Post"})

		from ragapp.templates.includes.likes.likes import like
...
def make_test_blog(category_title="Test Blog Category"):
	...
	if not ragapp.db.exists("Blog Category", category_name):
		ragapp.get_doc(doctype="Blog Category", title=category_title).insert()
	if not ragapp.db.exists("Blogger", "test-blogger"):
		ragapp.get_doc(doctype="Blogger", short_name="test-blogger", full_name="Test Blogger").insert()
