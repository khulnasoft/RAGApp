...
class Blogger(Document):
	...
	def validate(self):
		if self.user and not ragapp.db.exists("User", self.user):
			# for data import
			ragapp.get_doc(
	...
	def on_update(self):
		...
		from ragapp.website.doctype.blog_post.blog_post import clear_blog_cache

		clear_blog_cache()

		if self.user:
			for blog in ragapp.db.sql_list(
				"""select name from `tabBlog Post` where owner=%s
				and ifnull(blogger,'')=''""",
				self.user,
			):
				b = ragapp.get_doc("Blog Post", blog)
				b.blogger = self.name
				b.save()

			ragapp.permissions.add_user_permission("Blogger", self.name, self.user)
