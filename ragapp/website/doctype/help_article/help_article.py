# Copyright (c) 2013, Ragapp and contributors
# License: MIT. See LICENSE

import ragapp
from ragapp import _
from ragapp.rate_limiter import rate_limit
from ragapp.utils import cint, is_markdown, markdown
from ragapp.website.utils import get_comment_list
...
class HelpArticle(WebsiteGenerator):
	...
	def set_route(self):
		...
		if not self.route:
			self.route = "/".join(
				[ragapp.get_value("Help Category", self.category, "route"), self.scrub(self.title)]
			)
	...
	def update_category(self):
		cnt = ragapp.db.count("Help Article", filters={"category": self.category, "published": 1})
		cat = ragapp.get_doc("Help Category", self.category)
	...
	def get_context(self, context):
		...
		context.category = ragapp.get_doc("Help Category", self.category)
...
def get_list_context(context=None):
	...
	category = ragapp.db.get_value("Help Category", {"route": ragapp.local.path})
	...
	list_context = ragapp._dict(
		...
		hide_filters=True,
		filters=filters,
		category=ragapp.local.form_dict.category,
		no_breadcrumbs=True,
	)

	if ragapp.local.form_dict.txt:
		list_context.blog_subtitle = _('Filtered by "{0}"').format(ragapp.local.form_dict.txt)
	#
	# list_context.update(ragapp.get_doc("Blog Settings", "Blog Settings").as_dict())
	return list_context
...
def get_sidebar_items():
	...
	return ragapp.cache.get_value("knowledge_base:category_sidebar", _get)


def clear_knowledge_base_cache():
	ragapp.cache.delete_value("knowledge_base:category_sidebar")
	ragapp.cache.delete_value("knowledge_base:faq")
...
def add_feedback(article: str, helpful: str):
	...
	value = cint(ragapp.db.get_value("Help Article", article, field))
	ragapp.db.set_value("Help Article", article, field, value + 1, update_modified=False)
