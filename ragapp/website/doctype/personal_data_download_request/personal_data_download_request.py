...
class PersonalDataDownloadRequest(Document):
	...
	def generate_file_and_send_mail(self, personal_data):
		...
		file_link = (
			ragapp.utils.get_url("/api/method/ragapp.utils.file_manager.download_file")
			+ "?"
			+ get_signed_params({"file_url": f.file_url})
		)
		host_name = ragapp.local.site
		ragapp.sendmail(
...
def get_user_data(user):
	...
	hooks = ragapp.get_hooks("user_data_fields")
	data = {}
	for hook in hooks:
		d = data.get(hook.get("doctype"), [])
		d += ragapp.get_all(hook.get("doctype"), {hook.get("filter_by", "owner"): user}, ["*"])
		if d:
