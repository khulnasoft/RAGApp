...
from ragapp.website.doctype.personal_data_download_request.personal_data_download_request import (
	get_user_data,
)
...
class TestRequestPersonalData(IntegrationTestCase):
	...
	def test_file_and_email_creation(self):
		ragapp.set_user("test_privacy@example.com")
		download_request = ragapp.get_doc(
		...
		email_queue = ragapp.get_all("Email Queue", fields=["message"], order_by="creation DESC", limit=1)
		self.assertIn(ragapp._("Download Your Data"), email_queue[0].message)

		ragapp.db.delete("Email Queue")


def create_user_if_not_exists(email, first_name=None):
	ragapp.delete_doc_if_exists("User", email)
	...
	user = ragapp.get_doc(
		{
			...
			"birth_date": ragapp.utils.now_datetime(),
		}
