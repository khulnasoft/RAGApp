ragapp.ui.form.on("Web Form", {
	setup: function () {
		ragapp.meta.docfield_map["Web Form Field"].fieldtype.formatter = (value) => {
			const prefix = {
				"Page Break": "--red-600",
		...
		ragapp.meta.docfield_map["Web Form Field"].fieldname.formatter = (value) => {
			if (!value) return;
			return ragapp.unscrub(value);
		};

		ragapp.meta.docfield_map["Web Form List Column"].fieldname.formatter = (value) => {
			if (!value) return;
			return ragapp.unscrub(value);
		};
	},
	...
	refresh: function (frm) {
		...
		frm.sidebar
			...
			.attr("href", "#")
			.on("click", () => {
				const url = ragapp.urllib.get_full_url(frm.doc.route);
				const code = `<iframe src="${url}" style="border: none; width: 100%; height: inherit;"></iframe>`;
				ragapp.utils.copy_to_clipboard(code, __("Embed code copied"));
			});

		if (frm.doc.is_standard && !ragapp.boot.developer_mode) {
			frm.disable_form();
			ragapp.show_alert(
	...
	validate: function (frm) {
		...
		if (!frm.doc.web_form_fields) {
			...
			ragapp.throw(__("At least one field is required in Web Form Fields Table"));
		}
		...
		if (page_break_count >= 10) {
			ragapp.throw(__("There can be only 9 Page Break fields in a Web Form"));
		}
	},
	...
	add_get_fields_button(frm) {
		frm.add_custom_button(__("Get Fields"), () => {
			let webform_fieldtypes = ragapp.meta
...
ragapp.ui.form.on("Web Form List Column", {
	fieldname: function (frm, doctype, name) {
		let doc = ragapp.get_doc(doctype, name);
		let df = ragapp.meta.get_docfield(frm.doc.doc_type, doc.fieldname);
		if (!df) return;
...
ragapp.ui.form.on("Web Form Field", {
...
function get_fields_for_doctype(doctype) {
	return new Promise((resolve) => ragapp.model.with_doctype(doctype, resolve)).then(() => {
		return ragapp.meta.get_docfields(doctype).filter((df) => {
			return (
