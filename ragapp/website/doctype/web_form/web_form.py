...
from ragapp.core.doctype.file.utils import remove_file_by_url
from ragapp.desk.form.meta import get_code_files_via_hooks
from ragapp.modules.utils import export_module_json, get_doc_module
from ragapp.rate_limiter import rate_limit
from ragapp.utils import dict_with_keys, strip_html
from ragapp.utils.caching import redis_cache
from ragapp.website.utils import get_boot_data, get_comment_list, get_sidebar_items
...
class WebForm(WebsiteGenerator):
	...
	if TYPE_CHECKING:
		...
		from ragapp.website.doctype.web_form_field.web_form_field import WebFormField
		from ragapp.website.doctype.web_form_list_column.web_form_list_column import WebFormListColumn
	...
	website = ragapp._dict(no_cache=1)
	...
	def validate(self):
		...
		in_user_env = not (
			ragapp.flags.in_install
			or ragapp.flags.in_patch
			or ragapp.flags.in_test
			or ragapp.flags.in_fixtures
		)
		if in_user_env and self.is_standard and not ragapp.conf.developer_mode:
			...
			if self.has_value_changed("published"):
				...
				ragapp.throw(_("You need to be in developer mode to edit a Standard Web Form"))

		if not ragapp.flags.in_import:
			self.validate_fields()

	def validate_fields(self):
		"""Validate all fields are present"""
		from ragapp.model import no_value_fields

		meta = ragapp.get_meta(self.doc_type)
		missing = [
			df.fieldname
		...
		if missing:
			ragapp.throw(_("Following fields are missing:") + "<br>" + "<br>".join(missing))
...
"""
	...
	def get_context(self, context):
		...
		if ragapp.form_dict.name:
			...
			if not ragapp.db.exists(self.doc_type, ragapp.form_dict.name):
				raise ragapp.PageDoesNotExistError()

			if not self.has_web_form_permission(self.doc_type, ragapp.form_dict.name):
				ragapp.throw(
					_("You don't have the permissions to access this document"), ragapp.PermissionError
				)

		if ragapp.local.path == self.route:
			path = f"/{self.route}/list" if self.show_list else f"/{self.route}/new"
			ragapp.redirect(path)

		if ragapp.form_dict.is_list and not self.show_list:
			ragapp.redirect(f"/{self.route}/new")

		if ragapp.form_dict.is_edit and not self.allow_edit:
			context.in_view_mode = True
			ragapp.redirect(f"/{self.route}/{ragapp.form_dict.name}")

		if ragapp.form_dict.is_edit:
			context.in_edit_mode = True

		if ragapp.form_dict.is_read:
			context.in_view_mode = True

		if (
			not ragapp.form_dict.is_edit
			and not ragapp.form_dict.is_read
			and self.allow_edit
			and ragapp.form_dict.name
		):
			context.in_edit_mode = True
			ragapp.redirect(f"/{ragapp.local.path}/edit")

		if (
			ragapp.session.user != "Guest"
			and self.login_required
			and not self.allow_multiple
			and not ragapp.form_dict.name
			and not ragapp.form_dict.is_list
		):
			condition_json = json.loads(self.condition_json) if self.condition_json else []
			condition_json.append(["owner", "=", ragapp.session.user])
			names = ragapp.get_all(self.doc_type, filters=condition_json, pluck="name")
			if names:
				context.in_view_mode = True
				ragapp.redirect(f"/{self.route}/{names[0]}")

		# Show new form when
		# - User is Guest
		# - Login not required
		route_to_new = ragapp.session.user == "Guest" or not self.login_required
		if not ragapp.form_dict.is_new and route_to_new:
			ragapp.redirect(f"/{self.route}/new")

		self.reset_field_parent()

		# add keys from form_dict to context
		context.update(dict_with_keys(ragapp.form_dict, ["is_list", "is_new", "is_edit", "is_read"]))
		...
		context.boot["link_title_doctypes"] = ragapp.boot.get_link_title_doctypes()
	...
	def load_translations(self, context):
		...
		context.translated_messages = ragapp.as_json({message: _(message) for message in messages if message})
	...
	def load_form_data(self, context):
		...
		if self.breadcrumbs:
			context.parents = ragapp.safe_eval(self.breadcrumbs, {"_": _})

		if self.show_list and ragapp.form_dict.is_new:
			context.title = _("New {0}").format(context.title)

		context.has_header = (ragapp.form_dict.name or ragapp.form_dict.is_new) and (
			ragapp.session.user != "Guest" or not self.login_required
		)

		if context.success_message:
			context.success_message = ragapp.db.escape(context.success_message.replace("\n", "<br>")).strip(
				"'"
			)
		...
		# load reference doc
		if ragapp.form_dict.name:
			context.doc_name = ragapp.form_dict.name
			context.reference_doc = ragapp.get_doc(self.doc_type, context.doc_name)
			...
			if self.show_attachments:
				context.attachments = ragapp.get_all(
					"File",
					filters={
	...
	def add_custom_context_and_script(self, context):
		...
		if self.is_standard:
			...
			if os.path.exists(js_path):
				script = ragapp.render_template(open(js_path).read(), context)

				for path in get_code_files_via_hooks("webform_include_js", context.doc_type):
					custom_js = ragapp.render_template(open(path).read(), context)
	...
	def has_web_form_permission(self, doctype, name, ptype="read"):
		...
		if self.apply_document_permissions:
			return ragapp.get_doc(doctype, name).has_permission(permtype=ptype)

		# owner matches
		elif ragapp.db.get_value(doctype, name, "owner") == ragapp.session.user:
			return True

		elif ragapp.has_website_permission(name, ptype=ptype, doctype=doctype):
			return True
...
def accept(web_form, data):
	"""Save the web form"""
	data = ragapp._dict(json.loads(data))

	files = []
	files_to_delete = []

	web_form = ragapp.get_doc("Web Form", web_form)
	doctype = web_form.doc_type
	user = ragapp.session.user

	if web_form.anonymous and ragapp.session.user != "Guest":
		ragapp.session.user = "Guest"

	if data.name and not web_form.allow_edit:
		ragapp.throw(_("You are not allowed to update this Web Form Document"))

	ragapp.flags.in_web_form = True
	meta = ragapp.get_meta(doctype)

	if data.name:
		# update
		doc = ragapp.get_doc(doctype, data.name)
	else:
		# insert
	...
	if doc.name:
		...
		# insert
		if web_form.login_required and ragapp.session.user == "Guest":
			ragapp.throw(_("You must login to submit this form"))
	...
	if web_form.anonymous and ragapp.session.user == "Guest" and user:
		ragapp.session.user = user

	ragapp.flags.web_form_doc = doc
	return doc


@ragapp.whitelist()
def delete(web_form_name, docname):
	web_form = ragapp.get_doc("Web Form", web_form_name)

	owner = ragapp.db.get_value(web_form.doc_type, docname, "owner")
	if ragapp.session.user == owner and web_form.allow_delete:
		ragapp.delete_doc(web_form.doc_type, docname, ignore_permissions=True)
	else:
		raise ragapp.PermissionError("Not Allowed")
...
def get_form_data(doctype, docname=None, web_form_name=None):
	...
	if ragapp.session.user != "Guest" and not docname and not web_form.allow_multiple:
		docname = ragapp.db.get_value(doctype, {"owner": ragapp.session.user}, "name")
...
def get_link_options(web_form_name, doctype, allow_read_on_all_link_options=False):
	web_form: WebForm = ragapp.get_doc("Web Form", web_form_name)

	if web_form.login_required and ragapp.session.user == "Guest":
		ragapp.throw(_("You must be logged in to use this form."), ragapp.PermissionError)

	if not web_form.published or not any(f for f in web_form.web_form_fields if f.options == doctype):
		ragapp.throw(
			_("You don't have permission to access the {0} DocType.").format(doctype), ragapp.PermissionError
		)

	link_options, filters = [], {}
	if web_form.login_required and not allow_read_on_all_link_options:
		filters = {"owner": ragapp.session.user}
	...
	link_options = ragapp.get_all(doctype, filters, fields)
...
def get_published_web_forms() -> dict[str, str]:
	return ragapp.get_all("Web Form", ["name", "route", "modified"], {"published": 1})
