...
class TestWebPage(IntegrationTestCase):
	def setUp(self):
		ragapp.db.delete("Web Page")
		for t in self.globalTestRecords["Web Page"]:
			ragapp.get_doc(t).insert()
	...
	def test_dynamic_route(self):
		web_page = ragapp.get_doc(
			...
			content_type="HTML",
			dynamic_template=1,
			main_section_html="<div>{{ ragapp.form_dict.doctype }}</div>",
		).insert()
		try:
			from ragapp.utils import get_html_for_route
