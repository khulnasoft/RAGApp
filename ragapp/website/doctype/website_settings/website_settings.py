...
from ragapp.utils import cint, encode, get_request_site_address
from ragapp.website.utils import get_boot_data
...
class WebsiteSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from ragapp.website.doctype.top_bar_item.top_bar_item import TopBarItem
		from ragapp.website.doctype.website_route_redirect.website_route_redirect import (
	...
	def validate_google_settings(self):
		if self.enable_google_indexing and not ragapp.db.get_single_value("Google Settings", "enable"):
		...
		for idx, row in enumerate(self.route_redirects):
			try:
				...
				if not ragapp.flags.in_migrate:
					ragapp.throw(_("Invalid redirect regex in row #{}: {}").format(idx, str(e)))
	...
	def get_access_token(self):
		...
		if not self.indexing_refresh_token:
			button_label = ragapp.bold(_("Allow API Indexing Access"))
...
def get_website_settings(context=None):
	...
	context = ragapp._dict(context or {})
	settings: WebsiteSettings = ragapp.client_cache.get_doc("Website Settings")
	...
	if context.disable_website_theme:
		context.theme = ragapp._dict()

	else:
		from ragapp.website.doctype.website_theme.website_theme import get_active_theme

		context.theme = get_active_theme() or ragapp._dict()

	if not context.get("favicon"):
		context["favicon"] = "/assets/ragapp/images/ragapp-favicon.svg"
	...
	context.read_only_mode = ragapp.flags.read_only
...
def get_auto_account_deletion():
	return ragapp.db.get_single_value("Website Settings", "auto_account_deletion")
