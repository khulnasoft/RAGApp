...
ragapp.provide("website");
ragapp.provide("ragapp.awesome_bar_path");
window.cur_frm = null;
...
$.extend(ragapp, {
	...
	require: async function (links, callback) {
		...
		links = links.map((link) => ragapp.bundled_asset(link));
	...
	bundled_asset(path, is_rtl = null) {
		if (!path.startsWith("/assets") && path.includes(".bundle.")) {
			...
			path = ragapp.boot.assets_json[path] || path;
			return path;
		}
		return path;
	},
	add_asset_to_head(link) {
		return new Promise((resolve) => {
			if (ragapp._assets_loaded.includes(link)) return resolve();
			let el;
			...
			el.onload = () => {
				ragapp._assets_loaded.push(link);
				resolve();
			};
		});
	},
	...
	call: function (opts) {
		...
		return $.ajax({
			...
			statusCode: opts.statusCode || {
				404: function () {
					ragapp.msgprint(__("Not found"));
				},
				403: function () {
					ragapp.msgprint(__("Not permitted"));
				},
				200: function (data) {
			...
			ragapp.process_response(opts, data);
		});
	},
	...
	process_response: function (opts, data) {
		...
		if (data._server_messages) {
			...
			if (opts.error_msg) {
				...
				ragapp.msgprint(server_messages);
			}
		}
	...
	has_permission: function (doctype, docname, perm_type, callback) {
		return ragapp.call({
			type: "GET",
			method: "ragapp.client.has_permission",
			no_spinner: true,
	...
	render_user: function () {
		if (ragapp.is_user_logged_in()) {
			$(".btn-login-area").toggle(false);
			$(".logged-in").toggle(true);
			$(".user-image").attr("src", ragapp.get_cookie("user_image"));

			$(".user-image-wrapper").html(
				ragapp.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-sidebar").html(
				ragapp.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-myaccount").html(
				ragapp.avatar(null, "avatar-large", null, null, null, true)
			);
		}
	},
	freeze_count: 0,
	...
	unfreeze: function () {
		if (!ragapp.freeze_count) return; // anything open?
		ragapp.freeze_count--;
		if (!ragapp.freeze_count) {
	...
	trigger_ready: function () {
		ragapp.ready_events.forEach(function (fn) {
			fn();
		});
	},
	...
	bind_filters: function () {
		...
		$(".filter").each(function () {
			...
			var val = ragapp.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function () {
		ragapp.get_navbar_search().on("keypress", function (e) {
			var val = $(this).val();
			if (e.which === 13 && val) {
				$(this).val("").blur();
				ragapp.do_search(val);
				return false;
			}
		});
	},
	do_search: function (val) {
		var path =
			(ragapp.awesome_bar_path && ragapp.awesome_bar_path[location.pathname]) ||
			window.search_path ||
			location.pathname;
	...
	set_search_path: function (path) {
		ragapp.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function () {
		return ragapp.get_cookie("user_id") !== "Guest" && ragapp.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (ragapp.session.user === "Guest" && window.show_language_picker) {
			ragapp
				.call("ragapp.translate.get_all_languages", {
					with_language_name: true,
				})
				.then((res) => {
					let language_list = res.message;
					let language = ragapp.get_cookie("preferred_language");
					let language_codes = [];
...
ragapp.setup_search = function (target, search_scope) {
	...
	$input.on(
		...
		ragapp.utils.debounce(() => {
			...
			ragapp
				.call({
					method: "ragapp.search.web_search",
					args: {
...
window.valid_email = function (id) {
	// copied regex from ragapp/utils.js validate_type
...
$(document).ready(function () {
	window.full_name = ragapp.get_cookie("full_name");
	var logged_in = ragapp.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	ragapp.bind_navbar_search();

	// add apps link
	let apps = ragapp.boot?.apps_data?.apps;
	let obj = {
		label: __("Apps"),
		route: "/apps",
	};
	if (apps?.length) {
		...
		let is_desk_apps = ragapp.boot?.apps_data?.is_desk_apps;
		!is_desk_apps && ragapp.add_apps(obj);
	}

	// switch to app link
	if (ragapp.get_cookie("system_user") === "yes" && logged_in) {
		ragapp.add_switch_to_desk();
	}
...
$(document).on("page-change", function () {
	...
	$.extend(ragapp, ragapp.get_cookies());
	ragapp.session = { user: ragapp.user_id };

	ragapp.datetime.refresh_when();
	ragapp.trigger_ready();
	ragapp.bind_filters();
	ragapp.highlight_code_blocks();
	ragapp.add_link_to_headings();
	ragapp.make_navbar_active();
	// scroll to hash
...
ragapp.ready(function () {
	ragapp.show_language_picker();
	ragapp.setup_videos();
	ragapp.realtime.init(window.socketio_port, true); // lazy connection
});
