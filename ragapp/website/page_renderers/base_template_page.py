import ragapp
from ragapp.website.doctype.website_settings.website_settings import get_website_settings
from ragapp.website.page_renderers.base_renderer import BaseRenderer
from ragapp.website.website_components.metatags import MetaTags
...
class BaseTemplatePage(BaseRenderer):
	...
	def init_context(self):
		self.context = ragapp._dict()
		self.context.update(get_website_settings())
		self.context.update(ragapp.local.conf.get("website_context") or {})

	def add_csrf_token(self, html):
		if ragapp.local.session:
			csrf_token = ragapp.local.session.data.csrf_token
			return html.replace(
				"<!-- csrf_token -->", f'<script>ragapp.csrf_token = "{csrf_token}";</script>'
			)
	...
	def set_base_template_if_missing(self):
		if not self.context.base_template_path:
			app_base = ragapp.get_hooks("base_template")
	...
	def set_missing_values(self):
		# set using ragapp.respond_as_web_page
		if hasattr(ragapp.local, "response") and ragapp.local.response.get("context"):
			self.context.update(ragapp.local.response.context)

		# to be able to inspect the context dict
		# Use the macro "inspect" from macros.html
		self.context.canonical = ragapp.utils.get_url(ragapp.utils.escape_html(self.path))
		...
		self.context.pathname = getattr(ragapp.local, "path", None) if hasattr(ragapp, "local") else self.path

	def update_website_context(self):
		# apply context from hooks
		update_website_context = ragapp.get_hooks("update_website_context")
		for method in update_website_context:
			values = ragapp.get_attr(method)(self.context)
			if values:
