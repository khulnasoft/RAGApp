...
import ragapp
from ragapp.website.page_renderers.document_page import DocumentPage
from ragapp.website.page_renderers.list_page import ListPage
from ragapp.website.page_renderers.not_found_page import NotFoundPage
from ragapp.website.page_renderers.print_page import PrintPage
from ragapp.website.page_renderers.redirect_page import RedirectPage
...
from ragapp.website.page_renderers.web_form import WebFormPage
from ragapp.website.router import evaluate_dynamic_routes
from ragapp.website.utils import can_cache, get_home_page
...
class PathResolver:
	...
	def resolve(self):
		...
		request = ragapp._dict()
		if hasattr(ragapp.local, "request"):
			request = ragapp.local.request or request
		...
		if request.url and can_cache() and ragapp.cache.hget("website_404", request.url):
			return self.path, NotFoundPage(self.path)

		try:
			resolve_redirect(self.path, request.query_string)
		except ragapp.Redirect as e:
			return ragapp.flags.redirect_location, RedirectPage(self.path, e.http_status_code)

		if ragapp.get_hooks("website_path_resolver"):
			for handler in ragapp.get_hooks("website_path_resolver"):
				endpoint = ragapp.get_attr(handler)(self.path)
		else:
			try:
				endpoint = resolve_path(self.path)
			except werkzeug.routing.exceptions.RequestRedirect as e:
				ragapp.flags.redirect_location = e.new_url
				return ragapp.flags.redirect_location, RedirectPage(e.new_url, e.code)
	...
	def get_custom_page_renderers():
		custom_renderers = []
		for renderer_path in ragapp.get_hooks("page_renderer") or []:
			try:
				renderer = ragapp.get_attr(renderer_path)
...
def resolve_redirect(path, query_string=None):
	...
	redirect_to = ragapp.cache.hget("website_redirects", path or "/")
	if redirect_to:
		if isinstance(redirect_to, dict):
			ragapp.flags.redirect_location = redirect_to["path"]
			raise ragapp.Redirect(redirect_to["status_code"])
		ragapp.flags.redirect_location = redirect_to
		raise ragapp.Redirect

	if redirect_to is False:
		return

	redirects = ragapp.get_hooks("website_redirects")
	redirects += ragapp.get_all(
	...
	for rule in redirects:
		...
		if query_string and rule.get("match_with_query_string"):
			path_to_match = path + "?" + ragapp.safe_decode(query_string)

		try:
			match = re.match(pattern, path_to_match)
		except re.error:
			ragapp.log_error("Broken Redirect: " + pattern)
		...
		if match:
			...
			raise ragapp.Redirect(status_code)

	ragapp.cache.hset("website_redirects", path_to_match or "/", False)
...
def get_website_rules():
	...
	def _get():
		rules = ragapp.get_hooks("website_route_rules")
		for d in ragapp.get_all("DocType", "name, route", dict(has_web_view=1)):
			if d.route:
	...
	return ragapp.cache.get_value("website_route_rules", _get)


def validate_path(path: str):
	if not PathResolver(path).is_valid_path():
		ragapp.throw(ragapp._("Path {0} it not a valid path").format(ragapp.bold(path)))
