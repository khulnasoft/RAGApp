...
import ragapp
from ragapp.website.utils import extract_title, get_frontmatter
...
def get_page_info_from_web_page_with_dynamic_routes(path):
	...
	"""
	from ragapp.website.doctype.web_page.web_page import get_dynamic_web_pages
...
def get_page_info_from_web_form(path):
	...
	from ragapp.website.doctype.web_form.web_form import get_published_web_forms
	...
	for d in get_published_web_forms():
		...
		if end_point:
			if path.endswith("/list"):
				ragapp.form_dict.is_list = True
			elif path.endswith("/new"):
				ragapp.form_dict.is_new = True
			elif path.endswith("/edit"):
				ragapp.form_dict.is_edit = True
			else:
				ragapp.form_dict.is_read = True
			return d
...
def evaluate_dynamic_routes(rules, path):
	...
	if hasattr(ragapp.local, "request") and ragapp.local.request.environ:
		urls = route_map.bind_to_environ(ragapp.local.request.environ)
		try:
			endpoint, args = urls.match("/" + path)
			if args:
				# don't cache when there's a query string!
				ragapp.local.no_cache = 1
				ragapp.local.form_dict.update(args)
		except NotFound:
			pass
...
def get_pages(app=None):
	...
	return ragapp.cache.get_value("website_pages", lambda: _build(app))
...
def setup_source(page_info):
	...
	if page_info.template.endswith((".md", ".html")):
		...
		if page_info.template.endswith(".md"):
			source = ragapp.utils.md_to_html(source)
...
def get_base_template(path=None):
	...
	"""
	if not path:
		path = ragapp.local.request.path

	base_template_map = ragapp.get_hooks("base_template_map") or {}
...
def load_properties_from_controller(page_info):
	...
	module = ragapp.get_module(page_info.controller)
	if not module:
		return
...
def get_doctypes_with_web_view():
	...
	def _get():
		...
		doctypes = ragapp.get_hooks("website_generators")
		doctypes_with_web_view = ragapp.get_all(
			"DocType", fields=["name", "module"], filters=dict(has_web_view=1)
		)
		module_app_map = ragapp.local.module_app
		doctypes += [
			d.name
			for d in doctypes_with_web_view
			if module_app_map.get(ragapp.scrub(d.module)) in installed_apps
		]
		return doctypes

	return ragapp.cache.get_value("doctypes_with_web_view", _get)


def get_start_folders():
	return ragapp.local.flags.web_pages_folders or ("www", "templates/pages")


def clear_routing_cache():
	from ragapp.website.doctype.web_form.web_form import get_published_web_forms
	from ragapp.website.doctype.web_page.web_page import get_dynamic_web_pages
	from ragapp.website.page_renderers.document_page import _find_matching_document_webview
	from ragapp.www.sitemap import get_public_pages_from_doctypes
	...
	ragapp.cache.delete_value("home_page")
