...
def delete_page_cache(path=None):
	if path:
		ragapp.cache.delete_value(f"{WEBSITE_PAGE_CACHE_PREFIX}{path}")
	else:
		ragapp.cache.delete_keys(WEBSITE_PAGE_CACHE_PREFIX)
...
def can_cache(no_cache=False):
	if ragapp.flags.force_website_cache:
		return True
	if ragapp.conf.disable_website_cache or ragapp.conf.developer_mode:
		return False
	if getattr(ragapp.local, "no_cache", False):
		return False
	return not no_cache


def get_comment_list(doctype, name):
	comments = ragapp.get_all(
		"Comment",
		...
		or_filters=[["owner", "=", ragapp.session.user], ["published", "=", 1]],
	)

	communications = ragapp.get_all(
		"Communication",
		fields=[
			"name",
			"creation",
		...
		or_filters=[
			["recipients", "like", f"%{ragapp.session.user}%"],
			["cc", "like", f"%{ragapp.session.user}%"],
			["bcc", "like", f"%{ragapp.session.user}%"],
		],
	)
...
def get_home_page():
	if ragapp.local.flags.home_page and not ragapp.flags.in_test:
		return ragapp.local.flags.home_page
	...
	def _get_home_page():
		...
		if ragapp.session.user != "Guest":
			# by role
			for role in ragapp.get_roles():
				home_page = ragapp.db.get_value("Role", role, "home_page")
				if home_page:
					break

			# portal default
			if not home_page:
				home_page = ragapp.db.get_single_value("Portal Settings", "default_portal_home")
		...
		# global
		if not home_page:
			home_page = ragapp.db.get_single_value("Website Settings", "home_page")

		if not home_page:
			home_page = "login" if ragapp.session.user == "Guest" else "me"

		home_page = home_page.strip("/")

		return home_page

	if ragapp.local.dev_server:
		# dont return cached homepage in development
		return _get_home_page()

	return ragapp.cache.hget("home_page", ragapp.session.user, _get_home_page)


def get_home_page_via_hooks():
	home_page = None

	home_page_method = ragapp.get_hooks("get_website_user_home_page")
	if home_page_method:
		home_page = ragapp.get_attr(home_page_method[-1])(ragapp.session.user)
	elif ragapp.get_hooks("website_user_home_page"):
		home_page = ragapp.get_hooks("website_user_home_page")[-1]

	if not home_page:
		role_home_page = ragapp.get_hooks("role_home_page")
		if role_home_page:
			for role in ragapp.get_roles():
	...
	if not home_page:
		home_page = ragapp.get_hooks("home_page")
		if home_page:
...
def get_boot_data():
	...
	return {
		...
		"sysdefaults": {
			"float_precision": cint(ragapp.get_system_settings("float_precision")) or 3,
			"date_format": get_date_format(),
			"time_format": get_time_format(),
			"first_day_of_the_week": get_first_day_of_the_week(),
			"number_format": get_number_format().string,
			"currency": ragapp.get_system_settings("currency"),
		},
		"time_zone": {
			"system": get_system_timezone(),
			"user": ragapp.get_cached_value("User", ragapp.session.user, "time_zone")
			or get_system_timezone(),
		},
		"assets_json": get_assets_json(),
		"sitename": ragapp.local.site,
	}


def is_signup_disabled():
	return ragapp.get_website_settings("disable_signup")
...
def get_toc(route, url_prefix=None, app=None):
	...
	return ragapp.get_template("templates/includes/full_index.html").render(
...
def get_full_index(route=None, app=None):
	...
	from ragapp.website.router import get_pages

	if not ragapp.local.flags.children_map:
		...
		children_map = ragapp.cache.get_value("website_full_index", _build)

		ragapp.local.flags.children_map = children_map

	return ragapp.local.flags.children_map
...
def clear_cache(path=None):
	...
	if path:
		ragapp.cache.hdel("website_redirects", path)
		delete_page_cache(path)
	else:
		clear_sitemap()
		ragapp.clear_cache("Guest")
		delete_page_cache()
		keys += [
			"portal_menu_items",
			"home_page",
	...
	ragapp.cache.delete_value(keys)

	for method in ragapp.get_hooks("website_clear_cache"):
		ragapp.get_attr(method)(path)
...
def get_sidebar_items(parent_sidebar, basepath=None):
	...
	hooks = ragapp.get_hooks("look_for_sidebar_json")
	look_for_sidebar_json = hooks[0] if hooks else ragapp.flags.look_for_sidebar
...
def get_portal_sidebar_items():
	sidebar_items = ragapp.cache.hget("portal_menu_items", ragapp.session.user)
	if sidebar_items is None:
		sidebar_items = []
		roles = ragapp.get_roles()
		portal_settings = ragapp.get_doc("Portal Settings", "Portal Settings")
		...
		items_via_hooks = ragapp.get_hooks("portal_menu_items")
		if items_via_hooks:
			for i in items_via_hooks:
				i["enabled"] = 1
			add_items(sidebar_items, items_via_hooks)

		ragapp.cache.hset("portal_menu_items", ragapp.session.user, sidebar_items)
...
def get_sidebar_items_from_sidebar_file(basepath, look_for_sidebar_json):
	...
	with open(sidebar_json_path) as sidebarfile:
		try:
			...
			ragapp.throw("Invalid Sidebar JSON at " + sidebar_json_path)
...
def cache_html(func):
	...
	def cache_html_decorator(*args, **kwargs):
		...
		if can_cache():
			html = None
			page_cache = ragapp.cache.get_value(cache_key)
			if page_cache and ragapp.local.lang in page_cache:
				html = page_cache[ragapp.local.lang]
			if html:
				ragapp.local.response.from_cache = True
				ragapp.local.response.can_cache = True
				return html
		html = func(*args, **kwargs)
		context = args[0].context
		if can_cache(context.no_cache):
			page_cache = ragapp.cache.get_value(cache_key) or {}
			page_cache[ragapp.local.lang] = html
			ragapp.cache.set_value(cache_key, page_cache, expires_in_sec=30 * 60)
			ragapp.local.response.can_cache = True
...
def build_response(path, data, http_status_code, headers: dict | None = None):
	...
	response.headers["X-From-Cache"] = ragapp.local.response.from_cache or False
...
def add_preload_for_bundled_assets(response):
	links = [f"<{css}>; rel=preload; as=style" for css in ragapp.local.preload_assets["style"]]
	links.extend(f"<{js}>; rel=preload; as=script" for js in ragapp.local.preload_assets["script"])

	version = get_build_version()
	links.extend(
		f"<{svg}?v={version}>; rel=preload; as=fetch; crossorigin"
		for svg in ragapp.local.preload_assets["icons"]
	)
