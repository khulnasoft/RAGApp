...
class MetaTags:
	def __init__(self, path, context):
		...
		self.tags = ragapp._dict(self.context.get("metatags") or {})
	...
	def init_metatags_from_context(self):
		...
		if self.tags.get("image"):
			self.tags["image"] = ragapp.utils.get_url(self.tags["image"])

		self.tags["language"] = ragapp.local.lang or "en"
	...
	def set_metatags_from_website_route_meta(self):
		...
		if route == "":
			# homepage
			route = ragapp.get_website_settings("home_page")

		route_exists = route and not route.endswith((".js", ".css")) and has_meta_tags(route)

		if route_exists:
			website_route_meta = ragapp.get_doc("Website Route Meta", route)
...
def has_meta_tags(route):
	return bool(ragapp.db.exists("Website Route Meta", route))
