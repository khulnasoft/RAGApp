...
class TestWorkflow(IntegrationTestCase):
	...
	def setUp(self):
		self.patcher = patch("ragapp.attach_print", return_value={})
		self.patcher.start()
		ragapp.db.delete("Workflow Action")
		self.workflow = create_todo_workflow()

	def tearDown(self):
		ragapp.set_user("Administrator")
		self.patcher.stop()
		ragapp.delete_doc("Workflow", "Test ToDo")
	...
	def test_if_workflow_actions_were_processed_using_role(self):
		...
		workflow_actions = ragapp.get_all("Workflow Action", fields=["*"])
...
def create_todo_workflow():
	from ragapp.tests.ui_test_helpers import UI_TEST_USER

	if ragapp.db.exists("Workflow", "Test ToDo"):
		ragapp.delete_doc("Workflow", "Test ToDo")

	TEST_ROLE = "Test Approver"

	if not ragapp.db.exists("Role", TEST_ROLE):
		ragapp.get_doc(doctype="Role", role_name=TEST_ROLE).insert(ignore_if_duplicate=True)
		if ragapp.db.exists("User", UI_TEST_USER):
			ragapp.get_doc("User", UI_TEST_USER).add_roles(TEST_ROLE)

	workflow = ragapp.new_doc("Workflow")
...
def create_new_todo():
	return ragapp.get_doc(doctype="ToDo", description="workflow " + random_string(10)).insert()
