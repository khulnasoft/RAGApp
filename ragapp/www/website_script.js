...
{% if enable_view_tracking %}
	if (navigator.doNotTrack != 1 && !window.is_404) {
		ragapp.ready(() => {
			let browser = ragapp.utils.get_browser();
			let query_params = ragapp.utils.get_query_params();

			// Get visitor ID based on browser uniqueness
			import('/assets/ragapp/js/lib/fingerprintjs.js')
				.then(fingerprint_js => fingerprint_js.load())
				.then(fp => fp.get())
				.then(result => {
					ragapp.call("ragapp.website.doctype.web_page_view.web_page_view.make_view_log", {
