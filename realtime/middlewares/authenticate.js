...
function authenticate_with_ragapp(socket, next) {
	...
	socket.ragapp_request = (path, args = {}, opts = {}) => {
	...
	socket
		.ragapp_request("/api/method/ragapp.realtime.get_user_info")
		.then((res) => res.json())
		.then(({ message }) => {
...
function get_site_name(socket) {
	...
	} else if (socket.request.headers["x-ragapp-site-name"]) {
		socket.site_name = get_hostname(socket.request.headers["x-ragapp-site-name"]);
	} else if (
		conf.default_site &&
...
module.exports = authenticate_with_ragapp;
